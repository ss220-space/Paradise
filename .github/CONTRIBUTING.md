...
- **Код должен быть модульным:** если вы добавляете новый функционал — он должен идти в отдельный файл в случае, если не подходящих для него файлов ещё не существует (например, новый инструмент имеет смысл добавить в `tools.dm`, а не создавать новый файл с нуля).
...
- Если вы использовали **регулярные выражения ("regex")** для поиска и замены кода при рефакторинге, **укажите их** в описании PR'а. Это может существенно упростить дальнейшую работу с затронутым кодом для будущим разработчиков.
...
Как и в других "C"-подобных языках, в DM существует оператор `.`, он же "dot-operator". Он используется для доступа к полям (переменным) и методам (функциям) экземплярам объекта.
