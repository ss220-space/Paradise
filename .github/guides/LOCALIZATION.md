...
**НЕ ИСПОЛЬЗУЙТЕ ЭТОТ ПРОК НАПРЯМУЮ!** Используйте готовые макросы в `code/__HELPERS/localization.dm`. Вы можете собрать из них всё, что может вам понадобиться.
...
**НЕ ИСПОЛЬЗУЙТЕ ЭТОТ ПРОК НАПРЯМУЮ!** Используйте готовые макросы в `code/__HELPERS/localization.dm`.

```DM
"[user] откашлива[PLUR_ET_UT(user)] бабочек!"
```
...
Данный прок не полностью стандартизирован под макросы, так что можно использовать его напрямую.

```DM
// DECL_CREDIT это тоже самое, что declension_ru(num, "", "а", "ов")
"Письмо содержит [amount] символ[DECL_CREDIT(amount)]."
```
