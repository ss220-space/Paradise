...
```DM
// DECL___A_OV это тоже самое, что declension_ru(num, "", "а", "ов")
amount = 102
"Письмо содержит [amount] символ[DECL___A_OV(amount)]."  // "Письмо содержит 102 символа."
```
...
2. **Комментарии**:
   В нашем репозитории все комментарии должны быть на английском языке (за редким исключением). Пожалуйста, не переводите их и не оставляйте комментарии на русском языке. Комментарии в _DreamMaker_ начинаются с `//` или `/**/` для многострочных комментариев.
