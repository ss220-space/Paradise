...
def make_error(source_loc: SourceLoc, msg) -> str:
    if os.getenv("GITHUB_ACTIONS") == "true":
        return f"::error file={source_loc.file_path},line={source_loc.line},title=Admin Verbs::{source_loc.file_path}:{source_loc.line}: {RED}{msg}{NC}"
    else:
...
class Walker:
    ...
    def visit_Call(self, node, source_info):
        if (
            str(node.name) in PROCS_NEED_USER
...
if __name__ == "__main__":
    print("check_admin_verb_params started")
    ...
    for pth in dme.subtypesof("/datum/admin_verb"):
        walker = Walker()
        td = dme.types[pth]
        ...
        if walker.bad_names:
            ...
            for name, source_loc in walker.bad_names:
                errors.append(
                    make_error(
                        source_loc,
                        f"admin verb {td.path} uses `{name}` instead of implicit 'user' arg",
                    )
                )
        ...
        if walker.bad_clean_inputs:
            ...
            for node, source_loc in walker.bad_clean_inputs:
                errors.append(
                    make_error(
                        source_loc,
                        f"call {node.name}() in {td.path} must have `user = user` as its last arg",
                    )
                )
    ...
    print(f"check_admin_verb_params tests completed in {end - start:.2f}s\n")
    sys.exit(exit_code)
