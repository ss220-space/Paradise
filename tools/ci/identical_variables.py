...
def main():
    ...
    for path in dme.subtypesof("/"):
        ...
        for variable_name in typepath.var_names(modified=True):
            ...
            # Fixing by LittleBoobs. I don't know how the scripts on Paradise work if there are errors in them.
            ...
            try:
                ...
                # Skipping variables that are not found in the parent type
                if "cannot find value for" in str(e):
                    continue
                else:
                    # Calling the exception again for other errors
                    raise e
