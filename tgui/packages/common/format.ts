...
export const formatPower = (value: number, minBase1000 = 0): string => {
  return formatSiUnit(value, minBase1000, 'W');
};

export const formatEnergy = (value: number, minBase1000 = 0): string => {
  return formatSiUnit(value, minBase1000, 'J');
};

// Formats a number as a currency string
export const formatMoney = (value: number, precision = 0): string => {
  if (!Number.isFinite(value)) {
    return String(value);
  }
