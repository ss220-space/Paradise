      ...
      <Stack.Item grow>
        <Section fill scrollable title={`${area_name}`} buttons={sectionButton}>
          <Stack vertical>
            ...
            />
            <SupermatterEntry
              title="Передача энергии разряда"
              alwaysShowChevron
              content={
                <ProgressBar
            ...
            <SupermatterEntry
              ...
              content={
                ...
                >
                  {`${toFixed(gas_total_moles, 2)} молей`}
                </ProgressBar>
              }
            />
            <SupermatterEntry
        ...
        >
          <Stack vertical>
            {gas_composition.map(([gas_tlv, amount]) => (
              <SupermatterEntry
                key={gas_tlv}
                title={getGasLabel(gas_tlv) || 'Неизвестно'}
                content={
                  <ProgressBar
                  ...
                  gas_metadata[gas_tlv] ? (
                    <>
                      ...
                      {gas_metadata[gas_tlv].numeric_data.length ? (
                        ...
                        'Не имеет эффектов от концентрации'
                      )}
                    </>
                  ) : (
                    'Не имеет эффектов'
