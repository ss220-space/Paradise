...
export const matrix = <T>(initializer: () => T, ...dimensions: number[]) => {
...
export const useClickAndDragEventHandler = <T>(
  ref: React.Ref<T>,
...
export const copyLayer = <T>(layer: T[][]) => {
  return [...layer.map((row) => [...row])];
};

export const hasServerColorData = (
