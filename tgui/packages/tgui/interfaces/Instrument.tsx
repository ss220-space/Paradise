...
type InstrumentData = {
  ...
  instrument: string;
  canNoteShift: boolean;
  noteShift: number;
  noteShiftMin: number;
  noteShiftMax: number;
  sustainMode: number;
  sustainLinearDuration: number;
  sustainExponentialDropoff: number;
  legacy: boolean;
  sustainDropoffVolume: number;
  sustainHeldNote: boolean;
};

const InstrumentHelp = (properties) => {
  const { act, data } = useBackend<InstrumentData>();
  const { help } = data;
  if (!help) {
    return;
  }
    ...
    >
      <Section height="100%" title="Help" overflow="auto">
        <Box px="0.5rem" mt="-0.5rem">
          ...
          <Button color="grey" onClick={() => act('help')}>
            Close
          </Button>
        </Box>
      </Section>
...
const InstrumentStatus = (properties) => {
  ...
  return (
    <Section
      ...
      buttons={
        <>
          <Button icon="info" onClick={() => act('help')}>
            Help
          </Button>
    ...
    >
      <LabeledList>
        <LabeledList.Item label="Playback">
          ...
          <Button disabled={!playing} icon="stop" onClick={() => act('stop')}>
            Stop
          </Button>
        </LabeledList.Item>
            ...
            {!!(!legacy && canNoteShift) && (
              <>
                ...
                <LabeledList.Item label="Sustain indefinitely last held note">
                  ...
                  >
                    {sustainHeldNote ? 'Yes' : 'No'}
                  </Button>
                </LabeledList.Item>
              </>
            )}
          </LabeledList>
          <Button icon="redo" mt="0.5rem" onClick={() => act('reset')}>
            Reset to Default
          </Button>
        </Section>
      </Collapsible>
