...
export const PandemicSuper = (props: unknown) => {
  ...
  if(error_message !== null) {
    blocks.push((
      <p><b style={{ fontSize: 'big' }}> {error_message}</b></p>
    ))
  }
  if(beaker_exists) {
      blocks.push((<Box>
  ...
  if(error_message === null) {
      blocks.push((<Stack.Item>
        <Stack fill vertical>
          <Stack.Item>
            <Section
              title="Данные образца крови"
              mb="15px">
                <LabeledList>
          ...
          <Stack.Item grow>
            <Section
              title="Данные о заболеваниях"
              mb="15px">
                {diseases !== null && diseases.length > 0 ? diseases.map((disease) => (
                  <LabeledList key={disease.index}>
                    <LabeledList.Item label="Общепринятое название" labelStyle={{ fontWeight: 'bold', maxWidth: 200 }}>
                      {disease.name}
                      {disease.name === 'Неизвестно'
                      ? (
                        <Button
                          icon="tag"
                          onClick={() => act('renameDisease', {
                            index: disease.index,
                          })}
                        >
                          Задать название
                    ...
                    <LabeledList.Item label="Болезнетворный агент" labelStyle={{ fontWeight: 'bold', maxWidth: 200, textWrap: 'wrap' }}>
                      {disease.agent} <Button
                          icon="plus-circle"
                          onClick={() => act('createExample', {
                            index: disease.index,
                          })}
                        >
                          Создать образец
                    ...
                    <LabeledList.Item label="Описание" labelStyle={{ fontWeight: 'bold', maxWidth: 200, textWrap: 'wrap' }}>
                      {disease.description}
                    </LabeledList.Item>
                    <LabeledList.Item label="Путь передачи" labelStyle={{ fontWeight: 'bold', maxWidth: 200, textWrap: 'wrap' }}>
                      {disease.route}
                    </LabeledList.Item>
                    <LabeledList.Item label="Возможное лекарство" labelStyle={{ fontWeight: 'bold', maxWidth: 200, textWrap: 'wrap' }}>
                      {disease.possibleMedicine}
                    </LabeledList.Item>
                    <LabeledList.Item label="Возможность выработки антител" labelStyle={{ fontWeight: 'bold', maxWidth: 200, textWrap: 'wrap' }}>
