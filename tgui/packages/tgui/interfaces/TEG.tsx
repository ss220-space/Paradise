...
type TEGData = {
  ...
  cold_inlet_pressure: number;
  cold_outlet_temp: number;
  cold_outlet_pressure: number;
  hot_inlet_temp: number;
  hot_inlet_pressure: number;
  hot_outlet_temp: number;
  hot_outlet_pressure: number;
  warning_cold_pressure: BooleanLike;
  warning_hot_pressure: BooleanLike;
  warning_switched: BooleanLike;
  output_power: number;
};

export const TEG = (_props) => {
  const { act, data } = useBackend<TEGData>();
  if (data.error) {
    return (
      <Window width={500} height={400}>
        <Window.Content>
          <Section title="Error">
            {data.error}
            <Button icon="circle" onClick={() => act('check')}>
              Recheck
            </Button>
          </Section>
        </Window.Content>
