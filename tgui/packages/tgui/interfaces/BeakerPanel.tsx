...
const makeContainerState = (default_type: ContainerType) => {
...
const setContainerReagentVolume = (
  container: Container,
  setContainer: (container: Container) => void,
  reagent: typePath,
  volume: number = 10
...
const containerToSpawnInfo = (container: Container) => {
  return {
...
const readableContainerType = (container_type: ContainerType) => {
  return capitalizeFirst(`${container_type.text} (${container_type.volume}u)`);
};

const readableReagentType = (reagent: Reagent) => {
  return capitalizeFirst(reagent.text);
};

const grenadeCheck = (containers: Container[]) => {
