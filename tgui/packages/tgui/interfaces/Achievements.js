...
const Achievement = (props, properties, context) => {
  const { data } = useBackend(properties, context);
  const { achievement } = props;
  const { name, desc, icon_class, value, score } = achievement;
  const { allStylesPreview } = data;
  return (
    <Table.Row key={name}>
      <Table.Cell collapsing table-layout:auto >
        <img
          height="74px"
          width="74px"
          src={`data:image/jpeg;base64,${allStylesPreview[icon_class]}`}
        />
      </Table.Cell>
