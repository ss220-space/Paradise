...
const SeedVaultHeaderField = (props, context) => {
  ...
  return (
    <Button
      ...
      onClick={() => {
        _sortOrder.field !== props.name
          ? setSortOrder({ 'field': name, 'desc': false })
          : setSortOrder({ 'field': name, 'desc': !_sortOrder.desc });
      }}
    />
  );
};
...
const SeedVaultHeader = (props, context) => {
  const { seedsTotal, seedsCapacity } = props;
...
const SeedVaultContents = (props, context) => {
  ...
  const [sortOrder, _setSortOrder] = useLocalState(context, 'sort', {
    'field': 'plantName',
    'desc': false,
  });
  ...
  const nameSearch = createSearch(
    searchText,
    (pile) => pile.display_name + pile.strain_text
  );
  ...
  return (
    <Fragment>
      <Flex
        direction="row"
        textAlign="center"
        className="SeedExtractor__contents--row"
      >
        <Flex.Item basis={'15vw'} textAlign="left" bold>
          <Box m={1}>{displayName}</Box>
        </Flex.Item>
          ...
          <Flex direction="column">
            ...
            <Flex.Item minHeight="25px" pt={1}>
              ...
              <NumberInput
                ...
                stepPixelSize={3}
                onChange={(e, value) =>
                  act('vend', { index: vendIdx, amount: value })
                }
              />
              <Button
                icon="arrow-down"
