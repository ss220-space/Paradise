...
#define NO_ROBOPARTS 	"no_roboparts" // запрещает ставить робо-конечности, робо-органы(лёгкие, сердце, etc), аугменты
...
#define NO_CYBERIMPS "no_cyberimps" // запрещает ставить кибер-импланты(toolset, CNS Rebooter, etc), но не робо-органы
