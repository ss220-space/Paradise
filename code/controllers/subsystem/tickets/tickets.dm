...
/datum/controller/subsystem/tickets/proc/autoRespond(N)
	...
	var/response_phrases = list("Спасибо" = "Спасибо, желаем вам удачного дня!",
		...
		"Если ещё раз случиться" = "Спасибо, дайте нам знать, если это продолжит происходить.",
...
/datum/controller/subsystem/tickets/proc/showDetailUI(mob/user, ticketID)
	...
	dat +="<a href='?src=[UID()];refresh=1'>Показать все</a><a href='?src=[UID()];refreshdetail=[T.ticketNum]'>Обновить</a>"
