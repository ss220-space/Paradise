...
/datum/controller/subsystem/tickets/proc/autoRespond(N)
	...
	var/response_phrases = list(
		...
		"Сообщите об ошибке" = "Чтобы оформить баг-репорт перейдите в соответствующий канал нашего <a href='[config.discordurl]/issues/new/choose'>Discord сервера</a>. Заполните указанную в закреплённых сообщениях форму. Если раунд ещё идёт, пожалуйста, дождитесь окончания и отправьте сообщение об ошибке.",
