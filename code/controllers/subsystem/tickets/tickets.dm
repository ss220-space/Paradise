...
/datum/controller/subsystem/tickets/fire()
	...
	if(LAZYLEN(stales))
		...
		for(var/num in stales)
			report += "[num], "
		message_staff("<span class='[span_class]'>Тикет [report] был открыт [TICKET_TIMEOUT / 600] минут. Меняю статус на просроченный.</span>")
...
/datum/controller/subsystem/tickets/proc/newHelpRequest(client/C, text)
	...
	if((T = checkForOpenTicket(C)))
		...
		T.setCooldownPeriod()
		to_chat(C.mob, "<span class='[span_class]'>Ваш [ticket_name] #[ticketNum] остаётся открытым! Зайдите в \"My tickets\" во вкладке Admin чтобы просмотреть.</span>")
...
/datum/controller/subsystem/tickets/proc/convert_to_other_ticket(ticketId)
	if(!check_rights(rights_needed))
		return
	if(alert("Вы уверены что хотите перевести тикет в '[other_ticket_name]' тикет?",,"Yes","No") != "Yes")
		return
...
/datum/controller/subsystem/tickets/proc/other_ticket_system_staff_check()
	...
	if(!staff[1])
			if(alert("Нет активных администраторов, чтобы ответить на тикет. Вы уверены что хотите перевести тикет?",, "No", "Yes") != "Yes")
			return FALSE
	return TRUE
...
/datum/controller/subsystem/tickets/proc/autoRespond(N)
	...
	var/response_phrases = list("Спасибо" = "Спасибо, желаем вам удачного дня!",
		...
		"Сообщите об ошибке" = "Чтобы сообщить об ошибке, пожалуйста перейдите на <a href='[config.githuburl]'>GitHub</a>. После, в 'Issues' и 'New Issue'. Затем заполните все поля и отправьте. Если раунд всё ещё идёт, либо сохраните в текстовый файл, либо подождите пока раунд не закончится.",
		...
		"IC-Проблема" = "Это игровая (IC-In Character) ситуация и не будет обрабатываться администраторами. Вы можете поговорить со службой безопасности, АВД, главой отдела, представителем Nanotrasen, или обратиться в любой другой соответствующий орган, находящийся в настоящее время на станции.",
		"Отказано" = "Отказано",
		"Соберись с мыслями" = "Соберись с мыслями",
		"Обжалование в Discord" = "Обжалование бана должно происходить в дискорде. ЛС или adminhelp по поводу вашего бана не решат эту проблему. Чтобы обжаловать ваш бан, пожалуйста, перейдите по адресу <a href='[config.banappeals]'>[config.banappeals]</a>"
		)
...
/datum/controller/subsystem/tickets/proc/assignStaffToTicket(client/C, N)
	var/datum/ticket/T = allTickets[N]
	if(T.staffAssigned != null && T.staffAssigned != C && alert("Тикет уже назначен [T.staffAssigned.ckey]. Вы уверены что хотите забрать?","Забрать тикет","Да","Нет") != "Нет")
		return FALSE
	T.assignStaff(C)
	return TRUE
