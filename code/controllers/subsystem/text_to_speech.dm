...
/datum/controller/subsystem/tts/proc/sanitize_tts_input(message)
	...
	for(var/word in tts_replacement_list)
		. = replacetext(., regex("(?<!\[\\wа-яёА-ЯЁ])[word](?!\[\\wа-яёА-ЯЁ])", "igm"), tts_replacement_list[word])
	for(var/word in tts_replacement_list_insensitive)
		. = replacetext(., regex("(?<!\[\\wа-яёА-ЯЁ])[word](?!\[\\wа-яёА-ЯЁ])", "igm"), tts_replacement_list_insensitive[word])
