...
/datum/controller/subsystem/ticker/proc/setup()
	...
	for(var/mob/new_player/P in GLOB.player_list)
		...
		if(!P.client.prefs.check_any_job())
			to_chat(P, span_danger("Вы не выбрали ни одной роли, \
									а также опцию возврата в лобби, если выбранные роли недоступны. \
									Из-за этого вы не можете заполучить какую-либо роль с началом раунда. \
									Пожалуйста, измените свой список предпочитаемых ролей.")
			)
			if(has_antags)
