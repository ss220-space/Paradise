...
/datum/move_loop/has_target/setup(delay, timeout, atom/chasing)
	...
	RegisterSignal(target, COMSIG_PARENT_QDELETING, PROC_REF(handle_no_target))	// paradise turfs are special
...
/datum/move_manager/proc/move_to_pathfind(moving, chasing, min_dist, delay, timeout, subsystem, priority, flags, datum/extra_info)
...
/datum/move_loop/has_target/move_towards/move()
	...
	if(home)
		if(get_turf(moving) == get_turf(target))
			x_rate = 0
			y_rate = 0
			x_ticker = 0
			y_ticker = 0
