...
/**
  ...
  */
/datum/controller/subsystem/ghost_spawns/proc/poll_candidates(question = "Вы хотите сыграть за специальную роль?", role, antag_age_check = FALSE, poll_time = 30 SECONDS, ignore_respawnability = FALSE, min_hours = 0, flash_window = TRUE, check_antaghud = TRUE, source, role_cleanname, reason)
	...
	for(var/mob/dead/observer/M in (ignore_respawnability ? GLOB.player_list : GLOB.respawnable_list))
		...
		A.name = "Рассматриваются кандидаты"
		...
		var/act_signup = "<a href='byond://?src=[A.UID()];signup=1'>\[Подписаться]</a>"
		to_chat(M, span_boldnotice(span_big("В настоящее время рассматриваются кандидаты [role ? "игры за [role_cleanname || role]" : "\"[question]\""]. [act_jump] [act_signup] [reason?"<i>\nПричина: [reason]</i>":""]")))
