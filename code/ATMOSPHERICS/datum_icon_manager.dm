...
/datum/pipe_icon_manager/proc/gen_pipe_icons()
	...
	var/icon/pipe = new('icons/obj/atmos/pipes.dmi')
	...
	for(var/state in pipe.IconStates())
		...
		var/image/I = image('icons/obj/atmos/pipes.dmi', icon_state = state)
		pipe_icons[cache_name] = I

		for(var/pipe_color in GLOB.pipe_colors)
			I = image('icons/obj/atmos/pipes.dmi', icon_state = state)
			I.color = GLOB.pipe_colors[pipe_color]
			pipe_icons[state + "[GLOB.pipe_colors[pipe_color]]"] = I

	pipe = new ('icons/obj/atmos/heat.dmi')
	for(var/state in pipe.IconStates())
		if(!state || findtext(state, "map"))
			continue
		pipe_icons["hepipe" + state] = image('icons/obj/atmos/heat.dmi', icon_state = state)

	pipe = new ('icons/obj/atmos/junction.dmi')
	for(var/state in pipe.IconStates())
		if(!state || findtext(state, "map"))
			continue
		pipe_icons["hejunction" + state] = image('icons/obj/atmos/junction.dmi', icon_state = state)

/datum/pipe_icon_manager/proc/gen_manifold_icons()
	if(!manifold_icons)
		manifold_icons = new()

	var/icon/pipe = new('icons/obj/atmos/manifold.dmi')

	for(var/state in pipe.IconStates())
		if(findtext(state, "clamps"))
			var/image/I = image('icons/obj/atmos/manifold.dmi', icon_state = state)
		...
		if(findtext(state, "core") || findtext(state, "4way"))
			...
			manifold_icons[state] = I
			for(var/pipe_color in GLOB.pipe_colors)
				I = image('icons/obj/atmos/manifold.dmi', icon_state = state)
				I.color = GLOB.pipe_colors[pipe_color]
...
/datum/pipe_icon_manager/proc/gen_device_icons()
	...
	device = new('icons/obj/atmos/vent_pump.dmi')
	for(var/state in device.IconStates())
		if(!state || findtext(state, "map"))
			continue
		device_icons["vent" + state] = image('icons/obj/atmos/vent_pump.dmi', icon_state = state)

	device = new('icons/obj/atmos/vent_scrubber.dmi')
	for(var/state in device.IconStates())
		if(!state || findtext(state, "map"))
			continue
		device_icons["scrubber" + state] = image('icons/obj/atmos/vent_scrubber.dmi', icon_state = state)
...
/datum/pipe_icon_manager/proc/gen_underlay_icons()
	...
	var/icon/pipe = new('icons/obj/atmos/pipe_underlays.dmi')
	...
	for(var/state in pipe.IconStates())
		...
		for(var/D in GLOB.cardinal)
			var/image/I = image(icon('icons/obj/atmos/pipe_underlays.dmi', icon_state = state, dir = D), layer = GAS_PIPE_HIDDEN_LAYER)
			underlays[cache_name + "[D]"] = I
			for(var/pipe_color in GLOB.pipe_colors)
				I = image(icon('icons/obj/atmos/pipe_underlays.dmi', icon_state = state, dir = D), layer = GAS_PIPE_HIDDEN_LAYER)
				I.color = GLOB.pipe_colors[pipe_color]
