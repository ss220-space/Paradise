...
/obj/machinery/disposal/attackby(obj/item/I, mob/user, params)
		...
		if((storage.allow_quick_empty || storage.allow_quick_gather) && length(storage.contents))
			...
			if(length(storage))
				user.visible_message(
					span_notice("[capitalize(user.declent_ru(NOMINATIVE))] выгружает предметы из [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)]."),
					span_notice("Вы выгружаете предметы из [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)].")
				)
			else
				user.visible_message(
					span_notice("[capitalize(user.declent_ru(NOMINATIVE))] опустошает [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)]."),
	...
	user.visible_message(
		span_notice("[capitalize(user.declent_ru(NOMINATIVE))] помещает [I.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
...
/obj/machinery/disposal/shove_impact(mob/living/target, mob/living/attacker)
	target.visible_message(
		span_warning("[capitalize(attacker.declent_ru(NOMINATIVE))] заталкивает [target.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]!"),
		span_userdanger("[capitalize(attacker.declent_ru(NOMINATIVE))] заталкивает вас в [declent_ru(ACCUSATIVE)]!"),
...
/obj/machinery/disposal/MouseDrop_T(mob/living/target, mob/living/user, params)
	...
	for(var/mob/viewer in viewers(user))
		if(target == user)
			viewer.show_message("[capitalize(user.declent_ru(NOMINATIVE))] начинает залезать в мусоропровод.", 3)
		else
			viewer.show_message("[capitalize(user.declent_ru(NOMINATIVE))] начинает заталкивать [target.declent_ru(ACCUSATIVE)] в мусоропровод.", 3)
...
/obj/machinery/disposal/proc/put_in(mob/living/target, mob/living/user) // need this proc to use INVOKE_ASYNC in other proc. You're not recommended to use that one
	...
	if(target == user && !user.incapacitated())	// if drop self, then climbed in
		...
		msg = "[capitalize(user.declent_ru(NOMINATIVE))] залезает в [declent_ru(ACCUSATIVE)]."
		...
		msg = "[capitalize(user.declent_ru(NOMINATIVE))] заталкивает [target.name] в [declent_ru(ACCUSATIVE)]!"
...
/obj/machinery/disposal/proc/monkey_stuff_mob(mob/living/target, mob/living/user)
	...
	add_fingerprint(user)
	if(user == target)
		user.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] начинает забираться в [declent_ru(ACCUSATIVE)]."), span_notice("Вы начинаете забираться в [declent_ru(ACCUSATIVE)]..."))
	else
		target.visible_message(span_danger("[capitalize(user.declent_ru(NOMINATIVE))] начинает запихивать [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[capitalize(user.declent_ru(NOMINATIVE))] начинает запихивать вас в [declent_ru(ACCUSATIVE)]!"))
	if(do_after(user, 2 SECONDS, target))
		if(!loc)
			return
		target.forceMove(src)
	if(user == target)
		user.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] забирается в [declent_ru(ACCUSATIVE)]."), span_notice("Вы забираетесь [declent_ru(ACCUSATIVE)]."))
		. = TRUE
	else
		target.visible_message(span_danger("[capitalize(user.declent_ru(NOMINATIVE))] запихивает [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[capitalize(user.declent_ru(NOMINATIVE))] запихивает вас в [declent_ru(ACCUSATIVE)]."))
		add_attack_logs(user, target, "Disposal'ed")
		. = TRUE
	update()
...
/obj/machinery/disposal/click_alt(mob/user)
	user.visible_message(
		span_notice("[capitalize(user.declent_ru(NOMINATIVE))] пытается вручную извлечь содержимое [declent_ru(GENITIVE)]."),
	...
	user.visible_message(
		span_notice("[capitalize(user.declent_ru(NOMINATIVE))] извлекает содержимое [declent_ru(GENITIVE)]."),
