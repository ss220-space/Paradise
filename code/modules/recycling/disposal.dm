...
/obj/machinery/disposal/proc/can_be_inserted(obj/item/W, stop_messages = FALSE)
	...
	if(length(contents) >= storage_slots)
		if(!stop_messages)
			to_chat(usr, span_warning("[W.declent_ru_cap(NOMINATIVE)] не помещается в [declent_ru(ACCUSATIVE)], освободите место!"))
	...
	if(sum_w_class > max_combined_w_class)
		if(!stop_messages)
			to_chat(usr, span_notice("[declent_ru_cap(NOMINATIVE)] переполнен, освободите место."))
		return FALSE

	if(HAS_TRAIT(W, TRAIT_NODROP)) //SHOULD be handled in unEquip, but better safe than sorry.
		to_chat(usr, span_notice("[W.declent_ru_cap(NOMINATIVE)] прилип к вашей руке, вы не можете выкинуть его в [declent_ru(ACCUSATIVE)]."))
		return FALSE
...
/obj/machinery/disposal/attackby(obj/item/I, mob/user, params)
		...
		if((storage.allow_quick_empty || storage.allow_quick_gather) && length(storage.contents))
			...
			if(length(storage))
				user.visible_message(
					span_notice("[user.declent_ru_cap(NOMINATIVE)] выгружа[PLUR_ET_YUT(user)] предметы из [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)]."),
					span_notice("Вы выгружаете предметы из [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)].")
				)
			else
				user.visible_message(
					span_notice("[user.declent_ru_cap(NOMINATIVE)] опустоша[PLUR_ET_YUT(user)] [storage.declent_ru(GENITIVE)] в [declent_ru(ACCUSATIVE)]."),
	...
	user.visible_message(
		span_notice("[user.declent_ru_cap(NOMINATIVE)] помеща[PLUR_ET_YUT(user)] [I.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
...
/obj/machinery/disposal/grab_attack(mob/living/grabber, atom/movable/grabbed_thing)
	...
	grabber.visible_message(span_notice("[grabber.declent_ru_cap(NOMINATIVE)] начинает помещать [grabbed_thing.declent_ru(ACCUSATIVE)] в мусоропровод."), ignored_mobs = grabber)
	if(!do_after(grabber, 2 SECONDS, src, NONE) || !grabbed_thing || grabber.pulling != grabbed_thing)
		return .

	add_fingerprint(grabber)
	grabbed_thing.forceMove(src)
	grabber.visible_message(span_warning("[grabber.declent_ru_cap(NOMINATIVE)] поместил [grabbed_thing.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."))
...
/obj/machinery/disposal/shove_impact(mob/living/target, mob/living/attacker)
	target.visible_message(
		span_warning("[attacker.declent_ru_cap(NOMINATIVE)] заталкива[PLUR_ET_YUT(attacker)] [target.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]!"),
		span_userdanger("[attacker.declent_ru_cap(NOMINATIVE)] заталкива[PLUR_ET_YUT(attacker)] вас в [declent_ru(ACCUSATIVE)]!"),
...
/obj/machinery/disposal/MouseDrop_T(mob/living/target, mob/living/user, params)
	...
	for(var/mob/viewer in viewers(user))
		if(target == user)
			viewer.show_message("[user.declent_ru_cap(NOMINATIVE)] начина[PLUR_ET_YUT(user)] залезать в мусоропровод.", 3)
		else
			viewer.show_message("[user.declent_ru_cap(NOMINATIVE)] начина[PLUR_ET_YUT(user)] заталкивать [target.declent_ru(ACCUSATIVE)] в мусоропровод.", 3)
...
/obj/machinery/disposal/proc/put_in(mob/living/target, mob/living/user) // need this proc to use INVOKE_ASYNC in other proc. You're not recommended to use that one
	...
	if(target == user && !user.incapacitated())	// if drop self, then climbed in
		...
		msg = "[user.declent_ru_cap(NOMINATIVE)] залеза[PLUR_ET_YUT(user)] в [declent_ru(ACCUSATIVE)]."
		...
		msg = "[user.declent_ru_cap(NOMINATIVE)] заталкива[PLUR_ET_YUT(user)] [target.name] в [declent_ru(ACCUSATIVE)]!"
...
/obj/machinery/disposal/proc/monkey_stuff_mob(mob/living/target, mob/living/user)
	...
	if(target.mob_size > MOB_SIZE_HUMAN)
		to_chat(user, span_warning("[target.declent_ru_cap(NOMINATIVE)] не помещается в [declent_ru(ACCUSATIVE)]!"))
		return
	add_fingerprint(user)
	if(user == target)
		user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] начина[PLUR_ET_YUT(user)] забираться в [declent_ru(ACCUSATIVE)]."), span_notice("Вы начинаете забираться в [declent_ru(ACCUSATIVE)]..."))
	else
		target.visible_message(span_danger("[user.declent_ru_cap(NOMINATIVE)] начина[PLUR_ET_YUT(user)] запихивать [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[user.declent_ru_cap(NOMINATIVE)] начинает запихивать вас в [declent_ru(ACCUSATIVE)]!"))
	if(do_after(user, 2 SECONDS, target))
		if(!loc)
			return
		target.forceMove(src)
	if(user == target)
		user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] забира[PLUR_ET_YUT(user)]ся в [declent_ru(ACCUSATIVE)]."), span_notice("Вы забираетесь [declent_ru(ACCUSATIVE)]."))
		. = TRUE
	else
		target.visible_message(span_danger("[user.declent_ru_cap(NOMINATIVE)] запихива[PLUR_ET_YUT(user)] [target] в [declent_ru(ACCUSATIVE)]."), span_userdanger("[user.declent_ru_cap(NOMINATIVE)] запихивает вас в [declent_ru(ACCUSATIVE)]."))
		add_attack_logs(user, target, "Disposal'ed")
		. = TRUE
	update()
...
/obj/machinery/disposal/click_alt(mob/user)
	user.visible_message(
		span_notice("[user.declent_ru_cap(NOMINATIVE)] пыта[PLUR_ET_YUT(user)]ся вручную извлечь содержимое [declent_ru(GENITIVE)]."),
	...
	user.visible_message(
		span_notice("[user.declent_ru_cap(NOMINATIVE)] извлека[PLUR_ET_YUT(user)] содержимое [declent_ru(GENITIVE)]."),
...
/obj/machinery/disposal/CanAllowThrough(atom/movable/mover, border_dir)
	...
	if((isitem(mover) && !isprojectile(mover)) && mover.throwing && mover.pass_flags != PASSEVERYTHING)
		if((prob(75)  || mover.throwing.thrower && HAS_TRAIT(mover.throwing.thrower, TRAIT_BADASS)) && can_be_inserted(mover, TRUE))
			...
			visible_message("[mover.declent_ru_cap(NOMINATIVE)] приземляется в [declent_ru(ACCUSATIVE)].")
			update()
		else
			visible_message("[mover.declent_ru_cap(NOMINATIVE)] отскакивает от края [declent_ru(GENITIVE)]!")
		return FALSE
