...
/datum/action/cooldown/spell/rod_form/cast(atom/cast_on)
	...
	if(is_admin_level(start))
		return

	var/flight_dist = rod_max_distance + spell_level * 3
	var/turf/distant_turf = get_ranged_target_turf(start, cast_on.dir, flight_dist)
	new /obj/effect/immovablerod/wizard(start, distant_turf, null, rod_delay, FALSE, cast_on, flight_dist)
