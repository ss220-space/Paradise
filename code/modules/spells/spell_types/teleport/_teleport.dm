...
/datum/action/cooldown/spell/teleport/cast(atom/cast_on)
	...
	do_teleport(cast_on, pick(destinations), asoundout = post_teleport_sound, /*channel = teleport_channel,*/ bypass_area_flag = force_teleport, ignore_bluespace_interference = TRUE)
...
/datum/action/cooldown/spell/teleport/area_teleport/invocation(mob/living/invoker)
	...
	switch(invocation_type)
		if(INVOCATION_SHOUT)
			invoker.say("[invocation], [uppertext(last_chosen_area.name)]!")
		if(INVOCATION_WHISPER)
			invoker.whisper("[invocation], [uppertext(last_chosen_area.name)].")
