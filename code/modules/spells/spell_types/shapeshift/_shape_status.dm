...
/datum/status_effect/shapechange_mob/on_apply()
	...
	ADD_TRAIT(caster_mob, TRAIT_STASIS, TRAIT_STATUS_EFFECT(id))
...
/datum/status_effect/shapechange_mob/proc/on_mob_transformed(mob/living/source)
	...
	restore_caster()
	revealed_mob.Weaken(10 SECONDS, ignore_canweaken = TRUE)
...
/datum/status_effect/shapechange_mob/proc/restore_caster(kill_caster_after = FALSE)
	...
	REMOVE_TRAIT(caster_mob, TRAIT_STASIS, TRAIT_STATUS_EFFECT(id))
...
/datum/status_effect/shapechange_mob/from_spell/after_unchange()
	...
	caster_mob.rejuvenate() // Remove all of our damage before setting our health to a proportion of the former transformed mob's health
	...
	// Only transfer blood if both mobs are supposed to have a blood volume
	if(initial(caster_mob.blood_volume) > 0 && initial(owner.blood_volume) > 0)
