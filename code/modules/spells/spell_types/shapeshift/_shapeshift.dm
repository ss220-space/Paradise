...
/datum/action/cooldown/spell/shapeshift/proc/eject_from_vents(mob/living/cast_on)
	...
	var/obj/effect/gib_type = choose_gibs(cast_on)


	for(var/obj/machinery/atmospherics/unary/possible_vent in range(10, get_turf(cast_on)))
		if(length(possible_vent.parent) && possible_vent.parent == our_pipeline)
			new gib_type(get_turf(possible_vent))
			playsound(possible_vent, 'sound/misc/reee.ogg', 75, TRUE)

	GLOB.minor_announcement.announce("We detected a pipe blockage around [get_area(get_turf(cast_on))], please dispatch someone to investigate.", "[command_name()]")
...
/datum/action/cooldown/spell/shapeshift/proc/choose_gibs(mob/living/gibbed)
	if(is_clown_job(gibbed))
		return /obj/effect/gibspawner/confetti
	if(ishuman(gibbed))
		return /obj/effect/gibspawner/human
	if(isalien(gibbed))
		return /obj/effect/gibspawner/xeno
	if(isrobot(gibbed) || ismachineperson(gibbed) || isbot(gibbed))
...
/datum/action/cooldown/spell/shapeshift/proc/check_menu(mob/living/caster)
	...
	return !HAS_TRAIT(caster, TRAIT_INCAPACITATED)
