	...
	for(var/i in 1 to summon_amount)
		...
		if(ispath(summoned_object_type, /turf))
			if(isclosedturf(spawn_place))
				spawn_place.ChangeTurf(summoned_object_type, after_flags = CHANGETURF_IGNORE_AIR)
				return
			var/turf/simulated/floor/floor_turf = spawn_place
			floor_turf.ChangeTurf(summoned_object_type, after_flags = CHANGETURF_IGNORE_AIR)
			return
...
/datum/action/cooldown/spell/conjure/limit_summons/post_summon(atom/summoned_object, atom/cast_on)
	RegisterSignal(summoned_object, list(COMSIG_QDELETING, COMSIG_LIVING_DEATH), PROC_REF(delete_copy))
	number_of_summons++
