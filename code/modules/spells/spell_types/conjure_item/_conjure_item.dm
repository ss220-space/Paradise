...
/datum/action/cooldown/spell/conjure_item/cast(atom/cast_on)
	...
	if(istype(mob_caster))
		var/obj/existing_item = mob_caster.get_item_by_slot(mob_caster.get_active_hand())
		if(existing_item)
			mob_caster.drop_item_ground(existing_item)
	...
	if(istype(mob_caster))
		mob_caster.put_in_hands(created, qdel_on_fail = delete_on_failure)
