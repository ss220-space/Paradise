...
/datum/action/cooldown/spell/jaunt/bloodcrawl/proc/try_enter_jaunt(obj/effect/decal/cleanable/blood, mob/living/jaunter, forced = FALSE)
	...
	// Begin the jaunt
	ADD_TRAIT(jaunter, TRAIT_NO_TRANSFORM, MAGIC_TRAIT)
	var/obj/effect/dummy/phased_mob/holder = enter_jaunt(jaunter, jaunt_turf)
	if(!holder)
		REMOVE_TRAIT(jaunter, TRAIT_NO_TRANSFORM, MAGIC_TRAIT)
		return FALSE
	...
	playsound(jaunt_turf, 'sound/misc/enter_blood.ogg', 50, TRUE, -1)
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/on_jaunt_exited(obj/effect/dummy/phased_mob/jaunt, mob/living/unjaunter)
	...
	if(equip_blood_hands && iscarbon(unjaunter))
		for(var/obj/item/bloodcrawl/blood_hand in unjaunter.get_equipped_items(INCLUDE_HELD))
			unjaunter.temporarily_remove_item_from_inventory(blood_hand, force = TRUE)
			qdel(blood_hand)
	return ..()
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/proc/exit_blood_effect(mob/living/exited)
	...
	playsound(landing_turf, 'sound/misc/exit_blood.ogg', 50, TRUE, -1)
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon
	...
	var/consume_sound = 'sound/misc/demon_consume.ogg'
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon/proc/on_death(datum/source)
	SIGNAL_HANDLER

	var/turf/release_turf = get_turf(source)
	for(var/mob/living/dead as anything in consumed_mobs)

		// Unregister the signals first
		UnregisterSignal(dead, list(COMSIG_MOB_STATCHANGE, COMSIG_QDELETING))

		dead.forceMove(release_turf)
		// Heals them back to state one
		dead.playsound_local(release_turf, 'sound/misc/exit_blood.ogg', 50, TRUE, -1)

/**
 * Handle signal from a consumed mob being deleted. Clears any references.
 */
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon/proc/on_victim_deleted(datum/source)
	SIGNAL_HANDLER
 ...
 *
 * A signal handler for if one of the slaughter demon's consumed mobs has
 ...
 */
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon/proc/on_victim_statchange(mob/living/victim, new_stat)
	SIGNAL_HANDLER
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon/funny
	...
	consume_sound = 'sound/spookoween/scary_horn2.ogg'
 ...
 */
/datum/action/cooldown/spell/jaunt/bloodcrawl/slaughter_demon/funny/on_death(datum/source)
	...
	for(var/mob/living/friend as anything in consumed_mobs)
		...
		// Heals them back to state one
		friend.rejuvenate()
		friend.playsound_local(release_turf, 'sound/misc/exit_blood.ogg', 50, TRUE, -1)
...
/datum/action/cooldown/spell/jaunt/bloodcrawl/shadow_crawl
	...
	background_icon_state_active = "shadow_demon_bg"
	button_icon_state = "shadow_crawl"

/datum/action/cooldown/spell/jaunt/bloodcrawl/shadow_crawl/try_enter_jaunt(obj/effect/decal/cleanable/blood, mob/living/jaunter, forced)
	var/turf/jaunt_loc = jaunter.loc
	ADD_TRAIT(jaunter, TRAIT_NO_TRANSFORM, MAGIC_TRAIT)
	var/obj/effect/dummy/phased_mob/holder = enter_jaunt(jaunter, jaunt_loc)
	if(!holder)
		REMOVE_TRAIT(jaunter, TRAIT_NO_TRANSFORM, MAGIC_TRAIT)
		return FALSE
	...
	jaunt_loc.visible_message(span_danger("[jaunter] погружается во тьму..."))
	new /obj/effect/temp_visual/dir_setting/bloodcrawl(get_turf(jaunter), jaunter.dir, "shadowwalk_disappear")
	playsound(jaunt_loc, 'sound/misc/enter_blood.ogg', 50, TRUE, -1)
	jaunter.ExtinguishMob()

	REMOVE_TRAIT(jaunter, TRAIT_NO_TRANSFORM, MAGIC_TRAIT)
	if(!istype(jaunter, /mob/living/simple_animal/demon/shadow))
		return TRUE
	var/mob/living/simple_animal/demon/shadow/demon = jaunter
	demon.RegisterSignal(holder, COMSIG_MOVABLE_MOVED, TYPE_PROC_REF(/mob/living/simple_animal/demon/shadow, check_darkness))
	return TRUE

/datum/action/cooldown/spell/jaunt/bloodcrawl/shadow_crawl/can_cast_spell(feedback)
	var/turf/turf_to_check = get_turf(owner)
	if(turf_to_check.get_lumcount() < 0.2)
		return TRUE
	if(feedback)
		to_chat(owner, span_warning("Слишком светло!"))
	return FALSE

/datum/action/cooldown/spell/jaunt/bloodcrawl/shadow_crawl/try_exit_jaunt(obj/effect/decal/cleanable/blood, mob/living/jaunter, forced)
	if(!forced)
		if(HAS_TRAIT(jaunter, TRAIT_NO_TRANSFORM))
			to_chat(jaunter, span_warning("You cannot exit yet!!"))
			return FALSE

	if(!exit_jaunt(jaunter))
		return FALSE

	new /obj/effect/temp_visual/dir_setting/bloodcrawl(get_turf(jaunter), jaunter.dir, "shadowwalk_appear")
	return TRUE
