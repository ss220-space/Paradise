...
/datum/action/cooldown/spell/Grant(mob/grant_to)
	...
	if(spell_requirements & SPELL_REQUIRES_NO_CENTCOM)
	...
	if(invocation_type == INVOCATION_EMOTE)
		RegisterSignals(owner, list(SIGNAL_ADDTRAIT(TRAIT_EMOTE_MUTE), SIGNAL_REMOVETRAIT(TRAIT_EMOTE_MUTE)), PROC_REF(update_status_on_signal))
...
/datum/action/cooldown/spell/Remove(mob/living/remove_from)
	...
	UnregisterSignal(remove_from, list(
		...
		SIGNAL_REMOVETRAIT(TRAIT_EMOTE_MUTE),
...
/datum/action/cooldown/spell/proc/can_cast_spell(feedback = TRUE)
	...
	if((spell_requirements & SPELL_REQUIRES_NO_CENTCOM) && is_admin_level(caster_turf.z))
		if(feedback)
	...
	if(ishuman(owner))
		if(spell_requirements & SPELL_REQUIRES_WIZARD_GARB && !HAS_MIND_TRAIT(owner, TRAIT_NO_WIZARD_CLOTHES))
			...
			if(!(human_owner.head?.clothing_flags & CASTING_CLOTHES))
				...
				return FALSE
			if(!(human_owner.shoes?.clothing_flags & CASTING_CLOTHES))
				if(feedback)
					to_chat(owner, span_warning("You don't feel strong enough without your shoes!"))
				return FALSE
...
/datum/action/cooldown/spell/proc/before_cast(atom/cast_on)
	...
	// Bonus invocation check done h ere:
...
/datum/action/cooldown/spell/proc/cast(atom/cast_on)
	...
	if(owner)
		...
		if(owner.ckey)
			owner.create_attack_log("cast the spell [name][cast_on != owner ? " on / at [cast_on]":""].")
	...
	switch(used_invocation_type)
		if(INVOCATION_SHOUT)
			if(prob(used_invocation_garble_prob))
				invoker.say(replacetext(used_invocation_message," ","`"))
			else
				invoker.say(used_invocation_message)

		if(INVOCATION_WHISPER)
			if(prob(used_invocation_garble_prob))
				invoker.whisper(replacetext(used_invocation_message," ","`"))
			else
				invoker.whisper(used_invocation_message)

		if(INVOCATION_EMOTE)
			invoker.visible_message(
				capitalize(genderize_decode(invoker, replacetext(used_invocation_message, "%CASTER", invoker.name))),
				capitalize(genderize_decode(invoker, replacetext(invocation_self_message, "%CASTER", invoker.name))),
...
/datum/action/cooldown/spell/proc/try_invoke(mob/living/invoker, feedback = TRUE)
	...
	if(invocation_type == INVOCATION_EMOTE && HAS_TRAIT(invoker, TRAIT_EMOTE_MUTE))
		if(feedback)
