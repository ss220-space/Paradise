...
/datum/action/cooldown/spell/Grant(mob/grant_to)
	...
	if(spell_requirements & (SPELL_REQUIRES_NO_ANTIMAGIC|SPELL_REQUIRES_WIZARD_GARB))
		RegisterSignal(owner, list(COMSIG_MOB_EQUIPPED_ITEM, COMSIG_MOB_UNEQUIPPED_ITEM), PROC_REF(update_status_on_signal))
	if(invocation_type == INVOCATION_EMOTE)
		RegisterSignal(owner, list(SIGNAL_ADDTRAIT(TRAIT_EMOTE_MUTE), SIGNAL_REMOVETRAIT(TRAIT_EMOTE_MUTE)), PROC_REF(update_status_on_signal))
