...
/obj/item/rcs/examine(mob/user)
	. = ..()
	. += to_chat(user, span_notice("Осталось [round(rcell.charge/chargecost)] заряд[DECL___A_OV(round(rcell.charge/chargecost))]."))
...
/obj/item/rcs/proc/teleport(mob/user, obj/structure/closet/C, target)
	...
	to_chat(user, span_notice("Телепортация успешна. Осталось [round(rcell.charge/chargecost)] заряд[DECL___A_OV(round(rcell.charge/chargecost))]."))
