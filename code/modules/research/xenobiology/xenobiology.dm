...
/obj/item/slimepotion/sentience/afterattack(mob/living/M, mob/user, proximity_flag, params)
	...
	if(is_monkeybasic(M) && !M.ckey)
		...
		to_chat(user, span_notice("Вы предлагаете [src] [LF]... [GEND_HE_SHE_CAP(LF)] осторожно осматривает его"))
		being_used = TRUE
		...
		if(length(candidates))
			...
			to_chat(user, span_notice("[M] берет зелье и делает глоток. [GEND_HE_SHE_CAP(LF)] смотрит на вас грустными и понимающими глазами. Сработало!"))
			qdel(src)
