...
/datum/action/innate/feed_slime/Activate()
	...
	if(GLOB.cameranet.checkTurfVis(remote_eye.loc))
		...
		if(iswallturf(remote_eye.loc))
			...
			return
		if(!X.monkeys)
			to_chat(owner, "[X.declent_ru_cap(NOMINATIVE)] не содержит мартышек.")
			return
		if(X.monkeys >= 1)
	...
	if(GLOB.cameranet.checkTurfVis(remote_eye.loc))
		for(var/mob/living/carbon/human/M in remote_eye.loc)
			if(is_monkeybasic(M) && M.stat)
				M.visible_message("[M.declent_ru_cap(NOMINATIVE)] исчезает, [GEND_HE_SHE(M)] отправлен[GEND_A_O_Y(M)] на переработку!")
