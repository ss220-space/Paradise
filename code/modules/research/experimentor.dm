...
/obj/machinery/r_n_d/experimentor/proc/experiment(exp, obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		...
		if (!isRelict)
			...
			var/obj/item/relict_priduction/strange_teleporter/R = new /obj/item/relict_priduction/strange_teleporter(get_turf(exp_on))
...
/obj/item/relict_priduction/perfect_mix/afterattack(atom/target, mob/user, proximity)
	if(istype(target, /obj/item/reagent_containers/glass))
		...
		beaker.reagents.add_reagent(R.id, transfer)
