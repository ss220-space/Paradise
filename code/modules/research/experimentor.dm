	...
	if(exp == SCANTYPE_IRRADIATE)
		...
		if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			...
			for(var/mob/living/m in oview(1, src))
				...
				investigate_log("Experimentor has irradiated [m]", INVESTIGATE_EXPERIMENTOR) //One entry per person so we know what was irradiated.
			ejectItem(TRUE)
		if(prob(EFFECT_PROB_LOW-badThingCoeff))
...
/obj/machinery/r_n_d/experimentor/proc/warn_admins(mob/user, ReactionName)
	...
	add_game_logs("Experimentor reaction: [ReactionName] generated by [key_name(user)] in ([T.x], [T.y], [T.z])", user)
