...
/obj/machinery/computer/rdconsole/proc/start_destroyer(mob/user)
	...
	if(linked_destroy.busy)
		to_chat(user, span_danger("[linked_destroy.declent_ru_cap(NOMINATIVE)] в работе!"))
		return

	if(!linked_destroy.loaded_item)
		to_chat(user, span_danger("[linked_destroy.declent_ru_cap(NOMINATIVE)] пуст!"))
		return
...
/obj/machinery/computer/rdconsole/proc/finish_destroyer(list/temp_tech, mob/user)
	...
	if(!linked_destroy.hacked)
		if(!linked_destroy.loaded_item)
			to_chat(usr, span_danger("[linked_destroy.declent_ru_cap(NOMINATIVE)] пуст!"))
		else
			var/tech_log
			for(var/T in temp_tech)
...
/obj/machinery/computer/rdconsole/proc/start_machine(obj/machinery/r_n_d/machine, design_id, amount)
	...
	if(machine.busy)
		to_chat(usr, span_danger("[machine.declent_ru_cap(NOMINATIVE)] занят!"))
		return
...
/obj/machinery/computer/rdconsole/ui_act(action, list/params)
	...
	switch(action)
		...
		if("eject_item") //Eject the item inside the destructive analyzer.
			if(linked_destroy)
				if(linked_destroy.busy)
					to_chat(usr, span_danger("[linked_destroy.declent_ru_cap(NOMINATIVE)] занят!"))
...
/obj/machinery/computer/rdconsole/ui_interact(mob/user, datum/tgui/ui = null)
	...
	if(!ui)
		ui = new(user, src, "RndConsole", declent_ru_cap(NOMINATIVE))
		ui.open()
