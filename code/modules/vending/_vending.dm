...
/obj/machinery/vending/proc/build_inventory(list/productlist, list/recordlist, list/categories, start_empty = FALSE)
	...
	for(var/typepath in productlist)
		...
		record.name = item.declent_ru_cap(NOMINATIVE)
...
/obj/machinery/vending/proc/try_tilt(obj/item/I, mob/user)
	if(tiltable && !tilted && I.force)
		...
		if(COOLDOWN_FINISHED(src, last_hit_time))
			visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] странно покачивается..."))
...
/obj/machinery/vending/HasProximity(atom/movable/movable)
	...
	movable.visible_message(
		span_warning("[declent_ru_cap(NOMINATIVE)] внезапно опрокидывается на [movable.declent_ru(ACCUSATIVE)]!"),
		span_userdanger("[declent_ru_cap(NOMINATIVE)] обрушивается на вас без предупреждения!")
	)
...
/obj/machinery/vending/proc/insert_item(mob/user, obj/item/I)
	if(!item_slot || inserted_item)
		return
	if(!user.drop_transfer_item_to_loc(I, src))
		to_chat(user, span_warning("[I.declent_ru_cap(NOMINATIVE)] будто бы приклеен[GEND_A_O_Y(I)] к вашей руке! Вы не можете [GEND_HIS_HER(I)] скинуть!"))
		return
	inserted_item = I
...
/obj/machinery/vending/ui_interact(mob/user, datum/tgui/ui = null)
	...
	if(!ui)
		...
		ui = new(user, src, "Vending", declent_ru_cap(NOMINATIVE))
		ui.open()
...
/obj/machinery/vending/ui_data(mob/user)
	...
	data["coin_name"] = coin ? coin.declent_ru_cap(NOMINATIVE) : FALSE
	data["panel_open"] = panel_open ? TRUE : FALSE
	data["speaker"] = shut_up ? FALSE : TRUE
	data["item_slot"] = item_slot // boolean
	data["inserted_item_name"] = inserted_item ? inserted_item.declent_ru_cap(NOMINATIVE) : FALSE
	return data
...
/obj/machinery/vending/proc/collect_records_for_static_data(list/records, list/categories, premium, hidden, index)
	...
	for(var/datum/data/vending_product/product_record as anything in records)
		...
		var/list/static_record = list(
			...
			name = item.declent_ru_cap(NOMINATIVE),
			desc = item.desc,
...
/obj/machinery/vending/ui_act(action, list/params)
	...
	if(issilicon(usr) && !isrobot(usr))
		to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] отказывается взаимодействовать с вами, поскольку вы не входите в его целевую аудиторию!"))
		return
	switch(action)
		if("toggle_voice")
			if(panel_open)
		...
		if("vend")
			...
			if(issilicon(usr))
				to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] отказывается продавать вам товар, поскольку вы не входите в его целевую аудиторию!"))
				vend_ready = TRUE
				. = TRUE
				return
			...
			if(istype(usr.get_active_hand(), /obj/item/stack/spacecash))
				...
				to_chat(usr, span_notice("[declent_ru_cap(NOMINATIVE)] выдаёт товар в результате вмешательства администратора."))
				paid = TRUE
			else
...
/obj/machinery/vending/proc/throw_item()
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] метнул [throw_item.declent_ru(ACCUSATIVE)] в [target]!"))
...
/obj/machinery/vending/proc/handle_squish_carbon(mob/living/carbon/victim, damage_to_deal, crit, from_combat)
	...
	if(!from_combat && crit && critical_attack)
		...
		victim.visible_message(
			span_danger("[declent_ru_cap(NOMINATIVE)] давит [victim]!"),
			span_userdanger("[declent_ru_cap(NOMINATIVE)] давит вас!"),
...
/obj/machinery/vending/proc/tilt_over(mob/victim)
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] опрокидывается!"))
