...
/obj/machinery/newscaster/ui_interact(mob/user, datum/tgui/ui = null)
	...
	if(!ui)
		ui = new(user, src, "Newscaster", declent_ru_cap(NOMINATIVE))
		ui.open()
	...
	switch(action)
		...
		if("attach_photo")
			...
			if(ishuman(usr))
				...
				P.forceMove(src)
				photo = P
				visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] тихо жужжит, после чего из слота для фотографий выпадает [P.declent_ru(NOMINATIVE)]."))
...
/obj/machinery/newscaster/proc/eject_photo(mob/user)
	...
	if(ishuman(user) && user.put_in_active_hand(P, ignore_anim = FALSE))
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] выплёвывает [P.declent_ru(ACCUSATIVE)] из слота для фотографий прямо в руку [user]."))
	else
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] выплёвывает [P.declent_ru(ACCUSATIVE)] из слота для фотографий."))
	playsound(loc, 'sound/machines/terminal_insert_disc.ogg', 30, TRUE)
...
/obj/machinery/newscaster/proc/print_newspaper()
	...
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] тихо жужжит, печатая газету."))
