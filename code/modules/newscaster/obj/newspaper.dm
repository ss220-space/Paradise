...
/obj/item/newspaper/attack_self(mob/user)
	...
	if(user.is_literate())
		...
		dat += ".newspaper-subtitle { font-size: 1.2em; text-align: center; color: #666; margin-bottom: 20px; }"
		...
		switch(screen)
			if(SCREEN_COVER) // Cover
				dat += "<div class='newspaper-title'>Гриффон</div>"
		...
		user << browse(dat, "window=newspaper_main;size=600x800")
