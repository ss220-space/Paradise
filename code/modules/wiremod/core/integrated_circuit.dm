 ...
 **/
/obj/item/integrated_circuit/proc/set_cell(obj/item/stock_parts/cell/cell_to_set)
...
/obj/item/integrated_circuit/proc/add_component(obj/item/circuit_component/to_add, mob/living/user)
	...
	var/success = FALSE
	if(user)
		success = user.transfer_item_to_loc(to_add, src)
	else
...
/obj/item/integrated_circuit/ui_data(mob/user)
	...
	.["is_admin"] = (admin_only || user.can_advanced_admin_interact()) && check_rights_for(user.client, R_VAREDIT)
...
/obj/item/integrated_circuit/ui_interact(mob/user, datum/tgui/ui)
	...
	ui = new(user, src, "IntegratedCircuit", declent_ru_cap(NOMINATIVE))
	ui.open()
