...
/datum/martial_combo/cqc/consecutive/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	if(!target.stat)
		target.visible_message("<span class='warning'>[user] последовательно бь[pluralize_ru(user.gender,"ет","ют")] [target] в живот, шею и спину!</span>", \
							"<span class='userdanger'>[user] последовательно бь[pluralize_ru(user.gender,"ет","ют")] вас в живот, шею и спину!</span>")
