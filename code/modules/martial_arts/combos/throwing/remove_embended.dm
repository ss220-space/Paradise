...
/datum/martial_combo/throwing/remove_embended/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	for(var/obj/item/organ/external/limb as anything in H.bodyparts)
		...
		user.visible_message(
			span_warning("[user] пыта[pluralize_ru(user.gender,"ет","ют")]ся извлечь [I.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[limb][HENITIVE]] [H]."),
			span_notice("Вы пытаетесь извлечь [I.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[limb][HENITIVE]] [H]... (Это займёт [time_taken/10] секунд.)")
		)
		...
		if(do_after(user, time_taken, H))
			...
			if(H.has_pain())
				H.emote("scream")
			user.visible_message(
				span_notice("[user] успешно извлек[pluralize_ru(user.gender,"ет","ют")] [I.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[limb][HENITIVE]] [H]!"),
				span_notice("Вы успешно извлекли [I.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[limb][HENITIVE]] [H].")
			)
