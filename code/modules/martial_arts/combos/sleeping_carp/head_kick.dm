...
/datum/martial_combo/sleeping_carp/head_kick/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	if(!target.stat && !target.IsWeakened())
		...
		target.visible_message("<span class='warning'>[user] с разворота ударил [target] ногой в голову!</span>", \
						  "<span class='userdanger'>[user] с разворота попал ногой в ваш[hit_name]!</span>")
