...
/datum/martial_combo/sleeping_carp/gnashing_teeth/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	target.visible_message(span_danger("[user.declent_ru_cap(NOMINATIVE)] [atk_verb] [target.declent_ru(ACCUSATIVE)]!"),
					span_userdanger("[user.declent_ru_cap(NOMINATIVE)] [atk_verb] тебя!"))
