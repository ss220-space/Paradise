/datum/martial_combo/sleeping_carp/gnashing_teeth
	...
	explaination_text = "Каждый второй последовательный удар наносит дополнительный урон и вы кричите боевые фразы, вселяющие страх в сердца врагов."
...
/datum/martial_combo/sleeping_carp/gnashing_teeth/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	var/atk_verb = pick("метко ударяет", "брутально крушит", "бьет, словно кусает", "неудержимо уничтожает")
	...
	if(atk_verb == "бьет, словно кусает")
		playsound(get_turf(target), 'sound/weapons/bite.ogg', 50, 1, -1)
	if(atk_verb == "махает кулаком словно плавником и плавно, но жестоко атакует") // да, это специально так длинно что пиздец
		playsound(get_turf(target), 'sound/effects/hit_kick.ogg', 50, 1, -1)
