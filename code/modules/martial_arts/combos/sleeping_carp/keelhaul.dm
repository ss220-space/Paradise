...
/datum/martial_combo/sleeping_carp/keelhaul/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	if(!IS_HORIZONTAL(target))
		...
		target.visible_message(span_warning("Ударом в голову, [user.declent_ru(NOMINATIVE)] прибива[PLUR_ET_UT(user)] [target.declent_ru(ACCUSATIVE)] к полу!"),
						span_userdanger("Вас сбили с ног!"))
	else
		target.apply_damage(5, BRUTE, BODY_ZONE_HEAD)
		target.drop_l_hand()
		target.drop_r_hand()
		target.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] бь[PLUR_YOT_UT(user)] [target.declent_ru(ACCUSATIVE)] в голову, оставляя корчиться от боли!"),
