...
/datum/martial_combo/sleeping_carp/keelhaul/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	if(!IS_HORIZONTAL(target))
		...
		target.drop_l_hand()
		target.drop_r_hand()
		target.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] бь[PLUR_YOT_YUT(user)] [target.declent_ru(ACCUSATIVE)] в голову, оставляя корчиться от боли!"),
