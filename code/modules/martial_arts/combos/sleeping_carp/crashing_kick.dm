...
/datum/martial_combo/sleeping_carp/crashing_kick/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	target.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] пина[PLUR_ET_UT(user)] [target.declent_ru(ACCUSATIVE)] прямо в торс, отправляя [genderize_ru(target.gender, "его", "её", "их")] в полёт!"),
