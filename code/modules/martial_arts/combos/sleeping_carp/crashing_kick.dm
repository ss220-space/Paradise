...
/datum/martial_combo/sleeping_carp/crashing_kick/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	target.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] пина[PLUR_ET_YUT(user)] [target.declent_ru(ACCUSATIVE)] прямо в торс, отправляя [GEND_HIS_HER(target)] в полёт!"),
				span_userdanger("[user.declent_ru_cap(NOMINATIVE)] пинком в грудь отправляет тебя в полёт!"))
