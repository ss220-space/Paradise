/datum/martial_combo/sleeping_carp/crashing_kick
	...
	explaination_text = "Вы пинаете оппонента прямо в торс, отправляя его в полёт."
...
/datum/martial_combo/sleeping_carp/crashing_kick/perform_combo(mob/living/carbon/human/user, mob/living/target, datum/martial_art/MA)
	...
	target.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] пина[PLUR_ET_YUT(user)] [target.declent_ru(ACCUSATIVE)] прямо в торс, отправляя [GEND_HIS_HER(target)] в полёт!"),
				span_userdanger("[capitalize(user.declent_ru(NOMINATIVE))] пинком в грудь отправляет Вас в полёт!"))
	playsound(target, 'sound/effects/hit_kick.ogg', 50, TRUE, -1)
	var/atom/throw_target = get_edge_target_turf(target, user.dir)
	target.throw_at(throw_target, 4, 14, user)
