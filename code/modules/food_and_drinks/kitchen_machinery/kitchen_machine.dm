...
/obj/machinery/kitchen_machine
	...
	desc = "Если вы это читаете, значит картограф или кодер что-то напутал. Пожалуйста, сообщите об этом Discord."
	layer = 2.9
	density = TRUE
	anchored = TRUE
...
/obj/machinery/kitchen_machine/attackby(obj/item/I, mob/user, params)
	...
	if(dirty == MAX_DIRT)
		...
		if(istype(I, /obj/item/reagent_containers/spray/cleaner) || istype(I, /obj/item/soap))
			user.visible_message(
				span_notice("[user] начина[pluralize_ru(user.gender,"ет","ют")] чистить [declent_ru(ACCUSATIVE)]."),
			...
			if(broken == BROKEN_NONE)
				container_type = OPENCONTAINER
			user.visible_message(
				span_notice("[user] почистил[genderize_ru(user.gender,"","а","о","и")] [declent_ru(ACCUSATIVE)]."),
	...
	if(is_type_in_list(I, GLOB.cooking_ingredients[recipe_type]) || istype(I, /obj/item/mixing_bowl))
		if(length(contents) >= max_n_of_items)
			to_chat(user, span_warning("[capitalize(declent_ru(NOMINATIVE))] переполнена ингредиентами, больше добавить нельзя."))
			return ATTACK_CHAIN_PROCEED|ATTACK_CHAIN_NO_AFTERATTACK
		...
		SStgui.update_uis(src)
		user.visible_message(
			span_notice("[user] добавля[pluralize_ru(user.gender,"ет","ют")] [stack.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
			span_notice("Вы добавили один [stack] в [src]."),
		)
		return ATTACK_CHAIN_PROCEED_SUCCESS|ATTACK_CHAIN_NO_AFTERATTACK
	...
	if(is_type_in_typecache(I, acceptable_containers))
		...
		user.visible_message(
			span_notice("[user] добавля[pluralize_ru(user.gender,"ет","ют")] несколько ингредиентов из [container.declent_ru(GENITIVE)]."),
			span_notice("Вы добавили несколько ингредиентов из [container.declent_ru(GENITIVE)]."),
		)
		SStgui.update_uis(src)
		return ATTACK_CHAIN_PROCEED_SUCCESS|ATTACK_CHAIN_NO_AFTERATTACK

	to_chat(user, span_warning("Вы не представляете, как готовить с помощью [I.declent_ru(GENITIVE)]."))
	return ATTACK_CHAIN_PROCEED|ATTACK_CHAIN_NO_AFTERATTACK
...
/obj/machinery/kitchen_machine/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(broken != BROKEN_NEEDS_SCREWDRIVER)
		return FALSE
	user.visible_message(
		span_notice("[user] начина[pluralize_ru(user.gender,"ет","ют")] чинить [declent_ru(ACCUSATIVE)]."),
	...
	update_icon(UPDATE_ICON_STATE)
	user.visible_message(
		span_notice("[user] починил[genderize_ru(user.gender,"","а","о","и")] [declent_ru(ACCUSATIVE)]."),
...
/obj/machinery/kitchen_machine/proc/add_item(obj/item/I, mob/user)
	...
	. = TRUE
	user.visible_message(
		span_notice("[user] добавля[pluralize_ru(user.gender,"ет","ют")] [I.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
