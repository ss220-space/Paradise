...
/obj/machinery/kitchen_machine/ui_interact(mob/user, datum/tgui/ui = null)
	...
	if(!ui)
		ui = new(user, src, "KitchenMachine", declent_ru_cap(NOMINATIVE))
		ui.open()

/obj/machinery/kitchen_machine/ui_data(mob/user)
	var/list/data = list()
	data["name"] = declent_ru_cap(NOMINATIVE)
	...
	for(var/obj/O in contents)
		var/display_name = O.declent_ru_cap(NOMINATIVE)
...
/obj/machinery/kitchen_machine/proc/start()
	visible_message(
		span_notice("[declent_ru_cap(NOMINATIVE)] включается."),
...
/obj/machinery/kitchen_machine/proc/muck_finish()
	playsound(loc, 'sound/machines/ding.ogg', 50, TRUE)
	visible_message(span_alert("[declent_ru_cap(NOMINATIVE)] покрывается грязью!"))
...
/obj/machinery/kitchen_machine/proc/broke()
	...
	visible_message(span_alert("[declent_ru_cap(NOMINATIVE)] ломается!")) //Let them know they're stupid
	if(can_broke)
