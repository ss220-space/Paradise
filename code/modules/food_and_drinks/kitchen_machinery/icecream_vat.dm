...
/obj/machinery/icemachine/proc/generate_name(reagent_name)
	...
	var/name_suffix = pick(" Кремброс", " Взбивайс", " Сладкоежка", " Муссон", " Пломбир", " Эскимо", " Зефирко")
...
/obj/machinery/icemachine/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers/food/snacks/icecream))
		...
		I.desc += " С ароматической посыпкой."
		I.ru_names = list(
			NOMINATIVE = initial(I.ru_names[NOMINATIVE]) + " с посыпкой",
			GENITIVE = initial(I.ru_names[GENITIVE]) + " с посыпкой",
			DATIVE = initial(I.ru_names[DATIVE]) + " с посыпкой",
			ACCUSATIVE = initial(I.ru_names[ACCUSATIVE]) + " с посыпкой",
			INSTRUMENTAL = initial(I.ru_names[INSTRUMENTAL]) + " с посыпкой",
			PREPOSITIONAL = initial(I.ru_names[PREPOSITIONAL]) + " с посыпкой"
		)
