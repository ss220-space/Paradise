...
/obj/machinery/processor/process()
	...
	visible_message(span_notice("[picked_slime.declent_ru_cap(NOMINATIVE)] затягивается внутрь [declent_ru(GENITIVE)]."))
...
/datum/food_processor_process/mob/slime/process_food(loc, what, obj/machinery/processor/processor)
	...
	if(S.stat != DEAD)
		...
		S.visible_message(span_notice("[S.declent_ru_cap(NOMINATIVE)] выползает из комбайна!"))
		return
