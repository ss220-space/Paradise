...
/obj/machinery/juicer/interact(mob/user) // The microwave Menu
	...
	if(!beaker)
		beaker_contents = "[declent_ru_cap(NOMINATIVE)] не имеет присоединённой ёмкости."
	else if(!beaker.reagents.total_volume)
		beaker_contents = "[declent_ru_cap(NOMINATIVE)] содержит пустую ёмкость."
		is_beaker_ready = 1
	else if(beaker.reagents.total_volume < beaker.reagents.maximum_volume)
		beaker_contents = "[declent_ru_cap(NOMINATIVE)] содержит ёмкость с чем-то внутри."
		is_beaker_ready = 1
	else
		beaker_contents = "[declent_ru_cap(NOMINATIVE)] содержит заполненную ёмкость."
