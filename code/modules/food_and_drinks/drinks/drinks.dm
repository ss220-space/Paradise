...
/obj/item/reagent_containers/food/drinks/mouse_drop_dragged(atom/over_object, mob/user, src_location, over_location, params)
	...
	chugging = TRUE
	while(do_after(chugger, 4 SECONDS, chugger, show_progress = FALSE, show_cogbar = FALSE, max_interact_count = 1, cancel_on_max = TRUE, cancel_message = span_warning("You stop chugging [src].")))
