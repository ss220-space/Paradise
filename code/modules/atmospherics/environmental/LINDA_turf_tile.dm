...
/turf
	...
	//Я НЕНАВИЖУ ЧТО ОН ЗДЕСЬ, но turf/space и другие заставляют!
	///Our gas mix
	...
	**/
	var/initial_gas_mix = DEFAULT_ATMOS
...
/turf/simulated/assume_air(datum/gas_mixture/giver) //use this for machines to adjust air
	if(!giver)
		return FALSE
	air.merge(giver)
	update_visuals()
...
/turf/simulated/proc/copy_air_with_tile(turf/simulated/target_turf)
...
/turf/simulated/temperature_expose(datum/gas_mixture/air, exposed_temperature)
...
/turf/unsimulated/proc/process_cell(fire_count)
...
/turf/simulated/proc/process_cell(fire_count)
	...
	for(var/turf/simulated/enemy_tile as anything in adjacent_turfs)
	...
	for(var/turf/enemy_tile as anything in share_end)
		...
		if(difference > 0)
			consider_pressure_difference(enemy_tile, difference)
			// Должно сработать....
			if (is_station_level(loc.z) && our_air.total_moles() > 5 && isspaceturf(enemy_tile))
...
/turf/proc/consider_pressure_difference(turf/simulated/target_turf, difference, direction)
	SSair.high_pressure_delta |= src
	if(difference > pressure_difference)
		pressure_direction = direction ? direction : get_dir(src, target_turf)
...
/turf/simulated/proc/decompression(var/list/turfs, var/turf/space/space_turf, var/turn = 0)
	for (var/turf/simulated/enemy_turf in turfs)

		//archiving so we shouldn't space everything too fast
		LINDA_CYCLE_ARCHIVE(enemy_turf)
		enemy_turf.air.equalize(space_turf.space_gas)

		var/difference = round(enemy_turf.air.total_moles() / 2, 0.001)
		if(difference)
			var/decompression_direction = get_dir(enemy_turf, get_step_towards(enemy_turf, space_turf))
			if (!decompression_direction)
				decompression_direction = get_dir(enemy_turf, space_turf)
			enemy_turf.consider_pressure_difference(src, difference, decompression_direction)
...
/datum/excited_group/proc/add_turf(var/turf/simulated/target_turf)
...
/datum/excited_group/proc/merge_groups(datum/excited_group/target_group)
	if(turf_list.len > target_group.turf_list.len)
		...
		for(var/turf/simulated/group_member as anything in target_group.turf_list)
		...
		SSair.excited_groups -= src
		for(var/turf/simulated/group_member as anything in turf_list)
...
/datum/excited_group/proc/dismantle()
	for(var/turf/simulated/current_turf in turf_list)
...
/turf/simulated/conductivity_directions()
	if(blocks_air)
		return ..()
...
/turf/simulated/neighbor_conduct_with_src(turf/other)
	if(blocks_air)
		return ..()

	if(!other.blocks_air) //Both tiles are open
		var/turf/simulated/open_other = other
...
/turf/simulated/finish_superconduction()
...
/turf/simulated/consider_superconductivity(starting)
	if(air.temperature < (starting?MINIMUM_TEMPERATURE_START_SUPERCONDUCTION:MINIMUM_TEMPERATURE_FOR_SUPERCONDUCTION))
		return FALSE
	if(!blocks_air && (air.heat_capacity() < M_CELL_WITH_RATIO)) // Was: MOLES_CELLSTANDARD*0.1*0.05 Since there are no variables here we can make this a constant.
		return FALSE
	return ..()
...
/turf/simulated/proc/temperature_share_open_to_solid(turf/sharer)
