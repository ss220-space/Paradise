...
/obj/machinery/atmospherics/unary/hypertorus/core/process_atmos(seconds_per_tick)
	/*
	 *Pre-checks
	 */
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/fusion_process(seconds_per_tick)
...
//7 reworks
	...
	for(var/gas_tlv, amount in moderator_internal.get_interesting())
		moderator_list[gas_tlv] = amount
		scaled_moderator_list[gas_tlv] = max((amount - FUSION_MOLE_THRESHOLD) / scale_factor, 0)
	...
	for(var/gas_tlv, amount in internal_fusion.get_interesting())
		gas_power += (gas_meta[gas_tlv][META_GAS_FUSION_POWER] * amount)
	for(var/gas_tlv, amount in moderator_internal.get_interesting())
		gas_power += (gas_meta[gas_tlv][META_GAS_FUSION_POWER] * amount * 0.75)

	var/gas_power_instability = gas_power * INSTABILITY_GAS_POWER_FACTOR
	instability = MODULUS(POW2(gas_power_instability), toroidal_size) + (current_damper * 0.01) - iron_content * 0.05
	...
	var/energy_modifiers = scaled_moderator_list[TLV_N2] * 0.35 + \
								scaled_moderator_list[TLV_CO2] * 0.55 + \
								scaled_moderator_list[TLV_N2O] * 0.95 + \
								scaled_moderator_list[TLV_ZAUKER] * 1.55 + \
								scaled_moderator_list[TLV_ANTINOBLIUM] * 20
	// Gases that decrease the amount of energy
	energy_modifiers -= scaled_moderator_list[TLV_HYPERNOBLIUM] * 10 + \
								scaled_moderator_list[TLV_H2O] * 0.75 + \
								scaled_moderator_list[TLV_NITRIUM] * 0.15 + \
								scaled_moderator_list[TLV_HEALIUM] * 0.45 + \
								scaled_moderator_list[TLV_FREON] * 1.15
	...
	var/power_modifier = scaled_moderator_list[TLV_O2] * 0.55 + \
								scaled_moderator_list[TLV_CO2] * 0.95 + \
								scaled_moderator_list[TLV_NITRIUM] * 1.45 + \
								scaled_moderator_list[TLV_ZAUKER] * 5.55 + \
								scaled_moderator_list[TLV_PL] * 0.05 - \
								scaled_moderator_list[TLV_N2O] * 0.05 - \
								scaled_moderator_list[TLV_FREON] * 0.75
	///Minimum 0.25, this value is used to modify the behaviour of the energy emission based on the gases present in the mix
	var/heat_modifier = scaled_moderator_list[TLV_PL] * 1.25 - \
								scaled_moderator_list[TLV_N2] * 0.75 - \
								scaled_moderator_list[TLV_N2O] * 1.45 - \
								scaled_moderator_list[TLV_FREON] * 0.95
	///Between 0.005 and 1000, this value modify the radiation emission of the reaction, higher values increase the emission
	var/radiation_modifier = scaled_moderator_list[TLV_FREON] * 1.15 - \
									scaled_moderator_list[TLV_N2] * 0.45 - \
									scaled_moderator_list[TLV_PL] * 0.95 + \
									scaled_moderator_list[TLV_BZ] * 1.9 + \
									scaled_moderator_list[TLV_PROTO_NITRATE] * 0.1 + \
									scaled_moderator_list[TLV_ANTINOBLIUM] * 10
	...
	if(selected_fuel)
		...
		var/temp_value = (2 * (scaled_fuel_list[selected_fuel.requirements[1]] * CALCULATED_H2RADIUS) * (scaled_fuel_list[selected_fuel.requirements[2]] * CALCULATED_TRITRADIUS))
		internal_power = (scaled_fuel_list[selected_fuel.requirements[1]] * power_modifier / 100) * (scaled_fuel_list[selected_fuel.requirements[2]] * power_modifier / 100) * (PI * POW2(temp_value)) * energy
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/moderator_fuel_process(seconds_per_tick, production_amount, consumption_amount, datum/gas_mixture/internal_output, moderator_list, datum/hfr_fuel/fuel, fuel_list)
	...
	for(var/gas_tlv in fuel.requirements)
		internal_fusion.adjust_gas(gas_tlv, -min(fuel_list[gas_tlv], fuel_consumption))
	for(var/gas_tlv in fuel.primary_products)
		internal_fusion.adjust_gas(gas_tlv, fuel_consumption * 0.5)
 ...
 */
/obj/machinery/atmospherics/unary/hypertorus/core/proc/moderator_common_process(seconds_per_tick, scaled_production, datum/gas_mixture/internal_output, moderator_list, dirty_production_rate, heat_output, radiation_modifier)
	switch(power_level)
		if(1)
			if(moderator_list[TLV_PL] > 100)
				internal_output.adjust_gas(TLV_N2O, scaled_production * 0.5)
				moderator_internal.adjust_gas(TLV_PL, min(moderator_internal.toxins(), scaled_production * 0.85))
			if(moderator_list[TLV_BZ] > 150)
				internal_output.adjust_gas(TLV_HALON, scaled_production * 0.55)
				moderator_internal.adjust_gas(TLV_BZ, min(moderator_internal.bz(), scaled_production * 0.95))
		if(2)
			if(moderator_list[TLV_PL] > 50)
				internal_output.adjust_gas(TLV_BZ, scaled_production * 1.8)
				moderator_internal.adjust_gas(TLV_PL, min(moderator_internal.toxins(), scaled_production * 1.75))
			if(moderator_list[TLV_PROTO_NITRATE] > 20)
				radiation *= 1.55
				heat_output *= 1.025
				internal_output.adjust_gas(TLV_NITRIUM, scaled_production * 1.05)
				moderator_internal.adjust_gas(TLV_PROTO_NITRATE, min(moderator_internal.proto_nitrate(), scaled_production * 1.35))
		if(3, 4)
			if(moderator_list[TLV_PL] > 10)
				var/list/new_gases = list(TLV_FREON = scaled_production * 0.15, TLV_NITRIUM = scaled_production * 1.05)
				internal_output.adjust_multiple_gases(new_gases)
				moderator_internal.adjust_gas(TLV_PL, min(moderator_internal.toxins(), scaled_production * 0.45))
			if(moderator_list[TLV_FREON] > 50)
				heat_output *= 0.9
				radiation *= 0.8
			if(moderator_list[TLV_PROTO_NITRATE]> 15)
				var/list/new_gases = list(TLV_NITRIUM = scaled_production * 1.25, TLV_HALON = scaled_production * 1.15)
				internal_output.adjust_multiple_gases(new_gases)
				moderator_internal.adjust_gas(TLV_PROTO_NITRATE, min(moderator_internal.proto_nitrate(), scaled_production * 1.55))
				radiation *= 1.95
				heat_output *= 1.25
			if(moderator_list[TLV_BZ] > 100)
				var/list/new_gases = list(TLV_HEALIUM = scaled_production * 1.5, TLV_PROTO_NITRATE = scaled_production * 1.5)
				internal_output.adjust_multiple_gases(new_gases)
				visible_hallucination_pulse(src, HALLUCINATION_HFR(heat_output), 100 SECONDS * power_level * seconds_per_tick)

		if(5)
			if(moderator_list[TLV_PL] > 15)
				internal_output.adjust_gas(TLV_FREON, scaled_production *0.25)
				moderator_internal.adjust_gas(TLV_PL, min(moderator_internal.toxins(), scaled_production * 1.45))
			if(moderator_list[TLV_FREON] > 500)
				heat_output *= 0.5
				radiation *= 0.2
			if(moderator_list[TLV_PROTO_NITRATE] > 50)
				var/list/new_gases = list(TLV_NITRIUM = scaled_production * 1.95, TLV_PLUOXIUM = scaled_production)
				internal_output.adjust_multiple_gases(new_gases)
				moderator_internal.adjust_gas(TLV_PROTO_NITRATE, min(moderator_internal.proto_nitrate(), scaled_production * 1.35))
				radiation *= 1.95
				heat_output *= 1.25
			if(moderator_list[TLV_BZ] > 100)
				var/list/new_gases = list(TLV_HEALIUM = scaled_production, TLV_FREON = scaled_production * 1.15)
				internal_output.adjust_multiple_gases(new_gases)
				visible_hallucination_pulse(src, HALLUCINATION_HFR(heat_output), 100 SECONDS * power_level * seconds_per_tick)
			if(moderator_list[TLV_HEALIUM] > 100)
				if(critical_threshold_proximity > 400)
					critical_threshold_proximity = max(critical_threshold_proximity - (moderator_list[TLV_HEALIUM] / 100 * seconds_per_tick ), 0)
					moderator_internal.adjust_gas(TLV_HEALIUM, -min(moderator_internal.healium(), scaled_production * 20))
			if(moderator_internal.temperature() < 1e7 || (moderator_list[TLV_PL] > 100 && moderator_list[TLV_BZ] > 50))
				internal_output.adjust_gas(TLV_ANTINOBLIUM, dirty_production_rate * 0.9 / 0.065 * seconds_per_tick)
		if(6)
			if(moderator_list[TLV_PL] > 30)
				internal_output.adjust_gas(TLV_BZ, scaled_production * 1.15)
				moderator_internal.adjust_gas(TLV_PL, -min(moderator_internal.toxins(), scaled_production * 1.45))
			if(moderator_list[TLV_PROTO_NITRATE])
				var/list/new_gases = list(TLV_ZAUKER = scaled_production * 5.35, TLV_NITRIUM = scaled_production * 2.15)
				internal_output.adjust_multiple_gases(new_gases)
				moderator_internal.adjust_gas(TLV_PROTO_NITRATE, -min(moderator_internal.proto_nitrate(), scaled_production * 3.35))
				radiation *= 2
				heat_output *= 2.25
			if(moderator_list[TLV_BZ])
				visible_hallucination_pulse(src, HALLUCINATION_HFR(heat_output), 100 SECONDS * power_level * seconds_per_tick)
				internal_output.adjust_gas(TLV_ANTINOBLIUM, clamp(dirty_production_rate / 0.045, 0, 10) * seconds_per_tick)
			if(moderator_list[TLV_HEALIUM] > 100)
				if(critical_threshold_proximity > 400)
					critical_threshold_proximity = max(critical_threshold_proximity - (moderator_list[TLV_HEALIUM] / 100 * seconds_per_tick ), 0)
					moderator_internal.adjust_gas(TLV_HEALIUM, -min(moderator_internal.healium(), scaled_production * 20))
			internal_fusion.adjust_gas(TLV_ANTINOBLIUM, dirty_production_rate * 0.01 / 0.095 * seconds_per_tick)
	...
	if(internal_fusion.temperature() <= FUSION_MAXIMUM_TEMPERATURE * temperature_modifier)
		...
		internal_fusion.set_temperature(internal_fusion.temperature() - heat_limiter_modifier * 0.01 * seconds_per_tick)

	//heat up and output what's in the internal_output into the linked_output port
	if(internal_output.total_moles() > 0)
		if(moderator_internal.total_moles() > 0)
			internal_output.set_temperature(moderator_internal.temperature() * HIGH_EFFICIENCY_CONDUCTIVITY)
		else
			internal_output.set_temperature(internal_fusion.temperature() * METALLIC_VOID_CONDUCTIVITY)
		linked_output.air_contents.merge(internal_output)
	...
	if(moderator_list[TLV_O2] > 150)
		if(iron_content > 0)
			...
			moderator_internal.adjust_gas(TLV_O2, -iron_removed * OXYGEN_MOLES_CONSUMED_PER_IRON_HEAL)

	check_gravity_pulse(seconds_per_tick)

	radiation_pulse(src, max_range = 6, threshold = 0.3)

/obj/machinery/atmospherics/unary/hypertorus/core/proc/evaporate_moderator(seconds_per_tick)
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/process_damageheal(seconds_per_tick)
	...
	if(internal_fusion.total_moles() > 0 && (air_contents.total_moles() && coolant_temperature < HYPERTORUS_COLD_COOLANT_THRESHOLD) && power_level <= 4)
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_nuclear_particles(moderator_list)
	// New nuclear particle emission sytem.
	if(power_level < 4)
		return
	if(moderator_list[TLV_BZ] < (150 / power_level))
		return
	var/obj/machinery/hypertorus/corner/picked_corner = pick(corners)
	INVOKE_ASYNC(picked_corner.loc, TYPE_PROC_REF(/atom, fire_nuclear_particle), REVERSE_DIR(picked_corner.dir))

/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_lightning_arcs(moderator_list)
	if(power_level < 4)
		return
	if(moderator_list[TLV_ANTINOBLIUM] <= 50 && critical_threshold_proximity <= 500)
		return
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_gravity_pulse(seconds_per_tick)
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/remove_waste(seconds_per_tick)
	//Gases can be removed from the moderator internal by using the interface.
	if(!waste_remove)
		return
	var/filtering_amount = moderator_scrubbing.len
	var/list/scrub_list = moderator_internal.get_interesting() & moderator_scrubbing
	for(var/gas_tlv in scrub_list)
		var/datum/gas_mixture/removed = moderator_internal.remove_specific(gas_tlv, (moderator_filtering_rate / filtering_amount) * seconds_per_tick)
		if(removed)
			linked_output.air_contents.merge(removed)

	if(selected_fuel)
		var/datum/gas_mixture/internal_remove
		var/list/interesting_gasses = internal_fusion.get_interesting()
		for(var/gas_tlv in selected_fuel.primary_products)
			var/amount = interesting_gasses[gas_tlv]
			if(amount > 0)
...
/obj/machinery/atmospherics/unary/hypertorus/core/proc/process_internal_cooling(seconds_per_tick)
	if(moderator_internal.total_moles() > 0 && internal_fusion.total_moles() > 0)
		...
		internal_fusion.set_temperature(max(internal_fusion.temperature() - fusion_heat_amount / internal_fusion.heat_capacity(), TCMB))
		moderator_internal.set_temperature(max(moderator_internal.temperature() + fusion_heat_amount / moderator_internal.heat_capacity(), TCMB))

	if(air_contents.total_moles() * 0.05 <= MINIMUM_MOLE_COUNT)
		return
	var/datum/gas_mixture/cooling_port = air_contents
	...
	if(moderator_internal.total_moles() > 0)
		...
		cooling_remove.set_temperature(max(cooling_remove.temperature() - cooling_heat_amount / cooling_remove.heat_capacity(), TCMB))
		moderator_internal.set_temperature(max(moderator_internal.temperature() + cooling_heat_amount / moderator_internal.heat_capacity(), TCMB))
		...
		cooling_remove.set_temperature(max(cooling_remove.temperature() - cooling_heat_amount / cooling_remove.heat_capacity(), TCMB))
		internal_fusion.set_temperature(max(internal_fusion.temperature() + cooling_heat_amount / internal_fusion.heat_capacity(), TCMB))
	cooling_port.merge(cooling_remove)

/obj/machinery/atmospherics/unary/hypertorus/core/proc/inject_from_side_components(seconds_per_tick)
	update_pipenets()

	//Check and stores the gases from the moderator input in the moderator internal gasmix
	var/datum/gas_mixture/moderator_port = linked_moderator.air_contents
	...
	var/datum/gas_mixture/fuel_port = linked_input.air_contents
	for(var/gas_tlv in selected_fuel.requirements)
		internal_fusion.merge(fuel_port.remove_specific(gas_tlv, fuel_injection_rate * seconds_per_tick / length(selected_fuel.requirements)))
		linked_input.parent?.update = TRUE

/obj/machinery/atmospherics/unary/hypertorus/core/proc/check_deconstructable()
	if(!active)
		return
	if(power_level > 0)
		fusion_started = TRUE
