 ...
 */
/obj/machinery/atmospherics/binary/proc/crowbar_deconstruction_act(mob/living/user, obj/item/tool, internal_pressure = 0)
	if(!panel_open)
	...
	var/datum/gas_mixture/inside_air = air1
	if(inside_air.total_moles() > 0 || internal_pressure)
		filled_pipe = TRUE
	if(!node1 || (istype(node1, /obj/machinery/atmospherics/unary/portables_connector)))
		internal_pressure = internal_pressure > air1.return_pressure() ? internal_pressure : air1.return_pressure()

	inside_air = air2
	if(inside_air.total_moles() > 0 || internal_pressure)
		filled_pipe = TRUE
	if(!node2 || (istype(node2, /obj/machinery/atmospherics/unary/portables_connector)))
		internal_pressure = internal_pressure > air2.return_pressure() ? internal_pressure : air2.return_pressure()
