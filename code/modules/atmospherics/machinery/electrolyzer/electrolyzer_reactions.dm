...
/datum/electrolyzer_reaction/h2o_conversion/reaction_check(datum/gas_mixture/air_mixture, list/electrolyzer_args = list())
	if(air_mixture.water_vapor() < MINIMUM_MOLE_COUNT)
		return FALSE
	. = ..()
...
/datum/electrolyzer_reaction/h2o_conversion/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	...
	var/proportion = min(air_mixture.water_vapor() * INVERSE(2), (2.5 * (POW2(working_power))))
	air_mixture.set_water_vapor(air_mixture.water_vapor() - proportion * 2)
	air_mixture.set_oxygen(air_mixture.oxygen() + proportion)
	air_mixture.set_hydrogen(air_mixture.hydrogen() + proportion * 2)
	var/new_heat_capacity = air_mixture.heat_capacity()
	if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
		air_mixture.set_temperature(max(air_mixture.temperature() * old_heat_capacity / new_heat_capacity, TCMB))
...
/datum/electrolyzer_reaction/nob_conversion/reaction_check(datum/gas_mixture/air_mixture, list/electrolyzer_args = list())
	if(air_mixture.hypernoblium() < MINIMUM_MOLE_COUNT)
		return FALSE
...
/datum/electrolyzer_reaction/nob_conversion/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	...
	var/old_heat_capacity = air_mixture.heat_capacity()
	var/electrolysed = air_mixture.hypernoblium() * clamp(supermatter_power - POWER_PENALTY_THRESHOLD, 0, CRITICAL_POWER_PENALTY_THRESHOLD - POWER_PENALTY_THRESHOLD) / (CRITICAL_POWER_PENALTY_THRESHOLD - POWER_PENALTY_THRESHOLD)

	air_mixture.set_hypernoblium(air_mixture.hypernoblium() - electrolysed)
	air_mixture.set_antinoblium(air_mixture.antinoblium() + electrolysed)
	var/new_heat_capacity = old_heat_capacity + electrolysed * (SPECIFIC_HEAT_ANTINOBLIUM - SPECIFIC_HEAT_HYPER_NOBLIUM)
	if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
		air_mixture.set_temperature(max(air_mixture.temperature() * old_heat_capacity / new_heat_capacity, TCMB))
...
/datum/electrolyzer_reaction/halon_generation/reaction_check(datum/gas_mixture/air_mixture, list/electrolyzer_args)
	if(air_mixture.bz() < MINIMUM_MOLE_COUNT)
		return FALSE
	. = ..()
...
/datum/electrolyzer_reaction/halon_generation/react(datum/gas_mixture/air_mixture, working_power, list/electrolyzer_args = list())
	var/old_heat_capacity = air_mixture.heat_capacity()
	var/bz = air_mixture.bz()
	var/temperature = air_mixture.temperature()
	var/reaction_efficency = min(bz * (1 - NUM_E ** (-0.5 * temperature * working_power / FIRE_MINIMUM_TEMPERATURE_TO_EXIST)), bz)
	air_mixture.set_bz(bz - reaction_efficency)
	air_mixture.set_oxygen(air_mixture.oxygen() + reaction_efficency * 0.2)
	air_mixture.set_halon(air_mixture.halon() + reaction_efficency * 2)
	var/energy_used = reaction_efficency * HALON_FORMATION_ENERGY
	var/new_heat_capacity = air_mixture.heat_capacity()
	if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
		air_mixture.set_temperature(max(((temperature * old_heat_capacity + energy_used) / new_heat_capacity), TCMB))
