...
/datum/action/changeling/augmented_eyesight/on_purchase(mob/user, /datum/antagonist/changeling/antag)
	...
	var/obj/item/organ/internal/cyberimp/eyes/shield/ling/eyes = new(user)
	eyes.insert(user)
	active = FALSE
...
/datum/action/changeling/augmented_eyesight/sting_action(mob/living/carbon/user)
	...
	if(active)
		to_chat(user, span_notice("We adjust our eyes to protect them from bright lights."))
		eyes = new /obj/item/organ/internal/cyberimp/eyes/shield/ling(user)
		eyes.insert(user)
		active = FALSE
	else
		to_chat(user, span_notice("We adjust our eyes to sense prey through walls."))
		eyes = new /obj/item/organ/internal/cyberimp/eyes/thermals/ling(user)
		eyes.insert(user)
		active = TRUE
...
/obj/item/organ/internal/cyberimp/eyes/shield/ling/on_life()
	...
	if(owner.eye_blind || owner.eye_blurry || (eyes?.damage > 0))
