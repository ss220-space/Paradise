...
/datum/antagonist/blob_overmind/add_owner_to_gamemode()
	...
	if(add_to_mode && mode && !(owner in mode.blob_infected))
		...
		if(is_offspring)
			mode.blob_offsprings += owner
		else mode.blob_infected += owner
...
/proc/get_blob_help_messages(datum/reagent/blob/blob_reagent_datum)
	var/list/messages = list()
	messages += "<b>Как надразум, вы можете контролировать блоба!</b>"
	...
	messages += "<i>Обычная плитка</i> будет расширять ваше влияние и может быть улучшена до специальной плитки Блоба, выполняющей определённую функцию."
	messages += "<b>Вы можете улучшить обычные плитки блоба до следующих типов:</b>"
	...
	messages += "<i>Ресурсная плитка</i> это плитка, которая производит больше ресурсов для вас, стройте как можно больше таких, чтобы поглотить станцию. Этот тип плиток должен быть размещен рядом с узлами или вашим ядром, чтобы работать."
	messages += "<i>Фабрика</i> это плитка, которая порождает споры Блоба, атакующие ближайших врагов. Этот тип плиток должен быть размещен рядом с Узлами или вашим ядром, чтобы работать."
	...
	messages += "<b>Shortcuts:</b> ЛКМ = установить простую плитку <b>|</b> CTRL + ЛКМ = Улучшить плитку <b>|</b> СКМ = Указать цель спорам <b>|</b> Alt + ЛКМ = Удалить плитку"
