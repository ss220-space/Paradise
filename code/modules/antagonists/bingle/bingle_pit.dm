...
/obj/structure/bingle_hole/examine(mob/user)
	...
	. += span_alert("Внутри находится <b>[item_value_consumed]</b> предмет[DECL_A_OV(item_value_consumed)]!")
	. += span_notice("Существа смогут упасть туда, если в яме будет минимум <b>[BINGLE_PIT_GROW_VALUE]</b> предмет[DECL_A_OV(BINGLE_PIT_GROW_VALUE)]!")
...
/obj/structure/bingle_pit_overlay/examine(mob/user)
	...
	if(parent_pit && isbingle(user))
		. += span_alert("Внутри находится <b>[parent_pit.item_value_consumed]</b> предмет[DECL_A_OV(parent_pit.item_value_consumed)]!")
		. += span_notice("Существа смогут упасть туда, если в яме будет минимум <b>[BINGLE_PIT_GROW_VALUE]</b> предмет[DECL_A_OV(BINGLE_PIT_GROW_VALUE)]!")
