...
/obj/effect/proc_holder/spell/vampire/thrall_commune/cast(list/targets, mob/user)
	...
	var/title = is_thrall ? "(Раб Вампира) [user.real_name]" : "[span_dantalion(span_fontsize_3("(Мастер Вампир) [user.real_name]"))]"
	var/message = is_thrall ? "[span_dantalion(input)]" : "[span_dantalion(span_fontsize_3("<b>[input]</b>"))]"
