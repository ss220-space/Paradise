...
/obj/effect/proc_holder/spell/vampire/blood_tendrils/proc/apply_slowdown(turf/T, distance, slowed_amount, mob/user)
	for(var/mob/living/living in range(distance, T))
		if(living.affects_vampire(user))
			living.Slowed(slowed_amount)
			living.apply_damage(33, TOX)
			living.visible_message(span_warning("[living] опутыва[PLUR_ET_YUT(living)]ся кровавыми щупальцами, которые ограничивают [GEND_HIS_HER(living)] движение!"))
			var/turf/target_turf = get_turf(living)
