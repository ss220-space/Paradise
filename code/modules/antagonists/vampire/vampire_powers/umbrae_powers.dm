...
/obj/effect/proc_holder/spell/vampire/shadow_snare
	...
	desc = "Вы вызываете ловушку на земле. Когда ее пересекут, она ослепит цель, погасит все имеющиеся у нее источники света и захватит ее в капкан."
...
/obj/effect/proc_holder/spell/vampire/soul_anchor
	name = "Теневой якорь"
	desc = "Вы вызываете затемненный якорь после задержки, повторное заклинание телепортирует вас обратно к якорю. Вы будете вынуждены вернуться назад через 2 минуты, если не произнесли повторное заклинание."
	gain_desc = "Вы получили способность сохранять точку в пространстве и телепортироваться в нее по своему желанию. Если в течение 2 минут вы добровольно не телепортируетесь обратно в эту точку, вы будете вынуждены вернуться."
	required_blood = 30
