...
/datum/objective/spider_get_power/alife_spiders/generate_text()
	. = ..()
	explanation_text = "Расплодитесь. Для того, чтобы вы могли отложить яйцо Императрицы, в вашем гнезде должно быть не менее [targets_need] паук[DECL_A_OV_OV(targets_need)]."
...
/datum/objective/spider_get_power/spider_infections/generate_text()
	. = ..()
	explanation_text = "Заражайте. Для того, чтобы вы могли отложить яйцо Императрицы, должно быть заражено не менее [targets_need] гуманоид[DECL_A_OV_OV(targets_need)]."
