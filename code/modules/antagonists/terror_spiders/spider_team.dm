...
/datum/team/terror_spiders/proc/declare_results()
	if(SSticker?.mode?.station_was_nuked && !terror_stage == TERROR_STAGE_POST_END)
		to_chat(world, span_fontsize_3("<br><b>Частичная победа Пауков Ужаса!</b>"))
		...
		to_chat(world, span_fontsize_3("<br><b>Полная победа Пауков Ужаса!</b>"))
...
/datum/team/terror_spiders/declare_completion()
	...
	if(terror_queens.len || terror_princes.len || terror_princesses.len || terror_defilers.len)
		declare_results()
		var/text = span_fontsize_2("<br/><b>Основа гнезда:</b>")
		if(terror_queens.len)
			text += span_fontsize_1("<br/><b>Королев[(terror_queens?.len > 1 ? "ами были" : "ой был")]:</b>")
			for(var/datum/mind/spider in terror_queens)
				text += "<br/><b>[spider.key]</b> был <b>[spider.name]</b>"
		if(terror_princes.len)
			text += span_fontsize_1("<br/><b>Принц[(terror_queens?.len > 1 ? "ами были" : "ем был")]:</b>")
			for(var/datum/mind/spider in terror_princes)
				text += "<br/><b>[spider.key]</b> был <b>[spider.name]</b>"
		if(terror_princesses.len)
			text += span_fontsize_1("<br/><b>Принцесс[(terror_queens?.len > 1 ? "ами были" : "ой был")]:</b>")
			for(var/datum/mind/spider in terror_princesses)
				text += "<br/><b>[spider.key]</b> был <b>[spider.name]</b>"
		if(terror_defilers.len)
			text += span_fontsize_1("<br/><b>Осквернител[(terror_queens?.len > 1 ? "ями были" : "ем был")]:</b>")
			for(var/datum/mind/spider in terror_defilers)
				text += "<br/><b>[spider.key]</b> был <b>[spider.name]</b>"
		if(members.len)
			text += span_fontsize_2("<br/><b>Паук[(members?.len > 1 ? "ами Ужаса были" : "ом Ужаса был")]:</b>")
