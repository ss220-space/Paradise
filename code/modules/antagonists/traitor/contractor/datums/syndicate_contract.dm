...
/datum/syndicate_contract/proc/start_extraction_process(obj/item/contractor_uplink/uplink, mob/living/carbon/human/contractor)
	...
	contractor.visible_message(
		span_notice("[contractor] начинает вводить загадочную серию символов в [uplink.declent_ru(ACCUSATIVE)]..."),\
	...
	contractor.visible_message(span_notice("[contractor] вводит таинственный код в [uplink.declent_ru(ACCUSATIVE)] и достаёт \
						чёрно-золотую сигнальную ракету, после чего зажигает её."),\
...
/datum/syndicate_contract/proc/handle_target_return(mob/living/victim)
	...
	victim.visible_message(span_notice("[capitalize(victim.declent_ru(NOMINATIVE))] исчезает..."))
