...
/datum/contractor_hub/proc/on_completion(tc, creds)
	...
	owner?.initial_account?.credit(creds, pick(list(
		"ПОЗДРАВЛЯЕМ! Вы стали 10 000-м посетителем сайта SquishySlimes.squish! Прилагаем ваши [creds] кредит[DECL_0_A_OV(creds)].",
		"Поздравляем вас с выигрышем ставки в последнем матче \"Клоун против Мима\"! На ваш счёт было начислено [creds] кредит[DECL_0_A_OV(creds)].",
		"Дорогой бенефициар фонда, сообщаем вам, что оплата по просроченному платежу наконец-то завершена, и на ваш счёт зачислено [creds] кредит[DECL_0_A_OV(creds)].",
