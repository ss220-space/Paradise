...
/datum/antagonist/traitor/proc/assign_exchange_role(datum/mind/exchange_role)
	...
	to_chat(mob, span_info("<br><br>[where] is a folder containing <b>secret documents</b> that another Syndicate group wants. We have set up a meeting with one of their agents on station to make an exchange. Exercise extreme caution as they cannot be trusted and may be hostile.<br>"))
	mob.update_icons()
