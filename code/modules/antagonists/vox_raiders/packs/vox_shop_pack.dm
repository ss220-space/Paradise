...
/datum/vox_pack/proc/get_time_available()
	var/time = SSticker.time_game_started + time_until_available MINUTES
	return "[round(time / 36000)]:[add_zero(num2text(time / 600 % 60), 2)]"

/datum/vox_pack/proc/get_time_left()
	var/time = SSticker.time_game_started + time_until_available MINUTES - ROUND_TIME
	return "[round(time / 36000)]:[add_zero(num2text(time / 600 % 60), 2)]:[add_zero(num2text(time / 10 % 60), 2)]"
