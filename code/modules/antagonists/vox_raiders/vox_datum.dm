...
/datum/antagonist/vox_raider/greet()
	...
	. += {"Вы Вокс Рейдер
		...
		Используй '[get_language_prefix(LANGUAGE_VOX)]' для общения на воксском, :v для разговора по вашему зашифрованному каналу и не забудь включить подачу азота для дыхания!
