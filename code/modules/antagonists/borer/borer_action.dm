...
/datum/action/innate/borer/make_larvae/Activate()
	...
	if(borer.chemicals < cost)
		to_chat(borer.host, "Вам требуется [cost] химикат[DECL___A_OV(cost)] для размножения!")
		return
...
/datum/action/innate/borer/torment/Activate()
	...
	if(borer.chemicals < total_cost)
		to_chat(owner, "Вам требуется [total_cost] химикат[DECL___A_OV(total_cost)] для вызова психической агонии!")
		return
