...
/obj/item/card/id/advanced/heretic/examine(mob/user)
	...
	. += span_purple("Благословлена Обителью")
	...
	. += span_purple("<b>Альт-клик по карте</b> заставляет идентификатор создавать инвертированные порталы, которые будут телепортировать вас в случайный шлюз на станции, в то время как язычники телепортируются в пункт назначения.")
