...
/datum/antagonist/soultrapped_heretic/on_gain()
	...
	if(owner)
		to_chat(owner, span_ghostalert("Вы — запертая душа еретика, которым вы когда-то были. Вы можете попытаться убедить своих владельцев освободить вас, \
									...
									являетесь его членом и сохранили то, что осталось от вашей свободы воли. Помимо этого, вы мало что можете сделать, \
									кроме как наблюдать. Постарайтесь не попасть в ловушку со шкафом."))
		log_game("[key_name_log(owner)] был принесен в жертву кровавым культом как еретик и запечатан внутри меча.")
