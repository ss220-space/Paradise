...
/datum/heretic_knowledge_tree_column/main/beyond
	...
	path_pros = list(
		...
		"Три Runtime Error крашат цель: паралич, немота и сорванное действие.",
	)
	path_cons = list(
	...
	path_tips = list(
		...
		"\"Просмотр Переменных\" даёт добить цель через stat = UNCONSCIOUS, но только если на ней уже висят две ошибки.",
	)
...
/datum/heretic_knowledge/blade_upgrade/beyond
	...
	desc = "Ваш клинок начинает повреждать связь вещей с миром. Успешный удар по живой цели накладывает \
			Runtime Error, но не чаще раза в четыре секунды на одну и ту же жертву. \
...
/datum/heretic_knowledge/spell/view_variables
	...
	desc = "Даёт вам \"Просмотр Переменных\", заклинание, позволяющее временно переписать одно свойство цели: \
			...
			Если на цели уже висят две ошибки, вместо этого можно выключить ей сознание."
