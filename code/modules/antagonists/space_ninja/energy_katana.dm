...
/obj/item/melee/energy_katana/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] пронза[PLUR_ET_UT(user)] свой живот с помощью [src]! Похоже, [GEND_HE_SHE(user)] пыта[pluralize_ru(user.gender,"ется","ются")] совершить сеппуку!"))
	return BRUTELOSS
