/obj/item/clothing/suit/space/space_ninja/attackby(obj/item/I, mob/ninja, params)
	...
	if(istype(I, /obj/item/stack/sheet/mineral/uranium))
		...
		if(!uranium_stack.use(a_transfer))
			to_chat(ninja, span_warning("Вам нужно как минимум [a_transfer] лист[DECL_0_A_OV(a_transfer)] урана для восполнения запаса адреналина."))
			return ATTACK_CHAIN_PROCEED
	...
	if(isbluespacecrystal(I))
		...
		if(!crystal_stack.use(a_transfer))
			to_chat(ninja, span_warning("Вам нужно как минимум [a_transfer] блюспейс-кристалл[DECL_0_A_OV(a_transfer)] для восполнения запаса химикатов."))
			return ATTACK_CHAIN_PROCEED
