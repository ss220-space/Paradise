...
/obj/item/ninja_chameleon_scanner
	...
	desc = "Спрятанный в костюме ниндзи девайс. Сканирует внешний вид и голос существа, что позволяет ниндзя выдавать себя за него."
...
/obj/item/clothing/suit/space/space_ninja/proc/pick_form(mob/living/carbon/human/ninja)
	if(!disguise && !disguise_active)
		to_chat(ninja, span_warning("Вы ещё никого не сканировали! Используйте выданный вам девайс, чтобы просканировать чужую внешность!"))
		return
...
/obj/item/clothing/suit/space/space_ninja/proc/toggle_chameleon(extra_important_things = TRUE)
	...
	if(extra_important_things)
		...
		if(!ninja.wear_id)
			ninja.equip_to_slot_if_possible(n_id_card, ITEM_SLOT_ID)
		else
			qdel(n_id_card)
			n_id_card = null
		//Sparks
