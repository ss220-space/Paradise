...
/datum/team/xenomorph/declare_completion()
	if(members.len)
		...
		var/text = ""
		if(queens?.len)
			text += "<br/><<span style='font-size: 2;'><b>Королев[(queens.len > 1 ? "ами были" : "ой была")]:</b></spawn>"
			for(var/datum/mind/queen in queens)
				text += "<br/><b>[queen.key]</b> был <b>[queen.name]</b>"
		text += "<br/><<span style='font-size: 2;'><b>Ксеноморф[(members?.len > 1 ? "ами были" : "ом был")]:</b></spawn>"
