...
/mob/living/carbon/true_devil/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		if(INTENT_HARM)
			...
			visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] ударил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"), \
					span_userdanger("[M.declent_ru_cap(NOMINATIVE)] ударил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"))
		...
		if(INTENT_DISARM)
			...
			if(prob(5))
				...
				visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] повалил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"), \
						span_userdanger("[M.declent_ru_cap(NOMINATIVE)] повалил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"))
				return FALSE

			if(!prob(25))
				playsound(loc, 'sound/weapons/punchmiss.ogg', 25, TRUE, -1)
				visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] попытал[GEND_SYA_AS_OS_IS(M)] обезоружить [declent_ru(ACCUSATIVE)]!"))
				return FALSE

			drop_from_active_hand()
			playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, TRUE, -1)
			visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] обезоружил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"), \
			span_userdanger("[M.declent_ru_cap(NOMINATIVE)] обезоружил[GEND_A_O_I(M)] [declent_ru(ACCUSATIVE)]!"))
