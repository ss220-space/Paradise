...
/datum/devil_bane/toolbox
	...
	desc = "Похоже этот дьявол до смерти не любит ящики с инструментами."
...
/datum/devil_bane/toolbox/proc/toolbox_attack(datum/source, obj/item/item, mob/attacker, params)
	...
	attacker.visible_message(
		...
		span_notice("[item.declent_ru_cap(NOMINATIVE)] уничтожа[PLUR_ET_YUT(item)] [owner.declent_ru(ACCUSATIVE)]!"))
...
/datum/devil_bane/whiteclothes/proc/whiteclothes_attack(datum/source, obj/item/item, mob/attacker, params)
	...
	attacker.visible_message(span_warning("[owner.declent_ru_cap(NOMINATIVE)], кажется, получает вред от одежды [attacker.declent_ru(GENITIVE)]."),
...
/datum/devil_bane/silver
	...
	desc = "Похоже, жидкое серебро наносит этому дьяволу серьёзные раны."
	law = "Серебро в жидкой форме станет вашим падением."
