...
/datum/antagonist/mindslave/devil_pawn
	...
	clown_gain_text = "Дьявольская сила помогает вам побороть свою клоунскю натуру, позволяя использовать оружие без вреда для себя."
	clown_removal_text = "Длявольская сила покинула вас и к вам вернулась клоунская неуклюжесть, клоуничайте на здоровье."
...
/datum/antagonist/mindslave/devil_pawn/greet()
	...
	var/greet_text = "<b>Вы прдверглись дьявольскому воздействию [master.current.real_name], известному в аду как [devil.info.truename]. Следуйте каждому [genderize_ru(master.current.gender, "его", "её", "его", "их")] приказу.</b>"
	return span_dangerbigger(greet_text)

/datum/antagonist/mindslave/devil_pawn/farewell()
	if(issilicon(owner.current))
		to_chat(owner.current, span_userdanger("Вы превратились в робота! Вы больше не подвержены воздействию…"))
	else
		to_chat(owner.current, span_userdanger("Ваш разум очищен! Вы больше не подвержены воздействию."))
