/// This is intended to replace /icon, allowing rustlib to generate icons much faster than DM can at scale.
...
/datum/icon_transformer/proc/blend_icon(datum/universal_icon/icon_object, blend_mode, x = 1, y = 1)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_BLEND_ICON, "icon" = icon_object, "blend_mode" = blend_mode, "x" = x, "y" = y))
...
/datum/icon_transformer/proc/scale(width, height)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_SCALE, "width" = width, "height" = height))
...
/datum/icon_transformer/proc/crop(x1, y1, x2, y2)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_CROP, "x1" = x1, "y1" = y1, "x2" = x2, "y2" = y2))
...
/datum/icon_transformer/proc/flip(dir)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_FLIP, "dir" = dir))
...
/datum/icon_transformer/proc/rotate(angle)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_TURN, "angle" = angle))
...
/datum/icon_transformer/proc/shift(dir, offset, wrap=FALSE)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_SHIFT, "dir" = dir, "offset" = offset, "wrap" = wrap))
...
/datum/icon_transformer/proc/swap_color(src_color, dst_color)
	...
	#endif
	transforms += list(list("type" = RUSTLIB_ICONFORGE_SWAP_COLOR, "src_color" = src_color, "dst_color" = dst_color))
...
/datum/icon_transformer/proc/to_list()
	...
	for(var/list/transform as anything in transforms_original)
		...
		if(transform["type"] == RUSTLIB_ICONFORGE_BLEND_ICON)
