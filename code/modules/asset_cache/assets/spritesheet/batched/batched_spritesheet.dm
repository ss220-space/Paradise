...
/datum/asset/spritesheet_batched/should_refresh(yield)
	...
	if(isnull(cache_data) || isnull(cache_dmi_hashes_json))
		cache_data = rustlib_file_read("[ASSET_CROSS_ROUND_SMART_CACHE_DIRECTORY]/spritesheet_cache.[name].json")
...
/datum/asset/spritesheet_batched/proc/realize_spritesheets(yield)
	...
	for(var/size_id in sizes)
		...
		var/file_hash = rustlib_hash_file(RUSTLIB_HASH_MD5, file_directory)
	...
	rustlib_file_write(css, file_directory)
	var/css_hash = rustlib_hash_string(RUSTLIB_HASH_MD5, css)
...
/datum/asset/spritesheet_batched/proc/read_from_cache()
	...
	var/css_hash = rustlib_hash_file(RUSTLIB_HASH_MD5, css_file_directory)
	SSassets.transport.register_asset(css_name, fcopy_rsc(css_file_directory), file_hash=css_hash)
	for(var/size_id in sizes)
		var/fname = "data/spritesheets/[name]_[size_id].png"
		var/hash = rustlib_hash_file(RUSTLIB_HASH_MD5, fname)
...
/datum/asset/spritesheet_batched/proc/write_cache_meta(input_hash, dmi_hashes)
	...
	rustlib_file_write(json_encode(cache_data), "[ASSET_CROSS_ROUND_SMART_CACHE_DIRECTORY]/spritesheet_cache.[name].json")
