...
/datum/asset/spritesheet/proc/ensure_stripped(sizes_to_strip = sizes)
	for(var/size_id in sizes_to_strip)
		...
		var/error = rustlib_dmi_strip_metadata(file_directory)
		if(length(error))
...
/datum/asset/spritesheet/proc/read_css_from_cache()
	...
	while(find_background_urls.Find(replaced_css))
		...
		var/hash = rustlib_hash_file(RUSTLIB_HASH_MD5, file_path)
	...
	var/css_hash = rustlib_hash_string(RUSTLIB_HASH_MD5, replaced_css)
	rustlib_file_write(replaced_css, replaced_css_filename)
...
/datum/asset/spritesheet/proc/write_css_to_cache()
	...
	rustlib_file_write(mock_css, css_cache_filename())

/datum/asset/spritesheet/proc/write_data_to_cache()
	rustlib_file_write(json_encode(list(
		"sprites" = sprites,
	)), data_cache_filename())
