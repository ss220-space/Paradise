...
/datum/asset/spritesheet_batched/chem_master/create_spritesheets()
	...
	for(var/pill_type = 1 to 20)
		insert_icon("pill[pill_type]", uni_icon('icons/obj/medical/chemical.dmi', "pill[pill_type]", transform = transform))
	for(var/bottle_type in list("bottle", "wide_bottle", "round_bottle", "reagent_bottle"))
		insert_icon(bottle_type, uni_icon('icons/obj/medical/chemical.dmi', bottle_type, transform = transform))
	for(var/i = 1 to 20)
		insert_icon("bandaid[i]", uni_icon('icons/obj/medical/chemical.dmi', "bandaid[i]", transform = transform))
