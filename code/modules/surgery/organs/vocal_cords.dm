...
/datum/action/item_action/organ_action/use/adamantine_vocal_cords/Trigger(left_click = TRUE, attack_self = TRUE)
	if(!IsAvailable())
		return
...
/datum/action/item_action/organ_action/colossus/Trigger(left_click = TRUE, attack_self = TRUE)
	. = ..()
	if(!IsAvailable())
		if(world.time < cords.next_command)
