...
/obj/item/organ/internal/lungs/proc/check_breath(datum/gas_mixture/breath, mob/living/carbon/human/H)
	...
	if(safe_oxygen_max)
		if(O2_pp > safe_oxygen_max)
			var/ratio = (gases[GAS_O2][MOLES] / safe_oxygen_max / safe_oxygen_max) * 10
	...
	if(safe_oxygen_min)
		if(O2_pp < safe_oxygen_min)
			gas_breathed = handle_too_little_breath(H, O2_pp, safe_oxygen_min, gases[GAS_O2][MOLES])
			H.throw_alert(ALERT_NOT_ENOUGH_OXYGEN, /atom/movable/screen/alert/not_enough_oxy)
		else
			H.heal_damage_type(HUMAN_MAX_OXYLOSS, OXY)
			gas_breathed = gases[GAS_O2][MOLES]
			H.clear_alert(ALERT_NOT_ENOUGH_OXYGEN)

	//Exhale
	gases[GAS_O2][MOLES] = max(gases[GAS_O2][MOLES] - gas_breathed, 0)
	gases[GAS_CO2][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(safe_nitro_max)
		if(N2_pp > safe_nitro_max)
			var/ratio = (gases[GAS_N2][MOLES] / safe_nitro_max) * 10
	...
	if(safe_nitro_min)
		if(N2_pp < safe_nitro_min)
			gas_breathed = handle_too_little_breath(H, N2_pp, safe_nitro_min, gases[GAS_N2][MOLES])
			H.throw_alert(ALERT_NOT_ENOUGH_NITRO, /atom/movable/screen/alert/not_enough_nitro)
		else
			H.heal_damage_type(HUMAN_MAX_OXYLOSS, OXY)
			gas_breathed = gases[GAS_N2][MOLES]
			H.clear_alert(ALERT_NOT_ENOUGH_NITRO)

	//Exhale
	gases[GAS_N2][MOLES]-= gas_breathed
	gases[GAS_CO2][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(safe_co2_min)
		if(CO2_pp < safe_co2_min)
			gas_breathed = handle_too_little_breath(H, CO2_pp, safe_co2_min, gases[GAS_CO2][MOLES])
			H.throw_alert(ALERT_NOT_ENOUGH_CO2, /atom/movable/screen/alert/not_enough_co2)
		else
			H.adjustOxyLoss(-HUMAN_MAX_OXYLOSS)
			gas_breathed = gases[GAS_CO2][MOLES]
			H.clear_alert(ALERT_NOT_ENOUGH_CO2)

	//Exhale
	gases[GAS_CO2][MOLES] -= gas_breathed
	gases[GAS_O2][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(safe_toxins_max)
		if(Toxins_pp > safe_toxins_max)
			var/ratio = (gases[GAS_PL][MOLES] / safe_toxins_max) * 10
	...
	if(safe_toxins_min)
		if(Toxins_pp < safe_toxins_min)
			gas_breathed = handle_too_little_breath(H, Toxins_pp, safe_toxins_min, gases[GAS_PL][MOLES])
			H.throw_alert(ALERT_NOT_ENOUGH_TOX, /atom/movable/screen/alert/not_enough_tox)
		else
			H.heal_damage_type(HUMAN_MAX_OXYLOSS, OXY)
			gas_breathed = gases[GAS_PL][MOLES]
			H.clear_alert(ALERT_NOT_ENOUGH_TOX)

	//Exhale
	gases[GAS_PL][MOLES] -= gas_breathed
	gases[GAS_CO2][MOLES] += gas_breathed
	gas_breathed = 0
	...
	if(gases[GAS_N2O][MOLES])	// If there's some other shit in the air lets deal with it here.
