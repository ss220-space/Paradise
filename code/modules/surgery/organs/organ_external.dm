...
/obj/item/organ/external/remove(mob/living/user, special = ORGAN_MANIPULATION_DEFAULT, ignore_children = FALSE)
	...
	if(is_robotic() && sabotaged && !special)
		organ_owner.visible_message(
			span_danger("[declent_ru_cap(NOMINATIVE)] [organ_owner] взрыва[PLUR_ET_YUT(src)]ся!"),
	...
	if(!silent)
		switch(disintegrate)
			if(DROPLIMB_SHARP)
				if(!clean)
					...
					owner.visible_message(
						span_danger("[declent_ru_cap(NOMINATIVE)] [owner] отрыва[PLUR_ET_YUT(src)]ся!"),
			...
			if(DROPLIMB_BURN)
				...
				owner.visible_message(
					span_danger("[declent_ru_cap(NOMINATIVE)] [owner] испепеля[PLUR_ET_YUT(src)]ся!"),
			...
			if(DROPLIMB_BLUNT)
				...
				owner.visible_message(
					span_danger("[declent_ru_cap(NOMINATIVE)] [owner] отрыва[PLUR_ET_YUT(src)]ся, оставляя после себя [gore]!"),
...
/obj/item/organ/external/proc/disfigure(silent = FALSE)
	...
	if(owner)
		...
		if(!silent)
			owner.visible_message(
				span_warning("[declent_ru_cap(NOMINATIVE)] [owner] превраща[PLUR_ET_YUT(src)]ся в кровавую кашу, издавая тошнотворный звук!"),
