...
/obj/item/organ/external/attackby(obj/item/I, mob/user, params)
	if(is_sharp(I))
		...
		user.visible_message(
			span_warning("[user] начинает вырезать внутренности из [declent_ru(GENITIVE)]."),
		...
		playsound(loc, 'sound/weapons/slice.ogg', 50, TRUE, -1)
		user.visible_message(
			span_warning("[user] вырезает внутренности из [declent_ru(GENITIVE)]."),
...
/obj/item/organ/external/proc/release_restraints(mob/living/carbon/human/holder, silent = FALSE)
	...
	if(holder.handcuffed && (limb_zone in list(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM, BODY_ZONE_PRECISE_L_HAND, BODY_ZONE_PRECISE_R_HAND)))
		if(!silent)
			holder.visible_message(
				span_warning("[capitalize(holder.handcuffed.declent_ru(NOMINATIVE))] спадывает с [holder.name]."),
				span_warning("[capitalize(holder.handcuffed.declent_ru(NOMINATIVE))] спадывает с вас.")
			)
	...
	if(holder.legcuffed && (limb_zone in list(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG, BODY_ZONE_PRECISE_L_FOOT, BODY_ZONE_PRECISE_R_FOOT)))
		if(!silent)
			holder.visible_message(
				span_warning("[capitalize(holder.legcuffed.declent_ru(NOMINATIVE))] спадывает с [holder.name]."),
				span_warning("[capitalize(holder.legcuffed.declent_ru(NOMINATIVE))] спадывает с вас.")
			)
