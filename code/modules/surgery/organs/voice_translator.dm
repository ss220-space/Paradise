...
/obj/item/organ/internal/cyberimp/mouth/translator/proc/turn_languages_on()
	...
	to_chat(owner, span_notice("<font color=green>[declent_ru_cap(NOMINATIVE)] снова работает!</font>"))
	for(var/datum/language/lang as anything in given_languages)
...
/obj/item/organ/internal/cyberimp/mouth/translator/proc/turn_languages_off()
	active = FALSE
	can_wingdings = FALSE
	to_chat(owner, span_warning("[declent_ru_cap(NOMINATIVE)] временно вышел из строя из-за воздействия ЭМИ!"))
