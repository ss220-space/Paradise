...
/datum/surgery_step/extract_implant/begin_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(times_repeated >= max_times_to_check)
		user.visible_message(
				span_notice("[user] перестаёт искать инородные тела в теле [target]."),
	...
	user.visible_message(
		span_notice("[user] начинает искать инородные тела в [affected.declent_ru(PREPOSITIONAL)] [target], используя [tool.declent_ru(ACCUSATIVE)]."),
...
/datum/surgery_step/extract_implant/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message(
		span_warning("[user] ошибочно хватается [tool.declent_ru(INSTRUMENTAL)] за что-то в [affected.declent_ru(PREPOSITIONAL)] [target], повреждая внутренние ткани!"),
...
/datum/surgery_step/extract_implant/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(I && prob(80)) //implant removal only works on the chest.
		user.visible_message(
			span_notice("[user] вынимает [I.declent_ru(ACCUSATIVE)] из [affected.declent_ru(GENITIVE)] [target], используя [tool.declent_ru(ACCUSATIVE)]."),
		...
		if(case && !case.imp)
			...
			case.update_icon()
			user.visible_message(
				span_notice("[user] помещает [I.declent_ru(ACCUSATIVE)] в [case.declent_ru(ACCUSATIVE)]."),
		...
		user.visible_message(
			span_notice("[user] ничего не находит в [affected.declent_ru(PREPOSITIONAL)] [target] и достаёт [tool.declent_ru(ACCUSATIVE)] оттуда."),
