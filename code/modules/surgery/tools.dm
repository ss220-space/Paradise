...
/obj/item/surgicaldrill/suicide_act(mob/living/user)
	user.visible_message(
		span_suicide("[user] наматывает себя на [declent_ru(ACCUSATIVE)]!\n\
		Похоже, что [GEND_HE_SHE(user)] совершает суицид!")
		)
...
/obj/item/scalpel/suicide_act(mob/user)
	to_chat(viewers(user), pick(span_suicide("[user] [declent_ru(INSTRUMENTAL)] срезает свою кожу! Похоже, что [GEND_HE_SHE(user)] совершает суицид!"),
						span_suicide("[user] режет своё горло с помощью [declent_ru(GENITIVE)]! Похоже, что [GEND_HE_SHE(user)] совершает суицид!"),
						span_suicide("[user] вонзает [declent_ru(NOMINATIVE)] в свой желудок! Похоже, что [GEND_HE_SHE(user)] пытается совершить сэппуку!")))
	return BRUTELOSS
