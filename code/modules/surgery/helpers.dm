			...
			if(!current_surgery)
				...
				if(current_surgery.status == 1)
					...
					if(cautery_chance)
						...
						if(do_after(user, C.time * cautery_tool.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = M))
							if(!isrobot(user))
