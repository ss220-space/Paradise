...
/obj/machinery/papershredder/proc/empty_bin(mob/living/user, obj/item/storage/empty_into)
	...
	if(empty_into)
		if(paperamount)
			to_chat(user, span_notice("Вы заполняете [empty_into.declent_ru(ACCUSATIVE)] стольким количеством растерзанной бумаги, сколько [GEND_HE_SHE(empty_into)] мо[pluralize_ru(empty_into.gender, "жет", "гут")] вместить."))
		else
