...
/obj/machinery/photocopier/proc/copy(obj/item/C, scancopy = FALSE)
	...
	if(istype(C, /obj/item/paper))
		...
		to_chat(usr, span_warning("[capitalize(declent_ru(NOMINATIVE))] не способен отсканировать [copyitem.declent_ru(ACCUSATIVE)], [copyitem.declent_ru(NOMINATIVE)] будет извлеч[GEND_ENDING_EN_NA_NO_NY(copyitem)]."))
