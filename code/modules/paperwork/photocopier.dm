...
/obj/machinery/photocopier/proc/copy(obj/item/C, scancopy = FALSE)
	...
	if(istype(C, /obj/item/paper))
		...
		to_chat(usr, span_warning("[capitalize(declent_ru(NOMINATIVE))] не способен отсканировать [copyitem.declent_ru(ACCUSATIVE)], [copyitem.declent_ru(NOMINATIVE)] будет извлечен[GEND_ENDING_A_O_Y(copyitem)]."))
...
/obj/machinery/photocopier/grab_attack(mob/living/grabber, atom/movable/grabbed_thing)
	...
	visible_message(span_warning("[grabber] затаскива[PLUR_ET_UT(grabber)] [grabbed_thing.declent_ru(ACCUSATIVE)] на [declent_ru(ACCUSATIVE)]!"))
...
/obj/machinery/photocopier/MouseDrop_T(mob/target, mob/living/user)
	...
	add_fingerprint(user)
	if(target == user)
		visible_message(span_warning("[user] запрыгива[PLUR_ET_UT(user)] на [declent_ru(ACCUSATIVE)]!"))
	else if(target != user)
		if(target.anchored || !ishuman(user))
			return
		visible_message(span_warning("[user] затаскива[PLUR_ET_UT(user)] [target.declent_ru(ACCUSATIVE)] на [declent_ru(ACCUSATIVE)]!"))
	target.forceMove(get_turf(src))
	copymob = target
	if(copyitem)
		copyitem.forceMove(get_turf(src))
		visible_message(span_notice("[capitalize(copymob.declent_ru(NOMINATIVE))] сталкива[PLUR_ET_UT(user)] [copyitem.declent_ru(ACCUSATIVE)] со своего пути!"))
		copyitem = null
	playsound(loc, 'sound/machines/ping.ogg', 50, FALSE)
