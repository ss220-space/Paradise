...
/obj/machinery/photocopier/proc/copy(obj/item/C, scancopy = FALSE)
	...
	if(istype(C, /obj/item/paper))
		...
		balloon_alert(usr, "нельзя отсканировать!")
		to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] не способен отсканировать [copyitem.declent_ru(ACCUSATIVE)], [copyitem.declent_ru(NOMINATIVE)] будет извлечен[GEND_A_O_Y(copyitem)]."))
...
/obj/machinery/photocopier/proc/scan_document() //scan a document into a file
	...
	if(length(saved_documents) >= max_saved_documents)
		...
		to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] не способен отсканировать [copyitem.declent_ru(ACCUSATIVE)] в связи с тем, что лимит сохранённых файлов был достигнут. Для продолжения операции освободите память устройства."))
		return
	copying = TRUE
	var/obj/item/O
...
/obj/machinery/photocopier/MouseDrop_T(mob/target, mob/living/user)
	...
	if(copyitem)
		copyitem.forceMove(get_turf(src))
		visible_message(span_notice("[copymob.declent_ru_cap(NOMINATIVE)] сталкива[PLUR_ET_YUT(user)] [copyitem.declent_ru(ACCUSATIVE)] со своего пути!"))
		copyitem = null
	playsound(loc, 'sound/machines/ping.ogg', 50, FALSE)
