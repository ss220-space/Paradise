	...
	switch(action)
		...
		if("paper") // insert/eject paper/paperbundle/photo
			if(copyitem)
				...
				if(istype(I, /obj/item/paper) || istype(I, /obj/item/photo) || istype(I, /obj/item/paper_bundle))
					...
					to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] может принять только бумагу, фото и их стопки."))
					. = FALSE
		...
		if("send") // actually send the fax
			...
			if(cooldown_seconds > 0)
				playsound(loc, 'sound/machines/buzz-sigh.ogg', 50, FALSE)
				to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] не сможет работать ещё [cooldown_seconds] секунд[numeric_ending(cooldown_seconds, "", "а", "ы")]."))
				return
