...
/obj/structure/closet/supplypod/proc/handleReturnAfterDeparting(atom/movable/holder = src)
	...
	if(!effectQuiet && !ispath(style, /datum/pod_style/seethrough))
		audible_message(
			span_notice("[declent_ru_cap(NOMINATIVE)] шипит, закрываясь и улетая прочь от станции."),
...
/obj/structure/closet/supplypod/MouseDrop_T(atom/movable/O, mob/living/user, params)
	...
	user.visible_message(span_notice("[user.declent_ru_cap(NOMINATIVE)] начинает запихивать [O.declent_ru(ACCUSATIVE)] в [declent_ru(ACCUSATIVE)]."),
