...
/obj/item/clothing/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stack/nanopaste))
		...
		if(obj_integrity >= max_integrity)
			user.balloon_alert(user, "[declent_ru_cap(NOMINATIVE)] в полном порядке")
			return ATTACK_CHAIN_PROCEED
		...
		user.visible_message(
			span_notice("[user.declent_ru_cap(NOMINATIVE)] наносит немного нанопасты на [declent_ru(ACCUSATIVE)]. [declent_ru_cap(NOMINATIVE)] выглядит целее."),
			span_notice("Вы нанесли немного нанопасты на [declent_ru(ACCUSATIVE)]. [declent_ru_cap(NOMINATIVE)] выглядит целее."),
		)
