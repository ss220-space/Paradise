		...
		switch(sensor_mode)
			if(0)
				for(var/mob/V in viewers(user, 1))
					V.show_message(span_warning("[user] отключает датчики [src.loc]."), 1)
			if(1)
				for(var/mob/V in viewers(user, 1))
					V.show_message("[user] устанавливает датчики [src.loc] в бинарный режим.", 1)
			if(2)
				for(var/mob/V in viewers(user, 1))
					V.show_message("[user] устанавливает датчики [src.loc] в режим мониторинга жизненных показателей.", 1)
			if(3)
				for(var/mob/V in viewers(user, 1))
					V.show_message("[user] устанавливает датчики [src.loc] в режим мониторинга жизненных показателей и текущего местоположения.", 1)
		if(ishuman(src))
