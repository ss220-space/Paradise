...
/datum/tts_provider/ntts/request(text, datum/tts_seed/ntts/seed, datum/callback/proc_callback, output_file)
	...
	rustg_file_write("", output_file, "false")
	SShttp.create_async_request(RUSTG_HTTP_METHOD_GET, api_url, headers = headers, proc_callback = proc_callback, output_file = output_file)
