...
//			to_chat(src, "Traitor Items")
		...
		if("death")
			...
			if(prob(50))
				...
				if(fakemob)
					sleep(rand(30, 60))
					to_chat(src, span_deadsay("[span_name(fakemob.name)](FOLLOW) [pick("жалуется", "стонет", "хнычет", "причитает", "рыдает", "ноет")], \"[span_message([pick("мёртв","привет [name]","ты тоже?","ИИ взломан?",\
					 "я[prob(50)?", блять,":""] ненавижу [pick("культистов", "реву", "абдукторов", "предателей", "свармеров", "вирусы", "тебя")]")])]\""))
			sleep(rand(50,70))
			hal_screwyhud = SCREWYHUD_NONE
			SetSleeping(0)
		if("husks")
