	...
	if(isliving(dropping))
		...
		if(dropping != user && unlocked && (dropping.stat == DEAD || dropping.incapacitated()))
			if(length(passengers) >= max_passengers && !pilot)
				...
				return .
			if(length(passengers) < max_passengers)
				visible_message(span_danger("[user.name] начинает загрузку [dropping.declent_ru(GENITIVE)] в челнок!"))
...
/obj/spacepod/relaymove(mob/user, direction)
	...
	if(health <= 0)
		...
		to_chat(user, span_warning("Интерфейс не отвечает. Перезагрузка через [COOLDOWN_TIMELEFT(src, cooldown_emp)] секунд[DECL_SEC_MIN(COOLDOWN_TIMELEFT(src, cooldown_emp))]."))
		. = FALSE
