			...
			if("Authorize")
				if(!authorized.Find(W:registered_name))
					...
					if(auth_need - authorized.len > 0)
						...
						add_game_logs("[key_name(user)] has authorized early shuttle launch in ([x], [y], [z]).", user)
						...
						add_game_logs("[key_name(user)] has launched the emergency shuttle in ([x], [y], [z]) [seconds] seconds before launch.", user)
...
/obj/machinery/computer/emergency_shuttle/emag_act(mob/user)
	if(!emagged && SSshuttle.emergency.mode == SHUTTLE_DOCKED && user)
		...
		add_game_logs("[key_name(user)] has emagged the emergency shuttle in ([x], [y], [z]): [time] seconds before launch.", user)
