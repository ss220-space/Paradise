...
*/
		...
		if(SHUTTLE_ESCAPE)
			if(time_left <= 0)
				...
				if(devil_on_shuttle)
					GLOB.priority_announcement.Announce("Обнаружен сбой навигационных протоколов. Эвакуационный шаттл сошел с немеченного маршрута и движется в неизвестном направлении.")
				else
