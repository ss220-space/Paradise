...
/obj/item/clothing/suit/space/space_ninja
	...
	/// Абилки костюма
	actions_types = list(	/datum/action/item_action/SpiderOS,
...
//	Не используется			/datum/action/item_action/ninjastar,
							)
	...
	var/static/list/allowed_states = list(	"shuriken",
											"ninja_cloak",
											"ninja_spirit_form",
											"dust",
											"chameleon",
	...
	var/static/list/allowed_preview_states = list(	// Шарфа нет
													"ninja_preview_classic_hood_green",
													"ninja_preview_classic_hood_red",
													"ninja_preview_classic_hood_blue",
													"ninja_preview_classic_hood_green_f",
													"ninja_preview_classic_hood_red_f",
													"ninja_preview_classic_hood_blue_f",
													"ninja_preview_new_hood_green",
													"ninja_preview_new_hood_blue",
													"ninja_preview_new_hood_red",
													"ninja_preview_new_hood_green_f",
													"ninja_preview_new_hood_blue_f",
													"ninja_preview_new_hood_red_f",
													// Шарф отличается от стиля костюма
													"ninja_preview_new_scarf_classic_green",
													"ninja_preview_new_scarf_classic_blue",
													"ninja_preview_new_scarf_classic_red",
													"ninja_preview_new_scarf_classic_green_f",
													"ninja_preview_new_scarf_classic_blue_f",
													"ninja_preview_new_scarf_classic_red_f",
													"ninja_preview_classic_scarf_new_green",
													"ninja_preview_classic_scarf_new_red",
													"ninja_preview_classic_scarf_new_blue",
													"ninja_preview_classic_scarf_new_green_f",
													"ninja_preview_classic_scarf_new_red_f",
													"ninja_preview_classic_scarf_new_blue_f",
													// Шарф имеет такой же стиль как и костюм
													"ninja_preview_classic_scarf_classic_green",
													"ninja_preview_classic_scarf_classic_red",
													"ninja_preview_classic_scarf_classic_blue",
													"ninja_preview_classic_scarf_classic_green_f",
													"ninja_preview_classic_scarf_classic_red_f",
													"ninja_preview_classic_scarf_classic_blue_f",
													"ninja_preview_new_scarf_new_green",
													"ninja_preview_new_scarf_new_blue",
													"ninja_preview_new_scarf_new_red",
													"ninja_preview_new_scarf_new_green_f",
													"ninja_preview_new_scarf_new_blue_f",
													"ninja_preview_new_scarf_new_red_f")
	/// UI stuff end ///
	...
	var/list/boost_phrases = list(	"A CORNERED FOX IS MORE DANGEROUS THAN A JACKAL!",
		...
		if(!affecting)	//Чтобы уничтожить костюм и всё связанное в случае совершенно неправильной активации костюма
			...
			if(stealth) // If stealth is active.
				...
				if(prob(stealth_ambient_chance))
					var/sounds = pick(	'sound/ambience/ambifailure.ogg',
					...
					var/random_danger_text = pick(	"У вас по коже пробежали мурашки...",
													"Такое чувство, что на вас кто-то смотрит...",
													"Кажется вы врезались во что-то!")
					var/random_subtle_text = pick(	"Вам кажется вы слышали шаги...",
			...
			if(spirited) // If spirit form is active.
				cell.charge -= s_acost*10 //that shit is NOT cheap
		else
			cell.charge = 0
			if(stealth) // If stealth is active.
				cancel_stealth()
...
/obj/item/clothing/suit/space/space_ninja/ui_action_click(mob/ninja, action)
	if(!IS_SPACE_NINJA(ninja) && !anyone)
	...
	if(!s_initialized)
		...
		return FALSE
	if(action == /datum/action/item_action/ninja_autodust)
		ninja_toggle_autodust()
		return TRUE
	if(action == /datum/action/item_action/ninjastatus)
		ninjastatus()
		return TRUE
	if(action == /datum/action/item_action/ninjaboost)
		ninjaboost()
		return TRUE
	if(action == /datum/action/item_action/ninjaheal)
		ninjaheal()
		return TRUE
	if(action == /datum/action/item_action/ninjastar)
		ninjastar()
		return TRUE
	if(action == /datum/action/item_action/toggle_shuriken_fire_mode)
	...
	if(action == /datum/action/item_action/ninja_clones)
		spawn_ninja_clones()
		return TRUE
	if(action == /datum/action/item_action/ninjapulse)
		ninjapulse()
		return TRUE
	if(action == /datum/action/item_action/ninjanet)
		ninjanet()
		return TRUE
	if(action == /datum/action/item_action/ninja_sword_recall)
		ninja_sword_recall()
		return TRUE
	if(action == /datum/action/item_action/ninja_stealth)
		toggle_stealth()
		return TRUE
	if(action == /datum/action/item_action/ninja_chameleon)
		toggle_chameleon_scanner_mode()
		return TRUE
	if(action == /datum/action/item_action/ninja_smoke_bomb)
		prime_smoke()
		return TRUE
	if(action == /datum/action/item_action/ninja_caltrops)
		scatter_caltrops()
		return TRUE
	if(action == /datum/action/item_action/johyo)
		toggle_harpoon()
		return TRUE
	if(action == /datum/action/item_action/ninja_emergency_blink)
		emergency_blink()
		return TRUE
	if(action == /datum/action/item_action/ninja_spirit_form)
