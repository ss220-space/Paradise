...
/datum/prizes/proc/PlaceOrder(var/obj/machinery/prize_counter/prize_counter, var/itemID)
	if(!prize_counter.Adjacent(usr))
		to_chat(usr, span_warning("Вам нужно быть ближе!"))
		return
	if(!prize_counter)
		return 0
	...
	if(prize_counter.tickets >= item.cost)
		...
		prize_counter.visible_message(span_notice("Наслаждайтесь призом!"))
		return 1
	else
		prize_counter.visible_message(span_warning("Недостаточно билетов!"))
		return 0
