...
/obj/machinery/hydroponics/proc/mutatepest(mob/user)
	if(pestlevel > 5)
		...
		add_game_logs("[key_name(user)] caused spiderling pests to spawn in a hydro tray", user)
	...
	if(istype(O, /obj/item/reagent_containers))  // Syringe stuff (and other reagent containers now too)
		...
		if(!myseed)
			if(istype(O, /obj/item/seeds/kudzu))
				investigate_log("had Kudzu planted in it by [key_name(user)] at ([x],[y],[z])", INVESTIGATE_BOTANY)
			user.unEquip(O)
