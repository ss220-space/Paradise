...
/datum/station_goal/bluespace_tap/get_report()
	return {"<b>Блюспейс Харвестер Эксперимент</b><br>
	Очередная исследовательская станция изобрела устройство под названием Блюспейс Харвестер.
	...
	Добудьте плату, создайте Блюспейс Харвестер и подключите к нему провод питания. Добудьте [goal] очков добычи до конца смены.
	<br><br>
	Помните, что прототип - экспериментальный и может вести себя непредвиденно, если достаточно запитан.
	<br>
