...
/obj/machinery/power/bluespace_tap/emag_act(mob/user)
	...
	if(user)
		user.visible_message(span_warning("[user] переписывает протоколы безопасности [src.declent_ru(GENITIVE)]."), span_warning("Вы переписываете протоколы безопасности."))
