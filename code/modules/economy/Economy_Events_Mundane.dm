	...
	if(!newMsg.body)
		...
		switch(event_type)
			if(RESEARCH_BREAKTHROUGH)
				newMsg.body = "Крупный прорыв в области [pick("исследования плазмы","сверхсжатых материалов","наноаугментаций","исследования блюспейса", "управления нестабильной энергией")] был объявлен [pick("вчера","несколько дней назад","на прошлой неделе","ранее в этом месяце")], частной фирмой на [affected_dest.name]. Nanotrasen отказалась комментировать, может ли это повлиять на её прибыль."

			if(ELECTION)
				newMsg.body = "Сегодня было объявлено о предварительном отборе дополнительных кандидатов на предстоящие выборы в [pick("совет надзирателей","консультативный совет","губернаторство","коллегию инквизиторов")] на [affected_dest.name]. Среди них — [pick("медиамагнат","веб-знаменитость","титан индустрии","суперзвезда","знаменитый шеф-повар","популярный садовод","бывший армейский офицер","мультимиллиардер")] "
				...
				newMsg.body += "[pick("Моя единственная цель — помочь [pick("больным","бедным","детям")]","Я буду поддерживать рекордные прибыли Nanotrasen","Я верю в наше будущее","Мы должны вернуться к нашей моральной основе","Просто... расслабьтесь, ребята")]!"

			if(RESIGNATION)
				var/locvar = pick("MALE", "FEMALE")
				newMsg.body = "Nanotrasen с сожалением объявляет об отставке [random_name(locvar)] – [pick("секторного адмирала","дивизионного адмирала","корабельного адмирала","вице-адмирала")]."
				if(prob(25))
					var/locstring = pick("Сегунда","Салyса","Цефей","Андромеда","Груис","Корона","Акyила","Аселлyс") + " " + pick("I","II","III","IV","V","VI","VII","VIII")
					newMsg.body += " Сегодня днём на [affected_dest.name] состоится церемония, на которой [locvar == "MALE" ? "ему" : "ей"] будет вручена [pick("Красная Звезда Жертвенности","Пурпурное Сердце Героизма","Синий Орёл Верности","Зелёный Лев Изобретательности")] за "
					if(prob(33))
					...
					newMsg.body += " Ожидается, что [locvar == "MALE" ? "он" : "она"] поселится на [affected_dest.name], где [locvar == "MALE" ? "ему" : "ей"] была назначена щедрая пенсия."
				else if(prob(50))
					newMsg.body += " Новость была обнародована на [affected_dest.name] ранее сегодня, где [locvar == "MALE" ? "он" : "она"] назвал[locvar == "MALE" ? "" : "а"] причины своего ухода: '[pick("здоровье","семья","██████████")]'."
				else
			...
			if(CELEBRITY_DEATH)
				newMsg.body = "Сегодня, с сожалением, мы объявляем о внезапной кончине "
				if(prob(33))
			...
			if(BARGAINS)
				newMsg.body += "РАСПРОДАЖИ! РАСПРОДАЖИ! РАСПРОДАЖИ! Коммерческий контроль на [affected_dest.name] сообщает, что всё должно быть распродано! Во всех торговых центрах цены на все товары снижены, и все ритейлеры участвуют — так что приходите и проведите лучший шопинг в своей жизни!"
			...
			if(BIG_GAME_HUNTERS)
				...
				if(prob(33))
					...
					newMsg.body += "были атакованы свирепым [pick("нас'ром","дияабом","самаком","хищником, который ещё не идентифицирован")].\
					Власти призывают к осторожности, а местным жителям рекомендуется запастись оружием."
				else
			...
			if(GOSSIP)
				...
				if (locvar == "MALE")
					...
					newMsg.body += "[pick("Телеведущая","Веб-знаменитость","Суперзвезда","Модель","Актриса","Певица")] [random_name(FEMALE)] её супруг"
				if(prob(33))
					...
					newMsg.body += "[pick("недобрые комментарии","слухи, распущенные завистливыми друзьями","решение крупного спонсора прекратить сотрудничество","катастрофическое интервью на Nyx Tonight")]."

			if(TOURISM)
				var/locvar = pick("MALE", "FEMALE")
				var/locstring = ""
				if (locvar == "MALE")
					locstring = "туре популярного артиста [random_name(pick(MALE))]"
				else
					locstring = "туре популярной артистки [random_name(pick(FEMALE))]"
				newMsg.body += "Туристы стекаются на [affected_dest.name] после неожиданного объявления о [pick("крупных распродажах от крупного ритейлера","новой масштабной ARG от популярной развлекательной компании","[locstring]")]. Nyx Daily предлагает скидочные билеты для двоих на концерт [random_name(pick(MALE,FEMALE))] в обмен на репортажи очевидцев и оперативное освещение событий."
