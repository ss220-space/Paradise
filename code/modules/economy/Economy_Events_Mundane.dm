		...
		switch(event_type)
			...
			if(RESIGNATION)
				...
				if(prob(25))
					...
					newMsg.body += " Ожидается, что он[locvar == "MALE" ? "" : "а"] поселится на [affected_dest.name], где [locvar == "MALE" ? "ему" : "ей"] была назначена щедрая пенсия."
				else if(prob(50))
			...
			if(TOURISM)
				...
				newMsg.body += "Туристы стекаются на [affected_dest.name], после неожиданного объявления о [pick("распродажах от крупного ритейлера","новой масштабной ARG от популярной развлекательной компании","[locstring]")]. Nyx Daily предлагает скидочные билеты для двоих на концерт [random_name(pick(MALE,FEMALE))], в обмен на репортажи очевидцев и оперативное освещение событий."
