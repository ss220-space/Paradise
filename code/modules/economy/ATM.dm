...
/obj/machinery/atm/process()
	...
	if(linked_db && ((linked_db.stat & NOPOWER) || !linked_db.activated))
		...
		visible_message("[bicon(src)]<span class='warning'>[src] buzzes rudely, \"Connection to remote database lost.\""))
	...
	switch(action)
		if("transfer")
			...
			if(transfer_amount <= 0)
				to_chat(usr, "[bicon(src)]<span class='warning'>That is not a valid amount."))
				...
				if(linked_db.charge_to_account(target_account_number, authenticated_account, transfer_purpose, machine_id, transfer_amount))
					to_chat(usr, "[bicon(src)]<span class='notice'>Funds transfer successful."))
				else
					to_chat(usr, "[bicon(src)]<span class='warning'>Funds transfer failed."))
			else
				to_chat(usr, "[bicon(src)]<span class='warning'>You don't have enough funds to do that!"))
					...
					if(!authenticated_account)
						...
						if(previous_account_number == tried_account_num)
							if(number_incorrect_tries > max_pin_attempts)
								...
								to_chat(usr, "[bicon(src)]<span class='warning'>Incorrect pin/account combination entered, [max_pin_attempts - number_incorrect_tries] attempt\s remaining."))
								previous_account_number = tried_account_num
								playsound(src, 'sound/machines/buzz-sigh.ogg', 50, TRUE)
						else
							to_chat(usr, "[bicon(src)]<span class='warning'>Incorrect pin/account combination entered."))
							number_incorrect_tries = 0
					else
						...
						to_chat(usr, "[bicon(src)]<span class='notice'>Access granted. Welcome user '[authenticated_account.owner_name].'"))
					previous_account_number = tried_account_num
