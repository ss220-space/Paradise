...
/datum/event/economic_event/announce()
	...
	if(!newMsg.body)
		switch(event_type)
			...
			if(CORPORATE_ATTACK)
				newMsg.body = "A small [pick("pirate",AFFIL_CYBERSUN,AFFIL_GORLEX,"Syndicate")] fleet has precise-jumped into proximity with [affected_dest.name], [pick("for a smash-and-grab operation","in a hit and run attack","in an overt display of hostilities")]. Much damage was done, and security has been tightened since the incident."
			if(ALIEN_RAIDERS)
				if(prob(20))
