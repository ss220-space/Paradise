...
/datum/event/economic_event/announce()
	...
	if(!newMsg.body)
		switch(event_type)
			...
			if(BIOHAZARD_OUTBREAK)
				newMsg.body = "[pick("███████ █████","Биологическая угроза","Вспышка заболевания","Всмышка вируса", "\[ОТРЕДАКТИРОВАНО\]")] на [affected_dest.name] привела к введению карантина, что остановило многие поставки в регионе. Хотя карантин уже снят, власти призывают к доставке медицины для лечения заражённых."
			if(PIRATES)
				newMsg.body = "[pick("Пираты","Преступные элементы","Ударный отряд [pick("Синдиката","Donk Co.","Waffle Co.","██████████", "\[ОТРЕДАКТИРОВАНО\]")]")] сегодня [pick("установили блокаду","попытались шантажировать","атаковали")] [affected_dest.name]. Меры безопасности были усилены, но множество ценных ресурсов было похищено."
			if(CORPORATE_ATTACK)
				newMsg.body = "Небольшой флот [pick("пиратов","Cybersun Industries","Мародеров Горлекс","Синдиката")] совершил точный прыжок вблизи [affected_dest.name], [pick("для проведения операции «налет и захват»","для атаки по типу «ударил-убежал»","для открытой демонстрации враждебности")]. Было нанесено много ущерба, и с тех пор меры безопасности были усилены."
			if(ALIEN_RAIDERS)
				if(prob(20))
					newMsg.body = "Кооператив «Тигр» сегодня совершил набег на [affected_dest.name], несомненно, по приказу своих загадочных хозяев. Были похищены дикие животные, сельскохозяйственный скот, материалы для медицинских исследований, а также гражданские лица. Nanotrasen готовы противодействовать любым попыткам биотерроризма."
				else
			...
			if(AI_LIBERATION)
				newMsg.body = "[pick("██████████ был обнаружен ","Оперативник S.E.L.F проник","Злокачественный компьютерный вирус был обнаружен","Хакер был задержан на")] на [affected_dest.name] сегодня, и успел заразить [pick("██████████","разумную подсистему","ИИ первого класса","разумную оборонительную установку", "\[ОТРЕДАКТИРОВАНО\]")], прежде чем его остановили. Многие жизни были потеряны, так как система начала методично убивать гражданских лиц, и потребуется много сил для восстановления пострадавших районов."
			if(MOURNING)
				var/locvar = pick("MALE", "FEMALE")
				if (locvar == "MALE")
					newMsg.body = "[pick("Популярный","Любимый","Известный","Знаменитый")]  [pick("профессор","артист","певец","учёный","государственный служащий","администратор","капитан корабля","██████████")], [pick( random_name(MALE), 40; "█████ ███████" )] сегодня [pick("скончался","покончил с собой","был убит","погиб при странных обстоятвах")] на [affected_dest.name]. "
				else
					newMsg.body = "[pick("Популярная","Любимая","Известная","Знаменитая")]  [pick("профессор","артистка","учёная","государственная служащая","администратор","капитан корабля","██████████")]|, [pick( random_name(FEMALE), 40; "█████ ███████" )] сегодня [pick("скончалась","покончила с собой","была убита","погибла при странных обстоятвах")] на [affected_dest.name]. "
				newMsg.body += "Вся планета в трауре, а цены на товары упали из-за снижения морального духа рабочих."
			if(CULT_CELL_REVEALED)
				newMsg.body = "[pick("Коварный","Кровожадный","Злодейский","Безумный")] культ [pick("Древних Богов","Нар'си","апокалиптической секты","██████████")] [pick("был обнаружен","раскрыт","раскрыл себя","заявил о себе")] на [affected_dest.name] сегодня ранее. Моральный дух населения пошатнулся из-за того, что [pick("несколько","одна или две")] [pick("известных","популярных","уважаемых")] личности [pick("совершали \[ОТРЕДАКТИРОВАНО\] действия","признали свою принадлежность к культу","поклялись в верности лидеру культа","пообещали помочь культу")], прежде чем виновные были привлечены к ответственности. Редакторство напоминает всему персоналу, что всё сверхъестественное недопустимо на объектах Nanotrasen."
			if(SECURITY_BREACH)
				newMsg.body = "Сегодня утром [pick("произошло нарушение мер безопасности в","произошло несанкционированное проникновение в","произошла попытка кражи в","произошла атака анархистов на","произошёл акт насильственного саботажа в")] [pick("высокозащищённой","закрытой","секретной","\[ОТРЕДАКТИРОВАНО\]", "██████████")] [pick("\[ОТРЕДАКТИРОВАНО\]","секции","зоне","области")]. После инцидента меры безопасности на [affected_dest.name] были усилены, и редакторство заверяет весь персонал Nanotrasen, что такие инциденты случаются редко."
			if(ANIMAL_RIGHTS_RAID)
