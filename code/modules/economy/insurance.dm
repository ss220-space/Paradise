...
/proc/do_insurance_collection(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/money_account/connected_acc)
	...
	if(from_money_acc)
		if(!acc.insurance_auto_replen || !acc.charge(from_money_acc))
			...
			target.visible_message(
				span_danger("[user] безуспешно пытается списать страховку у [target]!"),
				span_userdanger("[user] безуспешно пытается списать вашу страховку!"),
				ignored_mobs = user,
			)
			return FALSE
	...
	target.visible_message(
		span_danger("[user] списывает страховку у [target] в размере: [req]."),
		span_userdanger("[user] списывает вашу страховку в размере: [req]."),
		ignored_mobs = user,
	)
	if(from_money_acc)
