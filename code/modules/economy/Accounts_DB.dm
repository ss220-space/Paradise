...
/obj/machinery/computer/account_database/proc/accounting_letterhead(report_name)
	...
	return {"
		...
		<center><small><i>Отчёт об аккаунтах [station_name()]</i></small></center>
		<u>Кем распечатан:</u> [L?.id?.registered_name ? L.id.registered_name : "Unknown"], [L?.id?.assignment ? L.id.assignment : "Unknown"]<br>
		<hr>
	"}
...
/obj/machinery/computer/account_database/ui_act(action, list/params)
	...
	switch(action)
		...
		if("finalise_create_account")
			...
			if(starting_funds > 0)
				GLOB.station_account.charge(starting_funds, null, "Активация нового аккаунта", "", "Активация нового аккаунта", M.owner_name)
		...
		if("print_records")
			...
			if(next_print > world.time)
				to_chat(usr, "<span class='warning'>Принтер ещё не готов к использованию. Подождите [(next_print - world.time) / 10] секунд.")
				return
			var/text
			playsound(loc, 'sound/goonstation/machines/printer_thermal.ogg', 50, 1)
			var/obj/item/paper/P = new(loc)
			P.name = "список аккаунтов"
			text = {"
				[accounting_letterhead("Список Всех Аккаунтов")]
			...
			for(var/i in 1 to length(GLOB.all_money_accounts))
				...
				text += {"
						<tr>
							...
							<td>[D.owner_name]</td>
							<td>$[D.money]</td>
							<td>[D.suspended ? "Приостановлен" : "Активен"]</td>
						</tr>
				"}
			...
			P.info = text
			visible_message("<span class='notice'>[src] распечатывает отчет.</span>")
		...
		if("print_account_details")
			...
			playsound(loc, 'sound/goonstation/machines/printer_thermal.ogg', 50, 1)
			var/obj/item/paper/P = new(loc)
			P.name = "детали аккаунта #[detailed_account_view.account_number]"
			var/title = "Детали аккаунта #[detailed_account_view.account_number]"
			text = {"
				[accounting_letterhead(title)]
				<u>Владелец:</u> [detailed_account_view.owner_name]<br>
				<u>Баланс:</u> $[detailed_account_view.money]<br>
				<u>Статус:</u> [detailed_account_view.suspended ? "Приостановлен" : "Активен"]<br>
				<u>Транзакции:</u> ([detailed_account_view.transaction_log.len])<br>
				<table>
					<thead>
						<tr>
							<td>Время</td>
			...
			P.info = text
			visible_message("<span class='notice'>[src] печетает отчёт.</span>")
			next_print = world.time + 30 SECONDS
