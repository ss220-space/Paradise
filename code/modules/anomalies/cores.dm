...
/obj/item/assembly/signaler/core/atmospheric/tier3/process()
	...
	if(H.bodytemperature < T0C - 50)
		visible_message("[declent_ru_cap(NOMINATIVE)] реагирует на контакт с холодным объектом, испуская языки пламени!")
	...
	visible_message("[declent_ru_cap(NOMINATIVE)] реагирует на контакт с горячим объектом, значительно охлаждая окружающую среду!")
