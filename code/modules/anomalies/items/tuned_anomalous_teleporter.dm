...
/obj/item/assembly/tuned_anomalous_teleporter/suicide_act(mob/user)
	...
	user.visible_message(span_suicide("[user] перенастраива[PLUR_ET_UT(user)] [declent_ru(NOMINATIVE)] случайным образом и пыта[PLUR_ET_UT(user)]ся телепортироваться! Выглядит, будто он[genderize_ru(gender, "", "а", "о", "и")] хо[genderize_ru(gender, "чет", "чет", "чет", "тят")] убить себя!"))
