...
/obj/item/assembly/tuned_anomalous_teleporter/suicide_act(mob/user)
	...
	user.visible_message(span_suicide("[user] перенастраива[PLUR_ET_UT(user)] [declent_ru(NOMINATIVE)] случайным образом и пыта[PLUR_ET_UT(user)]ся телепортироваться! Выглядит, будто он[GEND_ENDING_A_O_I(src)] хо[genderize_ru(gender, "чет", "чет", "чет", "тят")] убить себя!"))
