...
/obj/effect/anomaly/proc/collapse()
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] достигает критической массы и распадается!"))
...
/obj/effect/anomaly/proc/level_down()
	if(weaker_anomaly_type)
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] ослабевает!"))
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] теряет свою энергию и растворяется в пространстве!"))
	sleep(1 SECONDS)
	qdel(src)
...
/obj/effect/anomaly/proc/level_up()
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] становится мощнее!"))
...
/obj/effect/anomaly/proc/core_touch_effect(obj/item/assembly/signaler/core/core)
	...
	if(!iscoreempty(core))
		core.visible_message(span_warning("[core.declent_ru_cap(NOMINATIVE)] распадается, передавая свой заряд [declent_ru(DATIVE)]."))
	...
	if(new_charge <= 50)
		...
		core.visible_message(span_warning("[core.declent_ru_cap(NOMINATIVE)] заряжается от [declent_ru(GENITIVE)], \
	...
	new_core.visible_message(span_warning("[core.declent_ru_cap(NOMINATIVE)] заряжается от [declent_ru(GENITIVE)], \
...
/obj/effect/anomaly/proc/item_touch_effect(obj/item/item)
	...
	if(tier == 3 && istype(item, /obj/item/anomaly_upgrader))
		visible_message(span_danger("[item.declent_ru_cap(NOMINATIVE)] попадает в [declent_ru(ACCUSATIVE)], прикрепляется к ней и активируется!"))
