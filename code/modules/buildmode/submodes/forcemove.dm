...
/datum/buildmode_mode/forcemove/handle_click(mob/user, params, atom/A)
	...
	if(right_click)
		...
		to_chat(user, span_notice("\"[selected_atom.declent_ru_cap(NOMINATIVE)]\" выбран[GEND_A_O_Y(selected_atom)] для перемещения."))
		return
	...
	to_chat(user, span_notice("\"[selected_atom.declent_ru_cap(NOMINATIVE)]\" перемещен[GEND_A_O_Y(selected_atom)] \"[destination.declent_ru(ACCUSATIVE)]\"."))
