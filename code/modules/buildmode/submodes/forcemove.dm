...
/datum/buildmode_mode/forcemove/handle_click(mob/user, params, atom/A)
	...
	if(right_click)
		...
		to_chat(user, span_notice("'[capitalize(selected_atom.declent_ru(NOMINATIVE))]' выбран[GEND_ENDING_A_O_I(selected_atom)] для перемещения."))
		return
	...
	to_chat(user, span_notice("'[capitalize(selected_atom.declent_ru(NOMINATIVE))]' перемещен[GEND_ENDING_A_O_I(selected_atom)] '[destination.declent_ru(ACCUSATIVE)]'."))
