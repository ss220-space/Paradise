...
/obj/item/gun/proc/shoot_live_shot(mob/living/user, atom/target, pointblank = FALSE, message = TRUE)
	...
	if(suppressed)
		...
		if(message)
			if(pointblank)
				user.visible_message(span_danger("[user] стреля[PLUR_ET_UT(user)] из [declent_ru(GENITIVE)] в упор в [target]!"), span_danger("[pluralize_ru(user.gender,"Ты стреляешь","Вы стреляете")] из [declent_ru(GENITIVE)] в упор в [target]!"), span_italics("Вы слышите \a [fire_sound_text]!"), projectile_message = TRUE)
				...
				user.visible_message(span_danger("[user] стреля[PLUR_ET_UT(user)] из [declent_ru(GENITIVE)]!"), span_danger("[pluralize_ru(user.gender,"Ты стреляешь","Вы стреляете")] из [declent_ru(GENITIVE)]!"), "Вы слышите [fire_sound_text]!", projectile_message = TRUE)
...
/obj/item/gun/proc/handle_suicide(mob/living/carbon/human/user, mob/living/carbon/human/target, params)
	...
	if(user == target)
		target.visible_message(
			span_warning("[user] вставляет ствол [declent_ru(GENITIVE)] себе в рот, готовясь нажать на спуск..."),
			span_userdanger("Вы вставляеете ствол [declent_ru(GENITIVE)] себе в рот, готовясь нажать на спуск...")
		)
	else
		target.visible_message(
			span_warning("[user] направляет [declent_ru(ACCUSATIVE)] в голову [target], готовясь выстрелить..."),
	...
	if(!do_after(user, 12 SECONDS, target, NONE) || user.zone_selected != BODY_ZONE_PRECISE_MOUTH)
		if(user)
			if(user == target)
				...
				target.visible_message(
					span_notice("[user] решает пощадить [target]."),
					span_notice("[user] решает оставить вас в живых!")
				)
		semicd = 0
		return
	...
	target.visible_message(
		span_warning("[user] нажимает на спусковой крючок!"),
		span_userdanger("[user] нажимает на спусковой крючок!")
	)
