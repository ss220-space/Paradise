...
/obj/item/gun/proc/shoot_live_shot(mob/living/user, atom/target, pointblank = FALSE, message = TRUE)
	...
	if(suppressed)
		...
		if(message)
			if(pointblank)
				user.visible_message(span_danger("[user] стреляет из [src.declent_ru(GENITIVE)] в упор в [target]!"), span_danger("[pluralize_ru(user.gender,"Ты стреляешь","Вы стреляете")] из [src.declent_ru(GENITIVE)] в упор в [target]!"), span_italics("Вы слышите \a [fire_sound_text]!"), projectile_message = TRUE)
				...
				user.visible_message(span_danger("[user] стреляет из [src.declent_ru(GENITIVE)]!"), span_danger("[pluralize_ru(user.gender,"Ты стреляешь","Вы стреляете")] из [src.declent_ru(GENITIVE)]!"), "Вы слышите [fire_sound_text]!", projectile_message = TRUE)
	...
	if(flag)
		if(user.zone_selected == "mouth")
			if(target == user && HAS_TRAIT(user, TRAIT_BADASS))
				user.visible_message(span_danger("[user] дым с дула [src.declent_ru(GENITIVE )]. Как же [genderize_ru(user.gender,"он хорош","она хороша","оно хорошо","они хороши")]!"))
			else
