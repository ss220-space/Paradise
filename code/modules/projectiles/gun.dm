...
/obj/item/gun/examine(mob/user)
	...
	if(bayonet)
		...
		if(can_bayonet) // if it has a bayonet and this is false, the bayonet is permanent.
			. += span_notice("[bayonet.declent_ru_cap(NOMINATIVE)] можно [span_bold("открутить")] от [declent_ru(GENITIVE)].")
...
/obj/item/gun/proc/can_trigger_gun(mob/living/user)
	if(istype(user))
		...
		if(restricted_species && length(restricted_species) && !is_type_in_list(user.dna.species, restricted_species))
			to_chat(user, span_danger("[declent_ru_cap(NOMINATIVE)] несовместим с вашей биологией!"))
			return FALSE
