...
/obj/item/gun/examine(mob/user)
	...
	if(unique_rename)
		. += "<span class='notice'>Воспользуйтесь ручкой для переименования оружия.</span>"
	if(bayonet)
		. += "У оружия [can_bayonet ? "закрепленн[genderize_ru(bayonet.gender, "ый", "ая", "ое", "ые")]" : "встроен[genderize_ru(bayonet.gender, "ый", "ая", "ое", "ые")]"] [bayonet.declent_ru(ACCUSATIVE)]."
		if(can_bayonet) //if it has a bayonet and this is false, the bayonet is permanent.
			. += "<span class='info'>Похоже [bayonet.declent_ru(ACCUSATIVE)] можно снять <b>отверткой</b>.</span>"
	else if(can_bayonet)
		. += "На оружии есть крепление для <b>штыка</b>."
	...
	if(clumsy_check)
		if(istype(user))
			if((CLUMSY in user.mutations) && prob(40))
				to_chat(user, "<span class='userdanger'>Вы стреляете себе в ногу из [src.declent_ru(GENITIVE)]!</span>")
...
/obj/item/gun/screwdriver_act(mob/user, obj/item/I)
	...
	if(gun_light && can_flashlight)
		for(var/obj/item/flashlight/seclite/S in src)
			to_chat(user, "<span class='notice'>Вы открутили фонарик от [src.declent_ru(GENITIVE)].</span>")
			gun_light = null
			S.loc = get_turf(user)
			update_gun_light(user)
