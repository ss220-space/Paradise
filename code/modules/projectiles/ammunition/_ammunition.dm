...
/obj/item/ammo_casing/attackby(obj/item/item, mob/user, params)
	...
	if(!boolets)
		...
		return ATTACK_CHAIN_PROCEED
	box.update_appearance(UPDATE_ICON|UPDATE_DESC)
	to_chat(user, span_notice("Вы собрали [boolets] гильз[DECL_U_Y(boolets)]. Теперь в [box.declent_ru(GENITIVE)] [length(box.stored_ammo)] гильз[DECL_A_Y(length(box.stored_ammo))]."))
	if(box.can_fast_load)
