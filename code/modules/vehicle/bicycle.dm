...
/obj/vehicle/ridden/bicycle/zap_act(power, zap_flags)
	...
	for(var/mob in buckled_mobs)
		unbuckle_mob(mob, TRUE)

/obj/vehicle/ridden/bicycle/welder_act(mob/living/user, obj/item/welder)
	...
	while(obj_integrity < max_integrity)
		if(welder.use_tool(src, user, 2.5 SECONDS, volume = 50, extra_checks = CALLBACK(src, PROC_REF(can_still_fix))))
			did_the_thing = TRUE
