	...
	if(useMS && sendable && receivable) // only send the message if its going to work
		...
		for(var/mob/M in GLOB.dead_mob_list)
			if(isobserver(M) && M.client && (M.client.prefs.toggles & PREFTOGGLE_CHAT_GHOSTPDA))
				var/ghost_message = "[span_name(pda.owner)] ([ghost_follow_link(pda, ghost=M)]) [span_gamesay("PDA Message")] --> [span_name(P.owner)] ([ghost_follow_link(P, ghost=M)]): [span_message(t)]"
