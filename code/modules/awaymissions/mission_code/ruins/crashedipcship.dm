...
/obj/machinery/power/supermatter_shard/anchored/attackby(obj/item/W, mob/living/user, params)
	if(iswrench(W))
		user.visible_message(span_danger("As [user] tries to loose bolts of \the [src] with \a [W] but the tool disappears!"))
		consume_wrench(W)
