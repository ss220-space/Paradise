...
/datum/asset/rpd/register()
	for(var/state in icon_states('icons/obj/atmos/pipe-item.dmi'))
		...
		if(state in list("he", "simple"))
			for(var/D in GLOB.alldirs)
				assets["[state]-[dir2text(D)].png"] = icon('icons/obj/atmos/pipe-item.dmi', state, D)
		for(var/D in GLOB.cardinal)
			assets["[state]-[dir2text(D)].png"] = icon('icons/obj/atmos/pipe-item.dmi', state, D)
	for(var/state in icon_states('icons/obj/not_atmos_pipes/disposal.dmi'))
		if(!(state in list("pipe-c", "pipe-j1", "pipe-s", "pipe-t", "pipe-y", "intake", "outlet", "pipe-j1s"))) //Pipes we want sprites for
			continue
		for(var/D in GLOB.cardinal)
			assets["[state]-[dir2text(D)].png"] = icon('icons/obj/not_atmos_pipes/disposal.dmi', state, D)
	for(var/asset_name in assets)
		register_asset(asset_name, assets[asset_name])
