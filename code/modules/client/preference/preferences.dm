...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		...
		if(TAB_GAME) // General Preferences
			...
			dat += "<b>Runechat для призраков:</b> <a href='byond://?_src_=prefs;preference=runechat_dead'>[toggles3 & PREFTOGGLE_3_RUNECHAT_DEAD ? "Включить" : "Выключить"]</a><br>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("runechat_dead")
					toggles3 ^= PREFTOGGLE_3_RUNECHAT_DEAD
