...
/mob/living/simple_animal/hostile/mining_drone/emp_act(severity)
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] яростно трещит и искрит!"))
...
/mob/living/simple_animal/hostile/mining_drone/welder_act(mob/user, obj/item/I)
	...
	. = TRUE
	if(health == maxHealth)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] не требует ремонта!"))
		return
	if(!I.tool_use_check(user, 1))
		return
	if(AIStatus != AI_OFF && AIStatus != AI_IDLE)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] слишком активно двигается для ремонта!"))
		return
	...
	if(M.a_intent == INTENT_HELP)
		...
		switch(mode)
			if(MINEDRONE_COLLECT)
				to_chat(M, span_notice("[declent_ru_cap(NOMINATIVE)] переведён в режим поиска и сбора руды."))
			if(MINEDRONE_ATTACK)
				to_chat(M, span_notice("[declent_ru_cap(NOMINATIVE)] переведён в режим атаки на опасную фауну."))
		return
	..()
...
/obj/item/mine_bot_upgrade/proc/upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
	if(M.melee_damage_upper != initial(M.melee_damage_upper))
		to_chat(user, "[declent_ru_cap(NOMINATIVE)] уже имеет боевой модуль!")
		return
...
/obj/item/mine_bot_upgrade/health/upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
	if(M.maxHealth != initial(M.maxHealth))
		to_chat(user, "[declent_ru_cap(NOMINATIVE)] уже имеет усиленный корпус!")
		return
	M.maxHealth += 45
	M.updatehealth()
	qdel(src)
...
/obj/item/mining_drone_cube/attack_self(mob/user)
	user.visible_message(
		span_warning("[declent_ru_cap(NOMINATIVE)] раскрывается в полнофункционального шахтёрского бота!"),
