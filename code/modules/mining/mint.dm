	...
	switch(action)
		...
		if("ejectMat")
			...
			if(material.amount < MINERAL_MATERIAL_AMOUNT)
				to_chat(usr, span_warning("Не достаточно [material.name] для извлечения!"))
				return
...
/obj/machinery/mineral/mint/process()
	...
	if(length(money_bag.contents) >= money_bag.storage_slots)
		active = FALSE
		visible_message(span_notice("[declent_ru(NOMINATIVE)] прекращает производство, чтобы избежать переполнения."))
		update_icon(UPDATE_ICON_STATE)
		SStgui.update_uis(src)
		return
	...
	if(!materials.can_use_amount(COIN_COST, chosen_material))
		active = FALSE
		visible_message(span_notice("[declent_ru(NOMINATIVE)] прекращает производство из-за нехватки материала."))
		update_icon(UPDATE_ICON_STATE)
		SStgui.update_uis(src)
		return
...
/obj/machinery/mineral/mint/proc/try_make_coins(mob/user)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	if(!money_bag)
		visible_message(span_warning("[declent_ru(NOMINATIVE)] не может работать без денежного мешка!"))
		return
	if(length(money_bag.contents) == money_bag.storage_slots)
		visible_message(span_warning("[money_bag.declent_ru(NOMINATIVE)] полон!"))
		return
	if(!materials.can_use_amount(COIN_COST, chosen_material))
