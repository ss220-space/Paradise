...
/obj/machinery/anomalous_crystal/helpers/ActivationReaction(mob/user, method)
	if(..() && !ready_to_deploy)
		ready_to_deploy = 1
		notify_ghosts("Аномальный кристалл активирован в [get_area(src)]! Теперь призраки могут использовать его в любое время.", enter_link = "<a href=?src=\ref[src];ghostjoin=1>(Нажмите для входа)</a>", source = src, action = NOTIFY_ATTACK)
