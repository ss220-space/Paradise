...
/obj/effect/bubblegum_trigger/proc/activate()
	if(!length(targets_to_fuck_up))
		return
	var/spawn_locs = list()
	for(var/obj/effect/landmark/spawner/bubblegum_arena/bubblegum_arena in GLOB.landmarks_list)
		spawn_locs += get_turf(bubblegum_arena)
	for(var/mob/living/living in targets_to_fuck_up)
		var/turf/turf = get_turf(living)
		living.Immobilize(1 SECONDS)
		to_chat(living, span_colossus("<b>НЕТ! Я НЕ ПОЗВОЛЮ ТЕБЕ ДУМАТЬ, ЧТО ТЫ ПОБЕДИЛ! Я ПОЛОЖУ КОНЕЦ ТВОЕЙ ЖАЛКОЙ ЖИЗНИ!</b>"))
		new /obj/effect/temp_visual/bubblegum_hands/leftpaw(turf)
		new /obj/effect/temp_visual/bubblegum_hands/leftthumb(turf)
		sleep(8)
		playsound(turf, 'sound/misc/enter_blood.ogg', 100, TRUE, -1)
		var/turf/target_turf = pick(spawn_locs)
		living.forceMove(target_turf)
		playsound(target_turf, 'sound/misc/exit_blood.ogg', 100, TRUE, -1)
	for(var/obj/effect/landmark/spawner/bubblegum/bubblegum in GLOB.landmarks_list)
		new /mob/living/simple_animal/hostile/megafauna/bubblegum/round_2(get_turf(bubblegum))
...
/obj/effect/bubblegum_exit/proc/activate()
	var/spawn_exit = list()
	for(var/obj/effect/landmark/spawner/bubblegum_exit/bubblegum_exit in GLOB.landmarks_list)
		for(var/turf/turf in range(4, bubblegum_exit))
			if(turf.density)
				continue
			spawn_exit += get_turf(turf)
	var/area/probably_bubblearena = get_area(src)
	for(var/mob/living/living in probably_bubblearena)
		var/turf/turf = get_turf(living)
		living.Immobilize(1 SECONDS)
		to_chat(living, span_colossus("<b>А теперь... убирайся из моего дома.</b>"))
		new /obj/effect/temp_visual/bubblegum_hands/leftpaw(turf)
		new /obj/effect/temp_visual/bubblegum_hands/leftthumb(turf)
		sleep(8)
		playsound(turf, 'sound/misc/enter_blood.ogg', 100, TRUE, -1)
		...
		living.forceMove(target_turf)
		playsound(target_turf, 'sound/misc/exit_blood.ogg', 100, TRUE, -1)
	for(var/obj/obj in probably_bubblearena) //Mobs are out, lets get items / limbs / brains. Lets also exclude blood..
		if(iseffect(obj))
			continue
		if(istype(obj, /obj/structure/stone_tile)) //Taking the tiles from the arena is funny, but a bit stupid
			continue
		var/turf/target_turf = pick(spawn_exit)
		obj.forceMove(target_turf)
