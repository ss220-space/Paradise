...
/obj/item/blood_contract/attack_self(mob/user)
	...
	if(!choice)
		...
		for(var/mob/living/carbon/human/H in GLOB.player_list)
			if(H.stat == DEAD || H == L)
				continue
			to_chat(H, span_userdanger("Вы испытываете непреодолимое желание убить [L]. [GEND_HE_SHE_CAP(L)] помечен[genderize_ru(L.gender,"","а","о","ы")] красным! УБЕЙТЕ [GEND_HIS_HER_CAP(L)]!"))
