...
/obj/item/book_of_babel/attack_self(mob/living/carbon/user)
	if(HAS_TRAIT(user, TRAIT_NO_BABEL))
		user.visible_message(
			span_notice("[user] внезапно останавливается, недоумённо глядя на [declent_ru(GENITIVE)]."),
...
/obj/projectile/hook/on_hit(atom/target)
	...
	if(isliving(target))
		...
		if(!L.anchored && L.loc)
			L.visible_message(
				span_danger("[firer] зацепляет [L.declent_ru(ACCUSATIVE)] [declent_ru(INSTRUMENTAL)]!")
			)
...
/obj/item/immortality_talisman/attack_self(mob/user)
	...
	user.visible_message(span_danger("[user] исчезает из реальности, оставляя после себя дыру в пространстве!"))
...
/obj/item/immortality_talisman/proc/reappear(mob/user, obj/effect/immortality_talisman/effect)
	...
	user.visible_message(span_danger("[user] материализуется в пространстве, вновь возвращаясь в нашу реальность!"))
