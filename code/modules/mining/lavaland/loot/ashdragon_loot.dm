...
/obj/item/melee/ghost_sword/attack_self(mob/user)
	...
	notify_ghosts("[user] поднимает [declent_ru(ACCUSATIVE)], взывая о вашей помощи!", enter_link="<a href='byond://?src=[UID()];follow=1'>(Нажмите, чтобы помочь)</a>", source = user, action = NOTIFY_FOLLOW)
...
/obj/item/melee/ghost_sword/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	user.visible_message(
		span_danger("[user] наносит удар с силой [ghost_counter] [declension_ru(ghost_counter,"мстительного духа","мстительных духов","мстительных духов")]!"),
...
/obj/item/lava_staff/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(target in view(user.client.maxview(), get_turf(user)))
		...
		if(!istype(T, turf_type))
			...
			user.visible_message(span_danger("[user] направляет [declent_ru(ACCUSATIVE)] на [T.declent_ru(ACCUSATIVE)]!"))
			timer = world.time + create_delay + 1
			if(do_after(user, create_delay, T))
				user.visible_message(span_danger("[user] превращает [T.declent_ru(ACCUSATIVE)] в лаву!"))
			...
			user.visible_message(span_danger("[user] превращает [T.declent_ru(ACCUSATIVE)] в базаль!"))
