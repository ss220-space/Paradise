	...
	if(!beacon || QDELETED(beacon))
		if(isturf(user.loc))
			...
			if(do_after(user, 5 SECONDS, user) && !beacon)
				...
				span_hierophant("You detach the hierophant beacon, allowing you to teleport yourself and any allies to it at any time!</span>\n\
				<span class='notice'>You can remove the beacon to place it again by striking it with the club."))
			else
				timer = world.time
				INVOKE_ASYNC(src, PROC_REF(prepare_icon_update))
		else
