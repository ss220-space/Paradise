...
/obj/item/hierophant_club/suicide_act(mob/living/user)
	...
	user.visible_message(span_suicide("[user] поднимает [declent_ru(NOMINATIVE)] в воздух! Похоже, [GEND_HE_SHE(user)] собирается покончить с собой!"))
	new/obj/effect/temp_visual/hierophant/telegraph(get_turf(user))
	playsound(user,'sound/machines/airlock_open.ogg', 75, TRUE)
	user.visible_message(span_hierophant_warning("[user] растворяется в воздухе, оставляя свои вещи!"))
...
/obj/item/hierophant_club/ui_action_click(mob/user, datum/action/action, leftclick)
	...
	if(!beacon || QDELETED(beacon))
		if(isturf(user.loc))
			user.visible_message(
				span_hierophant_warning("[user] возится с навершием [declent_ru(GENITIVE)]..."),
			...
			if(do_after(user, 5 SECONDS, user) && !beacon)
				...
				user.visible_message(span_hierophant_warning("[user] размещает странный механизм у своих ног!"), "[span_hierophant("Вы отсоединяете маяк Иерофанта, позволяя телепортироваться к нему в любой момент!")]\n[span_notice("Можете ударить посохом, чтобы забрать его.")]")
			else
				timer = world.time
				INVOKE_ASYNC(src, PROC_REF(prepare_icon_update))
		else
	...
	user.update_action_buttons_icon()
	user.visible_message(span_hierophant_warning("[user] начинает слабо светиться..."))
	timer = world.time + 50
	INVOKE_ASYNC(src, PROC_REF(prepare_icon_update))
