...
/obj/item/eflowers/afterattack(atom/target, mob/user, proximity, params)
	...
	if(istype(M))
		if(M.client)
			to_chat(user, span_warning("[M.declent_ru_cap(NOMINATIVE)] слишком умён для приручения!"))
			return
		if(M.stat)
			to_chat(user, span_warning("[M.declent_ru_cap(NOMINATIVE)] мёртв!"))
			return
		if(M.faction == user.faction)
			to_chat(user, span_warning("[M.declent_ru_cap(NOMINATIVE)] уже на вашей стороне!"))
			return
		...
		if(M.sentience_type != SENTIENCE_ORGANIC)
			to_chat(user, span_warning("[M.declent_ru_cap(ACCUSATIVE)] невозможно приручить!"))
			return
		if(!do_after(user, 1.5 SECONDS, M))
			return
		M.visible_message(span_notice("[M.declent_ru_cap(NOMINATIVE)] выглядит умиротворённым после контакта с букетом!"))
...
/obj/item/organ/internal/cyberimp/arm/katana/Retract()
	...
	if(!katana.drew_blood)
		to_chat(owner, span_userdanger("[katana.declent_ru_cap(NOMINATIVE)] жадно атакует вас!"))
...
/obj/item/cursed_katana/proc/coagulate(mob/user)
	if(QDELETED(user))
		return
	to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] восстанавливается!"))
	shattered = FALSE
	playsound(user, 'sound/misc/demon_consume.ogg', 50, TRUE)
