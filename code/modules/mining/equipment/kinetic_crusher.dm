...
/obj/item/twohanded/kinetic_crusher/examine(mob/living/user)
	...
	if(length(trophies) != 0)
		...
		for(var/t in trophies)
			...
			. += span_notice("[icon2html(t, user)] <b>[T.declent_ru_cap(NOMINATIVE)]</b>: [T.effect_desc()].")
...
/obj/item/twohanded/kinetic_crusher/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	if(!HAS_TRAIT(src, TRAIT_WIELDED))
		var/warn_message = "[declent_ru_cap(NOMINATIVE)] слишком тяжёлый, чтобы использовать его одной рукой."
