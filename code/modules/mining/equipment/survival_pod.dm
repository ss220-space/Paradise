...
/obj/item/survivalcapsule/attack_self(mob/user)
	...
	if((is_station_level(UT.z)) && !emagged)
		...
		return
	loc.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] начинает вибрировать. Отойдите!"))
	used = TRUE
...
/obj/item/survivalcapsule/proc/expand(mob/triggerer)
	...
	switch(status)
		if(SHELTER_DEPLOY_BAD_AREA)
			loc.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] не функционирует в этой зоне."))
		if(SHELTER_DEPLOY_BAD_TURFS, SHELTER_DEPLOY_ANCHORED_OBJECTS)
			loc.visible_message(span_warning("[declent_ru_cap(DATIVE)] не хватает места для развертывания! Необходимо очистить площадь [template.width]x[template.height]!"))
