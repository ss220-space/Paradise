...
/obj/item/magmite_parts/proc/go_inert()
	if(inert)
		return
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] теряет свечение!"))
	inert = TRUE
...
/obj/item/magmite_parts/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(inert)
		to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)] кажется неактивным! Возможно, Мировая Кузня сможет восстановить его!"))
		return
