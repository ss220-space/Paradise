...
/obj/item/organ/internal/regenerative_core/ui_action_click(mob/user, datum/action/action, leftclick)
	if(inert)
		to_chat(owner, span_notice("[declent_ru_cap(NOMINATIVE)] рассыпается при попытке активации."))
	else
		owner.revive()
	after_use()
...
/obj/item/organ/internal/regenerative_core/insert(mob/living/carbon/M, special = ORGAN_MANIPULATION_DEFAULT)
	...
	if(!preserved && !inert)
		preserved(TRUE)
		owner.visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] стабилизируется при введении."))

/obj/item/organ/internal/regenerative_core/remove(mob/living/carbon/M, special = ORGAN_MANIPULATION_DEFAULT)
	if(!inert && !special)
		owner.visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] быстро разлагается при извлечении."))
		go_inert()
	return ..()
...
/obj/item/organ/internal/regenerative_core/legion/go_inert()
	..()
	desc = "[declent_ru_cap(NOMINATIVE)] утратило силу. Оно сгнило и совершенно бесполезно."

/obj/item/organ/internal/regenerative_core/legion/preserved(implanted = 0)
	..()
	desc = "[declent_ru_cap(NOMINATIVE)] стабилизированно. Теперь его можно безопасно использовать для полного исцеления."
