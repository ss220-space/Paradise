	...
	switch(action)
		...
		if("eject_id")
			...
			if(ishuman(usr))
				...
				usr.visible_message(
					span_notice("[usr] извлекает [inserted_id.declent_ru(ACCUSATIVE)] из [declent_ru(GENITIVE)]."),
		...
		if("eject_disk")
			...
			if(ishuman(usr))
				...
				usr.visible_message(
					span_notice("[usr] извлекает [inserted_disk.declent_ru(ACCUSATIVE)] из [declent_ru(GENITIVE)]."),
