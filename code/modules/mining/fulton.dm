...
/obj/item/extraction_pack/proc/check_use_pack(atom/movable/target, mob/living/carbon/human/user)
	if(!safe_for_living_creatures && check_for_living_mobs(target))
		to_chat(user, span_warning("[capitalize(declent_ru(NOMINATIVE))] небезопас[genderize_ru(gender, "ен", "на", "но", "ны")] для живых существ — они не переживут транспортировку!"))
		return FALSE
