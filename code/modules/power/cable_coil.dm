...
/obj/item/stack/cable_coil/update_icon_state()
	if(istype(target_type, /obj/structure/cable/multilayer))
		icon = 'icons/obj/engines_and_power/power.dmi'
		icon_state = "wire" // for some reason, this doesn't work but it still communicates in which mode is it... Works for me
		return
...
/obj/item/stack/cable_coil/proc/place_turf(turf/target_turf, mob/user)
	if(!isturf(user.loc))
		return
