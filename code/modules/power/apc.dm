...
/obj/machinery/power/apc/attack_hand(mob/user)
	...
	if(usr == user && opened && (!issilicon(user) || istype(user.get_active_hand(), /obj/item/gripper)))
		if(cell)
			user.visible_message(span_warning("[user.name] removes [cell] from [src]!", "You remove the [cell]."))
			cell.forceMove_turf()
		...
		if(prob(3))
			...
			if(cell.charge > 0)
				...
				for(var/mob/M in viewers(src))
					M.show_message(span_danger("The [name] suddenly lets out a blast of smoke and some sparks!", 3, span_danger("You hear sizzling electronics."), 2)
