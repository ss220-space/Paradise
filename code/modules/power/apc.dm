...
/obj/machinery/power/apc/proc/autoset(previous_value, change_value)
	if(change_value == CHANNEL_SETTING_OFF)
		if(previous_value == CHANNEL_SETTING_ON)			// if on, return off
			return CHANNEL_SETTING_OFF
		else if(previous_value == CHANNEL_SETTING_AUTO_ON)		// if auto-on, return auto-off
			return CHANNEL_SETTING_AUTO_OFF

	else if(change_value == CHANNEL_SETTING_AUTO_OFF)
		if(previous_value == CHANNEL_SETTING_AUTO_OFF)			// if auto-off, return auto-on
			return CHANNEL_SETTING_AUTO_ON

	else if(change_value == CHANNEL_SETTING_ON)
		if(previous_value == CHANNEL_SETTING_AUTO_ON)			// if auto-on, return auto-off
			return CHANNEL_SETTING_AUTO_OFF
