...
/obj/machinery/power/supermatter_crystal/proc/handle_high_power()
	...
	if(prob(5))
		supermatter_anomaly_gen(get_ranged_target_turf(src, pick(GLOB.cardinal), rand(5, 10)), HALLUCINATION_ANOMALY, 3)
	if(internal_energy > SEVERE_POWER_PENALTY_THRESHOLD && prob(5) || prob(1))
		supermatter_anomaly_gen(get_ranged_target_turf(src, pick(GLOB.cardinal), rand(5, 10)), GRAVITATIONAL_ANOMALY, 3)
	if((internal_energy > SEVERE_POWER_PENALTY_THRESHOLD && prob(2)) || (prob(0.3) && internal_energy > POWER_PENALTY_THRESHOLD))
		supermatter_anomaly_gen(get_ranged_target_turf(src, pick(GLOB.cardinal), rand(5, 10)), PYRO_ANOMALY, 3)
...
/proc/supermatter_anomaly_gen(turf/anomalycenter, type = FLUX_ANOMALY, anomalyrange = 5, has_changed_lifespan = TRUE)
	...
	switch(type)
		if(FLUX_ANOMALY)
			...
			new /obj/effect/old_anomaly/energetic(local_turf, has_changed_lifespan ? rand(25 SECONDS, 35 SECONDS) : null, FALSE, FALSE)
		if(GRAVITATIONAL_ANOMALY)
			new /obj/effect/old_anomaly/gravitational(local_turf, has_changed_lifespan ? rand(20 SECONDS, 30 SECONDS) : null, FALSE)
		if(PYRO_ANOMALY)
			new /obj/effect/old_anomaly/atmospheric(local_turf, has_changed_lifespan ? rand(15 SECONDS, 25 SECONDS) : null, FALSE)
		//if(HALLUCINATION_ANOMALY)
		//	new /obj/effect/old_anomaly/hallucination/supermatter(local_turf, has_changed_lifespan ? rand(15 SECONDS, 25 SECONDS) : null, FALSE)
		if(VORTEX_ANOMALY)
			new /obj/effect/old_anomaly/bhole(local_turf, 2 SECONDS, FALSE)
		//if(BIOSCRAMBLER_ANOMALY)
		//	new /obj/effect/old_anomaly/bioscrambler/docile(local_turf, null, FALSE)
		//if(DIMENSIONAL_ANOMALY)
		//	new /obj/effect/old_anomaly/dimensional(local_turf, null, FALSE)
