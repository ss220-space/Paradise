...
/obj/machinery/power/solar/crowbar_act(mob/user, obj/item/I)
	...
	user.visible_message(
		span_notice("[user] начинает снимать стекло с [declent_ru(GENITIVE)]."),
		span_notice("Вы начинаете снимать стекло с [declent_ru(GENITIVE)]...")
	)
	if(I.use_tool(src, user, 50, volume = I.tool_volume))
		user.visible_message(
			span_notice("[user] снимает стекло с [declent_ru(GENITIVE)]."),
...
/obj/item/solar_assembly/examine(mob/user)
	. = ..()
	. += span_notice("Похоже, что [GEND_HE_SHE(src)] <b>[anchored ? "прикручен[genderize_ru(gender, "", "а", "о", "ы")]" : "не прикручен[genderize_ru(gender, "", "а", "о", "ы")]"]</b>.")
	if(tracker)
...
/obj/item/solar_assembly/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/sheet/glass) || istype(I, /obj/item/stack/sheet/rglass))
		...
		user.visible_message(
			span_notice("[user] устанавливает стеклянные панели на солнечной батарее."),
	...
	if(istype(I, /obj/item/tracker_electronics))
		...
		user.visible_message(
			span_notice("[user] вставляет плату трекера в солнечную батарею."),
...
/obj/item/solar_assembly/wrench_act(mob/living/user, obj/item/I)
	...
	if(anchored)
		...
		user.visible_message(
			span_notice("[user] прикручивает солнечную батарею к полу."),
			span_notice("Вы прикручиваете солнечную батарею к полу."),
		)
	else
		balloon_alert(user, "откручено")
		user.visible_message(
			span_notice("[user] откручивает солнечную батарею от пола."),
...
/obj/item/solar_assembly/crowbar_act(mob/living/user, obj/item/I)
	...
	user.visible_message(
		span_notice("[user] достает плату солнечного датчика из [declent_ru(GENITIVE)]."),
