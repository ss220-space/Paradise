...
/datum/escape_menu/Destroy(force)
	...
	if(our_hud)
		REMOVE_TRAIT(our_hud, TRAIT_ESCAPE_MENU_OPEN, UNIQUE_TRAIT_SOURCE(src))
...
/datum/escape_menu/proc/on_tile_menu_open()
	SIGNAL_HANDLER
	PRIVATE_PROC(TRUE)

	return COMPONENT_BLOCK_OPEN
...
/datum/escape_menu/proc/refresh_hud()
	...
	if(old_hud)
		REMOVE_TRAIT(old_hud, TRAIT_ESCAPE_MENU_OPEN, UNIQUE_TRAIT_SOURCE(src))

	var/datum/new_hud = client.mob?.hud_used
	our_hud_ref = WEAKREF(new_hud)
	if(new_hud)
		ADD_TRAIT(new_hud, TRAIT_ESCAPE_MENU_OPEN, UNIQUE_TRAIT_SOURCE(src))
