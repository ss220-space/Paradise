...
/proc/generate_escape_menu_suicide_icon()
	...
	var/mob/living/carbon/human/dummy/clown = new
...
/datum/escape_menu/New(client/client)
	...
	window.send_asset(get_asset_datum(/datum/asset/spritesheet_batched/escape_menu_icons))
...
/datum/escape_menu/proc/send_init()
	...
	if(discordbugreporturl)
		resources += list(list("id" = "bug", "label" = "Баг репорт", "tooltip" = "Сообщить о баге"))

	var/githuburl = CONFIG_GET(string/githuburl)
	if(githuburl)
		resources += list(list("id" = "github", "label" = "Github", "tooltip" = "Открыть репозиторий игры"))

	var/forumurl = CONFIG_GET(string/discordurl)
	if(forumurl)
		resources += list(list("id" = "discord", "label" = "Дискорд", "tooltip" = "Присоединиться к Discord-серверу проекта."))

	var/rulesurl = CONFIG_GET(string/rulesurl)
	if(rulesurl)
		resources += list(list("id" = "rules", "label" = "Правила", "tooltip" = "Посмотреть правила сервера"))

	var/wikiurl = CONFIG_GET(string/wikiurl)
	if(wikiurl)
		resources += list(list("id" = "wiki", "label" = "Вики", "tooltip" = "Посмотреть вики по игре"))
	...
	*/
	resources += list(list("id" = "changelog", "label" = "Ченжлог", "tooltip" = "Посмотреть список изменений в билде"))
