...
/datum/hallucination/ice/start()
	...
	ADD_TRAIT(hallucinator, TRAIT_IMMOBILIZED, "hallucination_ice")
	to_chat(hallucinator, span_userdanger("Вы застываете в кубе льда!"))
...
/datum/hallucination/ice/proc/unfreeze()
	REMOVE_TRAIT(hallucinator, TRAIT_IMMOBILIZED, "hallucination_ice")
	if(ice_overlay)
		hallucinator.client?.images -= ice_overlay
		ice_overlay = null
