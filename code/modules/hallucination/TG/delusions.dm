...
/datum/hallucination/delusion/start()
	...
	feedback_details += "Иллюзия: [delusion_name]"
...
/datum/hallucination/delusion/preset/demon
	delusion_icon_file = 'icons/mob/demon.dmi'
...
/proc/get_dynamic_human_appearance(outfit_path, species_path = /datum/species/human, r_hand, l_hand, bloody_slots = NONE, animated = TRUE)
	if(!species_path)
		return FALSE
	...
	for(var/obj/item/carried_item in dummy)
		if(carried_item == dummy.get_active_hand())
