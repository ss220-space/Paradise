...
/obj/effect/client_image_holder/hallucination/fake_door_lock/Initialize(mapload, list/mobs_which_see_us, datum/hallucination/parent, obj/machinery/door/airlock/airlock)
	...
	src.airlock = airlock
	RegisterSignal(airlock, COMSIG_QDELETING, PROC_REF(bolted_airlock_deleted))
	// We need to grab these for our image before we run our parent's initialize
	src.image_icon = airlock.overlays_file
	src.image_state = "lights_bolts"
...
/obj/effect/client_image_holder/hallucination/fake_door_lock/proc/bolted_airlock_deleted(datum/source)
	SIGNAL_HANDLER
