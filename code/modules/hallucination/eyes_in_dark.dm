...
/datum/hallucination/eyes_in_dark/start()
	...
	var/list/valid = list()
	for(var/turf/nearby in view(hallucinator))
	...
	if(prob(5))
		to_chat(hallucinator, span_warning("Вам кажется, что за вами наблюдают..."))
...
/obj/effect/abstract/floating_eyes/update_overlays()
	. = ..()
	var/mutable_appearance/r_eye = mutable_appearance(icon = 'icons/mob/human_eyes.dmi', icon_state = "eyes_glow_r")
	r_eye.color = COLOR_DARK_RED
	. += r_eye
	var/mutable_appearance/l_eye = mutable_appearance(icon = 'icons/mob/human_eyes.dmi', icon_state = "eyes_glow_l")
	l_eye.color = COLOR_DARK_RED
	. += l_eye

	. += emissive_appearance('icons/mob/human_eyes.dmi', "eyes_glow_r", src)
	. += emissive_appearance('icons/mob/human_eyes.dmi', "eyes_glow_l", src)
