...
Важное замечание - значение "name" у темы модсьюта будет использовано во всех его частях в виде "Шлем модели \"name\" " Так что для примера будет использоваться "психологический". \
...
Появляется очевидный вопрос - зачем мы сначала добавляем ru_name, а после задаем стандартный список рунеймов - ответ прост, это необходимо для корректного отображения названия модуля в тгуи костюма. В других примерах кода прок на добавление ру-неймов более не будет дублироваться.
