...
/obj/item/reagent_containers/hypospray/on_reagent_change()
	if(safety_hypo && !emagged)
		...
		if(found_forbidden_reagent)
			if(ismob(loc))
				to_chat(loc, span_warning("[declent_ru_cap(NOMINATIVE)] определяет и удаляет недопустимое вещество."))
			else
				visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] определяет и удаляет недопустимое вещество."))
