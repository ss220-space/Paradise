...
/obj/item/reagent_containers/spray/proc/spray(atom/A)
	var/obj/effect/decal/chempuff/chempuff = new /obj/effect/decal/chempuff(get_turf(src))
	chempuff.create_reagents(amount_per_transfer_from_this)
	reagents.trans_to(chempuff, amount_per_transfer_from_this, 1/spray_currentrange)
	chempuff.color = mix_color_from_reagents(chempuff.reagents.reagent_list)
	var/turf/target_turf = get_turf(A)
	for(var/i in 1 to spray_currentrange)
		step_towards(chempuff, target_turf)
		chempuff.reagents.reaction(get_turf(chempuff))
		for(var/atom/atom in get_turf(chempuff))
			chempuff.reagents.reaction(atom)
		sleep(3)
	qdel(chempuff)
