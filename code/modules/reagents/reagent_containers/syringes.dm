	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(L) //living mob
				...
				to_chat(user, span_notice("Вы заполняете [declent_ru(ACCUSATIVE)] <b>[trans]</b> единиц[declension_ru(trans, "ей", "ами", "ами")] вещества. Теперь он содержит <b>[reagents.total_volume]</b> единиц[DECL_U_Y(reagents.total_volume)] вещества."))
			if(reagents.holder_full())
				mode = !mode
				update_icon()
		...
		if(SYRINGE_INJECT)
			...
			to_chat(user, span_notice("Вы вкололи <b>[amount_per_transfer_from_this]</b> единиц[DECL_U_Y(amount_per_transfer_from_this)] вещества с помощью [declent_ru(GENITIVE)]. В нём остаётся <b>[reagents.total_volume]</b> единиц[DECL_A_Y(reagents.total_volume)] вещества."))
