	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(L) //living mob
				...
				if(target != user)
					target.visible_message(span_danger("[user] пытается взять образец крови у [target]!"), \
											span_userdanger("[user] пытается взять у вас образец крови!"))
					busy = TRUE
		...
		if(SYRINGE_INJECT)
			...
			if(L) //living mob
				if(!L.can_inject(user, TRUE))
					return
				if(L != user)
					L.visible_message(span_danger("[user] пытается сделать [L] укол [declent_ru(INSTRUMENTAL)]!"), \
										span_userdanger("[user] пытается сделать вам укол [declent_ru(INSTRUMENTAL)]!"))
					...
					if(L.reagents.total_volume >= L.reagents.maximum_volume)
						return
					L.visible_message(span_danger("[user] делает [L] укол [declent_ru(INSTRUMENTAL)]!"), \
										span_userdanger("[user] делает вам укол [declent_ru(INSTRUMENTAL)]!"))
