...
/obj/item/reagent_containers/dropper/afterattack(atom/target, mob/user, proximity, params)
	...
	if(iscarbon(target))
		...
		if(!reagents.total_volume)
			return
		if(user != C)
			user.visible_message(span_danger("[user] начинает капать что-то в глаза [C], используя [declent_ru(ACCUSATIVE)]!"))
		...
		if(ishuman(target))
			...
			if(safe_thing)
				user.visible_message(span_danger("[user] пытается капнуть что-то в глаза [C], используя [declent_ru(ACCUSATIVE)], но [GEND_HIM_HER(user)] не удаётся!"))
		...
		user.visible_message(span_danger("[user] закапывает что-то в глаза [C], используя [declent_ru(ACCUSATIVE)]!"))
