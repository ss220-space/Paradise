...
/obj/item/reagent_containers/iv_bag/attack(mob/living/carbon/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(injection_target)
		...
		if(target != user)
			target.visible_message(
				span_danger("[user] пытается убрать иглу [declent_ru(GENITIVE)] из руки [target]!"),
				span_userdanger("[user] пытается убрать иглу [declent_ru(GENITIVE)] из вашей руки!"),
				ignored_mobs = user,
			)
			...
			if(!do_after(user, 3 SECONDS, target, NONE) || !injection_target)
				return .
			target.visible_message(
				span_danger("[user] убирает иглу [declent_ru(GENITIVE)] из руки [target]!"),
				span_userdanger("[user] убирает иглу [declent_ru(GENITIVE)] из вашей руки!"),
				ignored_mobs = user,
			)
			to_chat(user, span_notice("Вы убираете иглу [declent_ru(GENITIVE)] из руки [target]."))
		else
			user.visible_message(span_warning("[user] убирает иглу [declent_ru(GENITIVE)] из своей руки!"))
	...
	if(target != user)
		target.visible_message(
			span_danger("[user] пытается вставить иглу [declent_ru(GENITIVE)] в руку [target]!"),
			span_userdanger("[user] пытается вставить иглу [declent_ru(GENITIVE)] в вашу руку!"),
			ignored_mobs = user,
		)
		...
		if(!do_after(user, 3 SECONDS, target, NONE) || injection_target)
			return .
		target.visible_message(
				span_danger("[user] вставляет иглу [declent_ru(GENITIVE)] в руку [target]!"),
				span_userdanger("[user] вставляет иглу [declent_ru(GENITIVE)] в вашу руку!"),
			ignored_mobs = user,
		)
		balloon_alert(user, "игла вставлена")
	else
		user.visible_message(span_warning("[user] вставляет иглу [declent_ru(GENITIVE)] в свою руку!"))
		balloon_alert(user, "игла вставлена")
