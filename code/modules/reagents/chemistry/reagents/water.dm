...
/datum/reagent/holywater/on_mob_life(mob/living/M)
	...
	if(current_cycle >= 75 && prob(33))	// 30 units, 150 seconds
		...
		if(isvampirethrall(M))
			...
			M.visible_message(span_biggerdanger("[M] отшатывается, [GEND_HIS_HER(M)] кожа окрашивается в яркий цвет, [GEND_HE_SHE(M)] вновь обретает чувство контроля над собой!"))
			M.SetJitter(0)
			M.SetStuttering(0)
			M.SetConfused(0)
			return
		...
		if(vamp.bloodusable)
			...
			switch(current_cycle)
				...
				if(5 to 12)
					...
					vamp.base_nullification()
				if(13 to INFINITY)
					M.visible_message(span_danger("[M] внезапно вспыхивает!"),
