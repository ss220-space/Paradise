...
/datum/reagent/holywater/on_mob_life(mob/living/M)
	...
	if(current_cycle >= 75 && prob(33))	// 30 units, 150 seconds
		...
		if(isvampirethrall(M))
			...
			M.visible_message(span_dangerbigger("[М] отшатыва[pluralize_ru(M.gender, "ет", "ют")]ся, [genderize_ru(M.gender, "его", "её", "его", "их")] кожа окрашивается в яркий цвет, [genderize_ru(M.gender, "он", "она", "оно", "они")] вновь обрета[pluralize_ru(M.gender, "ет", "ют")] чувство контроля над собой!"))
			M.SetJitter(0)
			M.SetStuttering(0)
			M.SetConfused(0)
			return
