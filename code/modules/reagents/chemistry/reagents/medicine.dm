...
/datum/reagent/medicine/synaptizine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 1)
			M.visible_message(span_warning("[M] блю[PLUR_YOT_UT(M)]!"))
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] пошатыва[PLUR_ET_UT(M)]ся и пуска[PLUR_ET_UT(M)] слюни. Из [GEND_HIS_HER(M)] глаз течёт кровь!"))
			M.Dizzy(16 SECONDS)
			M.Weaken(8 SECONDS)
		if(effect <= 15)
			update_flags |= M.adjustToxLoss(1, FALSE)
...
/datum/reagent/medicine/omnizine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1) //lesser
		...
		if(effect <= 1)
			M.visible_message(span_warning("[M] хвата[PLUR_ET_UT(M)]ся за живот от боли!"))
			M.emote("scream")
			M.Weaken(8 SECONDS)
		else if(effect <= 3)
			M.visible_message(span_warning("[M] на мгновение теря[PLUR_ET_UT(M)] сознание!"))
			M.AdjustConfused(30 SECONDS)
		else if(effect <= 5)
			M.visible_message(span_warning("[M] спотыка[PLUR_ET_UT(M)]ся и едва не падает!"))
			M.Dizzy(10 SECONDS)
			M.Weaken(6 SECONDS)
		else if(effect <= 7)
			M.visible_message(span_warning("[M] тряс[PLUR_YOT_UT(M)]ся!"))
			M.Jitter(60 SECONDS)
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] резко дёрга[PLUR_ET_UT(M)]ся и падает!"))
			M.Paralyse(10 SECONDS)
			M.Weaken(8 SECONDS)
		else if(effect <= 8)
			M.visible_message(span_warning("[M] пошатыва[PLUR_ET_UT(M)]ся из стороны в сторону!"))
			M.Dizzy(10 SECONDS)
			M.Weaken(6 SECONDS)
	return list(effect, update_flags)
...
/datum/reagent/medicine/strange_reagent/reaction_mob(mob/living/mob, method = REAGENT_TOUCH, volume)
	...
	if(isanimal(mob) && method == REAGENT_TOUCH)
		...
		if(animal.stat == DEAD)
			...
			animal.visible_message(span_warning("[animal] ожива[PLUR_ET_UT(animal)]!"))
	...
	if(iscarbon(mob))
		...
		if(!mob.ghost_can_reenter())
			mob.visible_message(span_warning("[mob] слегка вздрагива[PLUR_ET_UT(mob)], но в остальном не реагиру[PLUR_ET_UT(mob)]!"))
			return
		if(!mob.suiciding && !HAS_TRAIT(mob, TRAIT_NO_CLONE) && (!mob.mind || mob.mind?.is_revivable()))
			var/time_dead = world.time - mob.timeofdeath
			mob.visible_message(span_warning("[mob] ожива[PLUR_ET_UT(mob)]!"))
			var/update = NONE
...
/datum/reagent/heparin/overdose_process(mob/living/carbon/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] тряс[PLUR_YOT_UT(M)]ся, кровь хлещет прямо из [GEND_HIS_HER(M)] пор!"))
			M.bleed(rand(10, 20))
		else if(effect <= 4)
			M.vomit(0, VOMIT_BLOOD, 0 SECONDS)
			M.AdjustBlood(-rand(5, 10))
				...
				for(var/obj/item/organ/external/bodypart as anything in M.bodyparts)
					if(prob(50)) // Each tick has a 50% chance of repearing a bone.
						if(bodypart.has_fracture()) //I can't just check for !E.status
							to_chat(M, span_notice("Вы чувствуете жжение в ваш[GEND_ENDING_EM_EI_EM_IH(bodypart)] [bodypart.declent_ru(PREPOSITIONAL)], по мере того как [GEND_HE_SHE(bodypart)] применяют правильную форму!"))
							bodypart.mend_fracture()
						if(bodypart.has_internal_bleeding())
							to_chat(M, span_notice("Вы чувствуете жжение в ваш[GEND_ENDING_EM_EI_EM_IH(bodypart)] [bodypart.declent_ru(PREPOSITIONAL)], по мере того как сосуды в [GEND_ON_IN_HIM(bodypart)] восстанавливаются!"))
							bodypart.stop_internal_bleeding()
