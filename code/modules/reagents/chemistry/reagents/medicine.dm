...
/datum/reagent/medicine/synaptizine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 1)
			M.visible_message(span_warning("[M] блюёт!"))
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] пошатывается и пускает слюни. Из [GEND_HIS_HER(M)] глаз течёт кровь!"))
			M.Dizzy(16 SECONDS)
			M.Weaken(8 SECONDS)
		if(effect <= 15)
			update_flags |= M.adjustToxLoss(1, FALSE)
...
/datum/reagent/medicine/omnizine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1) //lesser
		...
		if(effect <= 1)
			M.visible_message(span_warning("[M] хватается за живот от боли!"))
			M.emote("scream")
			M.Weaken(8 SECONDS)
		else if(effect <= 3)
			M.visible_message(span_warning("[M] на мгновение теряет сознание!"))
			M.AdjustConfused(30 SECONDS)
		else if(effect <= 5)
			M.visible_message(span_warning("[M] спотыкается и едва не падает!"))
			M.Dizzy(10 SECONDS)
			M.Weaken(6 SECONDS)
		else if(effect <= 7)
			M.visible_message(span_warning("[M] трясётся!"))
			M.Jitter(60 SECONDS)
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] резко дёргается и падает!"))
			M.Paralyse(10 SECONDS)
			M.Weaken(8 SECONDS)
		else if(effect <= 8)
			M.visible_message(span_warning("[M] пошатывается из стороны в сторону!"))
			M.Dizzy(10 SECONDS)
			M.Weaken(6 SECONDS)
	return list(effect, update_flags)
...
/datum/reagent/medicine/diphenhydramine/on_mob_life(mob/living/M)
	...
	if(prob(3))
		...
		M.visible_message(span_notice("[M] выглядит вялым."))
	return ..()
...
/datum/reagent/medicine/strange_reagent/reaction_mob(mob/living/mob, method = REAGENT_TOUCH, volume)
	...
	if(isanimal(mob) && method == REAGENT_TOUCH)
		...
		if(animal.stat == DEAD)
			...
			animal.visible_message(span_warning("[animal] оживает!"))
	...
	if(iscarbon(mob))
		...
		if(!mob.ghost_can_reenter())
			mob.visible_message(span_warning("[mob] слегка вздрагивает, но в остальном не реагирует!"))
			return
		if(!mob.suiciding && !HAS_TRAIT(mob, TRAIT_NO_CLONE) && (!mob.mind || mob.mind?.is_revivable()))
			var/time_dead = world.time - mob.timeofdeath
			mob.visible_message(span_warning("[mob] оживает!"))
			var/update = NONE
...
/datum/reagent/heparin/overdose_process(mob/living/carbon/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] трясётся, кровь хлещет прямо из [GEND_HIS_HER(M)] пор!"))
			M.bleed(rand(10, 20))
		else if(effect <= 4)
			M.vomit(0, VOMIT_BLOOD, 0 SECONDS)
			M.AdjustBlood(-rand(5, 10))
