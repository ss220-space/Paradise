...
/datum/reagent/spraytan/overdose_process(mob/living/M)
	...
	if(ishuman(M) && is_species(M, /datum/species/human))
		...
		if(prob(7))
			if(N.w_uniform)
				M.visible_message(span_notice(pick("Воротник [M] приподнимается без предупреждения.", "[M] играет своими бицепсами.")))
			else
				M.visible_message(span_notice("[M] играет своими бицепсами."))
	if(prob(10))
		M.say(pick(
