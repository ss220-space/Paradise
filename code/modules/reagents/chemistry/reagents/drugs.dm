...
/datum/reagent/nicotine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message(span_warning("[M] выглядит нервн[genderize_ru(M.gender, "ым", "ой", "ым", "ыми")]!"))
...
/datum/reagent/crank/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message(span_warning("[M] выглядет потерянно!"))
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] потеет как свинья!"))
			M.adjust_bodytemperature(rand(20,100))
			update_flags |= M.adjustToxLoss(5, FALSE)
			M.Stun(6 SECONDS)
		else if(effect <= 4)
			M.visible_message(span_warning("[M] трясётся как эпилептик!"))
			...
			M.emote("scream")
			M.visible_message(span_warning("[M] нервно скребёт свою кожу!"))
			M.Jitter(20 SECONDS)
...
/datum/reagent/krokodil/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message(span_warning("[M] выглядит потерянно!"))
			M.Stun(6 SECONDS)
			M.emote("drool")
		else if(effect <= 4)
			M.emote("shiver")
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] качается и валится на землю!"))
			update_flags |= M.adjustToxLoss(3, FALSE)
...
/datum/reagent/methamphetamine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] машет руками во все стороны, роняя предметы в руках!"))
			M.drop_l_hand()
			M.drop_r_hand()
		else if(effect <= 7)
			M.emote("laugh")
...
/datum/reagent/bath_salts/on_mob_life(mob/living/M)
	...
	if(ishuman(M))
		...
		if(check < 8 && head_organ.h_style != "Very Long Beard")
			...
			H.update_hair()
			H.update_fhair()
			H.visible_message(span_warning("[H] выглядит ДИКО!"))
	if(check < 30)
...
/datum/reagent/consumable/laughter/addiction_act_stage5(mob/living/carbon/M)
	...
	switch(chance)
		...
		if(151 to 200)
			...
			M.visible_message(span_warning("[M] складывается пополам от боли!"))
			M.Weaken(6 SECONDS)
		if(201 to 280)
