...
/datum/reagent/nicotine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message(span_warning("[M] выгляд[PLUR_IT_YAT(M)] нервн[genderize_ru(M.gender, "ым", "ой", "ым", "ыми")]!"))
...
/datum/reagent/crank/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message(span_warning("[M] выгляд[PLUR_ET_UT(M)] потерянно!"))
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] поте[PLUR_ET_UT(M)] как свинья!"))
			M.adjust_bodytemperature(rand(20,100))
			update_flags |= M.adjustToxLoss(5, FALSE)
			M.Stun(6 SECONDS)
		else if(effect <= 4)
			M.visible_message(span_warning("[M] тряс[PLUR_YOT_UT(M)]ся как эпилептик!"))
			...
			M.emote("scream")
			M.visible_message(span_warning("[M] нервно скреб[PLUR_YOT_UT(M)] свою кожу!"))
			M.Jitter(20 SECONDS)
...
/datum/reagent/krokodil/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			M.visible_message(span_warning("[M] кача[PLUR_ET_UT(M)]ся и вал[PLUR_IT_YAT(M)]ся на землю!"))
			update_flags |= M.adjustToxLoss(3, FALSE)
...
/datum/reagent/methamphetamine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] маш[PLUR_ET_YT(M)] руками во все стороны, роняя предметы в руках!"))
			M.drop_l_hand()
			M.drop_r_hand()
		else if(effect <= 7)
			M.emote("laugh")
...
/datum/reagent/consumable/laughter/addiction_act_stage5(mob/living/carbon/M)
	...
	switch(chance)
		...
		if(151 to 200)
			...
			M.visible_message(span_warning("[M] складыва[PLUR_ET_UT(M)]ся пополам от боли!"))
			M.Weaken(6 SECONDS)
		if(201 to 280)
