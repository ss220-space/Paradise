...
/datum/reagent/nicotine/on_mob_life(mob/living/M)
	...
	if(prob(PRIME(50,25)))
		...
		update_flags |= M.adjustStaminaLoss(-PRIME(1,2)*REAGENTS_EFFECT_MULTIPLIER, FALSE)
	return ..() | update_flags
...
/datum/reagent/aranesp/on_mob_life(mob/living/M)
	var/update_flags = STATUS_UPDATE_NONE
	update_flags |= M.adjustStaminaLoss(-PRIME(40,9), FALSE)
	if(prob(90))
		update_flags |= M.adjustToxLoss(1, FALSE)
	if(prob(5))
...
/datum/reagent/lube/ultra/on_mob_life(mob/living/M)
	...
	update_flags |= M.AdjustStunned(-2, FALSE)
	update_flags |= M.AdjustWeakened(-2, FALSE)
	update_flags |= M.adjustStaminaLoss(-PRIME(2,7), FALSE)
	M.status_flags |= GOTTAGOFAST
	M.Jitter(3)
