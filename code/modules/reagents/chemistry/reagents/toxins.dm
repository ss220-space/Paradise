...
/datum/reagent/slimejelly/on_mob_life(mob/living/M)
	...
	if(prob(10))
		...
		update_flags |= M.adjustToxLoss(rand(2, 6), FALSE) // avg 0.4 toxin per cycle, not unreasonable
	else if(prob(40))
...
/datum/reagent/staminatoxin/on_mob_life(mob/living/M)
	var/update_flags = STATUS_UPDATE_NONE
	update_flags |= M.adjustStaminaLoss(data, FALSE)
	data = max(data - 1, 3)
	return ..() | update_flags
...
/datum/reagent/beer2/on_mob_life(mob/living/M)
	...
	switch(current_cycle)
		...
		if(51 to INFINITY)
			update_flags |= M.Sleeping(2, FALSE)
			update_flags |= M.adjustToxLoss((current_cycle - 50), FALSE)
	return ..() | update_flags
