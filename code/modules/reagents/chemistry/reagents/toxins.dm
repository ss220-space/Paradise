	...
	if(method == REAGENT_TOUCH)
		...
		if(volume > 9 && (H.wear_mask || H.head))
			if(H.wear_mask && !(H.wear_mask.resistance_flags & ACID_PROOF))
				to_chat(H, span_danger("Ваш[GEND_ENDING_A_O_I(H.wear_mask)] [H.wear_mask.declent_ru(NOMINATIVE)] плавится!"))
				qdel(H.wear_mask)
				H.update_worn_mask()

			if(H.head && !(H.head.resistance_flags & ACID_PROOF))
				to_chat(H, span_danger("Ваш[GEND_ENDING_A_O_I(H.head)] [H.head.declent_ru(NOMINATIVE)] плавится!"))
				qdel(H.head)
				H.update_worn_head()
...
/datum/reagent/histamine/on_mob_life(mob/living/M)
	...
	if(prob(10))
		M.visible_message(span_danger("[M] расчёсывает кожу до крови!"))
...
/datum/reagent/histamine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] оглушительно кашляет, сгинаясь пополам!"))
			M.Jitter(20 SECONDS)
			M.AdjustStuttering(rand(0, 10 SECONDS))
			M.emote("cough")
			if(prob(40))
...
/datum/reagent/itching_powder/on_mob_life(mob/living/M)
	...
	if(prob(15))
		M.visible_message(span_danger("[M] сильно чешется!"))
		update_flags |= M.adjustBruteLoss(1, FALSE)
	...
	if(prob(2))
		...
		M.visible_message(span_danger("[M] валится на землю, истерично рассчёсывая свою кожу до крови!"))
		M.emote("scream")
	return ..() | update_flags
