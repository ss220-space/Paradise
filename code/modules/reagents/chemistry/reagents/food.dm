	...
	if(method == REAGENT_TOUCH)
		if(ishuman(M))
			...
			if(eyes_covered && mouth_covered)
				to_chat(victim, span_danger("[safe_thing] защищает ваше лицо от перца!"))
				return
			else if(mouth_covered) // Reduced effects if partially protected
				to_chat(victim, span_danger("[safe_thing] почти полностью защищает ваше лицо от перца!"))
				if(prob(20))
					victim.emote("scream")
				...
				to_chat(victim, span_danger("[safe_thing] частично защищает ваше лицо от перца!"))
				if(prob(20))
					victim.emote("scream")
