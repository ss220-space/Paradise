	...
	switch(volume)
		...
		if(40 to 69)
			if(prob(40))
				...
				M.say(pick(list("АААААААХАХАХАХ!", "ИХИХИХИХИХИХХИХИХИХИ!", "УАААААХАХАХ!", "МЬЯХАХАХАХАХАХАХААХАХХАХАХА!", "НЬЯЯЯХАХАХАХАХА!")))
			else if(prob(5))
...
/datum/reagent/consumable/laughter/addiction_act_stage5(mob/living/carbon/M)
	...
	if(prob(8))
		...
		update_flags |= M.adjustBrainLoss(rand(1, 3))
	else if(prob(6))
		...
		update_flags |= M.Stun(5)
		update_flags |= M.Weaken(7)
	else if(prob(8))
