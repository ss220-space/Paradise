...
/obj/machinery/chem_dispenser
	name = "chem dispenser"
	desc = "Высокотехнологичная машина, способная синтезировать определённые реагенты с помощью сложных физико-химических процессов. Даже не спрашивайте, как оно работает - вы всё равно не поймёте."
	ru_names = list(
...
/obj/machinery/chem_dispenser/mutagensaltpeter
	...
	desc = "Узкоспециализированная модель химического раздатчика, настроенная на синтез ограниченного числа реагентов, специально для ботанических нужд."
	ru_names = list(
	...
	switch(mode)
		...
		if("remove")
			if(!target.reagents.remove_reagent(current_reagent, amount))
				to_chat(user, span_notice("Вы забираете [amount] единиц[declension_ru(amount, "у", "ы", "")] [current_reagent] из [target.declent_ru(GENITIVE)]."))
		if("isolate")
