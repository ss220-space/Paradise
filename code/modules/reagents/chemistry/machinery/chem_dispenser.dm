...
/obj/machinery/chem_dispenser/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += span_notice("<br>Монитор состояния сообщает: скорость зарядки - <b>[recharge_amount]</b> единиц[DECL_U_Y(recharge_amount)] энергии за единицу времени.<br>Энергоэффективность увеличена на <b>[round((powerefficiency * 1000) - 100, 1)]%</b>")
...
/obj/item/handheld_chem_dispenser/afterattack(atom/target, mob/user, proximity_flag, list/modifiers, status)
	...
	switch(mode)
		if("dispense")
			...
			if(actual)
				to_chat(user, span_notice("Вы наливаете [amount] единиц[DECL_U_Y(amount)] [current_reagent] в [target.declent_ru(ACCUSATIVE)]."))
			update_icon(UPDATE_OVERLAYS)
		if("remove")
			if(!target.reagents.remove_reagent(current_reagent, amount))
				to_chat(user, span_notice("Вы удаляете [amount] единиц[DECL_U_Y(amount)] [current_reagent] из [target.declent_ru(GENITIVE)]."))
		if("isolate")
