/obj/machinery/computer/pandemic_super
	name = "PanD.E.M.I.C 220 Super"
	desc = "Новейшая модель PanD.E.M.I.C 220, добавлена возможность работать с вирусами напрямую в машине без необходимости ручного смешивания крови. Новый лучший друг вирусолога, руки больше не будут в крови!"
	icon = 'icons/obj/chemical.dmi'
	icon_state = "mixer0"
	circuit = /obj/item/circuitboard/pandemic_super
	idle_power_usage = 30
...
/obj/machinery/computer/pandemic_super/get_ru_names()
	return alist(
		NOMINATIVE = "Панд.Е.М.И.К 220 Super",
		GENITIVE = "Панд.Е.М.И.К 220 Super",
		DATIVE = "Панд.Е.М.И.К 220 Super",
		ACCUSATIVE = "Панд.Е.М.И.К 220 Super",
		INSTRUMENTAL = "Панд.Е.М.И.К 220 Super",
		PREPOSITIONAL = "Панд.Е.М.И.К 220 Super",
	)

/obj/machinery/computer/pandemic_super/examine(mob/user)
	. = ..()
	if(panel_open)
		. += span_notice("Панель техобслуживания открыта.")

/obj/machinery/computer/pandemic_super/Initialize(mapload)
	. = ..()
	update_icon()

/obj/machinery/computer/pandemic_super/set_broken()
	stat |= BROKEN
	update_icon()

/obj/machinery/computer/pandemic_super/update_icon_state()
	if(stat & BROKEN)
		icon_state = "mixer[beaker ? "1" : "0"]_b"
		return
	icon_state = "mixer[beaker ? "1" : "0"][(powered()) ? "" : "_nopower"]"

/obj/machinery/computer/pandemic_super/update_overlays()
	. = ..()
	if(!(stat & BROKEN) && !wait)
		. += "waitlight"

/obj/machinery/computer/pandemic_super/attack_ai(mob/user)
	add_hiddenprint(user)
	attack_hand(user)

/obj/machinery/computer/pandemic_super/attack_hand(mob/user)
...
/obj/machinery/computer/pandemic_super/attackby(obj/item/item, mob/user, params)
...
/obj/machinery/computer/pandemic_super/screwdriver_act(mob/user, obj/item/item)
	. = TRUE
	if(!beaker)
		add_fingerprint(user)
...
/obj/machinery/computer/pandemic_super/wrench_act(mob/living/user, obj/item/item)
	return default_unfasten_wrench(user, item)

// MARK: TGUI
/obj/machinery/computer/pandemic_super/ui_interact(mob/user, datum/tgui/ui = null)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "PandemicSuper")
		ui.open()

/obj/machinery/computer/pandemic_super/ui_data(mob/user)
	var/list/data = list()
...
/obj/machinery/computer/pandemic_super/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	if(..())
		return FALSE
	add_fingerprint(usr)
...
/obj/machinery/computer/pandemic_super/proc/ui_act_modal(action, params)
	return FALSE


// MARK: Actions
/obj/machinery/computer/pandemic_super/proc/eject_beaker()
...
/obj/machinery/computer/pandemic_super/proc/print_form(datum/disease/virus/advance/disease, mob/living/user)
...
/obj/machinery/computer/pandemic_super/proc/GetDiseaseByIndex(index)
...
/obj/machinery/computer/pandemic_super/proc/GetResistancesByIndex(index)
	if(length(beaker?.reagents?.reagent_list))
		for(var/datum/reagent/BL in beaker.reagents.reagent_list)
...
/obj/machinery/computer/pandemic_super/proc/GetDiseaseTypeByIndex(index)
	var/datum/disease/disease = GetDiseaseByIndex(index)
	if(disease)
		return disease.GetDiseaseID()

/obj/machinery/computer/pandemic_super/proc/replicator_cooldown(waittime)
	wait = 1
	update_icon()
	spawn(waittime)
		wait = null
