		...
		if(ailist.len)
			...
			if(character.mind)
				if((character.mind.assigned_role != JOB_TITLE_CYBORG) && (character.mind.assigned_role != character.mind.special_role))
					...
					GLOB.global_announcer.autosay(arrivalmessage, "Автоматическая система оповещения", follow_target_override = character)
		...
		if(length(ailist))
			...
			if(character.mind)
				if(character.mind.assigned_role != character.mind.special_role)
					...
					GLOB.global_announcer.autosay("A new[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "прибыл на станцию"].", "Система оповещения", follow_target_override = character)
