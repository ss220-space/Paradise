		...
		if(ailist.len)
			...
			if(character.mind)
				if((character.mind.assigned_role != JOB_TITLE_CYBORG) && (character.mind.assigned_role != character.mind.special_role))
					...
					GLOB.global_announcer.autosay(arrivalmessage, "Система оповещения")
		...
		if(ailist.len)
			...
			if(character.mind)
				if(character.mind.assigned_role != character.mind.special_role)
					...
					GLOB.global_announcer.autosay("A new[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "прибыл на станцию"].", "Система оповещения")
