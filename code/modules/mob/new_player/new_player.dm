		...
		if(ailist.len)
			...
			if(character.mind)
				if((character.mind.assigned_role != "Cyborg") && (character.mind.assigned_role != character.mind.special_role))
					if(character.mind.role_alt_title)
						rank = character.mind.role_alt_title
					GLOB.global_announcer.autosay("[character.real_name],[rank ? " [rank]," : " посетитель," ] [join_message ? join_message : "прибыл на станцию"].", "Arrivals Announcement Computer")
		...
		if(ailist.len)
			...
			if(character.mind)
				if(character.mind.assigned_role != character.mind.special_role)
					...
					GLOB.global_announcer.autosay("Новый[rank ? " [rank]" : " посетитель" ] [join_message ? join_message : "прибыл на станцию"].", "Arrivals Announcement Computer")
