...
/client/verb/toggle_throw_mode()
	...
	if(iscarbon(mob))
		...
		to_chat(usr, "<span class='danger'>Этот тип моба не может бросать предметы.</span>")
		...
		for(var/X in mob.grabbed_by)
			...
			switch(G.state)
				...
				if(GRAB_NECK)
					...
					if(!prob(5))
						return TRUE
					mob.visible_message("<span class='danger'>[mob] вырвался из захвата головы [G.assailant]!</span>")
					qdel(G)
	return FALSE
...
/mob/proc/toggle_move_intent()
	if(iscarbon(src))
		...
		if(C.legcuffed)
			to_chat(C, "<span class='notice'>Ваши ноги связаны! Вы не можете бежать пока не снимете [C.legcuffed]!</span>")
