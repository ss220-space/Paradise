...
/datum/ai_behavior/find_hunt_target/find_hive/valid_dinner(mob/living/source, obj/structure/beebox/hive, radius)
	if(!length(hive.honeycombs))
		return FALSE
	return source.can_see(hive, radius)
...
/datum/ai_behavior/hunt_target/find_honeycomb/target_caught(mob/living/hunter, obj/item/reagent_containers/honeycomb/food_target)
