...
/datum/ai_controller/basic_controller/legion/advanced
	blackboard = list(
		BB_TARGETING_STRATEGY = /datum/targeting_strategy/basic/legion,
...
/datum/ai_controller/basic_controller/legion_brood/advanced
	blackboard = list(
		BB_TARGETING_STRATEGY = /datum/targeting_strategy/basic/legion,
		BB_TARGET_MINIMUM_STAT = DEAD,
	)
...
/datum/ai_planning_subtree/random_speech/legion/speak(datum/ai_controller/controller)
	...
	if(victim.mind?.miming) // mimes cant talk
		return
	...
	controller.queue_behavior(/datum/ai_behavior/perform_speech_radio, pick(radio_speech), mob_radio, list(SUP_FREQ_NAME, PUB_FREQ_NAME))
