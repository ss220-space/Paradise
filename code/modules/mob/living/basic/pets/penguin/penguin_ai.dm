...
/datum/ai_behavior/find_hunt_target/penguin_egg/valid_dinner(mob/living/source, atom/dinner, radius)
	return source.can_see(dinner, radius) && !(dinner in source.contents)
...
/datum/ai_planning_subtree/find_and_hunt_target/drill_ice
	...
	hunt_targets = list(/turf/simulated/floor/plating/ice)
	hunt_range = 7

/datum/ai_behavior/find_hunt_target/search_turf_types/drillable_ice

/datum/ai_behavior/find_hunt_target/search_turf_types/drillable_ice/valid_dinner(mob/living/source, turf/simulated/floor/plating/ice/ice, radius)
	return ice.can_make_hole && source.can_see(ice, radius)
...
/datum/ai_behavior/find_and_set/in_list/drilled_ice/search_tactic(datum/ai_controller/controller, locate_paths, search_range = SEARCH_TACTIC_DEFAULT_RANGE)
	for(var/atom/possible_ice as anything in RANGE_TURFS(search_range, controller.pawn))
		if(!istype(possible_ice, /turf/simulated/floor/plating/ice))
			continue
