...
/mob/living/basic/pet/penguin/Initialize(mapload)
	...
	var/static/list/fishable_objects = typecacheof(list(/turf/simulated/floor/plating/ice))
...
/mob/living/basic/pet/penguin/UnarmedAttack(atom/attack_target, proximity_flag, list/modifiers)
	...
	if(!proximity_flag)
		return
	if(!istype(attack_target, /obj/item/reagent_containers/food/snacks/egg/penguin_egg))
		return
...
/mob/living/basic/pet/penguin/baby/Initialize(mapload)
	...
	var/grown_type = pickweight(weight_mobtypes)
	AddComponent(\
