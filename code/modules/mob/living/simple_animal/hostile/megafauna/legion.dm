...
/mob/living/simple_animal/hostile/megafauna/legion/proc/fire_disintegration_laser(location)
	...
	for(var/turf/turf as anything in get_line(src, location))
		if(ismineralturf(turf))
			var/turf/simulated/mineral/mineral = turf
			mineral.attempt_drill(src)
		if(iswallturf(turf))
			var/turf/simulated/wall/wall = turf
			wall.thermitemelt(time = 1 SECONDS)
		for(var/mob/living/mineral in turf)
			if(faction_check(mineral.faction, faction, FALSE))
				continue
			if(mineral.stat == DEAD)
				visible_message(span_danger("[mineral.declent_ru(ACCUSATIVE)] дезинтегрируется лучом!"))
				to_chat(mineral, span_userdanger("Вас поражает луч дезинтеграции!"))
				mineral.dust()
			else if(mineral != src)
				playsound(mineral,'sound/weapons/sear.ogg', 50, TRUE, -4)
				to_chat(mineral, span_userdanger("You're struck by a disintegration laser!"))
				var/limb_to_hit = mineral.get_organ(pick(BODY_ZONE_HEAD, BODY_ZONE_CHEST, BODY_ZONE_R_ARM, BODY_ZONE_L_ARM, BODY_ZONE_R_LEG, BODY_ZONE_L_LEG))
				var/armor = mineral.run_armor_check(limb_to_hit, LASER)
				mineral.apply_damage(70 - ((health / maxHealth) * 20), BURN, limb_to_hit, armor)
