...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/lava_arena()
	if(!target)
		return
	target.visible_message(span_boldwarning("[declent_ru_cap(NOMINATIVE)] заключает вас в арену огня!"))
	var/amount = 3
...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/arena_escape_enrage() // you ran somehow / teleported away from my arena attack now i'm mad fucker
	SLEEP_CHECK_DEATH(src, 0)
	SetRecoveryTime(80)
	visible_message(span_boldwarning("[declent_ru_cap(NOMINATIVE)] начинает ярко светиться, пока его раны закрываются!"))
...
/mob/living/simple_animal/hostile/megafauna/dragon/proc/swoop_attack(lava_arena = FALSE, atom/movable/manual_target, swoop_cooldown = 30)
	...
	icon_state = "shadow"
	visible_message(span_boldwarning("[declent_ru_cap(NOMINATIVE)] взмывает высоко вверх!"))
	...
	for(var/mob/living/L in orange(1, src))
		if(L.stat)
			visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] обрушивается на [L.declent_ru(NOMINATIVE)], раздавливая [GEND_HIS_HER(L)]!"))
			L.gib()
		else
			...
			if(L && !QDELETED(L)) // Some mobs are deleted on death
				...
				visible_message(span_warning("[L.declent_ru_cap(NOMINATIVE)] отбрасывается в сторону от [declent_ru(ACCUSATIVE)]!"))
