...
/mob/living/simple_animal/hostile/poison/terror_spider/defiler/spider_specialattack(mob/living/carbon/human/L, poisonable)
	...
	if(HAS_TRAIT(L, TRAIT_INCAPACITATED) || L.can_inject(null, FALSE, inject_target, FALSE) && prob(50))
		...
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] вонзает свои длинные клыки глубоко в [inject_target] [target.declent_ru(ACCUSATIVE)]!"))
	else
		if(prob(20))
			new /obj/item/organ/internal/body_egg/terror_eggs(L)
			visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] пробивает броню и вонзает свои длинные клыки глубоко в [inject_target] [target.declent_ru(ACCUSATIVE)]!"))
	if(!ckey && !IsTSInfected(L))
		step_away(src, L)
		step_away(src, L)
		lose_target()
		step_away(src, L)
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] отскакивает от [L.declent_ru(ACCUSATIVE)]!"))
	...
	if(istype(C))
		if(!IsTSInfected(C) && ishuman(C))
			...
			if(C.can_inject(null, FALSE, inject_target, FALSE))
				to_chat(C, "[declent_ru_cap(NOMINATIVE)] врезается в вас!")
