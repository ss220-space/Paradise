...
/mob/living/simple_animal/hostile/poison/terror_spider/queen/proc/DoQueenScreech(light_range, light_chance, camera_range, camera_chance)
	...
	for(var/obj/machinery/light/light in orange(light_range, src))
		if(light.on && prob(light_chance))
			light.break_light_tube()
	for(var/obj/machinery/camera/camera in orange(camera_range, src))
		if(camera.status && prob(camera_chance))
