	...
	if(can_die() && !hasdied)
		...
		for(var/thing in GLOB.ts_spiderlist)
			...
			if(T.spider_tier < spider_tier)
				T.visible_message(span_danger("[T.declent_ru_cap(NOMINATIVE)] корчится от боли!"))
		...
		switch(neststep)
			if(0)
				...
				if(ok_to_nest && entry_vent)
					...
					neststep = 1
					visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] приживается, начиная строить гнездо."))
				else if(entry_vent)
					if(!path_to_vent)
						visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] настороженно оглядывается — затем ищет лучшее место для строительста гнезда."))
						path_to_vent = 1
				else
					neststep = -1
...
/mob/living/simple_animal/hostile/poison/terror_spider/queen/proc/DoQueenScreech(light_range, light_chance, camera_range, camera_chance)
	visible_message(span_userdanger("[declent_ru_cap(NOMINATIVE)] издает пронзительный визг!"))
