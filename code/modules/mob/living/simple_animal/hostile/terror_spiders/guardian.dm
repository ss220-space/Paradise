...
/mob/living/simple_animal/hostile/poison/terror_spider/guardian/spider_specialattack(mob/living/carbon/human/L)
	...
	if(prob(20))
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] врезается в [L.declent_ru(ACCUSATIVE)], сбивая с ног!"))
		...
		if(spider_myqueen.stat != DEAD && !spider_myqueen.ckey)
			if(get_dist(src, spider_myqueen) > max_queen_range)
				if(!degenerate && !spider_myqueen.degenerate)
					...
					visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] стрекочет в направлении [spider_myqueen.declent_ru(GENITIVE)]!"))
	return ..()
	...
	if(spider_myqueen)
		...
		if(Q)
			if(Q.stat == DEAD)
				spider_myqueen = null
				degenerate = TRUE
				to_chat(src, span_userdanger("[Q.declent_ru_cap(NOMINATIVE)] умерла! Её сила больше не поддерживает вас!"))
				return
			...
			if(queen_visible)
				cycles_noqueen = 0
				if(spider_debug)
					to_chat(src, span_notice("[Q.declent_ru_cap(NOMINATIVE)] в зоне видимости."))
			else
				cycles_noqueen++
				if(spider_debug)
					to_chat(src, span_danger("[Q.declent_ru_cap(NOMINATIVE)] НЕ в зоне видимости. Цикл: [cycles_noqueen]."))
	...
	if(statpanel(STATPANEL_STATUS) && ckey && stat == CONSCIOUS)
		if(spider_myqueen)
			...
			if(degenerate)
				...
				status_tab_data[++status_tab_data.len] = list("Связь:", "<font color='#32a852'>[spider_myqueen.declent_ru_cap(NOMINATIVE)] рядом</font>") // color=green
