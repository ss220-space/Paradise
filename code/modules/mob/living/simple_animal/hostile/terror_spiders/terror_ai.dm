		...
		if(path_to_vent)
			if(entry_vent)
				if(spider_steps_taken > spider_max_steps)
					...
					if(spider_debug)
						visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] движется к вентиляционному отверстию [entry_vent.declent_ru(GENITIVE)]."))
			else
				path_to_vent = 0
			...
			for(var/obj/machinery/light/L in range(1, src))
				if(!L.status)
					...
					visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] разбивает [L.declent_ru(ACCUSATIVE)]."))
					return
			...
			if(!T)
				new web_type(loc)
				visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] плетёт паутину."))
	...
	if(cocoon_target)
		if(get_dist(src, cocoon_target) <= 1)
			...
			if(spider_steps_taken > spider_max_steps)
				...
				if(spider_debug)
					visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] движется к [cocoon_target.declent_ru(DATIVE)], чтобы заплести в кокон."))
	...
	if(entry_vent)
		if(get_dist(src, entry_vent) <= 2)
			if(ai_ventbreaker && entry_vent.welded)
				...
				entry_vent.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] выбивает приваренную крышку [entry_vent.declent_ru(GENITIVE)]!"))
			var/list/vents = list()
			...
			visible_message("<b>[declent_ru_cap(NOMINATIVE)] залезает в вентиляционные каналы!</b>", span_notice("Слышно, как что-то сжимается в вентиляционных каналах."))
			spawn(rand(20,60))
				...
				spawn(travel_time)
					...
					spawn(travel_time)
						...
						if(ai_ventbreaker && exit_vent.welded)
							...
							exit_vent.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] выбивает приваренную крышку [exit_vent.declent_ru(GENITIVE)]!"))
