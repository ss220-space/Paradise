...
/mob/living/simple_animal/hostile/poison/terror_spider/proc/Web()
	...
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] начинает выделять липкое вещество."))
...
/obj/structure/spider/terrorweb/bullet_act(obj/projectile/Proj)
	if(Proj.damage_type != BRUTE && Proj.damage_type != BURN)
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] невосприимчива к [Proj.declent_ru(DATIVE)]!"), projectile_message = TRUE)
...
/mob/living/simple_animal/hostile/poison/terror_spider/proc/DoWrap()
	if(cocoon_target && busy != SPINNING_COCOON)
		...
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] начинает выделять липкое вещество вокруг [cocoon_target.declent_ru(GENITIVE)]."))
				...
				if(cocoon_target && isturf(cocoon_target.loc) && get_dist(src,cocoon_target) <= 1)
					...
					for(var/mob/living/L in C.loc)
						...
						if(iscarbon(L))
							...
							fed++
							visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] втыкает хоботок в [L.declent_ru(ACCUSATIVE)] и высасывает вязкое вещество."))
							to_chat(src, span_notice("Вы начинаете быстро восстанавливаться!"))
							if(L.mind && ishuman(L))
								SEND_SIGNAL(mind, COMSIG_HUMAN_EATEN)
						else
							visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] заматывает [L.declent_ru(ACCUSATIVE)] в паутину."))
						large_cocoon = 1
	...
	if(do_after(src, 4.3 SECONDS, loc))
		for(var/obj/machinery/atmospherics/unary/vent_pump/P in range(1, get_turf(src)))
			if(P.welded)
				P.set_welded(FALSE)
				forceMove(P.loc)
				P.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] выбивает приваренную крышку [P.declent_ru(GENITIVE)]!"))
				return
		for(var/obj/machinery/atmospherics/unary/vent_scrubber/C in range(1, get_turf(src)))
			if(C.welded)
				C.set_welded(FALSE)
				forceMove(C.loc)
				C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] выбивает приваренную крышку [C.declent_ru(GENITIVE)]!"))
				return
