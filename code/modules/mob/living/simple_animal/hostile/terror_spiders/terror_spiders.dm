...
/mob/living/simple_animal/hostile/poison/terror_spider/AttackingTarget()
	if(isterrorspider(target))
		...
		if(T.spider_tier > spider_tier)
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] съёживается перед [target.declent_ru(INSTRUMENTAL)]."))
		else if(T.spider_tier == spider_tier)
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] тычется носом в [target.declent_ru(ACCUSATIVE)]."))
		else if(T.spider_tier < spider_tier && spider_tier >= 4)
			target.attack_animal(src)
		else
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] безобидно тычет носом [target.declent_ru(ACCUSATIVE)]."))
		T.CheckFaction()
		CheckFaction()
		...
		if(F.density)
			if(F.welded)
				to_chat(src, "[F.declent_ru_cap(NOMINATIVE)] заварен.")
			else
				visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] открывает [F.declent_ru(ACCUSATIVE)]!"))
				F.open()
		else
...
/mob/living/simple_animal/hostile/poison/terror_spider/New()
	...
	name += " ([rand(1, 1000)])"
	real_name = name
	msg_terrorspiders("[declent_ru_cap(NOMINATIVE)] вырастает в локации \"[get_area(src)]\".")
	if(is_away_level(z))
...
/mob/living/simple_animal/hostile/poison/terror_spider/proc/announcetoghosts()
	...
	if(stat == DEAD)
		return
	if(ckey)
		notify_ghosts("[declent_ru_cap(NOMINATIVE)] (контролируется игроком) появляется в локации \"[get_area(src)]\".")
	else if(ai_playercontrol_allowtype)
		var/image/alert_overlay = image(icon, icon_state)
		notify_ghosts("[declent_ru_cap(NOMINATIVE)] появляется в локации \"[get_area(src)]\".", enter_link = "<a href=byond://?src=[UID()];activate=1>(Нажмите для взятия контроля)</a>", source = src, alert_overlay = alert_overlay, action = NOTIFY_ATTACK)
...
/mob/living/simple_animal/hostile/poison/terror_spider/death(gibbed)
	if(can_die())
		if(!gibbed)
			msg_terrorspiders("[declent_ru_cap(NOMINATIVE)] умирает в локации \"[get_area(src)]\".")
		handle_dying()
		if(mind)
...
/mob/living/simple_animal/hostile/poison/terror_spider/proc/try_open_airlock(obj/machinery/door/airlock/D)
	...
	if(D.welded)
		...
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] открывает дверь силой!"))
