...
/obj/effect/proc_holder/spell/terror_stealth/cast(list/targets, mob/user = usr)
	user.alpha = 0
	user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] внезапно исчезает!"), span_purple("Вы теперь невидимы!"))
...
/obj/effect/proc_holder/spell/terror_stealth/proc/reveal(mob/user)
	...
	user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] появляется из ниоткуда!"), span_purple("Вы снова видимы!"))
...
/obj/effect/proc_holder/spell/aoe/terror_healing/cast(list/targets, mob/user = usr)
	for(var/mob/living/simple_animal/hostile/poison/terror_spider/spider in targets)
		visible_message(span_green("[user.declent_ru_cap(NOMINATIVE)] источает целительные феромоны!"))
