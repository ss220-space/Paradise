...
/mob/living/simple_animal/hostile/gorilla/examine_more(mob/user)
	. = ..()
	. += span_notice("<b>Руководство по работе с гориллами для самых маленьких:</b>\
...
/mob/living/simple_animal/hostile/gorilla/say(message, verb = "говор[PLUR_IT_YAT(src)]", sanitize = TRUE, ignore_speech_problems = FALSE, ignore_atmospherics = FALSE, ignore_languages = FALSE, ignore_emotes = FALSE)
	. = ..()
	if(.)
...
/mob/living/simple_animal/hostile/gorilla/cargo_domestic/Login()
	. = ..()
	to_chat(src, span_boldnotice("Вы \"[name]\" - одомашненная горилла и питомец отдела грузоперевозок. \
			Вы преданная и трудолюбивая горилла и любите свою работу. Вы хорошая горилла, и карго любит Вас."))
	to_chat(src, span_boldnotice("В интенте \"HELP\" Вы можете подбирать ящики, щёлкнув по ним, и бросить их, щелкнув по открытому полу. \
			Вы можете переносить [crate_limit] ящик[DECL___A_OV(crate_limit)] единовременно. \
			В интенте \"HARM\" Вы можете толкать ящики, но не ломать их. Также Вы можете закрывать или открывать ящики используя Alt+Click."))
