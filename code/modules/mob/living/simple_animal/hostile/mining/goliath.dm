...
/mob/living/simple_animal/hostile/asteroid/goliath/bullet_act(obj/projectile/P)
	if(prob(reflect_chance) && !istype(P, /obj/projectile/destabilizer))
		visible_message(span_danger("[P.declent_ru_cap(NOMINATIVE)] отскакивает от крепкой шкуры [declent_ru(GENITIVE)]!"), span_userdanger("[P.declent_ru_cap(NOMINATIVE)] отскакивает от крепой шкуры [declent_ru(GENITIVE)]!"), projectile_message = TRUE)
...
/mob/living/simple_animal/hostile/asteroid/goliath/proc/melee_attack(list/dirs)
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] выпускает щупальца из-под земли вокруг себя!"))
	for(var/d in dirs)
...
/mob/living/simple_animal/hostile/asteroid/goliath/proc/ranged_attack()
	var/tturf = get_turf(target)
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] опутывает щупальцами [target.declent_ru(ACCUSATIVE)]!"))
...
/obj/effect/temp_visual/goliath_tentacle/proc/trip()
	...
	for(var/mob/living/L in loc)
		if((!QDELETED(spawner) && spawner.faction_check_mob(L)) || L.stat == DEAD)
			continue
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] захватывает [L.declent_ru(ACCUSATIVE)]!"))
		if(!L.IsStunned())
			L.Stun(10 SECONDS)
