...
/mob/living/simple_animal/hostile/asteroid/goliath/proc/melee_attack(list/dirs)
	...
	visible_message(span_warning("[capitalize(src.declent_ru(NOMINATIVE))] выпускает щупальца из-под земли вокруг себя!"))
	for(var/d in dirs)
...
/mob/living/simple_animal/hostile/asteroid/goliath/proc/ranged_attack()
	var/tturf = get_turf(target)
	visible_message(span_warning("[capitalize(src.declent_ru(NOMINATIVE))] опутывает щупальцами [target.declent_ru(ACCUSATIVE)]!"))
...
/obj/effect/temp_visual/goliath_tentacle/proc/trip()
	...
	for(var/mob/living/L in loc)
		if((!QDELETED(spawner) && spawner.faction_check_mob(L)) || L.stat == DEAD)
			continue
		visible_message(span_danger("[capitalize(src.declent_ru(NOMINATIVE))] захватывает [L]!"))
		if(!L.IsStunned())
			L.Stun(10 SECONDS)
