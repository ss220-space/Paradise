...
/mob/living/simple_animal/hostile/asteroid/goldgrub/GiveTarget(new_target)
	...
	if(!QDELETED(target))
		if(wanted_objects[target.type] && length(loot) < max_loot)
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] смотрит на [target.declent_ru(ACCUSATIVE)] голодными глазами."))
		else if(iscarbon(target) || issilicon(target))
			Aggro()
			visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] пытается сбежать от [target.declent_ru(ACCUSATIVE)]!"))
			retreat_distance = 10
			minimum_distance = 10
			if(will_burrow)
...
/mob/living/simple_animal/hostile/asteroid/goldgrub/proc/EatOre(atom/targeted_ore)
	...
	if(length(loot) < max_loot)
		...
		for(var/i in 1 to using)
			loot += O.type
		O.use(using)
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] целиком проглотил руду!"))
	else // We are now full! We will consume no more ore ever again.
		search_objects = 0
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] поклёвывает немного руды и останавливается. Похоже, он наелся."))

/mob/living/simple_animal/hostile/asteroid/goldgrub/proc/Burrow()//You failed the chase to kill the goldgrub in time!
	if(stat == CONSCIOUS)
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] зарывается в землю и исчезает из виду!"))
		qdel(src)
