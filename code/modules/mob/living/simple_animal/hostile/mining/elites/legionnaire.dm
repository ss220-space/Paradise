...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/legionnaire_charge(target)
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] готовится к рывку!"))
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/legionnaire_charge_to(move_dir, times_ran, list/hit_targets = list())
	...
	for(var/mob/living/L in T.contents - src)
		if(faction_check_mob(L))
			return
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] топчет и пинает [L.declent_ru(ACCUSATIVE)]!"))
		to_chat(L, span_userdanger("[declent_ru_cap(NOMINATIVE)] топчет вас и отбрасывает пинком!"))
		if(L in hit_targets)
...
/mob/living/simple_animal/hostile/asteroid/elite/legionnaire/proc/bonfire_teleport()
	...
	if(isnull(mypile))
		...
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] призывает костёр на [get_turf(src)]!"))
		return
	else
		...
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] распадается на горящую груду костей!"))
		forceMove(pileturf)
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] формируется из костра!"))
