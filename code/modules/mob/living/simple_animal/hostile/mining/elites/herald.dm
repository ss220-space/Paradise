...
/obj/item/clothing/accessory/necklace/herald_cloak/proc/mirror_walk()
	...
	if(chosen == null)
		return
	usr.visible_message(span_warning("[usr] начинает пролезать в [starting_mirror.declent_ru(ACCUSATIVE)]..."), span_notice("Вы начинаете пролезать в [starting_mirror.declent_ru(ACCUSATIVE)]..."))
	if(do_after(usr, 2 SECONDS, usr))
		...
		if(QDELETED(chosen) || !usr|| usr.incapacitated() || !chosen || (get_dist(src, starting_mirror) > 1 || destination.z != usr.z))
			return
		usr.visible_message(span_warning("[usr] пролезает в [starting_mirror.declent_ru(ACCUSATIVE)], и исчезает в нём!"), span_notice("Вы пролезаете в [starting_mirror.declent_ru(ACCUSATIVE)]..."))
		usr.forceMove(destination)
		usr.visible_message(span_warning("[usr] вылезает из [chosen.declent_ru(ACCUSATIVE)], разбивая его!"), span_warning("Вы вылезаете из собственного отражения, разбивая зеркало!"))
