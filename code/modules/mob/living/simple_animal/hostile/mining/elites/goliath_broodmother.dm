...
/mob/living/simple_animal/hostile/asteroid/elite/broodmother/proc/tentacle_patch(target)
	...
	if(!isturf(tturf))
		return
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] зарывает свои щупальца под [target]!"))
...
/mob/living/simple_animal/hostile/asteroid/elite/broodmother/proc/rage()
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] начинает набирать скорость!"))
	color = "#FF0000"
	set_varspeed(0)
	move_to_delay = 3
...
/mob/living/simple_animal/hostile/asteroid/elite/broodmother_child/death(gibbed)
	...
	if(!QDELETED(mother))
		mother.children_list -= src
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] взрывается!"))
...
/obj/effect/temp_visual/goliath_tentacle/broodmother/trip()
	...
	for(var/mob/living/L in loc)
		if((!QDELETED(spawner) && spawner.faction_check_mob(L)) || L.stat == DEAD)
			continue
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] опутывает щупальцами [L.declent_ru(ACCUSATIVE)]!"))
