...
/mob/living/simple_animal/hostile/asteroid/marrowweaver/adjustHealth(
	...
	if(!buttmad)
		if(health < maxHealth/3)
			buttmad = TRUE
			visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] яростно стрекочет, оскалив клыки!"))
		...
		if(health > maxHealth/2)
			buttmad = FALSE
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] вроде бы успокоился, но не до конца."))
	...
	if(. && isliving(target))
		...
		if((L.stat == DEAD) && (health < maxHealth) && ishuman(L))
			if(fiesta(L, FALSE))
				...
				src.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] впрыскивает токсичную слизь во внутренности [L]..."),
