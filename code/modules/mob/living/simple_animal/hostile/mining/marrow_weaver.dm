/mob/living/simple_animal/hostile/asteroid/marrowweaver
	...
	ru_names = list(
		...
		PREPOSITIONAL = "о костномозговом ткаче"
	)
...
/mob/living/simple_animal/hostile/asteroid/marrowweaver/adjustHealth(
	...
	if(!buttmad)
		if(health < maxHealth/3)
			buttmad = TRUE
			visible_message(span_danger("[capitalize(src.declent_ru(NOMINATIVE))] яростно стрекочет, оскалив клыки!"))
		...
		if(health > maxHealth/2)
			buttmad = FALSE
			visible_message(span_notice("[capitalize(src.declent_ru(NOMINATIVE))] вроде бы успокоился, но не до конца."))
	...
	if(. && isliving(target))
		...
		if((L.stat == DEAD) && (health < maxHealth) && ishuman(L))
			if(fiesta(L, FALSE))
				...
				src.visible_message(span_danger("[capitalize(src.declent_ru(NOMINATIVE))] впрыскивает токсичную слизь во внутренности [L]..."),
...
/mob/living/simple_animal/hostile/asteroid/marrowweaver/frost
	...
	ru_names = list(
		...
		PREPOSITIONAL = "о морозном костномозговом ткаче"
	)
