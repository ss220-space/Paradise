/mob/living/simple_animal/attackby(obj/item/I, mob/user, params)
	...
	if(is_type_in_list(I, food_type))
		...
		user.visible_message(
			span_notice("[user.declent_ru_cap(NOMINATIVE)] с кормит [declent_ru(ACCUSATIVE)] [I.declent_ru(INSTRUMENTAL)]."),
...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		...
		if(INTENT_HELP)
			if(health > 0)
				visible_message(
					span_notice("[M.declent_ru_cap(NOMINATIVE)] [response_help] [declent_ru(ACCUSATIVE)]."),
					span_notice("[M.declent_ru_cap(NOMINATIVE)] [response_help] вас.")
				)
		...
		if(INTENT_HARM, INTENT_DISARM)
			...
			visible_message(
				span_danger("[M.declent_ru_cap(NOMINATIVE)] [response_harm] [declent_ru(ACCUSATIVE)]!"),
				span_userdanger("[M.declent_ru_cap(NOMINATIVE)] [response_harm] вас!")
			)
...
/mob/living/simple_animal/attack_alien(mob/living/carbon/alien/humanoid/M)
	if(..()) //if harm or disarm intent.
		if(M.a_intent == INTENT_DISARM)
			...
			visible_message(
				span_danger("[M.declent_ru_cap(NOMINATIVE)] [response_disarm] [name]!"),
				span_userdanger("[M.declent_ru_cap(NOMINATIVE)] [response_disarm] вас!")
			)
			add_attack_logs(M, src, "Alien disarmed")
		else
			var/damage = M.attack_damage
			visible_message(
				span_danger("[M.declent_ru_cap(NOMINATIVE)] дела[PLUR_ET_YUT(M)] резкий выпад в сторону [declent_ru(ACCUSATIVE)]!"),
				span_userdanger("[M.declent_ru_cap(NOMINATIVE)] делает резкий выпад в вашу сторону!")
			)
...
/mob/living/simple_animal/proc/attack_threshold_check(damage, damagetype = BRUTE, armorcheck = MELEE)
	...
	if(temp_damage >= 0 && temp_damage <= force_threshold)
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] выглядит невредимым."))
		return FALSE
	else
