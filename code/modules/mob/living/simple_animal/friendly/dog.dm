...
/mob/living/simple_animal/pet/dog/corgi/proc/find_new_hat()
	...
	visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] надевает [inventory_head.declent_ru(ACCUSATIVE)] на голову, как-то."))

///Deadchat plays command that drops the current hat off Ian.
/mob/living/simple_animal/pet/dog/corgi/proc/drop_hat()
	if(!inventory_head)
		return
	visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] энергично трясет головой, бросая [inventory_head.declent_ru(ACCUSATIVE)] на землю."))
	inventory_head.forceMove(drop_location())
	inventory_head = null
	update_dog_fluff()
	regenerate_icons()
