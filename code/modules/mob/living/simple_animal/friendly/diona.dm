...
/mob/living/simple_animal/diona/OnUnarmedAttack(atom/A)
	if(isdiona(A) && (src in A.contents)) //can't attack your gestalt
		visible_message("[declent_ru_cap(NOMINATIVE)] слегка шевелится.")
	else
		..()
...
/mob/living/simple_animal/diona/proc/evolve()
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] начинает дрожать и разрывается, порождая новые побеги дионеи."), span_danger("Ваше сознание разделяется. Мы поглощаем питательные вещества и разрастаемся в гештальт-форму."))
...
/mob/living/simple_animal/diona/proc/consume(obj/item/reagent_containers/food/snacks/grown/G)
	if(nutrition >= nutrition_need) // Prevents griefing by overeating plant items without evolving.
		...
		if(do_after(src, 2 SECONDS, G, max_interact_count = 1))
			visible_message("[declent_ru_cap(NOMINATIVE)] жадно поглощает [G.declent_ru(ACCUSATIVE)].","Вы жадно пожираете [G.declent_ru(ACCUSATIVE)].")
