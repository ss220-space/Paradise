...
/mob/living/simple_animal/cow/attackby(obj/item/I, mob/user, params)
	...
	if(is_type_in_list(I, food_type))
		...
		user.visible_message(
			span_notice("[user] скармливает пшеницу [declent_ru(DATIVE)]! [GEND_HE_SHE_CAP(src)] [pick(feedMessages)]."),
...
/mob/living/simple_animal/cow/attack_hand(mob/living/carbon/M)
	if(!stat && M.a_intent == INTENT_DISARM && icon_state != icon_dead)
		M.visible_message(span_warning("[M] опрокидывает [declent_ru(ACCUSATIVE)]!"), \
...
/mob/living/simple_animal/chicken/attackby(obj/item/I, mob/user, params)
	...
	if(is_type_in_list(I, food_type)) //feedin' dem chickens
		...
		if(!user.drop_transfer_item_to_loc(I, src))
			return ATTACK_CHAIN_PROCEED
		user.visible_message(
			span_notice("[user] скармливает пшеницу [declent_ru(DATIVE)]. [GEND_HE_SHE_CAP(src)] радостно [pick(speak_emote)]."),
...
/obj/item/udder/proc/milkAnimal(obj/item/reagent_containers/glass/container, mob/user)
	...
	if(!transfered)
		...
		return FALSE
	user.visible_message(
		span_notice("[user] доит [declent_ru(ACCUSATIVE)]."),
