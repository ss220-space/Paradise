...
/mob/living/simple_animal/hostile/retaliate/goat/handle_automated_movement()
	...
	if(length(enemies) && prob(10))
		enemies = list()
		lose_target()
		visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] успокаивается."))
...
/mob/living/simple_animal/hostile/retaliate/goat/AttackingTarget()
	...
	if(. && isdiona(target))
		...
		H.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] отрывает большой кусок от [H]!"), \
				span_userdanger("[declent_ru_cap(NOMINATIVE)] отрывает от вас большой кусок [NB.declent_ru(GENITIVE)]!"))
		NB.droplimb()
	...
	if(!stat && M.a_intent == INTENT_DISARM && icon_state != icon_dead)
		...
		spawn(rand(20,50))
			if(!stat && M)
				...
				to_chat(M, span_notice("[declent_ru_cap(NOMINATIVE)][pick(responses)]"))
	else
		..()
