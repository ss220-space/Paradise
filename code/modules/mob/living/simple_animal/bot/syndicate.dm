...
/mob/living/simple_animal/bot/ed209/syndicate/ed209_ai()
	...
	if(spawn_turf && !are_zs_connected(src, spawn_turf))
		raise_alert("[declent_ru_cap(NOMINATIVE)] потерялся в пространстве.")
		raised_alert = FALSE
		raise_alert("[declent_ru_cap(NOMINATIVE)] активировал протокол само-уничтожения.")
		explode()
...
/mob/living/simple_animal/bot/ed209/syndicate/explode()
	if(!QDELETED(src))
		...
		raise_alert("[declent_ru_cap(NOMINATIVE)] уничтожен.")
		qdel(src)
