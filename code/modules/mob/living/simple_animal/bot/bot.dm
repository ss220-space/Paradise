...
/mob/living/simple_animal/bot/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/screwdriver))
		...
		if(open)
			...
			if(do_after(user, 30 * W.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src))
				if(paicard)
