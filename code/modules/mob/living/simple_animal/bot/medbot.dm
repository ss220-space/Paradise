...
/mob/living/simple_animal/bot/medbot/set_custom_texts()
	...
	text_dehack_fail = "[declent_ru_cap(NOMINATIVE)] выглядит повреждённым и не может быть перепрограммирован!"
...
/mob/living/simple_animal/bot/medbot/proc/medicate_patient(mob/living/carbon/C)
	...
	if(!reagent_id) //If they don't need any of that they're probably cured!
		...
		if(!emagged && !hijacked && check_overdose(patient, reagent_id, injection_amount))
			soft_reset()
			return
		C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] пытается сделать укол [patient]!"),
									span_userdanger("[declent_ru_cap(NOMINATIVE)] пытается сделать вам укол!"))
...
/mob/living/simple_animal/bot/medbot/proc/do_inject(mob/living/carbon/C, inject_beaker, reagent_id)
	...
	if(in_range(src, patient) && on && assess_patient(patient))
		...
		C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сделал укол [patient] своим шприцем!"),
						span_userdanger("[declent_ru_cap(NOMINATIVE)] сделал вам укол своим шприцем!"))
	else
		visible_message("[declent_ru_cap(NOMINATIVE)] убирает свой шприц.")
