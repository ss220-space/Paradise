...
/mob/living/simple_animal/bot/mulebot/proc/buzz(type)
	switch(type)
		if(SIGH)
			audible_message("[declent_ru_cap(NOMINATIVE)] разочарованно гудит.")
			playsound(loc, 'sound/machines/buzz-sigh.ogg', 50, FALSE)
		if(ANNOYED)
			audible_message("[declent_ru_cap(NOMINATIVE)] раздражённо жужжит.")
			playsound(loc, 'sound/machines/buzz-two.ogg', 50, FALSE)
		if(DELIGHT)
			audible_message("[declent_ru_cap(NOMINATIVE)] восторженно звенит!")
...
/mob/living/simple_animal/bot/mulebot/proc/at_target()
	if(!reached_target)
		...
		audible_message("[declent_ru_cap(NOMINATIVE)] громко звенит!")
		...
		if(pathset) //The AI called us here, so notify it of our arrival.
			...
			if(calling_ai)
				to_chat(calling_ai, span_notice("[icon2html(src, calling_ai)] [declent_ru_cap(NOMINATIVE)] удалённо проигрывает звук звонка!"))
...
/mob/living/simple_animal/bot/mulebot/Bump(mob/living/bumped_living)
	...
	if(isrobot(bumped_living))
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] врезается в [bumped_living]!"))
		return .
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сбивает [bumped_living]!"))
...
/mob/living/simple_animal/bot/mulebot/proc/RunOver(mob/living/carbon/human/H)
	...
	H.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] переезжает [H]!"),
					span_userdanger("[declent_ru_cap(NOMINATIVE)] переезжает вас!"))
