...
/mob/living/simple_animal/bot/cleanbot/set_custom_texts()
	...
	text_dehack_fail = "[declent_ru_cap(NOMINATIVE)] не отвечает на ваши команды!"
...
/mob/living/simple_animal/bot/cleanbot/emag_act(mob/user)
	..()
	if(emagged == 2 && user)
		to_chat(user, span_danger("[declent_ru_cap(NOMINATIVE)] странно жужжит!"))
	...
	if(emagged == 2) //Emag functions
		if(issimulatedturf(loc))
			...
			if(prob(5)) //Spawns foam!
				visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] издаёт громкие булькающие звуки, прежде чем выпустить шлейф пены!"))
...
/mob/living/simple_animal/bot/cleanbot/proc/start_clean(obj/effect/decal/cleanable/target)
	set_anchored(TRUE)
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] начинает очищать [target]."))
	mode = BOT_CLEANING
	update_icon()
