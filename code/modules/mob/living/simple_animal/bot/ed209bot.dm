...
/mob/living/simple_animal/bot/ed209/emag_act(mob/user)
	...
	if(emagged == 2)
		...
		audible_message(span_danger("[declent_ru_cap(NOMINATIVE)] странно жужжит!"))
...
/mob/living/simple_animal/bot/ed209/proc/look_for_perp()
	...
	for(var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
		...
		if(!threatlevel)
			...
			visible_message("<b>[declent_ru_cap(NOMINATIVE)]</b> указывает на [C.name]!")
			mode = BOT_HUNT
			INVOKE_ASYNC(src, PROC_REF(handle_automated_action))
			break
		else
...
/mob/living/simple_animal/bot/ed209/proc/stun_attack(mob/living/carbon/C)
	...
	C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] оглушил [C]!"),
					span_userdanger("[declent_ru_cap(NOMINATIVE)] оглушил вас!"))

/mob/living/simple_animal/bot/ed209/proc/start_cuffing(mob/living/carbon/C)
	mode = BOT_ARREST
	playsound(loc, 'sound/weapons/cablecuff.ogg', 30, TRUE, -2)
	C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] начинает надевать стяжки на [C]!"),
					span_userdanger("[declent_ru_cap(NOMINATIVE)] пытается надеть на вас стяжки!"))
...
/mob/living/simple_animal/bot/ed209/proc/cuff_callback(mob/living/carbon/C)
	...
	C.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] надел стяжки на [C]!"),
					span_userdanger("[declent_ru_cap(NOMINATIVE)] надел на вас стяжки!"))
