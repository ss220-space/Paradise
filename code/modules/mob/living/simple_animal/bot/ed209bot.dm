...
/mob/living/simple_animal/bot/ed209/proc/ed209_ai()
	...
	switch(mode)
		...
		if(BOT_ARREST)
			...
			if(!Adjacent(target) || !isturf(target.loc) || target.getStaminaLoss() <= target.get_max_stamina()) //if he's changed loc and about to get up or not adjacent or got into a closet, we prep arrest again.
				back_to_hunt()
				return
			else
				mode = BOT_PREP_ARREST
