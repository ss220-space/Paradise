...
/obj/effect/proc_holder/spell/hulk_transform/cast(list/targets, mob/user = usr)
	...
	if(istype(user,/mob/living/simple_animal/hulk))
		...
		return
	to_chat(user, "<span class='bold notice'>You can feel real POWER."))
...
/obj/effect/proc_holder/spell/hulk_dash/cast(list/targets, mob/living/user)
	...
	if (istype(user.loc,/turf) && !(isspaceturf(user.loc)))
		...
		if(failure)
			...
			user.visible_message(span_warning(" \the [user] attempts to dash away but was interrupted!"),
...
/obj/effect/proc_holder/spell/hulk_jump/cast(list/targets, mob/living/user)
	...
	if (istype(user.loc,/turf) && !(isspaceturf(user.loc)))
		...
		if(failure)
			user.Weaken(10 SECONDS)
			user.visible_message(span_warning(" \the [user] attempts to leap away but is slammed back down to the ground!"),
...
/obj/effect/proc_holder/spell/fireball/hulk_spit
	...
	action_background_icon_state = "bg_hulk"
	selection_activated_message	= span_notice("Your prepare to spit fire! <b>Left-click to spit at a target!</b></span>"
...
/obj/effect/proc_holder/spell/fireball/hulk_spit/hulk_lazor
	...
	selection_activated_message	= span_notice("You strained your eyes preparing the LAZOR! <b>Left-click to fire at a target!</b></span>"
