 ...
 * * INCAPABLE_RESTRAINTS - mob in a restraint (handcuffs/straightjacket) is not considered incapacitated
...
/mob/living/incapacitated(ignore_flags)
	...
	if(!(ignore_flags & INCAPABLE_RESTRAINTS) && HAS_TRAIT(src, TRAIT_RESTRAINED))
		return TRUE
	if(!(ignore_flags & INCAPABLE_GRAB) && pulledby && pulledby.grab_state > GRAB_PASSIVE)
		return TRUE
	return FALSE
