...
/obj/item/clothing/mask/facehugger/examine(mob/user)
	...
	if(real)//So that giant red text about probisci doesn't show up for fake ones
		switch(stat)
			if(DEAD,UNCONSCIOUS)
				. += span_boldannounceic("[declent_ru_cap(NOMINATIVE)] не двигается.")
			if(CONSCIOUS)
				. += span_boldannounceic("[declent_ru_cap(NOMINATIVE)] кажется, активен!")
		if(sterile)
...
/obj/item/clothing/mask/facehugger/proc/Attach(mob/living/attached_mob)
	...
	var/text_name = declent_ru_cap(NOMINATIVE)
...
/obj/item/clothing/mask/facehugger/proc/Die()
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сворачивается в клубок!"))
