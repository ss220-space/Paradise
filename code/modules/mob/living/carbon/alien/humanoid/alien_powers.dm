...
/datum/action/innate/xeno_action/resin/Activate()
	...
	if(plasmacheck(55))
		var/choice = input("Choose what you wish to shape.","Resin building") as null|anything in list("resin wall","resin door","resin membrane","resin nest") //would do it through typesof but then the player choice would have the type path and we don't want the internal workings to be exposed ICly - Urist

		if(!choice || !plasmacheck(55))
			return
		...
		switch(choice)
			...
			if("resin door")
				new /obj/structure/alien/resin/door(host.loc)
			if("resin membrane")
