...
/datum/strippable_item/mob_item_slot/mask/alternate_action(atom/source, mob/user, action_key)
	...
	if(action_key == "dislodge_headpocket")
		...
		if(!pocket.pocket.master_item)
			return
		user.visible_message(span_danger("[user] пытается достать что-то из головного кармана [source]!"),
		...
		if(do_after(user, POCKET_STRIP_DELAY, source, max_interact_count = 1))
			user.visible_message(span_danger("[user] достаёт что-то из головного кармана [source]!"),
	...
	muzzle.visible_message(span_danger("[user] пытается [muzzle.locked ? "раз" : "за"]блокировать [muzzle.declent_ru(ACCUSATIVE)] [source]!"), \
					span_userdanger("[user] пытается [muzzle.locked ? "раз" : "за"]блокировать [muzzle.declent_ru(ACCUSATIVE)] [source]!"))
	if(!do_after(user, POCKET_STRIP_DELAY, source, max_interact_count = 1))
		return
