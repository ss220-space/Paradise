	...
	if(cuff_break)
		...
		if(is_processed)
			...
			visible_message(
				...
				span_notice("Вы пытаетесь снять [cuffs.declent_ru(ACCUSATIVE)]. Это займёт примерно [breakout_time / 10] секунд[DECL_U_Y_0(breakout_time / 10)]."),
			)
...
/mob/living/carbon/resist_muzzle()
	...
	visible_message(
		...
		span_notice("Вы пытаетесь избавиться от [I.declent_ru(GENITIVE)]. Это займёт примерно [time / 10] секунд[DECL_U_Y_0(time / 10)]."),
	)
