...
/mob/living/carbon/proc/cuff_resist(obj/item/cuffs, cuff_break = FALSE)
	...
	if(cuff_break)
		if(is_processed)
			...
			visible_message(
				span_warning("[name] пытается сломать [cuffs.declent_ru(ACCUSATIVE)]!"),
		...
		while(do_after(src, breakout_iter, src, DA_IGNORE_USER_LOC_CHANGE|DEFAULT_DOAFTER_IGNORE|DA_IGNORE_HELD_ITEM, max_interact_count = 1,
			...
			if(cuff_breakout_attempts * breakout_iter >= breakout_time)
				...
				visible_message(span_warning("[name] пытается снять [cuffs.declent_ru(ACCUSATIVE)]!"))
		...
		if(is_processed)
			...
			visible_message(
				span_warning("[name] пытается снять [cuffs.declent_ru(ACCUSATIVE)]!"),
...
/mob/living/carbon/proc/clear_cuffs(obj/item/I, cuff_break)
	...
	if(I != handcuffed && I != legcuffed && I != wear_suit)
		return FALSE
	visible_message(
		span_danger("[name] [cuff_break ? "лома" : "снима"]ет [I.declent_ru(ACCUSATIVE)]!"),
...
/mob/living/carbon/resist_muzzle()
	...
	visible_message(
		span_warning("[name] грызёт [I.declent_ru(GENITIVE)], пытаясь освободиться!"),
	...
	visible_message(
		span_danger("[name] избавляется от [I.declent_ru(GENITIVE)]!"),
