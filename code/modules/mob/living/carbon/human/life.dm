	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		for(var/obj/item/thing in bodypart.embedded_objects)
			if(prob(thing.embedded_pain_chance))
				...
				to_chat(src, span_userdanger("[thing.declent_ru_cap(NOMINATIVE)] в ваш[GEND_EM_EI_EM_IH(bodypart)] [GLOB.body_zone[bodypart.limb_zone][PREPOSITIONAL]] причиняет боль!"))
			...
			if(prob(thing.embedded_fall_chance))
				...
				visible_message(
					span_danger("[thing.declent_ru_cap(NOMINATIVE)] выпадает из [GLOB.body_zone[bodypart.limb_zone][GENITIVE]] [name]!"),
					span_danger("[thing.declent_ru_cap(NOMINATIVE)] выпадает из [GEND_YOURS(bodypart)] [GLOB.body_zone[bodypart.limb_zone][GENITIVE]]!"),
				)
