	...
	if(dna.species.has_organ[INTERNAL_ORGAN_BRAIN])
		...
		if(sponge)
			...
			if(sponge.damage >= 120 && stat != DEAD)
				visible_message(span_alert("<b>[capitalize(declent_ru(NOMINATIVE))]</b> обмяк[genderize_ru(src.gender, "", "ла", "ло", "лии")], [GEND_HIS_HER(src)] выражение лица совершенно пустое."))
				death()
	if(updating_health)
