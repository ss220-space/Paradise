...
/datum/action/innate/tail_lash/Activate()
	...
	for(var/mob/living/carbon/human/C in orange(1))
		...
		if(E)
			...
			user.visible_message("<span class='danger'>[user.name] хлещет [C.name] в [E_ru] своим хвостом! </span>", "<span class='danger'>Вы хлестаете [C.name] в [E_ru] своим хвостом!</span>")
			...
			if(user.restrained())
				if(prob(50))
					user.Weaken(2)
					user.visible_message("<span class='danger'>[user.name] теря[pluralize_ru(user.gender,"ет","ют")] [genderize_ru(user.gender,"его","её","свое","их")] равновесие!</span>", "<span class='danger'>Вы теряете равновесие!</span>")
					return
