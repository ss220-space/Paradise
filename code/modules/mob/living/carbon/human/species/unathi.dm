...
/datum/action/innate/ignite_unathi/Activate()
	...
	if(world.time <= cooldown)
		to_chat(user, span_warning("Ваша пасть болит из-за прошлой попытки. Подождите [round((cooldown - world.time) / 10)] секунд[DECL_U_Y__(round((cooldown - world.time) / 10))] и попробуйте ещё раз"))
		return
