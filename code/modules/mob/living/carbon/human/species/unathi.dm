...
/datum/action/innate/tail_lash/Activate()
	...
	for(var/mob/living/carbon/human/C in orange(1))
		...
		if(E)
			...
			user.visible_message("<span class='danger'>[user.declent_ru(NOMINATIVE)] хлещет хвостом [C.declent_ru(ACCUSATIVE)] по [E.declent_ru(DATIVE)]! </span>", "<span class='danger'>Вы хлещете хвостом [C.declent_ru(ACCUSATIVE)] по [E.declent_ru(DATIVE)]!</span>")
			...
			if(user.restrained())
				if(prob(50))
					user.Weaken(2)
					user.visible_message("<span class='danger'>[user.declent_ru(NOMINATIVE)] теря[pluralize_ru(user.gender,"ет","ют")] равновесие!</span>", "<span class='danger'>Вы теряете равновесие!</span>")
					return
