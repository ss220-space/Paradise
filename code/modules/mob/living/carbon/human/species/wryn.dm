...
/datum/action/innate/wryn_sting
	name = "Жало Врина"
	desc = "Подготовка жала Врина для ужаливания."
...
/datum/action/innate/wryn_sting/Trigger()
	...
	if(user.wear_suit)	//Is your Wryn wearing a Hardsuit or a Laboat that's blocking their Stinger?
		to_chat(user, "<span class='notice'>Нужно снять верхнюю одежду перед тем как использовать жало Врина.</span>")
		return
	if(user.getStaminaLoss() >= 50)	//Does your Wryn have enough Stamina to sting?
		to_chat(user, "<span class='notice'>Вы чувствуете себя слишком усталым чтобы использовать жало Врина.</span>")
		return
	else
		button_on = TRUE
		UpdateButtonIcon()
...
/datum/action/innate/wryn_sting/proc/select_target(var/mob/living/carbon/human/user)
	...
	if(!target)		//No one's around!
		to_chat(user, "<span class='warning'>Вокруг некого ужалить! Жало втягивает обратно.</span>")
		user.visible_message("<span class='warning'[user] втягивает свое жало.</span>")
		button_on = FALSE
		UpdateButtonIcon()
		return
...
/datum/action/innate/wryn_sting/proc/sting_target(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(!(target in orange(1, user)))	//Dang, did they get away?
		...
		to_chat(user, "<span class='danger'> Вы жалите [target] в [organ] своим жалом!</span>")
		user.visible_message("<span class='danger'>[user] жалит [target] в [organ] его жалом! </span>")
...
/datum/species/wryn/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	if(target.handcuffed && target.get_int_organ(/obj/item/organ/internal/wryn/hivenode))
		...
		user.visible_message("<span class='notice'>[user] начинает яростно отрывать антенну [target].</span>")
		to_chat(target, "<span class='danger'><B>[user] схватил вашу антенну и яростно тянет её!<B></span>")
		if(do_mob(user, target, 250))
			...
			to_chat(user, "<span class='notice'>Вы слышите громкий хруст, когда безжалостно отрываете антенну [target].</span>")
			to_chat(target, "<span class='danger'>Вы слышите невыносимый громкий хруст, когда ваша антена оторвана [user].</span>")
