	...
	if(target.handcuffed && node && user.zone_selected == BODY_ZONE_HEAD)
		switch(alert(user, "Вы хотите вырвать усики этому существу?", "OH SHIT", "Да", "Нет"))
			if("Да")
				...
				to_chat(target, span_danger("<b>[user] схватил[genderize_ru(user.gender,"","а","о","и")] ваши усики и яростно тян[PLUR_ET_YT(user)] их!<b>"))
				if(do_after(user, 25 SECONDS, target, NONE))
					...
					to_chat(target, span_danger("Вы слышите невыносимый хруст, когда [user] вырыва[PLUR_ET_UT(user)] усики из вашей головы."))
