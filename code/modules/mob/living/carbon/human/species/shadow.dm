...
/datum/species/shadow/proc/empower_handler(mob/living/carbon/human/human, empowering = FALSE)
	switch(empowering)
		if(TRUE)
			if(do_after(human, TIME_TO_EMPOWER, human, ALL, show_progress = FALSE, show_cogbar = FALSE, max_interact_count = 1, extra_checks = CALLBACK(src, PROC_REF(light_check), human)))
				human.apply_status_effect(STATUS_EFFECT_SHADOW_EMPOWER)
		if(FALSE)
			if(do_after(human, TIME_TO_EXHAUST, human, ALL, show_progress = FALSE, show_cogbar = FALSE, max_interact_count = 1)) // NO extra_checks. Out in the light? Lose empower.
				human.remove_status_effect(STATUS_EFFECT_SHADOW_EMPOWER)
