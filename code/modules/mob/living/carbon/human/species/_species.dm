...
/datum/species/proc/grab(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	if(target.check_block())
		target.visible_message("<span class='warning'>[target.name] заблокировал[genderize_ru(target.gender,"","а","о","и")] попытку захвата [user.name]!</span>")
		return FALSE
...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, "<span class='warning'>Вы не хотите навредить [target.name]!</span>")
		return FALSE
	//Vampire code
	if(user.mind && user.mind.vampire && (user.mind in SSticker.mode.vampires) && !user.mind.vampire.draining && user.zone_selected == "head" && target != user)
		...
		//end vampire codes
	if(target.check_block())
		target.visible_message("<span class='warning'>[target.name] заблокировал[genderize_ru(target.gender,"","а","о","и")] атаку [user.name]!</span>")
		return FALSE
	if(attacker_style && attacker_style.harm_act(user, target) == TRUE)
		...
		if(attack.harmless)
			...
			target.visible_message("<span class='danger'>[user.name] [attack_species] [target.name]!</span>")
			return FALSE
		...
		if(!damage)
			...
			target.visible_message("<span class='danger'>[user.name] попытал[genderize_ru(user.gender,"ся","ась","ось","ись")], но не [attack_species] [target.name]!</span>")
			return FALSE
		...
		if((target.stat != DEAD) && damage >= user.dna.species.punchstunthreshold)
			target.visible_message("<span class='danger'>[user.name] ослабил[genderize_ru(user.gender,"","а","о","и")] [target.name]!</span>", \
							"<span class='userdanger'>[user.name] ослабил[genderize_ru(user.gender,"","а","о","и")] [target.name]!</span>")
...
/datum/species/proc/disarm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	if(target.check_block())
		target.visible_message("<span class='warning'>[target.name] заблокировал[genderize_ru(target.gender,"","а","о","и")] попытку обезоруживания [user.name]!</span>")
		return FALSE
	if(attacker_style && attacker_style.disarm_act(user, target) == TRUE)
		...
		if(randn <= 10)
			...
			target.visible_message("<span class='danger'>[user.name] толкнул[genderize_ru(user.gender,"","а","о","и")] [target.name]!</span>")
		...
		if(randn <= 60)
			...
			if(target.pulling)
				target.visible_message("<span class='danger'>[user.name] разорвал[genderize_ru(user.gender,"","а","о","и")] хватку [target.name] на [target.pulling]!</span>")
				talked = 1
				target.stop_pulling()
			...
			if(istype(target.l_hand, /obj/item/grab))
				...
				if(lgrab.affecting)
					target.visible_message("<span class='danger'>[user.name] разорвал[genderize_ru(user.gender,"","а","о","и")] хватку [target.name] на [lgrab.affecting]!</span>")
					talked = 1
				spawn(1)
					qdel(lgrab)
			if(istype(target.r_hand, /obj/item/grab))
				...
				if(rgrab.affecting)
					target.visible_message("<span class='danger'>[user.name] разорвал[genderize_ru(user.gender,"","а","о","и")] хватку [target.name] на [rgrab.affecting]!</span>")
					talked = 1
				spawn(1)
					qdel(rgrab)
			//End BubbleWrap
			...
			if(!talked)	//BubbleWrap
				if(target.drop_item())
					target.visible_message("<span class='danger'>[user.name] обезоружил[genderize_ru(user.gender,"","а","о","и")] [target.name]!</span>")
			playsound(target.loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
			return


	playsound(target.loc, 'sound/weapons/punchmiss.ogg', 25, 1, -1)
	target.visible_message("<span class='danger'>[user.name] попытал[genderize_ru(user.gender,"ся","ась","ось","ись")] обезоружить [target.name]!</span>")
...
/datum/species/proc/spec_attack_hand(mob/living/carbon/human/M, mob/living/carbon/human/H, datum/martial_art/attacker_style) //Handles any species-specific attackhand events.
	...
	if((M != H) && M.a_intent != INTENT_HELP && H.check_shields(M, 0, M.name, attack_type = UNARMED_ATTACK))
		...
		H.visible_message("<span class='warning'>[M.name] попытал[genderize_ru(M.gender,"ся","ась","ось","ись")] коснуться [H.name]!</span>")
		return FALSE
