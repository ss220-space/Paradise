...
/datum/species/proc/grab(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	var/message = span_warning("[target.declent_ru(NOMINATIVE)] блокирует попытку захвата [user.declent_ru(GENITIVE)]!")
...
/datum/species/proc/harm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	var/message = span_warning("[target.declent_ru(NOMINATIVE)] блокирует атаку [user.declent_ru(GENITIVE)]!")
	if(target.check_martial_art_defense(target, user, null, message))
		return FALSE
	if(attacker_style && attacker_style.harm_act(user, target) == TRUE)
		...
		if(!damage)
			...
			target.visible_message(span_danger("[user.declent_ru(NOMINATIVE)] [attack_species] [target.declent_ru(ACCUSATIVE)], но промахивается!"))
			return FALSE
		...
		if((target.stat != DEAD) && damage >= (user.dna.species.punchstunthreshold + user.physiology.punch_stun_threshold))
			target.visible_message(
				span_danger("[user.declent_ru(NOMINATIVE)] ослабляет [target.declent_ru(ACCUSATIVE)]!"), \
				span_userdanger("[user.declent_ru(NOMINATIVE)] ослабляет [target.declent_ru(ACCUSATIVE)]!")
			)
...
/datum/species/proc/disarm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	if(user == target)
		return FALSE
	var/message = span_warning("[target.declent_ru(NOMINATIVE)] блокирует попытку обезоруживания [user.declent_ru(GENITIVE)]!")
	if(target.check_martial_art_defense(target, user, null, message))
		return FALSE
	if(attacker_style && attacker_style.disarm_act(user, target) == TRUE)
		...
		if(randn <= 5 + extra_knock_chance)
			...
			target.visible_message(span_danger("[user.declent_ru(NOMINATIVE)] толкает [target.declent_ru(ACCUSATIVE)]!"))
...
/datum/species/proc/spec_attack_hand(mob/living/carbon/human/M, mob/living/carbon/human/H, datum/martial_art/attacker_style) //Handles any species-specific attackhand events.
	...
	if((M != H) && M.a_intent != INTENT_HELP && H.check_shields(M, 0, M.name, attack_type = UNARMED_ATTACK))
		...
		H.visible_message(span_warning("[M.declent_ru(NOMINATIVE)] пытается коснуться [H.declent_ru(ACCUSATIVE)]!"))
		return FALSE
...
/datum/species/proc/equip_delay_self_check(obj/item/I, slot, mob/living/carbon/human/user)
	user.visible_message(span_notice("[user] начинает надевать [I.declent_ru(ACCUSATIVE)]..."), span_notice("Вы начинаете надевать [I.declent_ru(ACCUSATIVE)]..."))
