/mob/living/carbon/human/examine(mob/user)
	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		...
		if(bodypart.tourniquet.applyed_bodypart != bodypart)
			continue
		msg += span_warning("<a href='byond://?src=[UID()];tourniquet_object=[bodypart.tourniquet.UID()];limb=[bodypart.UID()]' class='warning'>[genderize_ru(gender, "Его", "Её", "Его", "Их")] [bodypart.declent_ru(NOMINATIVE)] перевязан[genderize_ru(gender, "", "а", "о", "ы")] [icon2html(bodypart.tourniquet, src)] [bodypart.tourniquet.declent_ru(INSTRUMENTAL)]!</a>\n")
