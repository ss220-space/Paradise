...
/mob/living/carbon/human/proc/dchat_attack(intent)
	...
	if(!victim)
		switch(intent)
			if(INTENT_HARM)
				visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] бьет [implement] дико!"))
			if(INTENT_HELP)
				visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] кажется, делает глубокий вдох."))
		return
	implement = "[isnull(in_hand) ? "кулаки" : in_hand.declent_ru(ACCUSATIVE)]"
	if(isLivingSSD(victim))
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] [intent == INTENT_HARM ? "неохотно " : ""] опускает [implement]."))
		return
...
/mob/living/carbon/human/proc/dchat_resist()
	if(!can_resist())
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] кажется, ничего не cможет сделать!"))
		return
	if(!HAS_TRAIT(src, TRAIT_RESTRAINED))
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] кажется, ничего особенного не делает."))
		return

	visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] пытается освободиться!"))
	resist()
...
/mob/living/carbon/human/proc/dchat_pickup()
	...
	if(in_hand)
		if(HAS_TRAIT(in_hand, TRAIT_NODROP))
			visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] пытается уронить [in_hand.declent_ru(ACCUSATIVE)], но кажется, что он застрял в руке!"))
			return
		if(in_hand.flags & ABSTRACT)
			visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] кажется, что у [genderize_ru(gender, "него", "нее", "него", "них")] заняты руки!"))
			return
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] бросает [in_hand.declent_ru(ACCUSATIVE)] и поднимает [thing.declent_ru(ACCUSATIVE)]!"))
		do_unEquip(in_hand)
		in_hand.forceMove(old_loc)
	else
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] поднимает [thing.declent_ru(ACCUSATIVE)]!"))
	put_in_active_hand(thing)

/mob/living/carbon/human/proc/dchat_throw()
	var/obj/item/in_hand = get_active_hand()
	if(!in_hand || in_hand.flags & ABSTRACT)
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] делает движение, похожее на бросок!"))
		return
	...
	if(HAS_TRAIT(in_hand, TRAIT_NODROP))
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))]пытается бросить [in_hand.declent_ru(ACCUSATIVE)][isturf(possible_target) ? "" : " в сторону [possible_target]"], но [genderize_ru(gender, "он не отрывается", "она не отрывается", "оно не отрывается", "они не отрываются")] от руки!"))
		return
...
/mob/living/carbon/human/proc/dchat_shove()
	...
	if(!H)
		visible_message(span_notice("[capitalize(declent_ru(NOMINATIVE))] пытается что-то отодвинуть в сторону!"))
		return
...
/mob/living/carbon/human/proc/dchat_shoot()
	...
	if(!held_gun)
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] направляет палец в форме пистолета в сторону [possible_target.declent_ru(GENITIVE)]!"))
		return
	if(!istype(held_gun))
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] направляет [held_gun.declent_ru(ACCUSATIVE)] в сторону [possible_target.declent_ru(GENITIVE)]!"))
		return
	// for his neutral special, he wields a Gun
	held_gun.afterattack(possible_target, src)
	visible_message(span_danger("[capitalize(declent_ru(NOMINATIVE))] стреляет из [held_gun.declent_ru(GENITIVE)][isturf(possible_target) ? "" : " в сторону [possible_target.declent_ru(GENITIVE)]!"]"))
