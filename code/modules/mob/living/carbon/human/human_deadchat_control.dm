...
/mob/living/carbon/human/proc/dchat_attack(intent)
	...
	if(!victim)
		switch(intent)
			if(INTENT_HARM)
				visible_message(
					span_warning("[capitalize(declent_ru(NOMINATIVE))] дико бьёт [implement]!")
				)
			if(INTENT_HELP)
				visible_message(
					span_notice("[capitalize(declent_ru(NOMINATIVE))] кажется, делает глубокий вдох.")
				)
		return
	implement = "[isnull(in_hand) ? "кулаки" : in_hand.declent_ru(ACCUSATIVE)]"
	if(isLivingSSD(victim))
		visible_message(
			span_notice("[capitalize(declent_ru(NOMINATIVE))] [intent == INTENT_HARM ? "неохотно " : ""] опускает [implement].")
		)
		return
...
/mob/living/carbon/human/proc/dchat_resist()
	if(!can_resist())
		visible_message(
			span_warning("[capitalize(declent_ru(NOMINATIVE))] кажется, ничего не cможет сделать!")
		)
		return
	if(!HAS_TRAIT(src, TRAIT_RESTRAINED))
		visible_message(
			span_notice("[capitalize(declent_ru(NOMINATIVE))] кажется, ничего особенного не делает.")
		)
		return

	visible_message(
		span_warning("[capitalize(declent_ru(NOMINATIVE))] пытается освободиться!")
	)
	resist()
...
/mob/living/carbon/human/proc/dchat_pickup()
	...
	if(in_hand)
		if(HAS_TRAIT(in_hand, TRAIT_NODROP))
			visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] пытается выпустить [in_hand.declent_ru(ACCUSATIVE)], но кажется, что [genderize_ru(in_hand.gender, "он застрял", "она застряла", "оно зястряло", "они застряли")] в руке!"))
			return
...
/mob/living/carbon/human/proc/dchat_throw()
	...
	if(HAS_TRAIT(in_hand, TRAIT_NODROP))
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] пытается бросить [in_hand.declent_ru(ACCUSATIVE)][isturf(possible_target) ? "" : " в сторону [possible_target]"], но [GEND_HE_SHE(src)] не отрывается от руки!"))
		return
