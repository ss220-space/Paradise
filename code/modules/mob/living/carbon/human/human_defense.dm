		...
		if(body_position != LYING_DOWN && !HAS_TRAIT(src, TRAIT_HULK) && prob(mind.martial_art.reflection_chance)) //But only if they're not lying down, and hulks can't do it
			...
			if(istype(mind.martial_art, /datum/martial_art/ninja_martial_art))
				...
				if(!creeping_widow.check_katana(mind.current))
					checks_passed = FALSE
			if(checks_passed)
				visible_message(span_danger("[src] отража[PLUR_ET_UT(src)] [P.declent_ru(ACCUSATIVE)]!"), \
								span_userdanger("[src] отража[PLUR_ET_UT(src)] [P.declent_ru(ACCUSATIVE)]!"))
	...
	if(mind?.martial_art?.can_deflect) //Some martial arts users can deflect projectiles!
		if(body_position != LYING_DOWN && !HAS_TRAIT(src, TRAIT_HULK) && mind.martial_art.try_deflect(src)) //But only if they're not lying down, and hulks can't do it
			...
			if(HAS_TRAIT(src, TRAIT_PACIFISM) || !P.is_reflectable(REFLECTABILITY_PHYSICAL)) //if it cannot be reflected, it hits the floor. This is the exception to the rule
				...
				P.firer = src
				T.bullet_act(P)
				visible_message(span_danger("[src] отбива[PLUR_ET_UT(src)] [P.declent_ru(ACCUSATIVE)] в сторону!"), \
...
/mob/living/carbon/human/welder_act(mob/user, obj/item/item)
	...
	while(rembrute > 0)
		...
		rembrute = nrembrute
		user.visible_message(span_alert("[user] ремонтиру[PLUR_ET_UT(src)] вмятины на [E.declent_ru(INSTRUMENTAL)] [src], используя [item.declent_ru(ACCUSATIVE)]."))
	...
	if(bleed_rate && ismachineperson(src))
		bleed_rate = 0
		user.visible_message(span_alert("[user] устраня[PLUR_ET_UT(src)] протечки в корпусе [src], используя [item.declent_ru(ACCUSATIVE)]."))
	if(IgniteMob())
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_HEAD) //only if we didn't specify a zone or if that zone is the head.
		...
		if(head_clothes)
			if(!(head_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[capitalize(head_clothes.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(head_clothes)] вашу голову и лицо от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_CHEST)
		...
		if(chest_clothes)
			if(!(chest_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[capitalize(chest_clothes.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(chest_clothes)] ваше туловище от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_L_ARM || bodyzone_hit == BODY_ZONE_R_ARM)
		...
		if(arm_clothes)
			if(!(arm_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[capitalize(arm_clothes.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(arm_clothes)] ваши руки от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_L_LEG || bodyzone_hit == BODY_ZONE_R_LEG || bodyzone_hit == "feet")
		...
		if(leg_clothes)
			if(!(leg_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[capitalize(leg_clothes.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(leg_clothes)] ваши руки от кислоты!"))
		else
...
/mob/living/carbon/human/proceed_attack_results(obj/item/item, mob/living/user, params, def_zone)
	...
	if(check_martial_art_defense(src, user, item, span_warning("[src] блокиру[PLUR_ET_UT(src)] [item.declent_ru(ACCUSATIVE)]!")))
		return ATTACK_CHAIN_BLOCKED
...
/mob/living/carbon/human/send_item_attack_message(obj/item/item, mob/living/user, def_zone)
	...
	if(!item.force)
		visible_message(
			span_warning("[user] аккуратно тыкнул[genderize_ru(user.gender, "", "а", "о", "и")] [declent_ru(ACCUSATIVE)] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]."),
			span_warning("[user] аккуратно тыкнул[genderize_ru(user.gender, "", "а", "о", "и")] вас [item.declent_ru(INSTRUMENTAL)] [message_hit_area]."),
			ignored_mobs = user,
		)
	...
	visible_message(
		span_danger("[user] [message_verb][genderize_ru(user.gender, "", "а", "о", "и")] [declent_ru(ACCUSATIVE)] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]!"),
		span_userdanger("[user] [message_verb][genderize_ru(user.gender, "", "а", "о", "и")] вас [item.declent_ru(INSTRUMENTAL)] [message_hit_area]!"),
		ignored_mobs = user,
	)
...
/mob/living/carbon/human/proc/reagent_safety_check(hot = TRUE)
	if(wear_mask)
		to_chat(src, span_danger("[capitalize(wear_mask.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(wear_mask)] вас от [hot ? "горячей" : "холодной"] жидкости!"))
		return FALSE
	if(head)
		to_chat(src, span_danger("[capitalize(head.declent_ru(NOMINATIVE))] защища[PLUR_ET_UT(head)] вас от [hot ? "горячей" : "холодной"] жидкости!"))
		return FALSE
	return TRUE
