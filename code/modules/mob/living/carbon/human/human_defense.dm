...
/mob/living/carbon/human/attack_basic_mob(mob/living/basic/user)
	...
	if(!.)
		return
	if(check_shields(user, user.melee_damage_lower, "the [user.name]", MELEE_ATTACKS, user.armour_penetration))
		return FALSE
	...
	apply_damage(user.melee_damage_lower, user.melee_damage_type, affecting, armor)
