...
/mob/living/carbon/human/welder_act(mob/user, obj/item/item)
	...
	while(rembrute > 0)
		...
		rembrute = nrembrute
		user.visible_message(span_alert("[user] ремонтиру[pluralize_ru(src.gender, "ет", "ют")] вмятины на [E.declent_ru(INSTRUMENTAL)] [src], используя [item.declent_ru(ACCUSATIVE)]."))
...
/mob/living/carbon/human/proceed_attack_results(obj/item/item, mob/living/user, params, def_zone)
	...
	if(check_martial_art_defense(src, user, item, span_warning("[src] блокиру[pluralize_ru(src.gender, "ет", "ют")] [item.declent_ru(ACCUSATIVE)]!")))
		return ATTACK_CHAIN_BLOCKED
	...
	var/armor = run_armor_check(affecting, MELEE, span_warning("Ваша броня защитила [hit_area_name], полностью поглотив удар."), span_warning("Ваша броня защитила [hit_area_name], смягчив удар."), armour_penetration = item.armour_penetration)
	if(armor >= 100)
		return .
...
/mob/living/carbon/human/send_item_attack_message(obj/item/item, mob/living/user, def_zone)
	...
	if(!item.force)
		visible_message(
			span_warning("[user] аккуратно тыкнул[genderize_ru(user.gender, "", "а", "о", "и")] [src] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]."),
			span_warning("[user] аккуратно тыкнул[genderize_ru(user.gender, "", "а", "о", "и")] вас [item.declent_ru(INSTRUMENTAL)] [message_hit_area]."),
			ignored_mobs = user,
		)
		to_chat(user, span_warning("Вы аккуратно тыкнули [src] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]."))
		return
	...
	visible_message(
		span_danger("[user] [message_verb][genderize_ru(user.gender, "", "а", "о", "и")] [src] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]!"),
		span_userdanger("[user] [message_verb][genderize_ru(user.gender, "", "а", "о", "и")] вас [item.declent_ru(INSTRUMENTAL)] [message_hit_area]!"),
		ignored_mobs = user,
	)
	to_chat(user, span_danger("Вы [message_verb]и [src] [item.declent_ru(INSTRUMENTAL)] [message_hit_area]!"))
