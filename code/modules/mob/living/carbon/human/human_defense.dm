...
/mob/living/carbon/human/bullet_act(obj/projectile/P, def_zone)
	...
	if(P.is_reflectable(REFLECTABILITY_ENERGY))
		...
		if(reflected)
			visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] отражает [P.declent_ru(ACCUSATIVE)]!"), \
							span_userdanger("[declent_ru_cap(NOMINATIVE)] отражает [P.declent_ru(ACCUSATIVE)]!"),\
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_HEAD) //only if we didn't specify a zone or if that zone is the head.
		...
		if(head_clothes)
			if(!(head_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[head_clothes.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(head_clothes)] вашу голову и лицо от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_CHEST)
		...
		if(chest_clothes)
			if(!(chest_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[chest_clothes.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(chest_clothes)] ваше туловище от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_L_ARM || bodyzone_hit == BODY_ZONE_R_ARM)
		...
		if(arm_clothes)
			if(!(arm_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[arm_clothes.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(arm_clothes)] ваши руки от кислоты!"))
		else
	...
	if(!bodyzone_hit || bodyzone_hit == BODY_ZONE_L_LEG || bodyzone_hit == BODY_ZONE_R_LEG || bodyzone_hit == "feet")
		...
		if(leg_clothes)
			if(!(leg_clothes.resistance_flags & UNACIDABLE))
				...
				to_chat(src, span_notice("[leg_clothes.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(leg_clothes)] ваши руки от кислоты!"))
		else
...
/mob/living/carbon/human/proc/reagent_safety_check(hot = TRUE)
	if(wear_mask)
		to_chat(src, span_danger("[wear_mask.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(wear_mask)] вас от [hot ? "горячей" : "холодной"] жидкости!"))
		return FALSE
	if(head)
		to_chat(src, span_danger("[head.declent_ru_cap(NOMINATIVE)] защища[PLUR_ET_YUT(head)] вас от [hot ? "горячей" : "холодной"] жидкости!"))
		return FALSE
	return TRUE
