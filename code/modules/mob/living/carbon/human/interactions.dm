...
/mob/living/carbon/human/proc/make_interaction()
	...
	var/mob/living/carbon/human/partner_human = human.partner
	var/obj/item/organ/external/temp = human.bodyparts_by_name[BODY_ZONE_PRECISE_R_HAND]
	var/hashands = (temp?.is_usable())
	if(!hashands)
		temp = human.bodyparts_by_name[BODY_ZONE_PRECISE_L_HAND]
		hashands = (temp?.is_usable())
	temp = partner_human.bodyparts_by_name[BODY_ZONE_PRECISE_R_HAND]
	var/hashands_p = (temp?.is_usable())
	if(!hashands_p)
		temp = partner_human.bodyparts_by_name[BODY_ZONE_PRECISE_L_HAND]
		hashands = (temp?.is_usable())
	var/mouthfree = !((human.head && (human.head.flags_cover & HEADCOVERSMOUTH)) || (human.wear_mask && (human.wear_mask.flags_cover & MASKCOVERSMOUTH)))
	var/mouthfree_p = !((partner_human.head && (partner_human.head.flags_cover & HEADCOVERSMOUTH)) || (partner_human.wear_mask && (partner_human.wear_mask.flags_cover & MASKCOVERSMOUTH)))

	var/dat = span_fontsize3("<b><hr>[human.partner]</b><br><hr>")
	...
	if(hashands)
		...
		if(Adjacent(partner_human))
			...
			if(partner_human.dna.species.name == SPECIES_MOTH)
				dat +=  {"• <a href='byond://?src=[UID()];interaction=pullwing'><span style='color: darkred;'>Дёрнуть за крылья!</span></a><br>"}
			if((partner_human.dna.species.name == SPECIES_TAJARAN)  || (human.dna.species.name == SPECIES_VOX)|| (human.dna.species.name == SPECIES_VULPKANIN) || (human.dna.species.name == SPECIES_UNATHI))
				dat +=  {"• <a href='byond://?src=[UID()];interaction=pull'><span style='color: darkred;'>Дёрнуть за хвост!</span></a><br>"}
				if(partner_human.can_inject(human))
	...
	if(mouthfree && human.dna.species.name != SPECIES_DIONA)
