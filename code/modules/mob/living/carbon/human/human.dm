...
/mob/living/carbon/human/Topic(href, href_list)
	if(in_range(src, usr) && !usr.incapacitated() && !HAS_TRAIT(usr, TRAIT_HANDS_BLOCKED))
		...
		if(href_list["embedded_object"])
			...
			usr.visible_message(
				span_warning("[usr] пытается извлечь [thing.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[bodypart.limb_zone][GENITIVE]]."),
			...
			if(do_after(usr, time_taken, src))
				...
				if(ishuman(usr))
					...
					if(h_user.has_pain())
						h_user.emote("scream")
				usr.visible_message(
					span_warning("[usr] с усилием извлекает [thing.declent_ru(ACCUSATIVE)] из [GLOB.body_zone[bodypart.limb_zone][GENITIVE]]!"),
...
/mob/living/carbon/human/proc/play_xylophone()
	if(!src.xylophone)
		visible_message(
			span_warning("[src] начинает играть на своей грудной клетке, как на ксилофоне."),
...
/mob/living/carbon/human/verb/check_pulse()
	...
	if(!self)
		...
		usr.visible_message(
			span_notice("[usr] прижимает пальцы к артерии и замирает, будто бы что-то считая."),
			ignored_mobs = usr
		)
		balloon_alert(usr, "обнаружение пульса...")
...
/mob/living/carbon/human/proc/do_cpr(mob/living/carbon/human/H)
	...
	if(H.receiving_cpr) // To prevent spam stacking
		...
		return
	visible_message(
		span_danger("[src] пытается провести СЛР на [H.declent_ru(PREPOSITIONAL)]."),
		ignored_mobs = src
	)
	...
	if(do_after(src, 4 SECONDS, H, NONE))
		if(H.health <= HEALTH_THRESHOLD_CRIT)
			...
			visible_message(
				span_danger("[src] успешно проводит СЛР на [H.declent_ru(PREPOSITIONAL)]."),
				ignored_mobs = src
			)
...
/mob/living/carbon/human/extinguish_light(force = FALSE)
	...
	if(O?.glowing)
		...
		visible_message(
			span_danger("[src] растворяется во тьме."),
...
/mob/living/carbon/human/harvest(mob/living/user)
	...
	if(is_monkeybasic(src))
		while(meatleft > 0)
			...
			meatleft--
		visible_message(span_notice("[user] разделывает [declent_ru(ACCUSATIVE)]."))
		gib()
...
/mob/living/carbon/human/proc/fireman_carry(mob/living/carbon/target)
	...
	visible_message(
		span_notice("[src] начинает поднимать [target.declent_ru(ACCUSATIVE)] и закидывать на плечо."),
		ignored_mobs = src
	)
