	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		...
		if(bodypart.is_traumatized() || !bodypart.properly_attached)
			...
			if(!bodypart.properly_attached && has_pain())
				visible_message(
					span_warning("[src] [emote_scream]бросает предмет, который держал[GEND_ENDING_A_O_I(src)] в [bodypart.declent_ru(PREPOSITIONAL)]!"),
			...
			custom_emote(EMOTE_VISIBLE, "[(has_pain()) ? emote_scream :  "" ]бросает предмет, который держал[GEND_ENDING_A_O_I(src)] в [bodypart.declent_ru(PREPOSITIONAL)]!")
			...
			custom_emote(EMOTE_VISIBLE, "бросает предмет, который держал[GEND_ENDING_A_O_I(src)], [GEND_HIS_HER(src)] [bodypart.declent_ru(NOMINATIVE)] выходит из строя!")
