	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		...
		if(bodypart.is_traumatized() || !bodypart.properly_attached)
			...
			if(!bodypart.properly_attached && has_pain())
				visible_message(
					span_warning("[src] [emote_scream]броса[PLUR_ET_UT(src)] предмет, который держал[GEND_ENDING_A_O_I(src)] в [bodypart.declent_ru(PREPOSITIONAL)]!"),
			...
			custom_emote(EMOTE_VISIBLE, "[(has_pain()) ? emote_scream :  "" ]броса[PLUR_ET_UT(src)] предмет, который держал[GEND_ENDING_A_O_I(src)] в [bodypart.declent_ru(PREPOSITIONAL)]!")
			...
			custom_emote(EMOTE_VISIBLE, "броса[PLUR_ET_UT(src)] предмет, который держал[GEND_ENDING_A_O_I(src)], [GEND_HIS_HER(src)] [bodypart.declent_ru(NOMINATIVE)] выход[PLUR_IT_YAT(bodypart)] из строя!")
