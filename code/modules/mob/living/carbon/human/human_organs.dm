...
/mob/living/carbon/human/proc/handle_grasp()
	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		...
		if(bodypart.is_traumatized() || !bodypart.properly_attached)
			...
			var/emote_scream = pick("кричит от боли и ", "издаёт резкий крик и ", "вскрикивает и ")
			if(!bodypart.properly_attached && has_pain())
				visible_message(
					span_warning("[src] [emote_scream]бросает предмет, который держал[genderize_ru(gender,"","а","о","и")] в [bodypart.declent_ru(PREPOSITIONAL)]!"),
			...
			custom_emote(EMOTE_VISIBLE, "[(has_pain()) ? emote_scream :  "" ]бросает предмет, который держал[genderize_ru(gender,"","а","о","и")] в [bodypart.declent_ru(PREPOSITIONAL)]!")
			...
			custom_emote(EMOTE_VISIBLE, "бросает предмет, который держал[genderize_ru(gender,"","а","о","и")], [GEND_HIS_HER(src)] [bodypart.declent_ru(NOMINATIVE)] выходит из строя!")
