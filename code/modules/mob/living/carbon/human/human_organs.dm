...
/mob/living/carbon/human/proc/handle_grasp()
	...
	for(var/obj/item/organ/external/bodypart as anything in bodyparts)
		...
		if(bodypart.is_traumatized() || !bodypart.properly_attached)
			...
			var/emote_scream = pick("крич[pluralize_ru(gender,"ит","ат")] от боли и ", "изда[PLUR_YOT_UT(src)] резкий крик и ", "вскрикива[PLUR_ET_UT(src)] и ")
			if(!bodypart.properly_attached && has_pain())
				visible_message(
					span_warning("[src] [emote_scream]броса[PLUR_ET_UT(src)] предмет, который держал[genderize_ru(gender,"","а","о","и")] в [bodypart.declent_ru(PREPOSITIONAL)]!"),
			...
			custom_emote(EMOTE_VISIBLE, "[(has_pain()) ? emote_scream :  "" ]броса[PLUR_ET_UT(src)] предмет, который держал[genderize_ru(gender,"","а","о","и")] в [bodypart.declent_ru(PREPOSITIONAL)]!")
			...
			custom_emote(EMOTE_VISIBLE, "броса[PLUR_ET_UT(src)] предмет, который держал[genderize_ru(gender,"","а","о","и")], [GEND_HIS_HER(src)] [bodypart.declent_ru(NOMINATIVE)] выход[PLUR_IT_YAT(bodypart)] из строя!")
