...
/datum/click_intercept/give/New(client/C)
	...
	RegisterSignal(giver, list(COMSIG_QDELETING, COMSIG_ITEM_EQUIPPED, COMSIG_ITEM_DROPPED), PROC_REF(signal_qdel))
	RegisterSignal(giving_item, list(COMSIG_QDELETING, COMSIG_MOB_SWAP_HANDS, SIGNAL_ADDTRAIT(TRAIT_HANDS_BLOCKED)), PROC_REF(signal_qdel))
...
/atom/movable/screen/alert/take_item/proc/cancel_give()
	...
	to_chat(receiver, span_warning("[giver] передум[genderize_ru(giver.gender, "ал", "ала", "ало", "али")] передавать вам [locateUID(item_UID)]."))
