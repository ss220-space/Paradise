		...
		switch(ans)
			if("Взять")
				...
				if(!Adjacent(target))
					...
					return
				if(HAS_TRAIT(I, TRAIT_NODROP) || (I.item_flags & ABSTRACT))
					to_chat(usr, span_warning("[I.declent_ru_cap(NOMINATIVE)] прилип[GEND_LA_LO_LI(I)]  к вашей руке и не отдаётся!"))
					to_chat(target, span_warning("[I.declent_ru_cap(NOMINATIVE)] прилип[GEND_LA_LO_LI(I)] к руке [usr.name], когда вы попытались взять!"))
					return
...
/mob/living/carbon/verb/toggle_give()
	...
	if(HAS_TRAIT(I, TRAIT_NODROP))
		to_chat(src, span_warning("[I.declent_ru_cap(NOMINATIVE)] прилип[GEND_A_O_I(I)] к вашей руке и не отда[PLUR_YOT_YUT(I)]ся!"))
		return
...
/atom/movable/screen/alert/take_item/Click(location, control, params)
	...
	if(HAS_TRAIT(I, TRAIT_NODROP))
		to_chat(giver, span_warning("[I.declent_ru_cap(NOMINATIVE)] прилип к вашей руке при попытке передачи!"))
		to_chat(receiver, span_warning("[I.declent_ru_cap(NOMINATIVE)] прилип к руке [giver] когда вы пытались взять!"))
		return
