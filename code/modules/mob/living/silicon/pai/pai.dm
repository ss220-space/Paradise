...
/mob/living/silicon/pai/attackby(obj/item/I, mob/user, params)
	...
	if(!I.force)
		...
		visible_message(
			span_warning("[user] бережно стукает по [name] [I.declent_ru(INSTRUMENTAL)]."),
			span_warning("[user] бережно стукает вас [I.declent_ru(INSTRUMENTAL)]."),
		)
		return ATTACK_CHAIN_PROCEED_SUCCESS
	...
	visible_message(
		span_danger("[user] сильно бьёт по [name] [I.declent_ru(INSTRUMENTAL)]!"),
		span_userdanger("[user] сильно бьёт вас [I.declent_ru(INSTRUMENTAL)]!"),
	)
...
/mob/living/silicon/pai/MouseDrop(mob/living/carbon/human/user, src_location, over_location, src_control, over_control, params)
	...
	if(usr == src)
		switch(tgui_alert(user, "[src] хочет, чтобы вы его подобрали. Подобрать?", "Подбор", list("Да", "Нет")))
			...
			if("Нет")
				to_chat(src, span_warning("[user] не хочет вас подбирать..."))
	else
		if(Adjacent(user))
			get_scooped(user)
		else
