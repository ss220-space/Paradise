...
/datum/paiController/proc/findPAI(var/obj/item/paicard/p, var/mob/user)
	...
	dat += {"
		...
		<html>
			...
			<body>
	"}
	dat += "<p>Отображение доступных личностей ПИИ из центральной базы данных…</p><p>Если личности не будут обнаружены, либо если не будет найдена подходящая личность, попробуйте выполнить повторный поиск новых личностей ПИИ позже.</p><p>Поиск личностей начат, вы можете закрыть это окно до получения уведомления о появлении доступных личностей. Обычно это занимает от 15 секунд до нескольких минут.</p>"
