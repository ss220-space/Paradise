...
/mob/living/silicon/robot/drone/proc/transfer_personality(client/player)
	...
	to_chat(src, "Для связи с другими дронами используйте <b>'[get_language_prefix(LANGUAGE_DRONE_BINARY)]'</b>. Также вы можете тихо говорить на языке, понятном только вашим собратьям.")
