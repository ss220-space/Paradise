...
/obj/item/gripper/melee_attack_chain(mob/user, atom/target, list/modifiers)	// this shit requires massive refactoring
	...
	if(gripped_item) //Already have an item.
		...
		if(is_type_in_typecache(item, can_hold) && !HAS_TRAIT_FROM_ONLY(item, TRAIT_NODROP, CYBORG_ITEM_TRAIT)) // Make sure the item is something the gripper can hold
			. |= ATTACK_CHAIN_SUCCESS
			balloon_alert(user, "подобрано")
			item.forceMove(src)
			gripped_item = item
			item.update_icon(UPDATE_OVERLAYS) //Some items change their appearance upon being pulled (IV drip as an example)
			update_icon(UPDATE_OVERLAYS)
			RegisterSignals(item, list(COMSIG_MOVABLE_MOVED, COMSIG_QDELETING), PROC_REF(handle_item_moving))
			RegisterSignals(item, list(COMSIG_ATOM_UPDATED_ICON), PROC_REF(handle_item_icon_update))
		else
