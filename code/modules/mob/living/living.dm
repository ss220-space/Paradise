...
/mob/living/proc/ZImpactDamage(turf/impacted_turf, levels)
	...
	if(levels <= 1 && ((istajaran(src) && functional_legs) || cat) && body_position != LYING_DOWN && can_help_themselves)
		...
		if(cat || HAS_TRAIT(src, TRAIT_DWARF)) // lil' bounce kittens
			visible_message(
				span_notice("[capitalize(declent_ru(NOMINATIVE))] жёстко приземляется на [impacted_turf.declent_ru(ACCUSATIVE)], и вскакивает на ноги!"),
		...
		visible_message(
			span_danger("[capitalize(declent_ru(NOMINATIVE))] жёстко приземляется на [impacted_turf.declent_ru(ACCUSATIVE)] и болезненно встаёт на ноги!"),
...
/mob/living/proc/MobBump(mob/living/bumped_mob)
	...
	if(get_confusion() && get_disoriented())
		...
		visible_message(
			span_danger("[name] врезается в [bumped_mob.declent_ru(ACCUSATIVE)], сбивая друг друга с ног!"),
...
/mob/living/proc/ObjBump(obj/object)
	if(get_confusion() && get_disoriented())
		...
		take_organ_damage(rand(5, 10))
		visible_message(
			span_danger("[name] врезается в [object.declent_ru(ACCUSATIVE)]!"),
...
/mob/living/run_pointed(atom/target)
	...
	if(isgun(hand_item) && target != hand_item)
		if(a_intent == INTENT_HELP || !ismob(target))
			visible_message("<b>[declent_ru(NOMINATIVE)]</b> указывает [hand_item.declent_ru(INSTRUMENTAL)] на [pointed_object].")
			return TRUE

		target.visible_message(
			span_danger("[declent_ru(NOMINATIVE)] направляет [hand_item.declent_ru(INSTRUMENTAL)] на [pointed_object]!"),
			span_userdanger("[declent_ru(NOMINATIVE)] направляет [hand_item.declent_ru(INSTRUMENTAL)] на вас!"),
		)
	...
	visible_message("<b>[declent_ru(NOMINATIVE)]</b> указывает на [pointed_object].")
	add_emote_logs(src, "point to [key_name(target)] [COORD(target)]")
	return TRUE
...
/mob/living/move_from_pull(atom/movable/puller, turf/target_turf, glide_size_override)
	...
	Knockdown(3 SECONDS)
	puller.stop_pulling()
	visible_message(span_danger("Ноги [name] путаются и [GEND_HE_SHE(src)] с грохотом падает на пол!"))
...
/mob/living/resist_grab(moving_resist = FALSE)
	...
	to_chat(pulledby, span_danger("[name] пытается вырваться из Вашего захвата!"))
	...
	switch(grabber.a_intent)
		if(INTENT_HARM)
			...
			victim.visible_message(
				span_danger("[attacker] наносит удар головой в мягкие ткани черепа [victim.name]!"),
				span_userdanger("[attacker] наносит удар головой в мягкие ткани Вашего черепа!"),
			)
...
/mob/living/hit_by_thrown_carbon(mob/living/carbon/human/C, datum/thrownthing/throwingdatum, damage, mob_hurt, self_hurt)
	...
	C.Weaken(3 SECONDS)
	C.visible_message(span_danger("[C.name] врезается в [name], сбивая друг друга с ног!"),
...
/mob/living/proc/knockOver(mob/living/carbon/target)
	if(target.key) //save us from monkey hordes
		target.visible_message(span_warning("[pick( \
			"[target] спотыкается об [declent_ru(GENITIVE)]!", \
			"[target] опрокидывается на [declent_ru(GENITIVE)]!", \
			"[target] отлетает с пути [declent_ru(GENITIVE)]!", \
