...
/mob/living/proc/ZImpactDamage(turf/impacted_turf, levels)
	...
	if(can_help_themselves)
		...
		if(bodypart_wing && !bodypart_wing.has_fracture()) // wings can soften
			visible_message(
				span_notice("[declent_ru_cap(NOMINATIVE)] жёстко приземля[PLUR_ET_YUT(src)]ся на [impacted_turf.declent_ru(ACCUSATIVE)], но оста[PLUR_YOT_YUT(src)]ся невредим[GEND_A_O_Y(src)] после падения."),
	...
	if(levels <= 1 && ((istajaran(src) && functional_legs) || cat) && body_position != LYING_DOWN && can_help_themselves)
		...
		if(cat || HAS_TRAIT(src, TRAIT_DWARF)) // lil' bounce kittens
			visible_message(
				span_notice("[declent_ru_cap(NOMINATIVE)] жёстко приземля[PLUR_ET_YUT(src)]ся на [impacted_turf.declent_ru(ACCUSATIVE)], и вскакива[PLUR_ET_YUT(src)] на ноги!"),
		...
		visible_message(
			span_danger("[declent_ru_cap(NOMINATIVE)] жёстко приземля[PLUR_ET_YUT(src)]ся на [impacted_turf.declent_ru(ACCUSATIVE)] и болезненно вста[PLUR_YOT_YUT(src)] на ноги!"),
...
/mob/living/proc/harvest(mob/living/user)
	...
	visible_message(span_notice("[user.declent_ru_cap(NOMINATIVE)] разделывает [declent_ru(ACCUSATIVE)]."))
	gib()
...
/mob/living/proc/knockOver(mob/living/carbon/target)
	if(target.key) //save us from monkey hordes
		target.visible_message(span_warning("[pick( \
			...
			"[declent_ru_cap(NOMINATIVE)] сбивает [target]!", \
			"[declent_ru_cap(NOMINATIVE)] влетает в [target], заставляя [GEND_HIS_HER(target)] упасть!", \
			"[declent_ru_cap(NOMINATIVE)] опрокидывает [target]!")]")
		)
