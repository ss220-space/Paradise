...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if(stat != DEAD)
		...
		M.do_attack_animation(src)
		visible_message("<span class='danger'>\The [M.name] поглощает [src]!</span>", "<span class='userdanger'>\The [M.name] поглощает вас!</span>")
		return TRUE
...
/mob/living/attack_larva(mob/living/carbon/alien/larva/L)
	switch(L.a_intent)
		if(INTENT_HELP)
			visible_message("<span class='notice'>[L.name] трется головой о [src].</span>")
			return 0
			...
			if(prob(90))
				...
				visible_message("<span class='danger'>[L.name] кусает [src]!</span>", \
						"<span class='userdanger'>[L.name] кусает [src]!</span>")
				playsound(loc, 'sound/weapons/bite.ogg', 50, 1, -1)
				return 1
			else
				visible_message("<span class='danger'>[L.name] пытается укусить [src]!</span>", \
					"<span class='userdanger'>[L.name] пытается укусить [src]!</span>")
	return 0

/mob/living/attack_alien(mob/living/carbon/alien/humanoid/M)
	switch(M.a_intent)
		if(INTENT_HELP)
			visible_message("<span class='notice'>[M] ласкает [src] своей серповидной рукой.</span>")
			return FALSE
		if(INTENT_GRAB)
			grabbedby(M)
			return FALSE
