...
/mob/living/hitby(atom/movable/AM, skipcatch, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum)
	...
	visible_message(
		span_danger("[capitalize(declent_ru(NOMINATIVE))] получает удар [thrown_item.declent_ru(INSTRUMENTAL)]."),
		span_userdanger("[capitalize(declent_ru(NOMINATIVE))] получает удар [thrown_item.declent_ru(INSTRUMENTAL)].")
	)
...
/mob/living/mech_melee_attack(obj/mecha/M)
	if(M.occupant.a_intent == INTENT_HARM)
		...
		visible_message(
			span_danger("[capitalize(M.declent_ru(NOMINATIVE))] ударяет [declent_ru(ACCUSATIVE)]!"),
			span_userdanger("[capitalize(M.declent_ru(NOMINATIVE))] ударяет вас!")
		)
		...
		if(!is_strong())
			...
			visible_message(
				span_warning("[capitalize(M.declent_ru(NOMINATIVE))] отталкивает [declent_ru(ACCUSATIVE)] в сторону.")
			)
		else
			M.occupant_message(span_warning("Вы пытаетесь оттолкнуть [declent_ru(ACCUSATIVE)] в сторону, но это не срабатывает."))
			visible_message(
				span_warning("[capitalize(M.declent_ru(NOMINATIVE))] безуспешно пытается оттолкнуть [declent_ru(ACCUSATIVE)] в сторону.")
			)
...
/mob/living/proc/IgniteMob()
	if(fire_stacks > 0 && !on_fire)
		on_fire = TRUE
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] загорается!"), span_userdanger("Вы загораетесь!"))
...
/mob/living/proc/grippedby(mob/living/grabber, grab_state_override)
	...
	if(isnull(grab_state_override) && grabber.grab_state) //only the first upgrade is instantaneous
		...
		visible_message(
			span_danger("[grabber.name] начинает усиливать хватку над [name]!"),
			span_userdanger("[grabber.name] начинает усиливать хватку над Вами!"),
	...
	switch(grabber.grab_state)
		if(GRAB_AGGRESSIVE)
			...
			if(HAS_TRAIT(grabber, TRAIT_PACIFISM) || GLOB.pacifism_after_gt)
				visible_message(
					span_danger("[grabber.name] крепко сжимает [name]!"),
					span_danger("[grabber.name] крепко сжимает Вас!"),
					span_italics("Вы слышите агрессивную возню!"),
					ignored_mobs = grabber,
				)
				...
				visible_message(
					span_danger("[grabber.name] агрессивно хватает [name]!"),
					span_danger("[grabber.name] агрессивно хватает Вас!"),
					span_italics("Вы слышите агрессивную возню!"),
					ignored_mobs = grabber,
				)
		...
		if(GRAB_NECK)
			...
			visible_message(
				span_danger("[grabber.name] хватает [name] за шею!"),
				span_userdanger("[grabber.name] хватает Вас за шею!"),
				span_italics("Вы слышите агрессивную возню!"),
				ignored_mobs = grabber,
			)
		...
		if(GRAB_KILL)
			...
			visible_message(
				span_danger("[grabber.name] душит [name]!"),
				span_userdanger("[grabber.name] душит Вас!"),
				span_italics("Вы слышите агрессивную возню!"),
				ignored_mobs = grabber,
			)
...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if(stat != DEAD)
		...
		M.do_attack_animation(src)
		visible_message(
			span_danger("[capitalize(M.declent_ru(NOMINATIVE))] поглощает [declent_ru(ACCUSATIVE)]!"),
			span_userdanger("[M.declent_ru(NOMINATIVE)] поглощает вас!")
		)
		return TRUE
...
/mob/living/attack_animal(mob/living/simple_animal/M)
	...
	M.do_attack_animation(src)
	visible_message(
		span_danger("[capitalize(M.declent_ru(NOMINATIVE))] [M.attacktext] [declent_ru(ACCUSATIVE)]!"),
...
/mob/living/attack_larva(mob/living/carbon/alien/larva/L)
	switch(L.a_intent)
		if(INTENT_HELP)
			visible_message(
				span_notice("[L.declent_ru(NOMINATIVE)] трётся головой о [declent_ru(ACCUSATIVE)].")
			)
			return FALSE
			...
			if(prob(90))
				...
				visible_message(
					span_danger("[capitalize(L.declent_ru(NOMINATIVE))] кусает [declent_ru(ACCUSATIVE)]!"),
					span_userdanger("[capitalize(L.declent_ru(NOMINATIVE))] кусает [declent_ru(ACCUSATIVE)]!")
				)
				playsound(loc, 'sound/weapons/bite.ogg', 50, TRUE, -1)
				return TRUE
			else
				visible_message(
					span_danger("[capitalize(L.declent_ru(NOMINATIVE))] пытается укусить [declent_ru(ACCUSATIVE)]!"),
					span_userdanger("[capitalize(L.declent_ru(NOMINATIVE))] пытается укусить [declent_ru(ACCUSATIVE)]!")
				)
	return FALSE
...
/mob/living/attack_alien(mob/living/carbon/alien/humanoid/M)
	...
	switch(M.a_intent)
		if(INTENT_HELP)
			visible_message(span_notice("[M.declent_ru(NOMINATIVE)] гладит [declent_ru(ACCUSATIVE)] своей серповидной рукой."))
			return FALSE
		if(INTENT_GRAB)
			grabbedby(M)
			return FALSE
