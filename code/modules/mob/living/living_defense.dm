...
/mob/living/mech_melee_attack(obj/mecha/mecha)
	if(mecha.occupant.a_intent == INTENT_HARM)
		if(HAS_TRAIT(mecha.occupant, TRAIT_PACIFISM) || GLOB.pacifism_after_gt)
			to_chat(mecha.occupant, span_warning("Вы не хотите навредить живым существам!"))
			return
		...
		switch(mecha.damtype)
			...
			if(STAMINA)
				...
				return
		mecha.occupant_message(span_danger("Вы ударяете [declent_ru(ACCUSATIVE)]."))
		visible_message(span_danger("[DECLENT_RU_CAP(mecha, NOMINATIVE)] ударя[PLUR_ET_YUT(mecha)] [declent_ru(ACCUSATIVE)]!"), span_userdanger("[DECLENT_RU_CAP(mecha, NOMINATIVE)] ударя[PLUR_ET_YUT(mecha)] вас!"))
		...
		if(!is_strong())
			...
			mecha.occupant_message(span_warning("Вы толкаете [declent_ru(ACCUSATIVE)] в сторону."))
			visible_message(span_warning("[DECLENT_RU_CAP(mecha, NOMINATIVE)] отталкива[PLUR_ET_YUT(mecha)] [declent_ru(ACCUSATIVE)] в сторону."))
		else
			mecha.occupant_message(span_warning("Вы пытаетесь оттолкнуть [declent_ru(ACCUSATIVE)] в сторону, но это не срабатывает."))
			visible_message(span_warning("[DECLENT_RU_CAP(mecha, NOMINATIVE)] безуспешно пытается оттолкнуть [declent_ru(ACCUSATIVE)] в сторону."))
