...
/mob/living/hitby(atom/movable/AM, skipcatch, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum)
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] получа[PLUR_ET_YUT(src)] удар [thrown_item.declent_ru(INSTRUMENTAL)]."),
					span_userdanger("[declent_ru_cap(NOMINATIVE)] получа[PLUR_ET_YUT(src)] удар [thrown_item.declent_ru(INSTRUMENTAL)]."))
...
/mob/living/mech_melee_attack(obj/mecha/M)
	if(M.occupant.a_intent == INTENT_HARM)
		...
		visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] ударя[PLUR_ET_YUT(M)] [declent_ru(ACCUSATIVE)]!"), span_userdanger("[M.declent_ru_cap(NOMINATIVE)] ударя[PLUR_ET_YUT(M)] вас!"))
		...
		if(!is_strong())
			...
			visible_message(span_warning("[M.declent_ru_cap(NOMINATIVE)] отталкива[PLUR_ET_YUT(M)] [declent_ru(ACCUSATIVE)] в сторону."))
		else
			M.occupant_message(span_warning("Вы пытаетесь оттолкнуть [declent_ru(ACCUSATIVE)] в сторону, но это не срабатывает."))
			visible_message(span_warning("[M.declent_ru_cap(NOMINATIVE)] безуспешно пытается оттолкнуть [declent_ru(ACCUSATIVE)] в сторону."))

//Mobs on Fire
/mob/living/proc/IgniteMob()
	if(fire_stacks > 0 && !on_fire)
		on_fire = TRUE
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] загора[PLUR_ET_YUT(src)]ся!"), span_userdanger("Вы загораетесь!"))
...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if(stat != DEAD)
		...
		M.do_attack_animation(src)
		visible_message(
			span_danger("[M.declent_ru_cap(NOMINATIVE)] поглоща[PLUR_ET_YUT(M)] [declent_ru(ACCUSATIVE)]!"),
...
/mob/living/attack_animal(mob/living/simple_animal/M)
	...
	M.do_attack_animation(src)
	visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] [M.attacktext] [declent_ru(ACCUSATIVE)]!"), \
					span_userdanger("[M.declent_ru_cap(NOMINATIVE)] [M.attacktext] [declent_ru(ACCUSATIVE)]!"))
	...
	switch(L.a_intent)
		if(INTENT_HELP)
			...
			if(prob(90))
				...
				visible_message(span_danger("[L.declent_ru_cap(NOMINATIVE)] куса[PLUR_ET_YUT(L)] [declent_ru(ACCUSATIVE)]!"), \
						span_userdanger("[L.declent_ru_cap(NOMINATIVE)] куса[PLUR_ET_YUT(L)] [declent_ru(ACCUSATIVE)]!"))
				playsound(loc, 'sound/weapons/bite.ogg', 50, TRUE, -1)
				return TRUE
			else
				visible_message(span_danger("[L.declent_ru_cap(NOMINATIVE)] пыта[PLUR_ET_YUT(L)]ся укусить [declent_ru(ACCUSATIVE)]!"), \
					span_userdanger("[L.declent_ru_cap(NOMINATIVE)] пыта[PLUR_ET_YUT(L)]ся укусить [declent_ru(ACCUSATIVE)]!"))
	return FALSE
