...
/mob/proc/show_message(msg, type, alt, alt_type)//Message, type of message (1 or 2), alternative message, alt message type (1 or 2)
	...
	if(stat == UNCONSCIOUS || (sleeping > 0 && stat != DEAD))
		to_chat(src, "<I>... Вам почти что удаётся расслышать чей-то разговор...</I>")
	else
		to_chat(src, msg)
	return
	...
	if(ishuman(M))
		...
		switch(slot)
			...
			if(slot_s_store)
				if(!H.wear_suit)
					if(!disable_warning)
						to_chat(H, "<span class='warning'>Вам нужен костюм перед тем как вы сможете прикрепить [name].</span>")
					return 0
...
/mob/verb/examinate(atom/A as mob|obj|turf in view())
	...
	if(!has_vision(information_only = TRUE) && !isobserver(src))
		to_chat(src, "<span class='notice'>Что-то здесь есть, но вы не можете разглядеть.</span>")
		return 1
...
/mob/proc/fakevomit(green = 0, no_text = 0) //for aesthetic vomits that need to be instant and do not stun. -Fox
	...
	if(istype(location, /turf/simulated))
		if(green)
			...
			if(!no_text)
				visible_message("<span class='warning'>[src.name] наблевал на себя!</span>","<span class='warning'>Вы наблевали на себя!</span>")
