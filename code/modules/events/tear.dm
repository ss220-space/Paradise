...
/datum/event/tear/announce(false_alarm)
	...
	if(!target_area)
		if(false_alarm)
			...
			kill()
			return
	GLOB.event_announcement.Announce("Ожидайте важное сообщение от нашего сотрудника. Дополнение к сообщению: [target_area.name].", "ВНИМАНИЕ: ОБНАРУЖЕНА АНОМАЛИЯ.", 'sound/announcer/tear.ogg')
