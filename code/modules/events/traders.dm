...
/datum/event/traders/start()
	...
	if(seclevel2num(get_security_level()) >= SEC_LEVEL_RED)
		GLOB.event_announcement.Announce("Торговому шаттлу со станции Юпитер было отказано в разрешении на стыковку из-за повышенной угрозы безопасности на борту [station_name()].", "Запрос на стыковку шаттла торговцев отклонен")
	...
	spawn()
		...
		while(spawn_count > 0 && candidates.len > 0)
			...
			if(C)
				...
				success_spawn = 1
		if(success_spawn)
			GLOB.event_announcement.Announce("Торговый шаттл со станции Юпитер получил разрешение на стыковку в порту прибытия 4 [station_name()].", "Запрос на стыковку шаттла торговцев принят")
		else
