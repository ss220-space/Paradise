	...
	spawn()
		...
		if(success_spawn)
			...
			if(station_name() == "NSS Cyberiad")
				map_trader_port = 4
			GLOB.event_announcement.Announce("A trading shuttle from Jupiter Station has been granted docking permission at [station_name()] arrivals port [map_trader_port].", "Trader Shuttle Docking Request Accepted")
		else
