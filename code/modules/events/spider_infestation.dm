...
/datum/event/spider_infestation/announce(false_alarm)
	if(successSpawn || false_alarm)
		GLOB.event_announcement.Announce("Ожидайте важное сообщение от нашего сотрудника.", "ВНИМАНИЕ: НЕОПОЗНАННЫЕ ФОРМЫ ЖИЗНИ.", new_sound = 'sound/announcer/spiders.ogg')
	else
