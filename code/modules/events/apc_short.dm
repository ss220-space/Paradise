...
/proc/power_restore(announce=TRUE)
	if(announce)
		GLOB.event_announcement.Announce("Питание на станции [station_name()] было восстановлено. Приносим извинения за неудобства.", "ВНИМАНИЕ: ЭНЕРГОПИТАНИЕ ВОССТАНОВЛЕНО.", new_sound = 'sound/AI/poweron.ogg')
...
/proc/power_restore_quick(announce=TRUE)
	if(announce)
		GLOB.event_announcement.Announce("Все СМЕСы на станции [station_name()] были перезаряжены. Приносим извинения за неудобства.", "ВНИМАНИЕ: ЭНЕРГОПИТАНИЕ ВОССТАНОВЛЕНО.", new_sound = 'sound/AI/poweron.ogg')
