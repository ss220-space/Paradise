...
/obj/structure/necropolis_gate/proc/toggle_the_gate(mob/user, legion_damaged)
	...
	if(open)
		new /obj/effect/temp_visual/necropolis(turf)
		visible_message(span_boldwarning("Врата с грохотом захлопываются!"))
		sleep(1)
		playsound(turf, 'sound/effects/stonedoor_openclose.ogg', 300, TRUE, frequency = 80000)
		sleep(1)
		set_density(TRUE)
		sleep(1)
		var/turf/sight_blocker_turf = get_turf(src)
		...
		sleep(2.5)
		playsound(turf, 'sound/magic/clockwork/invoke_general.ogg', 30, TRUE, frequency = 15000)
		add_overlay(door_overlay)
		open = FALSE
	else
		cut_overlay(door_overlay)
		new /obj/effect/temp_visual/necropolis/open(turf)
		sleep(2)
		visible_message(span_warning("Врата со скрежетом начинают открываться..."))
		playsound(turf, 'sound/effects/stonedoor_openclose.ogg', 300, TRUE, frequency = 20000)
		sleep(22)
...
/obj/structure/stone_tile/proc/toggle_fallen(new_fallen, init)
	...
	if(!give_turf_traits)
		give_turf_traits = string_list(list(TRAIT_LAVA_STOPPED, TRAIT_CHASM_STOPPED, TRAIT_TURF_IGNORE_SLOWDOWN, TRAIT_IMMERSE_STOPPED))
	if(fallen)
