...
/obj/machinery/cryopod/proc/free_taipan_role(job = null)
	switch(job)
		if(JOB_TITLE_TAIPAN_SCIENTIST)
			var/obj/effect/mob_spawn/human/space_base_syndicate/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_MEDIC)
			var/obj/effect/mob_spawn/human/space_base_syndicate/medic/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_BOTANIST)
			var/obj/effect/mob_spawn/human/space_base_syndicate/botanist/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_CARGO)
			var/obj/effect/mob_spawn/human/space_base_syndicate/cargotech/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_CHEF)
			var/obj/effect/mob_spawn/human/space_base_syndicate/chef/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_ENGINEER)
			var/obj/effect/mob_spawn/human/space_base_syndicate/engineer/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_COMMS)
			var/obj/effect/mob_spawn/human/space_base_syndicate/command/comms/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
		if(JOB_TITLE_TAIPAN_RD)
			var/obj/effect/mob_spawn/human/space_base_syndicate/command/rd/space_base_syndicate = new(get_turf(src))
			space_base_syndicate.setDir(dir)
			Destroy()
