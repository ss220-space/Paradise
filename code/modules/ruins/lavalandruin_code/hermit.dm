// The basic hermit mob spawner and related unique items for the hermit role.
...
/obj/effect/mob_spawn/human/hermit/Initialize(mapload)
	...
	switch(background_choice)
		if(1)
			...
			Сделки заключались в тишине кабинетов, а безопасность обеспечивали частные армии ТСФ. \
			Заказ казался рядовым — доставить чертежи на окраину системы. Условия были просты: никаких вопросов, двойная ставка. \
			Вы приняли его, как и десятки других. И теперь за вами охотятся. Вы чудом сбежали на этом аварийном шаттле. \
			Эти чертежи с вами — смертный приговор. Но это и ваш единственный шанс. Найдите на них покупателя и забудьте об этом кошмаре. \
			Вам нужно получить не менее 20 000 кредитов чтобы \"стереть свой след\" в этой истории."
		...
		if(2)
			flavour_text += "Вы были инженером проектировщиком на экспериментальной фабрике \"Системы Эйнштейна\". Вы работали над проектом \"Прометей\" — новым типом термоядерного \
			двигателя, который должен был дать второй шанс ядерной энергетике показать себя. Вы были горды за свои достижения, пока не увидели итоговый отчёт о стабильности. \
			Цифры не сходились. Реакция была неконтролируемой, ей было суждено стать катастрофой. Вы возмутились, но вам вежливо предложили \"не вмешиваться не в своё дело\". \
			Вы были уже готовы уйти, но оказалось, что теперь ваши разработки были переписаны под \"кооперативную собственность\" компании — вы более не владели своим изобретением. \
			Тогда вы приняли решение. На презентации проекта вы выгрузили все свои разработки на дискету и скрылись. Вас сразу же объявили вором и несостоявшимся сотрудником, что хочет заполучить всю славу себе, а \
			...
			Закончите свой проект и попытайтесь найти инвесторов на стороне."
		...
		if(3)
			...
			Неофициально — вы были смотрителем для \"испытуемых\" каторжников с Рида, по делу психологической и физической устойчивости к агрессивной флоре. \
			В одну из смен испытуемый заражённый грибком буквально разложился в состояние первичного бульона. Вы больше не могли молчать. \
		...
		if(4)
			flavour_text += "Вы были осуждённым каторжником на Риде. Рид — это ад, вырезанный изо льда и камня. Вы отбывали срок за преступление, которого не помните, \
			...
			Но лучше умереть свободным, чем сгнить заживо. Вы организовали бунт. Пока охранники отбивались от обезумевших зеков, вы прорвались в ангар. \
...
/obj/machinery/bodyscanner/hermit
	...
	desc = "Сложное медицинское устройство, используемое для сканирования физического состояния гуманоидов и поддержания их в состоянии криосна."
...
/obj/item/storage/backpack/satchel/hermit
	...
	desc = "Портфель с лямкой из тёмной дублёной кожи. Потёртый, не первой молодости, но безупречно ухоженный. На нём тисненая символика \"Солнечной гвардии\"."
...
/obj/item/documents/hermit/shellguard
	...
	desc = "\"Совершенно секретные\" документы корпорации \"Стальная Гвардия\", напечатанные на специальной бумаге, защищенной от копирования. Содержат технические характеристики, чертежи и подробности о новейших импульсных винтовках основанных на химических кислородо-йодный лазерах. Эти бумаги содержат превосходные оружейные данные!"
	icon_state = "docs_red"
	sell_multiplier = 1
	sell_interest = ALL
...
/obj/item/clothing/shoes/magboots/hermit
	name = "retro magboots"
	desc = "Магнитные ботинки от корпорации \"Системы Эйнштейна\", выполненные в старом стиле со всеми нюансами. Они не такие удобные, как современные модели, но надёжности им не занимать."
...
/obj/item/documents/hermit/einstein_engine
	...
	desc = "\"Совершенно секретные\" документы корпорации \"Системы Эйнштейна\", напечатанные на специальной бумаге, защищенной от копирования. Содержат технические характеристики, чертежи и подробности проекта \"Прометей\". Эти бумаги могут сильно ударить по активам \"Нанотрейзен\" и всех причастных к её плазменной промышленности, если попадут в нужные руки. Стоп... это печати ТСФ и СССП?!"
	icon_state = "docs_red"
	sell_multiplier = 1
...
/obj/item/documents/hermit/medical
	...
	desc = "\"Совершенно секретные\" архивные документы корпорации \"Нанотрейзен\", напечатанные на специальной бумаге, защищенной от копирования. Здесь содержится компрометирующая информация о неэтичных опытах над преступниками со всеми подробностями. Подписаны как вторым красным крестом, так и \"Нанотрейзен\"."
