...
ADMIN_VERB(advanced_proc_call, R_DEBUG|R_PROCCALL, "Advanced ProcCall", "Call a proc on any datum in the server.", ADMIN_CATEGORY_DEBUG)
	user.callproc_blocking()

/client/proc/callproc_blocking(list/get_retval)
	if(!check_rights(R_DEBUG|R_PROCCALL))
		return
...
/proc/WrapAdminProcCall(datum/target, procname, list/arguments)
	...
	if(target != GLOBAL_PROC)
		GLOB.LastAdminCalledTargetUID = UID_of(target)
...
ADMIN_VERB_ONLY_CONTEXT_MENU(call_proc_datum, R_DEBUG|R_PROCCALL, "Atom ProcCall", atom/thing as null|area|mob|obj|turf)
	...
	message_admins("[key_name(user)] called [thing]'s [procname]() with [length(lst) ? "the arguments [list2params(lst)]":"no arguments"].")
