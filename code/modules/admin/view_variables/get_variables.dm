...
/client/proc/vv_get_value(class, default_class, current_value, list/restricted_classes, list/extra_classes, list/classes, var_name)
	...
	switch(.["class"])
		...
		if(VV_NUM)
			.["value"] = tgui_input_number(usr, "Enter new number:", "Num", current_value, max_value = INFINITY, min_value = -INFINITY, round_value = FALSE)
			if(.["value"] == null)
				.["class"] = null
				return
		...
		if(VV_NEW_ATOM)
			...
			if(!type)
				...
				return
			.["type"] = type
			var/atom/new_type = new type()
			new_type.datum_flags |= DF_VAR_EDITED
			.["value"] = new_type
		...
		if(VV_NEW_LIST)
			...
			var/expectation = tgui_alert(usr, "Would you like to populate the list", "Populate List?", list("Yes", "No"))
		...
		if(VV_TEXT_LOCATE)
			...
			do
				...
				if(!ref)
					break
				datum_ref = locate(ref)
				if(!datum_ref)
					tgui_alert(usr, "Invalid ref!")
					continue
				if(!datum_ref.can_vv_mark())
		...
		if(VV_COLOR)
			.["value"] = tgui_input_color(usr, "Enter new color:", "Color", current_value)
			if(.["value"] == null)
				.["class"] = null
				return
