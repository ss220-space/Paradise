...
/client/proc/massmodify_variables(datum/target, var_name = "", strict_type = FALSE)
	...
	to_chat(src, "Variable contains: [translate_bitfield(default, variable, var_value)]", confidential = TRUE)
	...
	switch(class)
		if(VV_RESTORE_DEFAULT)
			...
			for(var/thing in items)
				...
				if(datum.vv_edit_var(variable, initial(datum.vars[variable])) != FALSE)
					accepted++
				else
					rejected++
				CHECK_TICK
		...
		if(VV_TEXT)
			...
			if(varsvars && length(varsvars))
				...
				if(unique == "Unique")
					...
					unique = FALSE
					for(var/edited_var in varsvars)
						new_value = replacetext(new_value,"\[[edited_var]]","[target.vars[edited_var]]")
			...
			for(var/thing in items)
				...
				if(unique)
					...
					for(var/edited_var in varsvars)
						new_value = replacetext(new_value,"\[[edited_var]]","[datum.vars[edited_var]]")
	...
	message_admins("[key_name_admin(src)] mass modified [original_name]'s [variable] to [html_encode(translate_bitfield(default, variable, target.vars[variable]))] (Type: [class]) ([accepted] objects modified)")

/// NOT using global lists as VV is a debug function and debug functions should really on as less things as possible.
/proc/get_all_of_type(target_type, subtypes = TRUE)
	var/list/typecache = list()
	typecache[target_type] = 1
	if(subtypes)
	...
	else if(ispath(target_type, /obj/machinery/door))
		for(var/obj/machinery/door/thing in world)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /obj/machinery))
		for(var/obj/machinery/thing in world)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /obj/item))
		for(var/obj/item/thing in world)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /obj))
	...
	else if(ispath(target_type, /turf))
		for(var/turf/thing in world)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /atom))
		for(var/atom/thing in world)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /client))
		for(var/client/thing in GLOB.clients)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK

	else if(ispath(target_type, /datum))
		for(var/datum/thing)
			if(typecache[thing.type])
				. += thing
			CHECK_TICK
