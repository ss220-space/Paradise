...
/proc/Centcomm_announce(text, mob/Sender)
	...
	for(var/client/client in GLOB.admins)
		if(R_EVENT & client.holder.rights)
			to_chat(client, msg)
	...
	for(var/client/client in GLOB.admins)
		if(check_rights(R_EVENT, FALSE, client.mob))
			...
			if(nuke_status == NUKE_MISSING)
				to_chat(client, span_userdanger("The nuclear device is not on station!"))
			else
				to_chat(client, span_adminnotice("<b>The nuke code is [nuke_code].</b>"))
				if(nuke_status == NUKE_CORE_MISSING)
					to_chat(client, span_userdanger("The nuclear device does not have a core, and will not arm!"))
			if(client.prefs.sound & SOUND_ADMINHELP)
				SEND_SOUND(client, sound('sound/effects/adminhelp.ogg'))
