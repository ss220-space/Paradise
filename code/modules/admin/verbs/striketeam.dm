	...
	for(var/obj/effect/landmark/L in GLOB.landmarks_list)
		...
		if(L.name == "Commando")
			...
			if(use_ds_borg)
				...
				R.key = ghost_mob.key
				if(nuke_code)
					R.mind.store_memory("<b>Коды от боеголовки:</b> <span class='warning'>[nuke_code]."))
				R.mind.store_memory("<b>Миссия:</b> <span class='warning'>[input]."))
				to_chat(R, span_userdanger("Вы борг отдела Специальных Операций, подчиняющийся Центральному Командованию. \nВаша миссия: <span class='danger'>[input]")))
			else
				...
				if(nuke_code)
					new_commando.mind.store_memory("<b>Коды от боеголовки:</b> <span class='warning'>[nuke_code]."))
				new_commando.mind.store_memory("<b>Миссия:</b> <span class='warning'>[input]."))
