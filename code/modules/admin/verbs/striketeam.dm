...
/client/proc/strike_team()
	...
	if(GLOB.sent_strike_team)
		to_chat(src, span_userdanger("Центральное Командование уже отправила один отряд."))
		if(alert("Вы хотите послать еще один?",,"Да","Нет")!="Да")
			return
	else if(alert("Вы хотите отправить отряд смерти Центрального Коммандования? После согласия это необратимо.",,"Да","Нет")!="Да")
		return
	alert("Этот «режим» будет продолжаться до тех пор, пока все не умрут или станция не будет уничтожена. Также, при необходимости, можно вызвать эвакуационный шаттл через админские кнопки. Появившиеся коммандос имеют внутренние камеры, которые можно просматривать через монитор внутри Офиса Спецопераций. Руководить командой рекомендуется оттуда. Первый выбранный/появившийся будет лидером команды.")

	message_admins(span_notice("[key_name_admin(usr)] has started to spawn a CentComm DeathSquad."))

	var/input = null
	while(!input)
		input = sanitize(copytext_char(input(src, "Пожалуйста, уточните, какую миссию будет выполнять Отряд Смерти.", "Укажите миссию", ""),1,MAX_MESSAGE_LEN))
		if(!input)
	...
	for(var/obj/effect/landmark/L in GLOB.landmarks_list)
		...
		if(L.name == "Commando")
			...
			if(!is_leader)
				var/new_dstype = alert(ghost_mob.client, "Выберете тип члена Отряда Смерти.", "Создание персонажа.", "Органик", "Борг")
				if(new_dstype == "Борг")
					use_ds_borg = TRUE
