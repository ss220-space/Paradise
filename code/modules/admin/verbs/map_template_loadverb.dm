/client/proc/map_template_load()
	...
	if(alert(usr,"Confirm location.","Template Confirm","Yes","No") == "Yes")
		...
		message_admins("<span class='adminnotice'>[key_name_admin(usr)] has started to place the map template ([template.name]) at [ADMIN_COORDJMP(T)]</span>")
		if(template.load(T, centered = TRUE))
			message_admins("<span class='adminnotice'>[key_name_admin(usr)] has placed a map template ([template.name]) at [ADMIN_COORDJMP(T)]. Took [stop_watch(timer)]s.</span>")
		else
