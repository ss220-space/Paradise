...
/datum/appearance_debugger/New(datum/admins/owner)
	...
	proxy_view_hover.generate_view("appearance_debugger_[owner.UID()]_proxy_hover")
	proxy_view_selected = new()
	proxy_view_selected.generate_view("appearance_debugger_[owner.UID()]_proxy_selected")
...
/datum/appearance_debugger/proc/get_appearance_data(atom/appearance_owner)
	...
	var/owner_uid = UID_of(appearance_owner)
	if(isatom(appearance_owner))
		target = appearance_cache["[owner_uid]"] || new /mutable_appearance(appearance_owner.appearance)
		appearance_cache["[owner_uid]"] = target
		if(!fake_vis["[owner_uid]"] && (ismovable(appearance_owner) || isturf(appearance_owner)))
			var/atom/movable/as_movable = appearance_owner
			var/list/false_vis_contents = list()
			fake_vis["[owner_uid]"] = false_vis_contents
