/client/proc/cmd_admin_say(msg as text)
	...
	for(var/client/C in GLOB.admins)
		if(check_rights(R_ADMIN|R_MOD, 0, C.mob))
			...
			to_chat(C, span_admin_channel("ADMIN: <span class='name'>[key_name(usr, 1)]</span> ([admin_jump_link(mob)]): <span class='message'>[msg]")), MESSAGE_TYPE_ADMINCHAT, confidential = TRUE)
...
/client/proc/cmd_mentor_say(msg as text)
	...
	for(var/client/C in GLOB.admins)
		if(check_rights(R_ADMIN|R_MOD|R_MENTOR, 0, C.mob))
			...
			msg = span_emojienabled("[msg]")
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "mentor_channel_admin" : "mentor_channel"]'>MENTOR: <span class='name'>[display_name]</span> ([admin_jump_link(mob)]): <span class='message'>[msg]")), MESSAGE_TYPE_MENTORCHAT, confidential = TRUE)
		...
		if(check_rights(R_ADMIN | R_VIEWRUNTIMES, FALSE, C.mob))
			...
			if(holder.fakekey)
				if(C.holder && C.holder.rights & R_ADMIN)
					...
					display_name = holder.fakekey
			msg = span_emojienabled("[msg]")
			to_chat(C, "<span class='[check_rights(R_ADMIN, FALSE) ? "dev_channel_admin" : "dev_channel"]'>DEV: <span class='name'>[display_name]</span> ([admin_jump_link(mob)]): <span class='message'>[msg]")), MESSAGE_TYPE_DEVCHAT, confidential = TRUE)
