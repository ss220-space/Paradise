...
/datum/plane_master_debug/proc/get_target()
	...
	if(!target?.hud_used || !explicit_mirror)
		...
		if(ismob(target.orbiting)) // If we're orbiting someone, swap to them if possible
			var/mob/as_mob = target.orbiting
			if(as_mob.hud_used)
...
/datum/plane_master_debug/ui_data()
	...
	data["mob_ref"] = reference_frame.UID()
	data["our_ref"] = owner.owner.mob.UID()
