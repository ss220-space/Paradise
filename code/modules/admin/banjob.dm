...
/proc/jobban_loadbanfile()
	if(CONFIG_GET(flag/ban_legacy_system))
		...
		//Job permabans
		var/datum/db_query/permabans = SSdbcore.NewQuery("SELECT ckey, job FROM [CONFIG_GET(string/utility_database)].[format_table_name("ban")] WHERE bantype = 'JOB_PERMABAN' AND isnull(unbanned)")
		...
		// Job tempbans
		var/datum/db_query/tempbans = SSdbcore.NewQuery("SELECT ckey, job FROM [CONFIG_GET(string/utility_database)].[format_table_name("ban")] WHERE bantype = 'JOB_TEMPBAN' AND isnull(unbanned) AND expiration_time > Now()")
...
/mob/verb/displayjobbans()
	set category = "OOC"
	set name = "Показать текущие джоббаны"
