...
#define TAG_CARBON_HEALTH(X) " \
	<font color='#ff0000'><b>Health: [X.health]</b></font> ( \
		...
		<font color='#00ffff'>[X.getStaminaLoss()]</font> \
		<font color='#800080'>[X.getCloneLoss()]</font> \
		<font color='#ffc0cb'>[X.getBrainLoss()]</font> \
"
...
ADMIN_VERB(display_tags, R_ADMIN, "View Tags", "Display all of the tagged datums.", ADMIN_CATEGORY_GAME)
	...
	dat += "<br><a href='byond://?_src_=holder;show_tags=1'>Refresh</a><br>"
	if(LAZYLEN(tagged_datums))
		for(var/datum/iter_datum as anything in tagged_datums)
			...
			if(isnull(iter_datum))
				...
				specific_info = "[TAG_CARBON_HEALTH(resolved_carbon)] | [AREACOORD(resolved_carbon)] [ADMIN_PP(iter_datum, "PP")] [ADMIN_FLW(iter_datum, "FLW")]"
			else if(isliving(iter_datum))
				var/mob/living/resolved_living = iter_datum
				specific_info = "[TAG_SIMPLE_HEALTH(resolved_living)] | [AREACOORD(resolved_living)] [ADMIN_PP(iter_datum, "PP")] [ADMIN_FLW(iter_datum, "FLW")]"
			else if(ismob(iter_datum))
				var/atom/resolved_atom = iter_datum // needed for ADMIN_JMP
				specific_info = "[AREACOORD(resolved_atom)] [ADMIN_PP(iter_datum, "PP")] [ADMIN_FLW(iter_datum, "FLW")]"
			else if(ismovable(iter_datum))
				var/atom/resolved_atom = iter_datum // needed for ADMIN_JMP
				specific_info = "[AREACOORD(resolved_atom)] [ADMIN_FLW(iter_datum, "FLW")]"
			...
			dat += "\t[index]: [iter_datum] | [specific_info] | [ADMIN_VV(iter_datum, "VV")] | [TAG_DEL(iter_datum)] | [iter_datum == marked_datum ? "<b>Marked</b>" : TAG_MARK(iter_datum)] "
