...
/atom/movable/proc/user_buckle_mob(mob/living/target, mob/living/user, check_loc = TRUE)
	...
	if(target != user) // Cheks if user interacts with himself
		target.visible_message(
			span_warning("[user] пытается пристегнуть [target.declent_ru(ACCUSATIVE)] к [declent_ru(DATIVE)]!"),
			span_userdanger("[user] пытается пристегнуть вас к [declent_ru(DATIVE)]!"),
	...
	if(.)
		if(target == user)
			target.visible_message(
				span_notice("[target] пристегивает себя к [declent_ru(DATIVE)]."),
				span_notice("Вы пристегиваете себя к [declent_ru(DATIVE)]."),
				span_italics("Слышен лязг металла."),
			)
		else
			target.visible_message(
				span_warning("[user] пристегивает [target.declent_ru(ACCUSATIVE)] к [declent_ru(DATIVE)]!"),
				span_warning("[user] пристегивает вас к [declent_ru(DATIVE)]!"),
				span_italics("Слышен лязг металла."),
			)
	...
	if(buckled_mob)
		if(buckled_mob != user)
			buckled_mob.visible_message(
				span_notice("[user] отстегивает [buckled_mob] от [declent_ru(GENITIVE)]."),
				span_notice("[user] отстегивает вас от [declent_ru(GENITIVE)]."),
				span_italics("Слышен лязг металла."),
			)
		else
			buckled_mob.visible_message(
				span_notice("[buckled_mob] отстегивает себя от [declent_ru(GENITIVE)]."),
