...
/datum/training_task/basic_2_1
	...
	"Для этого достаточно встать на клетку со стулом и перетащить с помощью ЛКМ своего персонажа на стул",
...
/datum/training_task/basic_2_2
	description = list("Чтобы встать со стула достаточно нажать B",
		"Помните, что если вы связаны и прикованы к стулу, то у вас не получится встать быстро",
		"Встаньте со стула, нажав B")
...
/datum/training_task/basic_3_1
	...
	"Поменяйте активную руку с помощью клавиши X")
	var/saved_hand
...
/datum/training_task/basic_3_3
	description = list("Имея в активной руке предмет вы можете его 'использовать'",
		...
		"Для этого используется клавиша Z",
		"Правило не распространяется на еду, напитки, шприцы и некоторые другие вещи",
		"Для их использования нажмите ЛКМ на своего персонажа, держа в активной руке предмет",
		"Важно понимать, когда предмет просто используется, а когда используется на персонажа",
		"В первом случае используйте Z, во втором ЛКМ на персонажа",
		"Попробуйте открыть ПДА с помощью клавиши Z, держа ПДА в активной руке")
...
/datum/training_task/basic_3_6
	description = list("Теперь, держа карту в активной руке, нажмите клавишу Е для того, чтобы быстро экипировать ее в нужный слот",
		"Горячая клавиша E подходит для почти любых предметов и позволяет быстро поместить их в нужное место",
		"После этого снова возьмите ПДА в активную руку и 'используйте' его клавишей Z",
		"На этот раз у вас будет открыто меню ПДА",
		"В ПДА есть немало полезных вещей, однако это вы узнаете позднее",
		"Сейчас закройте окно ПДА и поместите ПДА в ваш слот для ПДА. Используйте клавишу E)",
...
/datum/training_task/basic_4_1
	description = list("А сейчас жизненно важная задача",
		...
		"Для этого откройте рюкзак, нажав на него ЛКМ",
		"Откроется панель с вещами, лежащими в рюкзаке. Нажмите на маску ЛКМ чтобы взять ее в руку")
...
/datum/training_task/basic_4_2
	description = list("Хорошо, теперь наденьте маску на лицо",
		...
		"1 - держа маску в активной руке нажмите E. Она автоматически наденется на лицо, если сейчас на нем ничего нет",
...
/datum/training_task/basic_4_3
	description = list("Одной маски будет недостаточно",
		"Достаньте из рюкзака баллон и наденьте его в подходящий слот (можно и с помощью клавиши E)",
...
/datum/training_task/basic_4_4
	description = list("Осталось последнее - включить баллон",
		"Конечно можно взять баллон в руку, нажать Z, чтобы открылось меню баллона, и там нажать 'Set internals', но можно и проще",
...
/datum/training_task/basic_11_1
	...
	"Всего есть 4 интент",
	"Первый, зеленый - Помощь. Интент выставлен по умолчанию. С этим интентом вы будете поднимать людей, будить их, если у вас нет предметов в руках",
	...
	"Второй, желтый - Обезоруживание. Вы попытаетесь обезоружить кого-нибудь, выбив у него предмет из рук",
	"Третий, синий - Захват. Позволяет захватить человека. Это всегда агрессия, не используйте это для перетаскивания людей",
	"Четвертый, красный - Вред. Вы не будете бить сильнее с этим интентом, но если у вас в руках дубинка, то в добавок к оглушению вы будете наносить урон",
	...
	"Интенты можно переключать клавишами 1,2,3,4 согласно списку выше",
...
/datum/training_task/basic_13_1
	...
	"Однако не забывайте, существует википедия SS220, которая поможет вам освоиться как на любой из работ, так и на станции в целом",
