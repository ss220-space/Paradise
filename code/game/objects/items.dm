...
/obj/item/attack_hand(mob/user, pickupfireoverride = FALSE)
	...
	if(loc == user)
		...
		if(equip_delay_self > 0 && !user.is_general_slot(user.get_slot_by_item(src)))
			...
			if(!do_after(user, equip_delay_self, user, timed_action_flags = (DA_IGNORE_LYING|DA_IGNORE_USER_LOC_CHANGE), max_interact_count = 1, cancel_on_max = TRUE, cancel_message = span_warning("Снятие [name] было прервано!")))
				return
...
/obj/item/hit_by_thrown_mob(mob/living/C, datum/thrownthing/throwingdatum, damage, mob_hurt, self_hurt)
	return
