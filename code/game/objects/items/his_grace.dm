...
/obj/item/his_grace/proc/consume(mob/living/meal) //Here's your dinner, Mr. Grace.
	...
	if(!mind || mind.madeby_sentience_potion)
		meal.visible_message(span_his_grace("[capitalize(declent_ru(NOMINATIVE))] не получает насыщения от подобной пищи. [capitalize(declent_ru(NOMINATIVE))] недоволен!"))
		meal.forceMove(src)
		return
