...
/obj/item/implanter/attack(mob/living/carbon/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(target != user)
		target.visible_message(
			span_warning("[user] пытается имплантировать био-чип в [target].")
		)
	...
	if(user == target)
		...
		target.visible_message(
			span_warning("[user] имплантировал[GEND_ENDING_A_O_I(user)] био-чип в [target]."),
			span_notice("[user] имплантировал[GEND_ENDING_A_O_I(user)] вам био-чип."),
		)
	imp = null
	update_state()
