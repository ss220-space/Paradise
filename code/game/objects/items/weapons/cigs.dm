...
/obj/item/clothing/mask/cigarette/catch_fire()
	if(!lit)
		light(span_warning("[declent_ru_cap(NOMINATIVE)] зажигается от огня!"))
...
/obj/item/clothing/mask/cigarette/cigar/attackby(obj/item/item, mob/user, params)
	...
	if(!is_type_in_typecache(item, acceptable_lighters))
		add_fingerprint(user)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] просто ОТКАЗЫВА[uppertext(PLUR_ET_YUT(src))]СЯ быть прикуренной столь нецивилизованными методами."))
...
/obj/item/clothing/mask/cigarette/pipe/attackby(obj/item/item, mob/user, params)
	...
	if(!is_type_in_typecache(item, acceptable_lighters))
		add_fingerprint(user)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] просто ОТКАЗЫВАЕТСЯ быть прикуренной столь нецивилизованными методами."))
		return ATTACK_CHAIN_PROCEED
	return ..()
