...
/obj/item/clothing/mask/cigarette/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	if(target.on_fire)
		...
		light(span_notice("[user] хладнокровно прикуривает [declent_ru(ACCUSATIVE)] от горящего тела [target.declent_ru(GENITIVE)]. Очевидно, [GEND_HE_SHE(user)] желает [target.declent_ru(DATIVE)] всего хорошего."))
...
/obj/item/clothing/mask/cigarette/attackby(obj/item/item, mob/user, params)
	...
	if(isigniter(item))
		add_fingerprint(user)
		light(span_notice("[user] возится с [item.declent_ru(INSTRUMENTAL)], но в конце концов прикуривает [declent_ru(ACCUSATIVE)]."))
		return ATTACK_CHAIN_PROCEED_SUCCESS
	...
	if(istype(item, /obj/item/flashlight/flare))
		...
		if(!flare.on || !flare.can_fire_cigs)
			return ..()
		light(span_notice("[user] не находит ничего лучше [flare.declent_ru(GENITIVE)], чтобы прикурить [declent_ru(ACCUSATIVE)]. Бедолага..."))
		return ATTACK_CHAIN_PROCEED_SUCCESS
	...
	if(istype(item, /obj/item/clothing/mask/cigarette))
		...
		if(!cigarette.lit)
			return ..()
		light(span_notice("[user] прикуривает свою [declent_ru(ACCUSATIVE)] другой [cigarette.declent_ru(INSTRUMENTAL)]. Бедолага..."))
		return ATTACK_CHAIN_PROCEED_SUCCESS
...
/obj/item/clothing/mask/cigarette/attack_self(mob/user)
	if(lit)
		user.visible_message(span_notice("[user] спокойно зажимает прикуренный конец [declent_ru(GENITIVE)], мгновенно погасив [GEND_HIS_HER(src)]."))
		die()
	return ..()
