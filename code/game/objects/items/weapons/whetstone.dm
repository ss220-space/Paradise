...
/obj/item/whetstone/attackby(obj/item/I, mob/user, params)
	...
	if(I.force >= max || I.throwforce >= max) //So the whetstone never reduces force or throw_force
		to_chat(user, span_warning("[I.declent_ru_cap(NOMINATIVE)] и так уже предельно остр[GEND_YI_AYA_OE_YE(I)]!"))
		return .
	...
	if(signal_out & COMPONENT_BLOCK_SHARPEN_BLOCKED)
		to_chat(user, span_warning("[I.declent_ru_cap(NOMINATIVE)] нельзя заточить!"))
		return .
	if((signal_out & COMPONENT_BLOCK_SHARPEN_ALREADY) || (!signal_out && I.force > initial(I.force))) //No sharpening stuff twice
		to_chat(user, span_warning("[I.declent_ru_cap(NOMINATIVE)] уже было заточено ранее. Дальнейшая заточка невозможна!"))
		return .
