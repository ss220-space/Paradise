...
/obj/item/twohanded/fireaxe/energized/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	user.visible_message(
		span_danger("[user.declent_ru_cap(NOMINATIVE)] со всей силы вгоня[PLUR_ET_YUT(user)] заряженный топор в [target.declent_ru(ACCUSATIVE)]!"),
...
/obj/item/twohanded/spear/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/organ/external/head))	//Putting heads on spears
		...
		if(!user.drop_transfer_item_to_loc(I, src))
			return ..()
		user.visible_message(
			span_warning("[user.declent_ru_cap(NOMINATIVE)] насажива[PLUR_ET_YUT(user)] [I.declent_ru(ACCUSATIVE)] на копьё перед собой!"),
...
/obj/structure/headspear/attack_hand(mob/living/user)
	user.visible_message(
		span_warning("[user.declent_ru_cap(NOMINATIVE)] сбива[PLUR_ET_YUT(user)] [declent_ru(ACCUSATIVE)] ногой!"),
...
/obj/item/twohanded/mjollnir/proc/shock(mob/living/target)
	...
	target.visible_message(
		span_danger("[target.declent_ru_cap(NOMINATIVE)] поражён[GEND_A_O_Y(target)] разрядом [declent_ru(GENITIVE)]!"),
...
/obj/item/twohanded/knighthammer/afterattack(atom/A, mob/user, proximity, params)
	...
	if(charged == 5)
		...
		if(isliving(A))
			...
			if(Z.health >= 1)
				Z.visible_message(
					span_danger("[Z.declent_ru_cap(NOMINATIVE)] отброшен[GEND_A_O_Y(Z)] сокрушительным ударом [declent_ru(GENITIVE)]!"),
				...
				Z.visible_message(
					span_danger("[Z.declent_ru_cap(NOMINATIVE)] разрыва[PLUR_ET_YUT(Z)]ся на куски силой [declent_ru(GENITIVE)]!"),
...
/obj/item/twohanded/pitchfork/demonic/greater/krampus/afterattack(atom/target, mob/user, proximity, params)
	...
	if(is_airlock(target))
		...
		user.visible_message(span_danger("[user.declent_ru_cap(NOMINATIVE)] разрушает [target.declent_ru(ACCUSATIVE)] с помощью [declent_ru(INSTRUMENTAL)]"))
...
/obj/item/twohanded/pitchfork/suicide_act(mob/user)
	user.visible_message(span_suicide("[user.declent_ru_cap(NOMINATIVE)] пронза[PLUR_ET_YUT(user)] свой живот [declent_ru(INSTRUMENTAL)]! Похоже, [GEND_HE_SHE(user)] пытается покончить с собой..."))
	return BRUTELOSS
...
/obj/item/twohanded/pitchfork/demonic/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)] пылают в ваших руках!"))
