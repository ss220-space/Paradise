...
/obj/item/twohanded/fireaxe/energized/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	user.visible_message(
		span_danger("[capitalize(user.declent_ru(NOMINATIVE))] со всей силы вгоняет заряженный топор в [target.declent_ru(ACCUSATIVE)]!"),
...
/obj/item/twohanded/spear/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/organ/external/head))	//Putting heads on spears
		...
		if(!user.drop_transfer_item_to_loc(I, src))
			return ..()
		user.visible_message(
			span_warning("[capitalize(user.declent_ru(NOMINATIVE))] насаживает [I.declent_ru(ACCUSATIVE)] на копьё перед собой!"),
...
/obj/structure/headspear/attack_hand(mob/living/user)
	user.visible_message(
		span_warning("[capitalize(user.declent_ru(NOMINATIVE))] сбивает [declent_ru(ACCUSATIVE)] ногой!"),
	...
	if(charged == 5)
		...
		if(isliving(A))
			...
			if(Z.health >= 1)
				...
				Z.visible_message(
					span_danger("[capitalize(Z.declent_ru(NOMINATIVE))] разрывается на куски силой [declent_ru(GENITIVE)]!"),
...
/obj/item/twohanded/pitchfork/suicide_act(mob/user)
	user.visible_message(span_suicide("[capitalize(user.declent_ru(NOMINATIVE))] пронзает свой живот [declent_ru(INSTRUMENTAL)]! Похоже, [GEND_HE_SHE(user)] пытается покончить с собой..."))
	return BRUTELOSS
...
/obj/item/twohanded/pitchfork/demonic/pickup(mob/user)
	...
	living_user.visible_message(span_warning("Когда [living_user.declent_ru(NOMINATIVE)] поднимает [declent_ru(ACCUSATIVE)], [GEND_HIS_HER(living_user)] руки на мгновение загораются."), \
