...
/obj/item/rcd/proc/mode_turf(atom/A, mob/user)
	...
	if(isfloorturf(A))
		...
		if(checkResource(3, user))
			...
			playsound(loc, 'sound/machines/click.ogg', 50, 1)
			if(do_after(user, 20 * toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = A))
...
/obj/item/rcd/proc/mode_airlock(atom/A, mob/user)
	if(isfloorturf(A))
		...
		if(checkResource(10, user))
			...
			playsound(loc, 'sound/machines/click.ogg', 50, 1)
			if(do_after(user, 50 * toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = A))
...
/obj/item/rcd/proc/mode_decon(atom/A, mob/user)
	if(iswallturf(A))
		...
		if(checkResource(5, user))
			...
			playsound(loc, 'sound/machines/click.ogg', 50, 1)
			if(do_after(user, 40 * toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = A))
