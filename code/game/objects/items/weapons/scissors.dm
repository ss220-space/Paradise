...
/obj/item/scissors/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(ishuman(M))
		...
		if(do_after(user, 50 * toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = H)) //this is the part that adds a delay. delay is in deciseconds. --Made it 5 seconds, because hair isn't cut in one second in real life, and I want at least a little bit longer time, (TGameCo)
			if(!(M in view(1))) //Adjacency test
...
/obj/item/scissors/safety/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(ishuman(M))
		...
		if(!is_cutting)
			...
			playsound(loc, 'sound/goonstation/misc/scissor.ogg', 100, 1)
			if(do_after(user, 50 * toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = H))
