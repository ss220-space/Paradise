...
/obj/item/lighter/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(istype(src, /obj/item/lighter/zippo))
		cig.light(span_rose("[user] достает [declent_ru(ACCUSATIVE)] и держит [GEND_HIS_HER(src)] у [target.declent_ru(GENITIVE)]. Рука [user] тверда, как немигающее пламя, которым [GEND_HE_SHE(user)] прикуривает [cig.declent_ru(ACCUSATIVE)]."))
	else
...
/obj/item/lighter/zippo/turn_off_lighter(mob/living/user)
	...
	user.balloon_alert(user, "выключено")
	if(world.time > next_off_message)
		user.visible_message(span_rose("Вы слышите тихий щелчок, когда [user] закрывает [src.declent_ru(ACCUSATIVE)], даже не смотря в её сторону. Во даёт!"))
...
/obj/item/match/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(istype(src, /obj/item/match/unathi))
		if(prob(50))
			cig.light(span_rose("[user] извергает пламя на [target.declent_ru(ACCUSATIVE)] и зажигает [cig.declent_ru(ACCUSATIVE)], чуть не опалив [GEND_HIS_HER(target)] лицо!"))
			matchburnout()
		else
			cig.light(span_rose("[user] извергает пламя на [target.declent_ru(ACCUSATIVE)] , опаливая [GEND_HIS_HER(target)] лицо и зажигая [cig.declent_ru(ACCUSATIVE)]."))
			target.apply_damage(5, BURN, def_zone = BODY_ZONE_HEAD)
			playsound(src, 'sound/effects/unathiignite.ogg', 40, FALSE)
	else
		cig.light(span_notice("[user] держит [declent_ru(ACCUSATIVE)] у [target.declent_ru(GENITIVE)], и зажигает [cig.declent_ru(ACCUSATIVE)]."))
		playsound(src, 'sound/items/lighter/light.ogg', 25, TRUE)
