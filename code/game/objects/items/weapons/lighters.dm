...
/obj/item/lighter/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(istype(src, /obj/item/lighter/zippo))
		cig.light(span_rose("[user] доста[PLUR_ET_UT(user)] [declent_ru(ACCUSATIVE)] и держ[pluralize_ru(user.gender, "ит", "ат")] [GEND_HIS_HER(src)] у [target.declent_ru(GENITIVE)]. Рука [user] тверда, как немигающее пламя, которым [GEND_HE_SHE(user)] прикурива[PLUR_ET_UT(user)] [cig.declent_ru(ACCUSATIVE)]."))
	else
...
/obj/item/match/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(istype(src, /obj/item/match/unathi))
		...
		cig.light(span_notice("[user] держ[pluralize_ru(user.gender, "ит", "ат")] [declent_ru(ACCUSATIVE)] у [target.declent_ru(GENITIVE)], и зажига[PLUR_ET_UT(user)] [cig.declent_ru(ACCUSATIVE)]."))
		playsound(src, 'sound/items/lighter/light.ogg', 25, TRUE)
