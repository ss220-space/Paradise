...
/obj/item/melee/energy/cleaving_saw/examine(mob/user)
	. = ..()
	. += span_notice("It is [active ? "open, will cleave enemies in a wide arc and deal additional damage to fauna":"closed, and can be used for rapid consecutive attacks that cause fauna to bleed"].<br>\
