...
/obj/item/restraints/legcuffs/beartrap/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] засовывает свою голову в [declent_ru(NOMINATIVE)]! Похоже, что [GEND_HE_SHE(user)] пытается совершить самоубийство!"))
...
/obj/item/restraints/legcuffs/bola/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	target.visible_message(span_danger("[capitalize(declent_ru(NOMINATIVE))] сковывает ноги [target.declent_ru(GENITIVE)]!"))
	to_chat(target, span_userdanger("[capitalize(declent_ru(NOMINATIVE))] сковывает ваши ноги!"))
