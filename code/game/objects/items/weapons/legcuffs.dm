...
/obj/item/restraints/legcuffs/beartrap/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/grenade/iedcasing))	//Let's get explosive.
		...
		if(IED)
			to_chat(user, span_warning("[IED.declent_ru_cap(NOMINATIVE)] уже прикреплен[GEND_A_O_Y(IED)] к [declent_ru(DATIVE)]!"))
	...
	if(issignaler(I))
		...
		add_fingerprint(user)
		if(sig)
			to_chat(user, span_warning("[signaler.declent_ru_cap(NOMINATIVE)] уже подключен[GEND_A_O_Y(signaler)] к [declent_ru(DATIVE)]!"))
			return ATTACK_CHAIN_PROCEED
		if(signaler.secured)
			to_chat(user, span_notice("[signaler.declent_ru_cap(NOMINATIVE)] не долж[GEND_EN_NA_NO_NY(signaler)] быть закреплен[GEND_A_O_Y(signaler)]."))
...
/obj/item/restraints/legcuffs/bola/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	target.visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сковыва[PLUR_ET_YUT(target)] ноги [target.declent_ru(GENITIVE)]!"))
	to_chat(target, span_userdanger("[declent_ru_cap(NOMINATIVE)] сковыва[PLUR_ET_YUT(target)] ваши ноги!"))
