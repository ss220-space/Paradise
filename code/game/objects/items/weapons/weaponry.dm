...
/obj/item/banhammer/suicide_act(mob/user)
	to_chat(viewers(user), span_suicide("[user] бьёт себя [declent_ru(INSTRUMENTAL)]! Похоже, [GEND_HE_SHE(user)] хочет заблокировать себя!"))
...
/obj/item/sord/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] пытается насадить себя на [declent_ru(ACCUSATIVE)]! Выглядит как попытка суицида, если бы не было так жалко."), span_suicide("Вы пытаетесь насадить себя на [declent_ru(ACCUSATIVE)], но это БЕСПОЛЕЗНО..."))
	return SHAME
...
/obj/item/melee/claymore/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] падает на [declent_ru(ACCUSATIVE)]! Похоже, [GEND_HE_SHE(user)] пытается покончить с собой."))
	return BRUTELOSS
...
/obj/item/melee/baseball_bat/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "the attack", final_block_chance = 0, damage = 0, attack_type = ITEM_ATTACK)
	...
	if(I.w_class <= WEIGHT_CLASS_NORMAL || istype(I, /obj/item/beach_ball)) // baseball bat deflecting
		if(deflectmode)
			if(prob(10))
				visible_message(
					span_boldwarning("[owner] отбивает [I.declent_ru(ACCUSATIVE)] прямо в метателя! Это хоум-ран!"),
					span_boldwarning("Вы отбиваете [I.declent_ru(ACCUSATIVE)] прямо в метателя! Это хоум-ран!")
				)
				...
				visible_message(
					span_warning("[owner] замахивается... и промахивается! Как неловко..."),
					span_warning("Вы замахиваетесь... и промахиваетесь! Вот чёрт!")
				)
				...
				visible_message(
					span_warning("[owner] замахивается и отбивает [I.declent_ru(ACCUSATIVE)]!"),
					span_warning("Вы отбиваете [I.declent_ru(ACCUSATIVE)]!")
				)
