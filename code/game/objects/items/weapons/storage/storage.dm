...
/obj/item/storage/proc/can_be_inserted(obj/item/W, stop_messages = FALSE)
	...
	if(length(can_hold))
		if(!is_type_in_typecache(W, can_hold))
			if(!stop_messages)
				to_chat(usr, span_warning("[declent_ru_cap(NOMINATIVE)] не подход[PLUR_IT_YAT(src)] для [W.declent_ru(GENITIVE)]!"))
			return FALSE
...
/obj/item/storage/verb/toggle_gathering_mode()
	...
	switch(pickup_all_on_tile)
		if(TRUE)
			to_chat(usr, "[declent_ru_cap(NOMINATIVE)] теперь будет собирать все предметы с тайла за раз.")
		if(FALSE)
			to_chat(usr, "[declent_ru_cap(NOMINATIVE)] теперь будет собирать один предмет с тайла за раз")
