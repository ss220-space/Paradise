...
/obj/item/storage/bible/attack(mob/living/carbon/human/target, mob/living/carbon/human/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(target.stat == DEAD)
		target.visible_message(
			span_danger("[user] ударяет  безжизненное тело [target] [declent_ru(INSTRUMENTAL)]."),
	...
	if(prob(60))
		...
		if(user == target)
			target.visible_message(
				span_danger("[user] излечивает себя с силой Бога \"[deity_name]\"!"),
		...
		if(user == target)
			target.visible_message(
				span_danger("[user] огревает себя [declent_ru(INSTRUMENTAL)] по голове!"),
