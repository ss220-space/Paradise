		...
		if(response == "Yes")
			...
			if (user.mind.special_role || livingtime > 9000)
				if(do_after(user, 30, target=src))
					investigate_log("has become a singularity. Caused by [user.key]", INVESTIGATE_ENGINE)
					...
					add_game_logs("[key_name(user)] detonated a bag of holding", user)
					qdel(src)
				else
				...
				user.visible_message("After careful consideration, [user] has decided that putting a Bag of Holding inside another Bag of Holding would not yield the ideal outcome.","You come to the realization that this might not be the greatest idea.")
				investigate_log("could potentially become a singularity (feature disabled for non-special roles). Caused by [user.key]", INVESTIGATE_ENGINE)
				...
				add_game_logs("[key_name(user)] tried to detonate a bag of holding (feature disabled for non-special roles)", user)
	else
		. = ..()
