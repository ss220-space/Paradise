...
/obj/item/storage/secure/attackby(obj/item/W as obj, mob/user as mob, params)
	if(locked)
		...
		if(istype(W, /obj/item/screwdriver))
			if(do_after(user, 20 * W.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
				open = !open
		...
		if((istype(W, /obj/item/multitool)) && (open == 1) && (!l_hacking))
			...
			l_hacking = 1
			if(do_after(usr, 100 * W.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
				if(prob(40))
					l_setshort = 1
					l_set = 0
