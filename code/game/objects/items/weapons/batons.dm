...
/obj/item/melee/baton/proc/add_baton_notes()
	...
	if(affect_cyborg)
		readout += "- При ударе оглушает роботов на [round((stun_time_cyborg/10), 1)] секунд[DECL_U_Y(round((stun_time_cyborg/10), 1))]."
	...
	if(active)
		readout += "- Потребуется примерно [span_warning("[HITS_TO_CRIT(stamina_damage)] удар[DECL_A_OV(HITS_TO_CRIT(stamina_damage))]")], чтобы <b>[span_blue("нелетально")]</b> обезвредить противника."
	return readout.Join("\n")
