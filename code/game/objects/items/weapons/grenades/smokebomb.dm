...
/obj/item/grenade/smokebomb/proc/start_smoke()
	...
	for(var/obj/structure/blob/blob_structure in view(8,src))
		var/damage = round(30 / (get_dist(blob_structure, src) + 1))
		blob_structure.take_damage(damage, BURN, MELEE, 0)
	sleep(80)
	qdel(src)
