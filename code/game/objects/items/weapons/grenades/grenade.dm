...
/obj/item/grenade/proc/arm_grenade(mob/user, delayoverride = det_time, msg = TRUE, volume = 75)
	...
	if(user && msg)
		...
		balloon_alert(user, "взведено на [time] секунд[DECL_U_Y__(delayoverride / 10)]!")
...
/obj/item/grenade/examine(mob/user)
	...
	if(display_timer)
		if(det_time > 1)
			. += span_notice("Таймер установлен на <b>[det_time/10]</b> секунд[DECL_U_Y__(det_time/10)].")
		else
...
/obj/item/grenade/screwdriver_act(mob/living/user, obj/item/I)
	...
	balloon_alert(user, "время детонации — [det_time == 0.1 SECONDS ? "мгновенно" : "[det_time/10] секунд[DECL_U_Y__(det_time/10)]"]")
	add_fingerprint(user)
	return TRUE
