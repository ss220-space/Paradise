...
/obj/item/extinguisher/proc/AttemptRefill(atom/target, mob/user)
	if(istype(target, /obj/structure/reagent_dispensers/watertank) && target.Adjacent(user))
		...
		if(reagents.total_volume == reagents.maximum_volume)
			to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] уже полностью заправлен!"))
			safety = safety_save
			return TRUE
		var/obj/structure/reagent_dispensers/watertank/watertank = target
		var/transferred = watertank.reagents.trans_to(src, max_water)
		if(transferred > 0)
			to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] был заправлен на [transferred] единиц[DECL_SEC_MIN(transferred)]."))
			...
			to_chat(user, span_notice("[watertank.declent_ru_cap(NOMINATIVE)] пуст!"))
		safety = safety_save
		return TRUE
	return FALSE
...
/obj/item/extinguisher/afterattack(atom/target, mob/user, flag, params)
	...
	if(reagents.total_volume < 1)
		to_chat(user, span_danger("[declent_ru_cap(NOMINATIVE)] пуст."))
		return
