...
/obj/item/mixing_bowl
	name = "mixing bowl"
	desc = "Для смешивания ингредиентов на кухне."
	ru_names = list(
...
/obj/item/mixing_bowl/attackby(obj/item/stack/I, mob/user, params)
	if(istype(I, /obj/item/soap))
		...
		user.visible_message(
			span_notice("[user] начина[pluralize_ru(user.gender,"ет","ют")] мыть [declent_ru(ACCUSATIVE)]."),
		...
		clean()
		user.visible_message(
			span_notice("[user] вымыл[genderize_ru(user.gender,"","а","о","и")]] [declent_ru(ACCUSATIVE)]."),
	...
	if(is_type_in_list(I, containers))
		...
		user.visible_message(
			span_notice("[user] перелива[pluralize_ru(user.gender,"ет","ют")] немного раствора из [cached_name] в [declent_ru(ACCUSATIVE)]."),
