...
/obj/item/stack/medical/attack(mob/living/target, mob/living/user, params, def_zone, skip_attack_anim = FALSE)
	...
	if(ishuman(target))
		...
		if(human_target == user && !unique_handling)
			user.visible_message(
				span_notice("[human_target] начинает применять [declension_ru(NOMINATIVE)] на себе."),
	...
	if(isanimal(target))
		...
		user.visible_message(
			span_green("[user] применяет [declent_ru(NOMINATIVE)] на [critter.declent_ru(NOMINATIVE)]."),
	...
	user.visible_message(
		span_green("[user] применяет [declent_ru(NOMINATIVE)] к [target]."),
...
/obj/item/stack/medical/proc/human_heal(mob/living/carbon/human/target, mob/user)
	...
	user.visible_message(
		span_green("[user] использует [declent_ru(NOMINATIVE)] на [affecting.declent_ru(ACCUSATIVE)] [target]."),
	...
	while(rembrute + remburn > 0) // Don't bother if there's not enough leftover heal
		...
		remburn = nremburn
		user.visible_message(
			span_green("[user] обрабатывает раны на [organ.declent_ru(ACCUSATIVE)] [target] остатками медикаментов."),
