...
/obj/item/stack/sheet/animalhide/attackby(obj/item/W as obj, mob/user as mob, params)
	if(W.sharp)
		...
		if(do_after(user, 50 * W.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src))
