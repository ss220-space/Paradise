...
/obj/item/toy/katana/suicide_act(mob/user)
	var/dmsg = pick("[user] пытается воткнуть [declent_ru(ACCUSATIVE)] себе в живот, но он ломается! Выглядит так, будто [GEND_HE_SHE(user)] умрёт от стыда.",
					"[user] пытается воткнуть [declent_ru(ACCUSATIVE)] себе в живот, но он гнётся и ломается пополам! Выглядит так, будто [GEND_HE_SHE(user)] умрёт от стыда.",
					"[user] пытается перерезать себе горло, но тупое пластиковое лезвие приводит к тому, что [GEND_HE_SHE(user)] поскальзывается и ломает шею с громким хрустом!")
	user.visible_message(
		span_suicide("[dmsg] Похоже, [GEND_HE_SHE(user)] пытается покончить с собой.")
	)
	return BRUTELOSS
...
/obj/item/toy/nuke/attack_self(mob/user)
	if(cooldown < world.time)
		...
		user.visible_message(span_warning("[user] нажимает кнопку на [declent_ru(GENITIVE)]"), span_notice("Вы активируете [declent_ru(NOMINATIVE)], раздаётся громкий звук!"), span_notice("Слышишь щелчок кнопки."))
...
/obj/item/toy/plushie/attack_self(mob/user as mob)
	var/cuddle_verb = pick("обнимает", "тискает", "прижимает")
	...
	if(spam_flag == 0)
		...
		if(message_spam_flag == 0)
			...
			user.visible_message(
				...
				span_notice("Вы [msg] [declent_ru(ACCUSATIVE)]!")
			)
			spawn(30)
				message_spam_flag = 0
		spawn(3)
...
/obj/item/toy/redbutton/attack_self(mob/user)
	if(cooldown < world.time)
		...
		user.visible_message(span_warning("[user] нажимает большую красную кнопку."), span_notice("Вы нажимаете кнопку, раздаётся громкий звук!"), span_notice("Кнопка громко щёлкает."))
...
/obj/item/toy/codex_gigas/attack_self(mob/user)
	...
	user.visible_message(
		span_notice("[user] нажимает кнопку на [declent_ru(PREPOSITIONAL)]."),
...
/obj/item/toy/toy_xeno/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		user.visible_message(span_notice("[user] дергаетет верёвку на [declent_ru(PREPOSITIONAL)]."))
		INVOKE_ASYNC(src, PROC_REF(async_animation))
	else
...
/obj/item/toy/russian_revolver/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] быстро заряжает шесть патронов в барабан [declent_ru(GENITIVE)], приставляет к виску и нажимает на курок! Похоже, [GEND_HE_SHE(user)] пытается покончить с собой."))
...
/obj/item/toy/eight_ball/attack_self(mob/user as mob)
	if(!cooldown)
		...
		user.visible_message(span_notice("[user] сосредотачивается на своём вопросе и [use_action]..."))
