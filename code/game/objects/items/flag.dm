...
/obj/item/flag/chameleon/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/grenade) && !boobytrap)
		if(user.drop_item())
			...
			add_game_logs("[key_name_log(user)] has hidden [I] in the [src] ready for detonation at [AREACOORD(bombturf)].", user)
		...
		var/turf/bombturf = get_turf(src)
		add_game_logs("[key_name_log(user)] has lit the [src] trapped with [boobytrap] by [key_name_log(trapper)] at [AREACOORD(bombturf)].", user)
