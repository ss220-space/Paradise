...
/obj/item/pizza_bomb/attackby(var/obj/item/I, var/mob/user, params)
	...
	if(istype(I, /obj/item/wirecutters) && disarmed)
		...
		if(do_after(user, 40 * I.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src))
			playsound(src, I.usesound, 50, 1, 1)
			user.unEquip(src)
