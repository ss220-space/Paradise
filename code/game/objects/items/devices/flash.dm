...
/obj/item/flash/proc/burn_out()
	broken = TRUE
	update_icon(UPDATE_ICON_STATE)
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] выгорает!"))
...
/obj/item/flash/attack_self(mob/living/carbon/user, flag = 0, emp = FALSE)
	...
	user.visible_message(
		span_disarm("[declent_ru_cap(NOMINATIVE)] в руках [user.declent_ru(GENITIVE)] озаряется яркой вспышкой!"),
		ignored_mobs = user,
	)
