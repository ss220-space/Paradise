...
/obj/item/dna_notepad/proc/create_report_paper()
	...
	if(in_range(usr, src))
		...
		usr.visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] дребезжит, после чего из окна печати выпадает лист бумаги."))
	printing = FALSE
...
/obj/item/dna_notepad/proc/load_unknown_disabilities_from_console(obj/machinery/computer/scan_consolenew/dna_console, mob/living/user)
	...
	if(!connected)
		to_chat(user, span_warning("[dna_console.declent_ru_cap(NOMINATIVE)] не подключен."))
		balloon_alert(user, "ошибка загрузки")
		return
	if(!connected.occupant)
		to_chat(user, span_warning("[connected.declent_ru_cap(NOMINATIVE)] пуст."))
		balloon_alert(user, "ошибка загрузки")
		return
