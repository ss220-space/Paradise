...
/obj/item/radio/proc/receive_range(freq, level)
	...
	if(!freq) //recieved on main frequency
		if(!listening)
			return -1
	else if((syndiekey && !(freq in SSradio.syndicate_blacklist) && !istype(syndiekey, /obj/item/encryptionkey/syndicate/taipan)) || (istype(syndiekey, /obj/item/encryptionkey/syndicate/taipan) && !(freq in SSradio.taipan_blacklist)))
