...
/obj/item/radio/intercom/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/stack/tape_roll)) //eww
		...
		if(coil.get_amount() >= 5)
			...
			if(do_after(user, 10 * coil.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src) && buildstage == 1 && coil.use(5))
		...
		if(do_after(user, 10 * W.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src) && buildstage == 0)
			qdel(W)
