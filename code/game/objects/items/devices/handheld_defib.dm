...
/obj/item/handhelddefibrillator/syndie
	name = "handheld defibrillator"
	...
	var/icon_base = "sdefib"
	var/cooldown = FALSE
	var/charge_time = 50
	var/emagged = FALSE

/obj/item/handhelddefibrillator/syndie/emag_act(mob/user)
	if(!emagged)
		emagged = TRUE
		desc += " The screen only shows the words FUCK NANOTRASEN flashing over and over."
		if(user)
			to_chat(user, "<span class='warning'>you short out the safeties on [src]</span>")
	else
		emagged = FALSE
...
/obj/item/handhelddefibrillator/syndie/emp_act(severity)
	if(emagged)
		emagged = FALSE
		desc = "Used to restart stopped hearts(Not NanoTrasen's pigs hearts)."
		visible_message("<span class='notice'>[src] beeps: Safety protocols enabled!</span>")
...
/obj/item/handhelddefibrillator/syndie/attack(mob/living/carbon/human/H, mob/user)
	if(!istype(H))
		return ..()
...
/obj/item/handhelddefibrillator/syndie/proc/short_charge()
	icon_state = "[icon_base]-off"

/obj/item/handhelddefibrillator/syndie/proc/recharge()
	cooldown = FALSE
	icon_state = "[icon_base]-on"
	playsound(loc, "sound/weapons/flash.ogg", 75, 1)
