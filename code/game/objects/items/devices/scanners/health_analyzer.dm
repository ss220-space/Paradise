...
/obj/item/healthanalyzer/attack_self(mob/user)
	if(!scan_data)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] не содержит сохранённых данных."))
		return
	show_results(user)
...
/obj/item/healthanalyzer/proc/print_report(mob/living/user)
	...
	if(in_range(user, src))
		...
		user.visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] дребезжит, после чего из окна печати выпадает лист бумаги."))
