...
/obj/item/uplink/hidden/proc/check_trigger(mob/user, value, target)
	if(is_jammed)
		to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)], похоже, заблокирован — нельзя использовать!"))
		return FALSE
	if(value == target)
		trigger(user)
		return TRUE
	...
	switch(action)
		...
		if("purchase_cart")
			...
			if(calculate_cart_tc() > uses)
				to_chat(ui.user, span_warning("[declent_ru_cap(NOMINATIVE)] вибрирует, в нём недостаточно телекристаллов!"))
				return
			if(is_jammed)
				to_chat(ui.user, span_warning("[declent_ru_cap(NOMINATIVE)], похоже, заблокирован — нельзя использовать!"))
				return
