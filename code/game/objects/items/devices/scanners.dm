...
/obj/item/dna_notepad/ui_state(mob/user)
...
/obj/item/dna_notepad/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("editDnaBlock")
			...
			if(bufferId < 1 || bufferId > 54)
				to_chat(usr, "Ошибка выбора блока [bufferId]")
				return
			to_chat(usr, "Вы редактируете блок [bufferId]")
			var/list/choices = all_dna_names()
			ui_modal_choice(src, "editDnaBlockName", "Выберите эффект блока:", null, list("id" = bufferId), null, choices)
			//ui_modal_input(src, "editDnaBlockName", "Введите название для блока:", arguments = list("id" = bufferId))
		...
		if(UI_MODAL_ANSWER)
			...
			switch(id)
				if("editDnaBlockName")
					...
					if(buffer_id < 1 || buffer_id > 54)
						to_chat(usr, "invalid buffer id [buffer_id]")
						return
					var/gene = find_gene_by_name(answer)
					if (!gene)
						write_dna_data(buffer_id, answer, "gray")
						to_chat(usr, "you edit [buffer_id] to '[answer]' default color")
					else
						var/color = "gray"
						...
						if(istype(gene, /datum/dna/gene/basic))
							color = "green"
						write_dna_data(buffer_id, answer, color)
						to_chat(usr, "you edit [buffer_id] to '[answer]' color=[color]")
				else
					return FALSE
		else
			return FALSE
...
/obj/item/dna_notepad/proc/write_dna_data(block, name, marker)
	var/list/current_dna_detail_data = null
	for(var/list/dna_detail_data in dna_data)
		if (dna_detail_data["block"] == "[block]")
	...
	current_dna_detail_data["marker"] = "[marker]"
...
/obj/item/dna_notepad/proc/print_report(var/mob/living/user)
	...
	for(var/list/block in dna_data)
		paper.header += "[block["block"]]: [block["name"]]<br>"
	paper.header += "<hr>"
	if(in_range(user, src))
