	...
	if(on && user.zone_selected == BODY_ZONE_PRECISE_EYES)
		...
		if(M == user)	//they're using it on themselves
			...
			if(istype(H)) //robots and aliens are unaffected
				...
				else if(HAS_TRAIT(M, TRAIT_XRAY_VISION) || H.nightvision >= 8) //The mob's either got the X-RAY vision or has a tapetum lucidum (extreme nightvision, i.e. Vulp/Tajara with COLOURBLIND & their monkey forms).
