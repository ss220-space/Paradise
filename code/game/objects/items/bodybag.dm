...
/obj/item/bodybag/suicide_act(mob/living/user)
	if(isfloorturf(user.loc))
		user.visible_message(span_suicide("[user] заползает в [declent_ru(ACCUSATIVE)]! Похоже, что [GEND_HE_SHE(user)] пытается совершить самоубийство!"))
...
/obj/item/bodybag/bluespace/container_resist(mob/living/user)
	...
	if(user.incapacitated())
		to_chat(loc, span_warning("Давление ослабевает. Похоже, [GEND_HE_SHE(user)] переста[genderize_ru(user.gender, "л", "ла", "ло", "ли")] сопротивляться..."))
		return
