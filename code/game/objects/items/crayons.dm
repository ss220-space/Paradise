...
/obj/item/toy/crayon/afterattack(atom/target, mob/user, proximity)
	...
	if(is_type_in_list(target,validSurfaces))
		...
		busy = TRUE
		if(instant || do_after(user, 50 * toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = target))
