...
/obj/item/nuke_core/supermatter_sliver/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(victim.incorporeal_move || HAS_TRAIT(victim, TRAIT_GODMODE) || isnucleation(victim)) //try to keep this in sync with supermatter's consume fail conditions
		return ..()
...
/obj/item/nuke_core_container/supermatter/attack_hand(mob/user)
	if(cracked && sliver) //What did we say about touching the shard...
		if(!isliving(user) || HAS_TRAIT(user, TRAIT_GODMODE) || isnucleation(user))
			return FALSE
		user.visible_message(
