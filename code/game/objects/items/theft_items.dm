...
/obj/item/nuke_core_container/examine(mob/user)
	...
	if(cracked) // Cracked open.
		...
		. += span_notice("[declent_ru_cap(NOMINATIVE)] выглядит помятым. Возможно, более мощный взрыв сможет его разрушить.")
...
/obj/item/nuke_core_container/proc/seal()
	if(!QDELETED(core))
		...
		if(ismob(loc))
			to_chat(loc, span_warning("[declent_ru_cap(NOMINATIVE)] наглухо запечатан, радиация от [core.declent_ru(GENITIVE)] теперь изолирована."))
...
/obj/item/nuke_core_container/proc/unseal(mob/user)
	...
	sealed = FALSE
	playsound(src, 'sound/items/deconstruct.ogg', 60, TRUE)
	to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)] распечатан, радиация от [core.declent_ru(GENITIVE)] больше не изолирована."))
	unload(user)
...
/obj/item/nuke_core_container/proc/crack_open()
	visible_message(span_boldnotice("[declent_ru_cap(NOMINATIVE)] распахивается!"))
	if(core)
		START_PROCESSING(SSobj, core)
		REMOVE_TRAIT(core, TRAIT_BLOCK_RADIATION, UNIQUE_TRAIT_SOURCE(src))
	cracked = TRUE
...
/obj/item/nuke_core/supermatter_sliver/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/retractor/supermatter))
		...
		if(tongs.sliver)
			to_chat(user, span_warning("[tongs.declent_ru_cap(NOMINATIVE)] уже удерживают осколок суперматерии!"))
			return .
		if(ismob(loc))
			var/mob/holder = loc
...
/obj/item/nuke_core/supermatter_sliver/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	victim.visible_message(
		span_danger("[victim.declent_ru_cap(NOMINATIVE)], поражённый [declent_ru(INSTRUMENTAL)], вспыхивает пламенем, в комнате воцаряется тишина..."),
...
/obj/item/nuke_core/supermatter_sliver/pickup(mob/living/user)
	...
	user.visible_message(
		span_danger("[user.declent_ru_cap(NOMINATIVE)] тянется к [declent_ru(DATIVE)]. [GEND_HIS_HER(user)] тело начинает светиться и мгновенно вспыхивает!"),
...
/obj/item/nuke_core_container/supermatter/seal()
	if(!QDELETED(sliver))
		...
		update_icon(UPDATE_ICON_STATE)
		if(ismob(loc))
			to_chat(loc, span_warning("[declent_ru_cap(NOMINATIVE)] наглухо запечатан, [sliver.declent_ru(NOMINATIVE)] теперь в безопасности."))
...
/obj/item/nuke_core_container/supermatter/attack_hand(mob/user)
	if(cracked && sliver) //What did we say about touching the shard...
		if(!isliving(user) || HAS_TRAIT(user, TRAIT_GODMODE) || HAS_TRAIT(user, TRAIT_SUPERMATTER_IMMUNE))
			return FALSE
		user.visible_message(
			span_danger("[user.declent_ru_cap(NOMINATIVE)] тянется к [sliver.declent_ru(DATIVE)]. [GEND_HIS_HER_CAP(user)] тело начинает светиться и вспыхивает!"),
...
/obj/item/retractor/supermatter/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum) // no instakill supermatter javelins
	if(sliver)
		...
		visible_message(span_notice("[sliver.declent_ru_cap(NOMINATIVE)] выпадает из [declent_ru(GENITIVE)] при ударе о землю."))
		sliver = null
		update_icon(UPDATE_ICON_STATE)
	return ..()
