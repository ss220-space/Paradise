...
/obj/item/ecig/proc/inject_nicotine(mob/living/carbon/user, cycle_count)
	...
	if(emagged && cycle_count >= SAFE_THRESHOLD)
		applying = FALSE
		to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)] становится обжигающе горячей!"))
...
/obj/item/ecig/proc/perform_sparks()
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] начинает пищать и искрить!"))
...
/obj/item/krampus_bag/proc/consume(mob/living/victim, mob/user)
	...
	victim.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] открывается нараспашку и захватывает [victim.declent_ru(ACCUSATIVE)]!"), span_his_grace("[span_big("[declent_ru(NOMINATIVE)] захватывает вас!")]"))
	victim.death()
