...
/obj/structure/proc/do_climb(mob/living/user)
	...
	user.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] начинает забираться на [declent_ru(ACCUSATIVE)]!"))
	climber = user
	...
	if(get_turf(user) == get_turf(src))
		user.visible_message(span_warning("[capitalize(user.declent_ru(NOMINATIVE))] забирается на [declent_ru(ACCUSATIVE)]!"))
