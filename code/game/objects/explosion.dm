			...
			for(var/MN in GLOB.player_list)
				...
				if(M_turf && M_turf.z == z0)
					...
					if(dist <= round(max_range + world.view - 2, 1))
						M.playsound_local(epicenter, null, 100, 1, frequency, sound = explosion_sound)
					...
					// You hear a far explosion if you're outside the blast radius. Small bombs shouldn't be heard all over the station.
						...
						if(creaking_explosion)
							M.playsound_local(epicenter, null, far_volume, 1, frequency, sound = creaking_explosion_sound, distance_multiplier = 0)
						else if(prob(PROB_SOUND)) // Sound variety during meteor storm/tesloose/other bad event
							M.playsound_local(epicenter, null, far_volume, 1, frequency, sound = far_explosion_sound, distance_multiplier = 0) // Far sound
						else
							M.playsound_local(epicenter, null, far_volume, 1, frequency, sound = explosion_echo_sound, distance_multiplier = 0) // Echo sound
						...
						if(devastation_range)
							...
							shake_camera(M, 10, clamp(baseshakeamount * 0.25, 0, SHAKE_CLAMP))
							echo_volume = 60
						M.playsound_local(epicenter, null, echo_volume, 1, frequency, sound = explosion_echo_sound, distance_multiplier = 0)
