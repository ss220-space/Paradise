...
/obj/effect/portal/New(loc, turf/target, creator = null, lifespan = 300)
	...
	for(var/obj/effect/portal/P in loc)
		if(P != src)
			QDEL_IN(P, 0)
			//stack_trace("More than one portal has been opened in [atom_loc_line(loc)]") 	//WHY WE NEED TO STACK TRACE IF ALL PORTALS ARE ALREADY REMOVED AND SITUATION IS RESOLVED?!
...
/obj/effect/portal/proc/teleport(atom/movable/M)
	...
	if(ismegafauna(M))
		message_admins("[ADMIN_LOOKUP(M)] has used a portal at [ADMIN_VERBOSEJMP(src)] made by [key_name_admin(usr)].")
