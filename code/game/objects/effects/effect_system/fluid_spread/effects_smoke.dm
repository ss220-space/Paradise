...
/// A factory which produces red smoke that makes you cough.
/datum/effect_system/fluid_spread/smoke/bad/red
	effect_type = /obj/effect/particle_effect/fluid/smoke/bad/red
...
/// A factory which produces blue smoke that makes you cough.
...
/datum/effect_system/fluid_spread/smoke/freezing
	...
	/// The radius in which to chill every simulated turf.
	var/blast = 0

/datum/effect_system/fluid_spread/smoke/freezing/proc/Chilled(atom/A)
...
/datum/effect_system/fluid_spread/smoke/chem/New()
	...
	chemholder.reagents.set_reacting(FALSE)
...
/datum/effect_system/fluid_spread/smoke/chem/set_up(range = 1, amount = DIAMOND_AREA(range), atom/holder, atom/location = null, datum/reagents/carry = null, silent = FALSE)
	...
	for(var/datum/reagent/reagent as anything in carry.reagent_list)
		contained_reagents += "[reagent.volume]u [reagent]"

	var/where = "[AREACOORD(location)]"
	var/contained = length(contained_reagents) ? "[contained_reagents.Join(", ", " \[", "\]")]" : null
...
/datum/effect_system/fluid_spread/smoke/chem/start(log = FALSE)
	...
	chemholder.reagents.copy_to(smoke, chemholder.reagents.total_volume)
