...
/obj/effect/particle_effect/fluid/foam/Initialize(mapload)
	. = ..()
	if(slippery_foam)
		AddComponent(/datum/component/slippery, "foam", 4 SECONDS)
...
/datum/effect_system/fluid_spread/foam/start(log = FALSE)
	...
	chemholder.reagents.copy_to(foam, chemholder.reagents.total_volume, reagent_scale) // Foam has an amplifying effect on the reagents it is supplied with. This is balanced by the reagents being diluted as the area the foam covers increases.
...
/obj/effect/particle_effect/fluid/foam/firefighting
	...
	result_type = /obj/effect/decal/cleanable/flour/plasma
...
/obj/effect/particle_effect/fluid/foam/firefighting/process()
	...
	air = air.toxins - scrub_amt
	absorbed_plasma += scrub_amt

	if (air.return_temperature() > T20C)
		air.temperature = max(air.return_temperature() / 2, T20C)
...
/obj/effect/particle_effect/fluid/foam/firefighting/make_result()
	...
	if(istype(deposit) && deposit.reagents && absorbed_plasma > 0)
		deposit.reagents.add_reagent("plasma_dust", absorbed_plasma)
		absorbed_plasma = 0
	return deposit
...
/obj/structure/foamedmetal/attack_alien(mob/user, list/modifiers)
