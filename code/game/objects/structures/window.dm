	...
	switch(grabber.grab_state)
		if(GRAB_AGGRESSIVE)
			victim.visible_message(
				span_warning("[grabber] стукает [victim] в [declent_ru(ACCUSATIVE)]!"),
				span_warning("[grabber] стукает вас в [declent_ru(ACCUSATIVE)]!")
			)
			if(prob(25))
		...
		if(GRAB_NECK)
			victim.visible_message(
				...
				span_warning("[grabber] бьет вас о [declent_ru(ACCUSATIVE)]!")
			)
		...
		if(GRAB_KILL)
			victim.visible_message(
				span_warning("[grabber] с размаху бьёт [victim] к [declent_ru(DATIVE)]!"),
				span_warning("[grabber] с размаху бьёт вас к [declent_ru(DATIVE)]!")
			)
