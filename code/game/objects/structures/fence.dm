...
/obj/structure/fence/wirecutter_act(mob/living/user, obj/item/W)
	...
	if(W.use_tool(src, user, CUT_TIME * W.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), volume = W.tool_volume))
...
/obj/structure/fence/attackby(obj/item/C, mob/user)
	...
	if(istype(C, /obj/item/stack/rods))
		...
		if(do_after(user, 3 SECONDS * C.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src) && hole_size != NO_HOLE && R.use(HOLE_REPAIR))
