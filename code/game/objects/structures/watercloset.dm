...
/obj/structure/toilet/attackby(obj/item/item, mob/user, params)
	...
	if(istype(item, /obj/item/reagent_containers))
		...
		if(!container.is_refillable())
			to_chat(user, span_warning("[container.declent_ru_cap(NOMINATIVE)] не предназначен для повторного заполнения."))
			balloon_alert(user, "не удалось")
			return ATTACK_CHAIN_PROCEED
		if(container.reagents.holder_full())
			to_chat(user, span_warning("[container.declent_ru_cap(NOMINATIVE)] уже полный..."))
