...
/obj/structure/closet/cardboard/attackby(obj/item/item, mob/user, params)
	if(issoap(item))
		...
		user.visible_message(
			"[user.declent_ru(NOMINATIVE)] начинает стирать рисунки с [declent_ru(GENITIVE)].",
			ignored_mobs = user
		)
	...
	user.visible_message(
		span_notice("[user.declent_ru(NOMINATIVE)] начинает красить [declent_ru(ACCUSATIVE)]."),
		ignored_mobs = user
	)
	if(!do_after(user, 3 SECONDS, src))
		return ATTACK_CHAIN_BLOCKED_ALL
...
/obj/structure/closet/cardboard/agent/nullspace/on_move(turf/old_loc, turf/new_loc, direction)
	...
	old_loc.visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] [direction == UP ? "улетает ввысь, сквозь потолок" : "проваливается сквозь пол"]!"))
