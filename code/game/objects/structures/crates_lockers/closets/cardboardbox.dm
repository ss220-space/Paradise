...
/obj/structure/closet/cardboard/agent/nullspace/on_move(turf/old_loc, turf/new_loc, direction)
	...
	old_loc.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] [direction == UP ? "улета[PLUR_ET_YUT(src)] ввысь, сквозь потолок" : "провалива[PLUR_ET_YUT(src)]ся сквозь пол"]!"))
	do_sparks(rand(2, 5), TRUE, old_loc)
	playsound(new_loc, 'sound/magic/blink.ogg', 50)
	new_loc.visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] [direction == UP ? "появля[PLUR_ET_YUT(src)]ся из под пола" : "пада[PLUR_ET_YUT(src)] сквозь потолок"]!"))
