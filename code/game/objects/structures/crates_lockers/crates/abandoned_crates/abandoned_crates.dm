...
/obj/structure/closet/crate/secure/loot/attack_hand(mob/user, list/modifiers)
	...
	var/input = tgui_input_text(user, title = "Дека-кодовый замок", message = "Введите [code_length] цифр[DECL_U_Y_0(code_length)]. Все цифры должны быть уникальными.", max_length = code_length)
