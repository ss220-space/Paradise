...
/obj/structure/closet/crate/secure/screwdriver_act(mob/living/user, obj/item/tool)
	...
	user.emote("scream")
	to_chat(user, span_warning("[tool.declent_ru(NOMINATIVE)] сорвал[GEND_ENDING_SYA_AS_OS_IS(tool)]ась и повредил[GEND_ENDING_A_O_I(tool)] [affecting.declent_ru(ACCUSATIVE)]!"))
