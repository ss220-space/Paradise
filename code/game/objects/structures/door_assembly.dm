...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/pen))
		...
		if(do_after(user, 40 * coil.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
				...
				if(!noglass)
					if(!glass)
						if(istype(S, /obj/item/stack/sheet/rglass) || istype(S, /obj/item/stack/sheet/glass))
							...
							if(do_after(user, 40 * S.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
