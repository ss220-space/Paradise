...
/obj/structure/displaycase/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stack/sheet/glass) && broken)
		...
		if(!do_after(user, 2 SECONDS * glass.toolspeed * building_mod, src, category = DA_CAT_TOOL) || !broken || QDELETED(glass))
			return ATTACK_CHAIN_PROCEED
		if(!glass.use(2))
...
/obj/structure/displaycase_chassis/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/access_control))
		...
		if(!do_after(user, 3 SECONDS * control.toolspeed * building_mod, src, category = DA_CAT_TOOL) || electronics || control.emagged)
			return ATTACK_CHAIN_PROCEED
	...
	if(istype(I, /obj/item/stack/sheet/glass))
		...
		if(!do_after(user, 2 SECONDS * glass.toolspeed * building_mod, src, category = DA_CAT_TOOL) || QDELETED(glass))
			return ATTACK_CHAIN_PROCEED
		if(!glass.use(10))
