	...
	if(climber)
		...
		if(user.pulling.loc == loc)
			user.visible_message(
				span_notice("[user.declent_ru_cap(NOMINATIVE)] кладёт [user.pulling.declent_ru(ACCUSATIVE)] на [declent_ru(ACCUSATIVE)]."),
...
/obj/structure/table/proc/tablepush(mob/living/victim, mob/user)
	if(HAS_TRAIT(user, TRAIT_PACIFISM) || GLOB.pacifism_after_gt)
		...
		return FALSE
	if(victim.buckled)
		to_chat(user, span_warning("[victim.declent_ru_cap(NOMINATIVE)] уже пристегнут к [victim.buckled.declent_ru(DATIVE)]!"))
		return FALSE
	...
	victim.visible_message(
		span_danger("[user.declent_ru_cap(NOMINATIVE)] толка[PLUR_ET_YUT(user)] [victim.declent_ru(GENITIVE)] на [declent_ru(ACCUSATIVE)]."),
...
/obj/structure/table/proc/actual_flip(mob/living/user)
	...
	if(!flipped)
		...
		user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] переворачивает [declent_ru(ACCUSATIVE)]!"))
...
/obj/structure/table/glass/proc/table_shatter(mob/living/L)
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] разбивается!"), span_danger("Вы слышите, как бьется стекло"))
	var/turf/T = get_turf(src)
...
/obj/structure/rack/attack_hand(mob/living/user)
	...
	user.visible_message(
		span_warning("[user.declent_ru_cap(NOMINATIVE)] пинает [declent_ru(ACCUSATIVE)]."),
...
/obj/item/gunrack_parts/attack_self(mob/user)
	...
	if(do_after(user, 2 SECONDS, user))
		...
		user.visible_message(
			span_notice("[user.declent_ru_cap(NOMINATIVE)] собирает [GR.declent_ru(ACCUSATIVE)]."),
...
/obj/item/rack_parts/attack_self(mob/user)
	...
	if(do_after(user, 2 SECONDS, user))
		...
		user.visible_message(
			span_notice("[user.declent_ru_cap(NOMINATIVE)] собирает [R.declent_ru(ACCUSATIVE)]."),
