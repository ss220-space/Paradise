...
/obj/structure/morgue/container_resist(mob/living/carbon/user)
	if(!iscarbon(user) || user.incapacitated() || user.restrained())
		return
...
/obj/structure/morgue/proc/ex_act_effect(severity, probability = 100)
	...
	for(var/atom/movable/check in src)
		check.forceMove(loc)
		check.ex_act(severity, severity)
	qdel(src)
...
/obj/structure/crematorium	// will be deleted soon
	name = "DELETE ME PLZ"
	desc = "Tell coders to replace me!"
	icon = 'icons/obj/stationobjs.dmi'
...
/obj/machinery/crematorium/relaymove(mob/user)
	if(user.incapacitated() || user.restrained() || cremating)
		return
	tray_toggle(user)


/obj/machinery/crematorium/container_resist(mob/living/carbon/user)
	if(!iscarbon(user) || user.incapacitated() || user.restrained() || cremating)
		return
	to_chat(user, span_alert("You attempt to slide yourself out of [src]..."))
	tray_toggle(user)
