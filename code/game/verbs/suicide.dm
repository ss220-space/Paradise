...
/mob/living/simple_animal/mouse/do_suicide()
	visible_message(span_danger("[src] бешено мечется! Уровень сыра упал до критической отметки, и [GEND_HE_SHE(src)] покинул[genderize_ru(gender, "", "а", "о", "и")] наш мир."))
...
/mob/living/carbon/brain/do_suicide()
	to_chat(viewers(loc), span_danger("Мозг [src] становится тусклым и безжизненным. Похоже, [GEND_HE_SHE(src)] потерял[genderize_ru(gender, "", "а", "о", "и")] волю к жизни."))
	spawn(5 SECONDS)
		death(gibbed = FALSE)
		suiciding = FALSE
...
/mob/living/carbon/alien/humanoid/do_suicide()
	to_chat(viewers(src), span_danger("[src] бьётся в конвульсиях! Это похоже на попытку суицида."))
	//put em at -175
