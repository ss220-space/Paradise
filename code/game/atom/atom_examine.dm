			...
			if(length(reagents.reagent_list))
				if(user.can_see_reagents()) //Show each individual reagent
					for(var/current in reagents.reagent_list)
						...
						. += span_notice("<b>[reagent.name]</b> - <b>[reagent.volume]</b> единиц[DECL_A_Y(reagent.volume)].")
				else //Otherwise, just show the total volume
					if(reagents && length(reagents.reagent_list))
						. += span_notice("<b>[reagents.total_volume]</b> единиц[DECL_A_Y(reagents.total_volume)] вещества.")
			else
				. += span_notice("Ничего.")
		else if(container_type & AMOUNT_VISIBLE)
			if(reagents.total_volume)
				. += span_notice("Осталось ещё <b>[reagents.total_volume]</b> единиц[DECL_A_Y(reagents.total_volume)] вещества.")
			else
