...
/datum/ambition_objective/proc/get_job_departament_ambition()
	...
	switch(owner.assigned_role)
		if(JOB_TITLE_JUDGE, JOB_TITLE_LAWYER)
			if(owner.assigned_role == JOB_TITLE_JUDGE && (prob(chance_other_departament_ambition))) //шанс что магистрат возьмёт общую амбицию глав.
	...
	if(owner.assigned_role in GLOB.command_positions)
		...
		if (prob(chance_other_departament_ambition))
			switch(owner.assigned_role)
				...
				if(JOB_TITLE_SCIENTIST_STUDENT, JOB_TITLE_INTERN, JOB_TITLE_ENGINEER_TRAINEE)
	...
	if(owner.assigned_role in (GLOB.security_positions - GLOB.support_positions))
		if(owner.assigned_role == JOB_TITLE_BRIGDOC && (prob(chance_other_departament_ambition)))	//шанс что бригмедик возьмёт амбицию мед. отдела.
			job = pick(GLOB.medical_positions)
			return pick_list_weight("ambition_objectives_medical.json", job)
