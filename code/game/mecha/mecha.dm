...
/obj/mecha/proc/operation_allowed(mob/living/carbon/human/H)
	if(!ishuman(H))
		return 0
	if(check_access(H.get_access(), operation_req_access))
		return 1
	return 0

/obj/mecha/proc/internals_access_allowed(mob/living/carbon/human/H)
	if(check_access(H.get_access(), internals_req_access))
		return 1
	return 0

/obj/mecha/check_access(list/user_access, list/access_list)
	if(!istype(access_list))
		return 1
	if(!length(access_list)) //no requirements
		return 1
	if(!length(user_access)) //not ID or no access
		return 0
