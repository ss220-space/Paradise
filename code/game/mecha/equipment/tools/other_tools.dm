...
/obj/item/mecha_parts/mecha_equipment/cage/action(mob/living/carbon/target)
	...
	occupant_message(span_notice("[target] не мо[PLUR_JET_GUT(target)] быть удержан[GEND_ENDING_A_O_Y(target)], так как [target] не наход[PLUR_IT_YAT(target)]ся в критическом состоянии."))
	return FALSE
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/prisoner_insertion_check(mob/living/carbon/target)
	if(target.buckled)
		occupant_message(span_warning("[target] не помест[PLUR_IT_YAT(target)]ся в клетку, так как [target] прикован[GEND_ENDING_A_O_Y(target)] к [target.buckled.declent_ru(DATIVE)]!"))
		return FALSE
