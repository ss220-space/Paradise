...
/obj/item/mecha_parts/mecha_equipment/cage/proc/supress_action(mob/living/carbon/target)
	if(holding)
		...
		chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] перестаёт удерживать [holding] и начинает удерживать [target]."))
		stop_supressing(holding)
	else
		occupant_message(span_notice("Вы начинаете удерживать [target]..."))
		chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] начинает удерживать [target]."))
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/handcuff_action(mob/living/carbon/target)
	...
	chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] начинает сковывать [target]."))
	if(!do_after_cooldown(target))
		return FALSE
	if(!prisoner)
		change_alert(CAGE_STAGE_TWO)
	...
	chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] успешно сковал [target]."))
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/insert_action(mob/living/carbon/target)
	...
	chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] начинает помещать [target] внутрь клетки."))
	if(!do_after_cooldown(target))
	...
	chassis.visible_message(span_warning("[chassis.declent_ru_cap(NOMINATIVE)] поместил [target] в клетку."))
