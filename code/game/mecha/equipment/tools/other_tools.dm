...
/obj/item/mecha_parts/mecha_equipment/repair_droid/process()
	...
	if(h_boost < 0 || chassis.obj_integrity < chassis.max_integrity)
		chassis.repair_damage(h_boost, chassis.max_integrity - chassis.obj_integrity)
		repaired = TRUE
	if(repaired)
...
/obj/item/mecha_parts/mecha_equipment/cage/action(mob/living/carbon/target)
	...
	occupant_message(span_notice("[target] не может быть удержан[GEND_ENDING_A_O_Y(target)], так как [target] не находится в критическом состоянии."))
	return FALSE
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/insert_action(mob/living/carbon/target)
	...
	occupant_message(span_notice("[target] успешно помещен[GEND_ENDING_A_O_Y(target)] в клетку."))
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/on_escape(mob/living/carbon/target)
	SIGNAL_HANDLER
	occupant_message(span_warning("[prisoner] сбежал[GEND_ENDING_A_O_I(prisoner)] из клетки."))
	prisoner = null
	if(holding)
		if(holding.handcuffed)
			change_alert(CAGE_STAGE_TWO)
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/prisoner_insertion_check(mob/living/carbon/target)
	if(target.buckled)
		occupant_message(span_warning("[target] не поместится в клетку, так как [target] прикован[GEND_ENDING_A_O_Y(target)] к [target.buckled.declent_ru(DATIVE)]!"))
		return FALSE
	if(target.has_buckled_mobs())
		occupant_message(span_warning("[target] не поместится в клетку, пока на [GEND_ON_IN_HIM(target)] висит слайм!"))
		return FALSE
	if(prisoner)
...
/obj/item/mecha_parts/mecha_equipment/cage/proc/eject(force)
	...
	if(!force)
		occupant_message("[prisoner] извлечен[GEND_ENDING_A_O_Y(prisoner)].")
	else
		occupant_message("[prisoner] сбежал[GEND_ENDING_A_O_I(prisoner)] из клетки.")
	prisoner = null
	change_state("mecha_cage")
