...
/obj/item/mecha_parts/mecha_equipment/weapon/honker/action(target, params)
	...
	add_game_logs("[key_name(chassis.occupant)] used a Mecha Honker in [COORD(T)]", chassis.occupant)
	do_after_cooldown()
	return
...
/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/missile_rack/action(target, params)
	...
	add_game_logs("[key_name(chassis.occupant)] fired a [src] in [COORD(T)]", chassis.occupant)
	do_after_cooldown()
	return
