...
/atom/movable/proc/can_be_pulled(atom/movable/puller, grab_state, force, supress_message)
	...
	if(anchored)
		if(!supress_message && ismob(puller))
			to_chat(puller, span_warning("Похоже, [declent_ru(NOMINATIVE)] прикрепл[GEND_ENDING_EN_NA_NO_NY(src)] к полу!"))
		return FALSE
...
/atom/movable/proc/force_push(atom/movable/AM, force = move_force, direction, silent = FALSE)
	...
	if(!silent && .)
		visible_message(span_warning("[capitalize(declent_ru(NOMINATIVE))] сильно толкает [AM.declent_ru(ACCUSATIVE)]!"), span_warning("Вы сильно толкаете [AM.declent_ru(ACCUSATIVE)]!"))

/atom/movable/proc/move_crush(atom/movable/AM, force = move_force, direction, silent = FALSE)
	. = AM.move_crushed(src, force, direction)
	if(!silent && .)
		visible_message(span_danger("[capitalize(declent_ru(NOMINATIVE))] сокрушает [AM.declent_ru(ACCUSATIVE)]!"), span_danger("Вы сокрушили [AM.declent_ru(ACCUSATIVE)]!"))
...
/atom/movable/proc/devoured(mob/living/carbon/gourmet)
	...
	gourmet.visible_message(span_danger("[capitalize(gourmet.declent_ru(NOMINATIVE))] пытается поглотить [capitalize(declent_ru(ACCUSATIVE))]!"))
	...
	if(!do_after(gourmet, get_devour_time(gourmet), target, NONE, extra_checks = CALLBACK(src, PROC_REF(can_devour), gourmet), max_interact_count = 1, cancel_on_max = TRUE, cancel_message = span_notice("Вы прекращаете поглощать [capitalize(declent_ru(ACCUSATIVE))]!")))
		gourmet.visible_message(span_notice("[capitalize(gourmet.declent_ru(NOMINATIVE))] прекращает поглощать [capitalize(declent_ru(ACCUSATIVE))]!"))
		return FALSE

	gourmet.visible_message(span_danger("[capitalize(gourmet.declent_ru(NOMINATIVE))] поглощает [capitalize(declent_ru(ACCUSATIVE))]!"))
