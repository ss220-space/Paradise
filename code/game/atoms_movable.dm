	...
	if(ismob(pulled_atom))
		...
		if(!supress_message)
			pulled_mob.visible_message(
				span_warning("[declent_ru_cap(NOMINATIVE)] схватил[GEND_A_O_I(src)] [pulled_mob.declent_ru(ACCUSATIVE)]!"),
				span_warning("[declent_ru_cap(NOMINATIVE)] схватил[GEND_A_O_I(src)] Вас!"),
			)
...
/atom/movable/proc/can_be_pulled(atom/movable/puller, grab_state, force, supress_message)
	...
	if(force < (move_resist * MOVE_FORCE_PULL_RATIO))
		if(!supress_message && ismob(puller))
			to_chat(puller, span_warning("[declent_ru_cap(NOMINATIVE)] слишком тяжел[GEND_YI_AYA_OE_YE(src)]!"))
		return FALSE
	return TRUE
...
/atom/movable/proc/can_z_move(direction, turf/start, turf/destination, z_move_flags = ZMOVE_FLIGHT_FLAGS, mob/living/rider)
	...
	if(z_move_flags & ZMOVE_CAN_FLY_CHECKS && !(movement_type & (FLYING|FLOATING)) && !no_gravity(start))
		if(z_move_flags & ZMOVE_FEEDBACK)
			if(rider)
				to_chat(rider, span_notice("[declent_ru_cap(NOMINATIVE)] не способен к полёту."))
			else
...
/atom/movable/proc/force_push(atom/movable/AM, force = move_force, direction, silent = FALSE)
	...
	if(!silent && .)
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] сильно толка[PLUR_ET_YUT(src)] [AM.declent_ru(ACCUSATIVE)]!"), span_warning("Вы сильно толкаете [AM.declent_ru(ACCUSATIVE)]!"))

/atom/movable/proc/move_crush(atom/movable/AM, force = move_force, direction, silent = FALSE)
	. = AM.move_crushed(src, force, direction)
	if(!silent && .)
		visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сокруша[PLUR_ET_YUT(src)] [AM.declent_ru(ACCUSATIVE)]!"), span_danger("Вы сокрушили [AM.declent_ru(ACCUSATIVE)]!"))
...
/atom/movable/proc/devoured(mob/living/carbon/gourmet)
	...
	gourmet.visible_message(span_danger("[gourmet.declent_ru_cap(NOMINATIVE)] пыта[PLUR_ET_YUT(gourmet)]ся поглотить [declent_ru(ACCUSATIVE)]!"))
	...
	if(!do_after(gourmet, get_devour_time(gourmet), target, NONE, extra_checks = CALLBACK(src, PROC_REF(can_devour), gourmet), max_interact_count = 1, cancel_on_max = TRUE, cancel_message = span_notice("Вы прекращаете поглощать [declent_ru(ACCUSATIVE)]!")))
		gourmet.visible_message(span_notice("[gourmet.declent_ru_cap(NOMINATIVE)] прекраща[PLUR_ET_YUT(gourmet)] поглощать [declent_ru(ACCUSATIVE)]!"))
		return FALSE

	gourmet.visible_message(span_danger("[gourmet.declent_ru_cap(NOMINATIVE)] поглоща[PLUR_ET_YUT(gourmet)] [declent_ru(ACCUSATIVE)]!"))
