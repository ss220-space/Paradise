...
/obj/machinery/porta_turret/crowbar_act(mob/living/user, obj/item/I)
	...
	if(!I.use_tool(src, user, 2 SECONDS * building_mod, volume = I.tool_volume) || !(stat & BROKEN))
		return .
	if(prob(70))
...
/obj/machinery/porta_turret/wrench_act(mob/living/user, obj/item/I)
	...
	wrenching = TRUE
	CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
	if(!I.use_tool(src, user, 2 SECONDS * building_mod, volume = I.tool_volume) || enabled || raised || (!anchored && isinspace()))
		wrenching = FALSE
		return .
	wrenching = FALSE
...
/obj/machinery/porta_turret_construct/crowbar_act(mob/living/user, obj/item/I)
	...
	. = TRUE
	CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
	if(!I.use_tool(src, user, 3 SECONDS * building_mod, volume = I.tool_volume) || build_step != TURRET_BUILD_LOOSEN)
		return .
...
/obj/machinery/porta_turret_construct/welder_act(mob/user, obj/item/I)
	...
	switch(build_step)
		if(TURRET_BUILD_ARMORED)
			...
			CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
			if(!I.use_tool(src, user, 2 SECONDS * building_mod, volume = I.tool_volume) || build_step != TURRET_BUILD_ARMORED)
				return .
		...
		if(TURRET_BUILD_COATED)
			...
			CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
			if(!I.use_tool(src, user, 5 SECONDS * building_mod, amount = 5, volume = I.tool_volume) || build_step != TURRET_BUILD_COATED)
				return .
