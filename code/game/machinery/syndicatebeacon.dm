	...
	if(ishuman(user) || istype(user, /mob/living/silicon/ai))
		if(is_special_character(user))
			...
			if(user.gender == FEMALE)
				honorific = "Ms."
			dat += span_fontcolor_red("<i>Identity not found in operative database. What can the Syndicate do for you today, [honorific] [user.name]?</i><br>")
			if(!selfdestructing)
...
/obj/machinery/syndicate_beacon/Topic(href, href_list)
	...
	if(href_list["betraitor"])
		...
		charges -= 1
		if(prob(50))
			temptext = span_fontcolor_red("<i><b>Двойной агент. Ты планировал предать нас с самого начала. Позвольте нам отплатить за услугу тем же.</b></i>")
			src.updateUsrDialog()
