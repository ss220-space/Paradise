...
/obj/machinery/clonepod/attackby(obj/item/I, mob/user, params)
	...
	if(HAS_TRAIT(src, TRAIT_CMAGGED))
		...
		user.visible_message(
			span_notice("[user] начинает счищать слизь с [declent_ru(GENITIVE)]."),
			span_notice("Вы начинаете счищать слизь с [declent_ru(GENITIVE)].")
		)
		if(!do_after(user, 5 SECONDS, src))
			return ATTACK_CHAIN_PROCEED
		user.visible_message(
			span_notice("[user] убирает слизь с [declent_ru(GENITIVE)]."),
...
/obj/machinery/clonepod/proc/malfunction(go_easy = FALSE)
	if(occupant)
		...
		if(!go_easy)
			...
			message += "<i>Это и есть ваша смерть? Да, это она.</i>"
