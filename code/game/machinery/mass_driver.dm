...
/obj/machinery/mass_driver_frame/wrench_act(mob/living/user, obj/item/I)
	...
	switch(build)
		if(MASS_DRIVER_BUILD_LOOSE)
			...
			if(!I.use_tool(src, user, 1 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_LOOSE)
				return .
			set_anchored(TRUE)
		...
		if(MASS_DRIVER_BUILD_ANCHORED)
			...
			if(!I.use_tool(src, user, 1 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_ANCHORED)
				return .
			set_anchored(FALSE)
...
/obj/machinery/mass_driver_frame/wirecutter_act(mob/living/user, obj/item/I)
	...
	CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
	if(!I.use_tool(src, user, 1 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_WIRED)
		return .
...
/obj/machinery/mass_driver_frame/crowbar_act(mob/living/user, obj/item/I)
	...
	CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
	if(!I.use_tool(src, user, 3 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_GRILLE)
		return .
	...
	switch(build)
		if(MASS_DRIVER_BUILD_WELDED)
			if(iscoil(I))
				...
				CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
				if(!do_after(user, 2 SECONDS * coil.toolspeed * building_mod, src, category = DA_CAT_TOOL) || build != MASS_DRIVER_BUILD_WELDED || QDELETED(coil) || !coil.use(2))
					return ATTACK_CHAIN_PROCEED
		...
		if(MASS_DRIVER_BUILD_WIRED)
			if(istype(I, /obj/item/stack/rods))
				...
				CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
				if(!do_after(user, 2 SECONDS * rods.toolspeed * building_mod, src, category = DA_CAT_TOOL) || build != MASS_DRIVER_BUILD_WIRED || QDELETED(rods) || !rods.use(2))
					return ATTACK_CHAIN_PROCEED
...
/obj/machinery/mass_driver_frame/welder_act(mob/user, obj/item/I)
	...
	switch(build)
		if(MASS_DRIVER_BUILD_LOOSE)
			...
			if(!I.use_tool(src, user, 3 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_LOOSE)
				return .
		...
		if(MASS_DRIVER_BUILD_ANCHORED)
			...
			if(!I.use_tool(src, user, 4 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_ANCHORED)
				return .
		...
		if(MASS_DRIVER_BUILD_WELDED)
			...
			if(!I.use_tool(src, user, 4 SECONDS * building_mod, volume = I.tool_volume) || build != MASS_DRIVER_BUILD_WELDED)
				return .
