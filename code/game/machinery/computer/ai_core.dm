	...
	switch(state)
		...
		if(SCREWED_CORE)
			if(iscoil(I))
				...
				if(!do_after(user, 2 SECONDS * coil.toolspeed * building_mod, src, category = DA_CAT_TOOL) || state != SCREWED_CORE || QDELETED(coil))
					return ATTACK_CHAIN_PROCEED
				if(!coil.use(5))
		...
		if(CABLED_CORE)
			if(istype(I, /obj/item/stack/sheet/rglass))
				...
				if(rglass.get_amount() < 2)
					...
					return ATTACK_CHAIN_PROCEED
				CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
				if(!do_after(user, 2 SECONDS * rglass.toolspeed * building_mod, src, category = DA_CAT_TOOL) || state != CABLED_CORE || QDELETED(rglass))
					return ATTACK_CHAIN_PROCEED
				if(!rglass.use(2))
