			...
			if(t1 == "Custom")
				...
				if(istype(H))
					if(jobban_isbanned(H, t1))
						to_chat(usr, span_warning(span_fontsize_18px("ЦК не одобряет данную должность для этого сотрудника. Причиной этому могла послужить его профнепригодность для данной профессии."))) //На русском, потому что не все поймут на инглише
