...
/obj/machinery/computer/card/Initialize()
	...
	Radio.listening = 0
	Radio.config(list("Командование" = 0))
					...
					for(var/datum/data/record/R in GLOB.data_core.security)
						if(R.fields["id"] == E.fields["id"])
							if(status_valid_for_demotion(R.fields["criminal"]))
								...
								Radio.autosay("[scan.registered_name] ([scan.assignment]) has set [tempname] ([temprank]) to demote for: [reason]", name, "Командование", list(z))
