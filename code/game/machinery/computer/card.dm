...
/obj/machinery/computer/card/ui_act(action, params)
	...
	switch(action)
		if("assign") // transfer to a new job
			...
			if(t1 == "Custom")
				...
				if(temp_t && scan && modify)
					...
					add_game_logs("[key_name(usr)] ([scan.assignment]) has reassigned \"[modify.registered_name]\" from \"[oldrank]\" to \"[temp_t]\".", usr)
					investigate_log("[key_name(usr)] ([scan.assignment]) has reassigned \"[modify.registered_name]\" from \"[oldrank]\" to \"[temp_t]\"." , INVESTIGATE_ACCESSCHANGES)
				...
				add_game_logs("[key_name(usr)] ([scan.assignment]) has reassigned \"[modify.registered_name]\" from \"[jobnamedata]\" to \"[assignment]\".", usr)
				investigate_log("[key_name(usr)] ([scan.assignment]) has reassigned \"[modify.registered_name]\" from \"[jobnamedata]\" to \"[assignment]\".", INVESTIGATE_ACCESSCHANGES)
				if(t1 == "Civilian")
		...
		if("demote")
			...
			add_game_logs("[key_name(usr)] ([scan.assignment]) has demoted \"[modify.registered_name]\" [m_ckey_text] the \"[jobnamedata]\" for: \"[reason]\".", usr)
			investigate_log("[key_name(usr)] ([scan.assignment]) has demoted \"[modify.registered_name]\" [m_ckey_text] the \"[jobnamedata]\" for: \"[reason]\".", INVESTIGATE_ACCESSCHANGES)
		...
		if("terminate")
			...
			var/m_ckey = modify.getPlayerCkey()
			var/m_ckey_text = m_ckey ? "([m_ckey])" : "(no ckey)"
			add_game_logs("[key_name(usr)] ([scan.assignment]) has terminated \"[modify.registered_name]\" [m_ckey_text] the \"[jobnamedata]\" for: \"[reason]\".", usr)
			investigate_log("[key_name(usr)] ([scan.assignment]) has terminated \"[modify.registered_name]\" [m_ckey_text] the \"[jobnamedata]\" for: \"[reason]\".", INVESTIGATE_ACCESSCHANGES)
		...
		if("make_job_available") // MAKE ANOTHER JOB POSITION AVAILABLE FOR LATE JOINERS
			...
			add_game_logs("[key_name(usr)] ([scan.assignment]) has opened a job slot for job \"[j.title]\".", usr)
			investigate_log("[key_name(usr)] ([scan.assignment]) has opened a job slot for job \"[j.title]\".", INVESTIGATE_ACCESSCHANGES)
		...
		if("make_job_unavailable") // MAKE JOB POSITION UNAVAILABLE FOR LATE JOINERS
			...
			j.total_positions--
			opened_positions[edit_job_target]--
			add_game_logs("[key_name(usr)] ([scan.assignment]) has closed a job slot for job \"[j.title]\".", usr)
			investigate_log("[key_name(usr)] ([scan.assignment]) has closed a job slot for job \"[j.title]\".", INVESTIGATE_ACCESSCHANGES)
					...
					for(var/datum/data/record/R in GLOB.data_core.security)
						if(R.fields["id"] == E.fields["id"])
							if(status_valid_for_demotion(R.fields["criminal"]))
								...
								add_game_logs("[key_name(usr)] ([scan.assignment]) has set \"[tempname]\" ([temprank]) to demote for: \"[reason]\".", usr)
								investigate_log("[key_name(usr)] ([scan.assignment]) has set \"[tempname]\" ([temprank]) to demote for: \"[reason]\".", INVESTIGATE_RECORDS)
	...
	switch(action)
		...
		if("prioritize_job") // TOGGLE WHETHER JOB APPEARS AS PRIORITIZED IN THE LOBBY
			...
			if(j in SSjobs.prioritized_jobs)
				...
				return FALSE
			add_game_logs("[key_name(usr)] ([scan.assignment]) [priority ?  "prioritized" : "unprioritized"] the job \"[j.title]\".", usr)
			investigate_log("[key_name(usr)] ([scan.assignment]) [priority ?  "prioritized" : "unprioritized"] the job \"[j.title]\".", INVESTIGATE_ACCESSCHANGES)
