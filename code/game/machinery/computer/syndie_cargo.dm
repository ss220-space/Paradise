	...
	switch(action)
		...
		if("teleport")
			...
			if(data_storage.telepads_status == "Pads ready")
				sell()
				//Телепорт
				investigate_log("[key_name(usr)] has sold '[data_storage.sold_atoms]' using syndicate cargo. Remaining credits: [data_storage.cash]", INVESTIGATE_SYNDIE_CARGO)
			...
			for(var/i in 1 to length(data_storage.requestlist))
				...
				if(SO.ordernum == ordernum)
					...
					if(data_storage.cash >= P.cost)
						...
						investigate_log("[key_name(usr)] has authorized an order for [P.name]. Remaining credits: [data_storage.cash].", INVESTIGATE_SYNDIE_CARGO)
					else
