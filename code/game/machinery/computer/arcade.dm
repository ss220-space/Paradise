...
/obj/machinery/computer/arcade/proc/prizevend(score)
	if(prob(0.0001)) //1 in a million
		...
		visible_message(
			span_notice("[declent_ru_cap(NOMINATIVE)] выда[PLUR_YOT_YUT(src)]... Ого, оружие! Это просто улёт!"),
		...
		if(emagged)
			...
			switch(event)
				...
				if(ORION_TRAIL_MALFUNCTION)
					...
					visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] сбоит, из-за чего внутриигровые показатели перемешиваются!"))
					var/oldfood = food
					var/oldfuel = fuel
					...
					if(electronics)
						...
						if(oldfuel > fuel && oldfood > food)
							audible_message(span_danger("[declent_ru_cap(NOMINATIVE)] издаёт какой-то успокаивающий гул."))
						else if(oldfuel < fuel || oldfood < food)
							audible_message(span_danger("[declent_ru_cap(NOMINATIVE)] издаёт какой-то зловещий гул."))
						food = oldfood
						fuel = oldfuel
...
/obj/item/orion_ship/attack_self(mob/user) //Minibomb-level explosion. Should probably be more because of how hard it is to survive the machine! Also, just over a 5-second fuse
	...
	active = 1
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] тихо пищит и жужжит, пробуждаясь к жизни!"))
	...
	sleep(20)
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] начинает вибрировать..."))
	...
	sleep(3.6)
	visible_message(span_userdanger("[declent_ru_cap(NOMINATIVE)] взрывается!"))
