...
/obj/machinery/computer/arcade/battle/Topic(href, href_list)
	...
	if(!blocked && !gameover)
		if(href_list["attack"])
			...
			temp = "Ваша атака нанесла [attackamt] единиц[DECL_U_Y__(attackamt)] урона!"
			...
			temp = "Вы использовали [pointamt] единиц[DECL_U_Y__(pointamt)] ману <br>и восстановили [healamt] единиц здоровья!"
			...
			temp = "Вы восстанавливаете [chargeamt] единиц[DECL_U_Y__(chargeamt)] маны"
...
/obj/machinery/computer/arcade/battle/proc/arcade_action()
	if((enemy_mp <= 0) || (enemy_hp <= 0))
		...
		temp = "[enemy_name] бросает бомбу, <br>которая наносит вам [boomamt] единиц[DECL_U_Y__(boomamt)] урона взрывом!"
		playsound(loc, 'sound/arcade/boom.ogg', 50, TRUE)
		player_hp -= boomamt

	else if((enemy_mp <= 5) && (prob(70)))
		var/stealamt = rand(2,3)
		temp = "[enemy_name] крадёт [stealamt] единиц[DECL_U_Y__(stealamt)] вашей маны!"
		...
		temp = "[enemy_name] наносит [attackamt] единиц[DECL_U_Y__(attackamt)] урона!"
		playsound(loc, 'sound/arcade/hit.ogg', 50, TRUE)
		player_hp -= attackamt
...
/obj/machinery/computer/arcade/orion_trail/Topic(href, href_list)
	...
	//they also call event() again, to regen the eventdata, which is kind of odd but necessary
		...
		if(prob(success))
			FU = rand(5,15)
			FO = rand(5,15)
			last_spaceport_action = "Вы успешно совершили налёт на космопорт! Вы получили [FU] единиц[DECL_U_Y__(FU)] Топлива и [FO] единиц[DECL_U_Y__(FO)] Пищи! (+[FU]FU,+[FO]FO)"
		else
			FU = rand(-5,-15)
			FO = rand(-5,-15)
			last_spaceport_action = "Вам не удалось совершить налёт на космопорт! Вы потеряли [FU*-1] единиц[DECL_U_Y__(FU*-1)] Топлива и [FO*-1] единиц[DECL_U_Y__(FO*-1)] Пищи, унося свои ноги оттуда! ([FU]FU,[FO]FO)"
			...
			if(prob(success*5))
				...
				last_spaceport_action = "Вам не удалось совершить налёт на космопорт! Вы потеряли [FU*-1] единиц[DECL_U_Y__(FU*-1)] Топлива, [FO*-1] единиц[DECL_U_Y__(FO*-1)] Пищи, и [lost_crew], унося свои ноги оттуда! ([FU]FI,[FO]FO,-Crew)"
				if(emagged)
	...
	switch(event)
		if(ORION_TRAIL_RAIDERS)
			...
			if(prob(50))
				...
				food -= sfood
				fuel -= sfuel
				eventdat += "<br>Они украли [sfood] единиц[DECL_U_Y__(sfood)] <b>Пищи</b> и [sfuel] единиц[DECL_U_Y__(sfuel)] <b>Топлива</b>."
			else if(prob(10))
		...
		if(ORION_TRAIL_COLLISION)
			...
			if(prob(25))
				...
				food -= sfood
				fuel -= sfuel
				eventdat += "<br>[sfood] единиц[DECL_U_Y__(sfood)] <b>Пищи</b> и [sfuel] единиц[DECL_U_Y__(sfuel)] <b>Топлива</b> выброшены в открытый космос.."
			if(prob(10))
				var/deadname = remove_crewmember()
