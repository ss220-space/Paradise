...
/obj/structure/computerframe/wrench_act(mob/living/user, obj/item/I)
	...
	if(!I.use_tool(src, user, 2 SECONDS * building_mod, volume = I.tool_volume))
		return .
...
/obj/structure/computerframe/attackby(obj/item/I, mob/user, params)
	...
	switch(state)
		...
		if(STATE_NOWIRES)
			...
			CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
			if(!do_after(user, 2 SECONDS * coil.toolspeed * building_mod, src, category = DA_CAT_TOOL) || state != STATE_NOWIRES || QDELETED(coil))
				return ATTACK_CHAIN_PROCEED
			if(!coil.use(5))
		...
		if(STATE_WIRES)
			...
			CALCULATE_SKILL_MOD(user, BUILDING_SPEED_MOD, building_mod)
			if(!do_after(user, 2 SECONDS * glass.toolspeed * building_mod, src, category = DA_CAT_TOOL) || state != STATE_WIRES || QDELETED(glass))
				return ATTACK_CHAIN_PROCEED
			if(!glass.use(2))
