	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/wrench))
				...
				if(do_after(user, 20 * P.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
		...
		if(2)
			...
			if(istype(P, /obj/item/stack/cable_coil))
				...
				if(C.get_amount() >= 5)
					...
					if(do_after(user, 20 * C.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
						if(state == 2 && C.use(5))
		...
		if(3)
			...
			if(istype(P, /obj/item/stack/sheet/glass))
				...
				if(G.get_amount() >= 2)
					...
					if(do_after(user, 20 * G.toolspeed * (ishuman(user) ? user.dna.species.toolspeedmod : 1), target = src))
						if(state == 3 && G.use(2))
