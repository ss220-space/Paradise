...
/obj/machinery/cryopod
	...
	var/on_store_message = "помещено в криохранилище."
...
/obj/machinery/cryopod/proc/despawn_occupant()
	...
	if(!syndicate)
		...
		if(ailist.len)
			...
			if (announce_rank)
				announcer.say("; Тело [occupant.real_name] ([announce_rank]) [on_store_message]")
			else
				announcer.say("; Тело [occupant.real_name] [on_store_message]")
		else
			if (announce_rank)
				announce.autosay("Тело [occupant.real_name]  ([announce_rank]) [on_store_message]", "[on_store_name]")
			else
				announce.autosay("Тело [occupant.real_name] [on_store_message]", "[on_store_name]")
...
/obj/machinery/cryopod/robot
	...
	on_store_message = "помещено в робохранилище."
