/obj/machinery/syndiepad
	...
	var/delete_crates = FALSE // Удаляем ли мы посылки которые получает телепад после отправки через консоль? Мы же не хотим кучи посылок на базе админов. Админ Флаг, не изменяемый игроками.
