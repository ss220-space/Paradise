...
/obj/machinery/bodyscanner/proc/generate_printing_text()
	...
	if(istype(occupant)) //is there REALLY someone in there?
		...
		dat += "Паралич, %: [occupant.paralysis] ([prlz_left_pretext] [prlz_left] [prlz_left_posttext]!)<br>"
		dat += "Температура тела: [occupant.bodytemperature-T0C] °C<br>"

		dat += "<hr>"

		if(occupant.has_brain_worms())
			dat += "Во фронтальной коре обнаружена крупная опухоль, потенциально злокачественная. Рекомендуется хирургическое вмешательство.<br>"
		...
		if(occupant.reagents)
			...
			dat += "[extra_font]\tСальбутамол: [salbutamol_amount] единиц[declension_ru(salbutamol_powder_amount,"а","ы","")]<BR>"
