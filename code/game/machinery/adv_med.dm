	...
	switch(action)
		...
		if("print_p")
			...
			if(GLOB.copier_items_printed >= GLOB.copier_max_items)
				...
				isPrinting = FALSE
				return
			visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] дребезжит, после чего из окна печати выпадает лист бумаги."))
...
/obj/machinery/bodyscanner/proc/generate_printing_text()
	...
	if(istype(occupant)) //is there REALLY someone in there?
		...
		for(var/obj/item/organ/internal/organ as anything in occupant.internal_organs)
			...
			dat += "<tr>"
			dat += "<td>[organ.declent_ru_cap(NOMINATIVE)]</td><td>Н/Д</td><td>[organ.damage]</td><td>[infection] [robot] [dead]</td>"
			dat += "</tr>"
		dat += "</table>"
		...
		dat += "[declent_ru_cap(NOMINATIVE)] пуст."
