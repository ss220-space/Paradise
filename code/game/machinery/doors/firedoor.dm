	...
	switch(constructionStep)
		if(CONSTRUCTION_PANEL_OPEN)
			if(istype(C, /obj/item/stack/sheet/plasteel))
				...
				if(do_after(user, 60 * C.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src))
		...
		if(CONSTRUCTION_GUTTED)
			if(iscoil(C))
				...
				if(do_after(user, 60 * B.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src))
		...
		if(CONSTRUCTION_NOCIRCUIT)
			if(istype(C, /obj/item/firelock_electronics))
				...
				playsound(get_turf(src), C.usesound, 50, 1)
				if(!do_after(user, 40 * C.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src))
					return
