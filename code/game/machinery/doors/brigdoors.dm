...
/obj/machinery/door_timer/proc/print_report()
	...
	Radio.autosay(announcetext, name, "Безопасность")
...
/obj/machinery/door_timer/process()
	...
	if(timing)
		if(emagged)
			...
			return PROCESS_KILL
		if(timeleft() <= 0)
			Radio.autosay("Timer has expired. Releasing prisoner.", name, "Безопасность", list(z))
			occupant = CELL_NONE
	...
	switch(action)
		...
		if("add_timer")
			if(timing)
				...
				Radio.autosay("Prisoner [occupant] had their timer increased by [prisoner_time_add / 600] minutes [addtext]", name, "Безопасность", list(z))
		...
		if("restart_timer")
			if(timing)
				...
				Radio.autosay("Prisoner [occupant] had their timer reset [resettext]", name, "Безопасность", list(z))
		...
		if("stop")
			if(timing)
				...
				Radio.autosay("Timer stopped manually [stoptext]", name, "Безопасность", list(z))
			else
				. = FALSE
		if("flash")
