...
/obj/machinery/door_timer/proc/print_report()
	...
	for(var/obj/machinery/computer/prisoner/C in GLOB.prisoncomputer_list)
		...
		P.info += {"<center>[station_name()] - Отдел Защиты Активов</center><br>
						...
						<small>Этот документ был сгенерирован при активации таймера.</small>"}
		...
		if("add_timer")
			if(timing)
				...
				if(timetoset + prisoner_time_add >= PERMABRIG_TIME MINUTES)
					Radio.autosay("Время заключения превышает 60 минут: переведите заключенного в пермабриг!", name, SEC_FREQ_NAME)
					return FALSE
