	...
	if(alert(user,"Would you like to start the countdown now?",,"Yes","No") == "Yes" && can_interact(user))
		if(defused || active)
			...
			if(payload && !istype(payload, /obj/item/bombcore/training))
				add_game_logs("[key_name_log(user)] has primed a [name] ([payload]) for detonation at [A.name] [COORD(bombturf)]", user)
				investigate_log("[key_name_log(user)] has has primed a [name] ([payload]) for detonation at [A.name] [COORD(bombturf)]", INVESTIGATE_BOMB)
...
/obj/item/syndicatedetonator/attack_self(mob/user)
	if(timer < world.time)
		...
		if(detonated)
			...
			detonated--
			investigate_log("[key_name_log(user)] has remotely detonated [detonated ? "syndicate bombs" : "a syndicate bomb"] using a [name] at [A.name] ([T.x],[T.y],[T.z])", INVESTIGATE_BOMB)
