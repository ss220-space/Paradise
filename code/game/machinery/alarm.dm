...
/obj/machinery/alarm/ui_status(mob/user, datum/ui_state/state)
	...
	if(aidisabled && (isAI(user) || isrobot(user)))
		to_chat(user, "<span class='warning'>Контроль ИИ над интерфейсом [src] был отключён.</span>")
		return STATUS_CLOSE
...
/obj/machinery/alarm/ui_act(action, list/params)
	...
	switch(action)
		...
		if("temperature")
			...
			var/input_temperature = input("Какую температуру должна поддерживать система? (между [min_temperature_c] °C и [max_temperature_c] °C)", "Управление термостатом") as num|null
		...
		if(1)
			if(iscoil(I))
				...
				if(coil.get_amount() < 5)
					to_chat(user, "Для этого вам нужно больше кабелей!")
					return
