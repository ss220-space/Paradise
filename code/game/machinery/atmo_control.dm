...
#define SENSOR_COMPOSITION_O2 (1<<2)
#define SENSOR_COMPOSITION_TOXINS (1<<3)
#define SENSOR_COMPOSITION_N2 (1<<4)
...
/obj/machinery/atmospherics/air_sensor/wrench_act(mob/user, obj/item/item)
	. = TRUE
	if(bolts)
		to_chat(user, "[src] is bolted to the floor! You can't detach it like this.")
		return .
	to_chat(user, span_notice("You begin to unfasten [src]..."))
	if(!item.use_tool(src, user, 4 SECONDS, volume = item.tool_volume) || bolts)
		return .
...
/obj/machinery/atmospherics/air_sensor/multitool_act(mob/living/user, obj/item/item)
	var/list/options = list(
		"Pressure: [ONOFF_TOGGLE(SENSOR_SCAN_PRESSURE)]" = SENSOR_SCAN_PRESSURE,
		"Temperature: [ONOFF_TOGGLE(SENSOR_SCAN_TEMPERATURE)]" = SENSOR_SCAN_TEMPERATURE,
		"Oxygen: [ONOFF_TOGGLE(SENSOR_COMPOSITION_O2)]" = SENSOR_COMPOSITION_O2,
		"Toxins: [ONOFF_TOGGLE(SENSOR_COMPOSITION_TOXINS)]" = SENSOR_COMPOSITION_TOXINS,
		"Nitrogen: [ONOFF_TOGGLE(SENSOR_COMPOSITION_N2)]" = SENSOR_COMPOSITION_N2,
		"Carbon Dioxide: [ONOFF_TOGGLE(SENSOR_COMPOSITION_CO2)]" = SENSOR_COMPOSITION_CO2,
		"Nitrous Oxide: [ONOFF_TOGGLE(SENSOR_COMPOSITION_N2O)]" = SENSOR_COMPOSITION_N2O,
		"Hydrogen: [ONOFF_TOGGLE(SENSOR_COMPOSITION_H2)]" = SENSOR_COMPOSITION_H2,
		"Water Vapor: [ONOFF_TOGGLE(SENSOR_COMPOSITION_H2O)]" = SENSOR_COMPOSITION_H2O,
	...
	if(temp_answer in options) // Null will break us out
		switch(options[temp_answer])
			if(SENSOR_SCAN_PRESSURE)
				output ^= SENSOR_SCAN_PRESSURE
			if(SENSOR_SCAN_TEMPERATURE)
				output ^= SENSOR_SCAN_TEMPERATURE
			if(SENSOR_COMPOSITION_O2)
				output ^= SENSOR_COMPOSITION_O2
			if(SENSOR_COMPOSITION_TOXINS)
				output ^= SENSOR_COMPOSITION_TOXINS
			if(SENSOR_COMPOSITION_N2)
				output ^= SENSOR_COMPOSITION_N2
			if(SENSOR_COMPOSITION_CO2)
				output ^= SENSOR_COMPOSITION_CO2
			if(SENSOR_COMPOSITION_N2O)
				output ^= SENSOR_COMPOSITION_N2O
			if(SENSOR_COMPOSITION_H2)
				output ^= SENSOR_COMPOSITION_H2
			if(SENSOR_COMPOSITION_H2O)
				output ^= SENSOR_COMPOSITION_H2O
			if("multitool")
...
/obj/machinery/computer/general_air_control/LateInitialize()
	// Setup sensors
	for(var/obj/machinery/atmospherics/air_sensor/air_sensor as anything in GLOB.gas_sensors)
		for(var/sensor_id in autolink_sensors)
			if(air_sensor.autolink_id == sensor_id)
				sensor_name_uid_map[autolink_sensors[sensor_id]] = air_sensor.UID()
				sensor_name_data_map[autolink_sensors[sensor_id]] = list()

	// Setup meters
	for(var/obj/machinery/atmospherics/meter/gas_meter as anything in GLOB.gas_meters)
		for(var/meter_id in autolink_sensors)
			if(gas_meter.autolink_id == meter_id)
				sensor_name_uid_map[autolink_sensors[meter_id]] = gas_meter.UID()
...
/obj/machinery/computer/general_air_control/multitool_act(mob/living/user, obj/item/item)
	if(!ismultitool(item)) // Should never happen
		return

	configure_sensors(user, item)
	return TRUE
...
/obj/machinery/computer/general_air_control/proc/configure_sensors(mob/living/user, obj/item/multitool/multitool)
	...
	switch(choice)
		if(ADD)
			...
			var/datum/linked_datum = locateUID(multitool.buffer_uid)
			...
			var/new_name = tgui_input_text(user, "Enter a name for the sensor/meter", "Name")
...
/obj/machinery/computer/general_air_control/proc/refresh_sensors()
	for(var/sensor_name in sensor_name_uid_map)
		var/obj/machinery/atmospherics/atmospherics_machine = locateUID(sensor_name_uid_map[sensor_name])
		if(QDELETED(atmospherics_machine))
			sensor_name_uid_map -= sensor_name
			sensor_name_data_map -= sensor_name
			continue

		if(istype(atmospherics_machine, /obj/machinery/atmospherics/air_sensor))
			// Cache here to avoid a ton of list lookups
			var/obj/machinery/atmospherics/air_sensor/air_sensor = atmospherics_machine
			var/list/sensor_data = sensor_name_data_map[sensor_name]
			var/turf/sensor_turf = get_turf(air_sensor)
			...
			if(air_sensor.output & SENSOR_SCAN_PRESSURE)
				sensor_data["pressure"] = air_sample.return_pressure()
			else
				sensor_data -= "pressure"

			if(air_sensor.output & SENSOR_SCAN_TEMPERATURE)
			...
			if(total_moles > 0)
				if(air_sensor.output & SENSOR_COMPOSITION_O2)
					sensor_data["o2"] = round(100 * air_sample.oxygen() / total_moles, 0.1)
				else
					sensor_data -= "o2"

				if(air_sensor.output & SENSOR_COMPOSITION_TOXINS)
					sensor_data["plasma"] = round(100 * air_sample.toxins() / total_moles, 0.1)
				else
					sensor_data -= "plasma"

				if(air_sensor.output & SENSOR_COMPOSITION_N2)
					sensor_data["n2"] = round(100 * air_sample.nitrogen() / total_moles, 0.1)
				else
					sensor_data -= "n2"

				if(air_sensor.output & SENSOR_COMPOSITION_CO2)
					sensor_data["co2"] = round(100 * air_sample.carbon_dioxide() / total_moles, 0.1)
				else
					sensor_data -= "co2"

				if(air_sensor.output & SENSOR_COMPOSITION_N2O)
					sensor_data["n2o"] = round(100 * air_sample.sleeping_agent() / total_moles, 0.1)
				else
					sensor_data -= "n2o"

				if(air_sensor.output & SENSOR_COMPOSITION_H2)
					sensor_data["h2"] = round(100 * air_sample.hydrogen() / total_moles, 0.1)
				else
					sensor_data -= "h2"

				if(air_sensor.output & SENSOR_COMPOSITION_H2O)
					sensor_data["h2o"] = round(100 * air_sample.water_vapor() / total_moles, 0.1)
				else
					sensor_data -= "h2o"

		else if(istype(atmospherics_machine, /obj/machinery/atmospherics/meter))
			var/list/meter_data = sensor_name_data_map[sensor_name]
			var/obj/machinery/atmospherics/meter/meter = atmospherics_machine
			if(meter.target)
				var/datum/gas_mixture/meter_environment = meter.target.return_obj_air()
				if(meter_environment)
					meter_data["pressure"] = meter_environment.return_pressure()
					meter_data["temperature"] = meter_environment.temperature()

/obj/machinery/computer/general_air_control/process()
	// We only care about refreshing if people are looking at us
	if(src in SStgui.open_uis)
		refresh_all()
		return

	return PROCESS_KILL

// MARK: large tank control
	...
	if(length(inlet_injector_autolink_ids))
		for(var/obj/machinery/atmospherics/unary/outlet_injector/inlet_injector as anything in GLOB.air_injectors)
			if(inlet_injector.autolink_id in inlet_injector_autolink_ids)
				...
				inlet_data += list(
					"[inlet_injector.UID()]" = list(
						"name"= inlet_injector.name,
						"on" = inlet_injector.on,
						"rate" = inlet_injector.volume_rate,
						"uid" = inlet_injector.UID(),
					),
				)
				refresh_inlets()
				break
		...
		for(var/obj/machinery/atmospherics/unary/vent_pump/outlet_vent as anything in GLOB.all_vent_pumps)
			if(outlet_vent.autolink_id in outlet_autolink_ids)
				...
				outlet_vent_data += list(
					"[outlet_vent.UID()]" = list(
						"name" = outlet_vent.name,
						"on" = outlet_vent.on,
						"checks" = outlet_vent.pressure_checks,
						"rate" = outlet_vent.pressure_checks == 1 ? outlet_vent.external_pressure_bound : outlet_vent.internal_pressure_bound,
						"uid" = outlet_vent.UID(),
					),
				)
				refresh_outlets()
...
/obj/machinery/computer/general_air_control/large_tank_control/multitool_act(mob/living/user, obj/item/item)
	...
	switch(choice)
		if(INLET)
			configure_inlet(user, item)
		if(OUTLET)
			configure_outlet(user, item)
		if(SENSORS)
			configure_sensors(user, item)
...
/obj/machinery/computer/general_air_control/large_tank_control/proc/configure_inlet(mob/living/user, obj/item/multitool/multitool)
	...
	switch(choice)
		if(ADD)
			if(multitool.buffer_uid in inlet_uids)
...
/obj/machinery/computer/general_air_control/large_tank_control/proc/configure_outlet(mob/living/user, obj/item/multitool/multitool)
	...
	switch(choice)
		if(ADD)
			if(multitool.buffer_uid in outlet_uids)
				to_chat(user, span_warning("Error: This device is already connected to the console."))
				return
			...
			if(istype(linked_datum, /obj/machinery/atmospherics/unary/vent_pump))
				...
				outlet_vent_data += list(
					"[linked_datum.UID()]" = list(
						...
						"on" = outlet_vent.on,
						"checks" = outlet_vent.pressure_checks,
						"rate" = outlet_vent.internal_pressure_bound,
						"uid" = outlet_vent.UID(),
					),
				)
				refresh_outlets()
...
#undef SENSOR_SCAN_PRESSURE
#undef SENSOR_SCAN_TEMPERATURE
#undef SENSOR_COMPOSITION_O2
#undef SENSOR_COMPOSITION_TOXINS
#undef SENSOR_COMPOSITION_N2
#undef SENSOR_COMPOSITION_CO2
#undef SENSOR_COMPOSITION_N2O
#undef SENSOR_COMPOSITION_H2
#undef SENSOR_COMPOSITION_H2O
