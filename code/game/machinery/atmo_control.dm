		...
		if(sensors.len)
			for(var/id_tag in sensors)
				...
				if(data)
					...
					sensor_part += span_fontcolor_red("[long_name] can not be found!<br>")
...
"}
			...
			output += "[span_fontcolor_red("ERROR: Can not find input port")] <a href='byond://?src=[UID()];in_refresh_status=1'>Search</a><br>"
	if(output_tag)
		if(output_info)
...
"}
			...
			output += "[span_fontcolor_red("ERROR: Can not find output port")] <a href='byond://?src=[UID()];out_refresh_status=1'>Search</a><br>"
