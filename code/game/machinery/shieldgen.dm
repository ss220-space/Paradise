...
/obj/machinery/shieldgen/proc/remove_shield(obj/machinery/shield/shield)
...
/obj/machinery/shieldgen/attackby(obj/item/I, mob/user, params)
	...
	if(iscoil(I))
		...
		if(!do_after(user, 3 SECONDS * coil.toolspeed * building_mod, src, category = DA_CAT_TOOL) || !malfunction || !is_open || QDELETED(coil) || !coil.use(1))
			return ATTACK_CHAIN_PROCEED
		health = max_health
		malfunction = FALSE
...
/obj/machinery/shieldwallgen/Initialize(mapload)
	. = ..()
	active_shields = alist() // Doing it here since you can't cast numeral defines to strings using "[NORTH]" in the definition
...
/obj/machinery/shieldwallgen/proc/try_charge_shields_power()
	...
	var/surplus = max(power_net.avail - power_net.load, 0)
	...
	power_net.load += shieldload //uses powernet power.
	return TRUE
...
/obj/machinery/shieldwallgen/proc/try_link_generators(direction)
	...
	for(var/turf in traveled_turfs)
		...
		shieldwall.dir = direction
		active_shields[direction] += shieldwall
		other_generator.active_shields[opposite_direction] += shieldwall

/obj/machinery/shieldwallgen/proc/deactivate()
	activated = FALSE
	STOP_PROCESSING(SSmachines, src)
	for(var/direction in GLOB.cardinal)
		var/list/shields_list = active_shields[direction]
		QDEL_LIST(shields_list) // Don't want to clean the assoc keys so no QDEL_LIST_ASSOC_VAL

/obj/machinery/shieldwallgen/proc/remove_active_shield(obj/machinery/shieldwall/shieldwall, direction)
	var/list/shields_list = active_shields[direction]
	shields_list -= shieldwall
