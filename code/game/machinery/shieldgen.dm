...
/obj/machinery/shieldgen/attackby(obj/item/I as obj, mob/user as mob, params)
	if(istype(I, /obj/item/card/emag))
		...
		if(do_after(user, 30 * coil.toolspeed * istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? istype(user, /mob/living/carbon/human) ? user.dna.species.toolspeedmod : 1 : 1 : 1, target = src))
			if(!src || !coil)
				return
			coil.use(1)
			health = max_health
