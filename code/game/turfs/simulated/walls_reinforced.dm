...
/turf/simulated/wall/r_wall/attackby(obj/item/I, mob/user, params)
	...
	if(d_state != RWALL_INTACT)
		...
		if(!do_after(user, max(2 SECONDS * d_state, 10 SECONDS) * metal.toolspeed, src, category = DA_CAT_TOOL, max_interact_count = 1) || d_state == RWALL_INTACT || QDELETED(metal))
			return .
	...
	if(istype(I, /obj/item/stack/sheet/plasteel))
		...
		if(!do_after(user, 4 SECONDS * plasteel.toolspeed, src, category = DA_CAT_TOOL, max_interact_count = 1) || d_state != RWALL_INTACT || QDELETED(plasteel))
			return .
		if(!plasteel.use(2))
...
/turf/simulated/wall/r_wall/welder_act(mob/user, obj/item/I)
	...
	if(d_state == RWALL_COVER)
		...
		if(I.use_tool(src, user, 6 SECONDS, volume = I.tool_volume) && d_state == RWALL_COVER)
			d_state = RWALL_CUT_COVER
			to_chat(user, span_notice("You press firmly on the cover, dislodging it."))
	else if(d_state == RWALL_SUPPORT_RODS)
		to_chat(user, span_notice("You begin slicing through the support rods..."))
		if(I.use_tool(src, user, 10 SECONDS, volume = I.tool_volume) && d_state == RWALL_SUPPORT_RODS)
		...
		if(I.use_tool(src, user, 6 SECONDS, volume = I.tool_volume) && d_state == RWALL_CUT_COVER)
...
/turf/simulated/wall/r_wall/crowbar_act(mob/user, obj/item/I)
	...
	switch(d_state)
		if(RWALL_CUT_COVER)
			...
			if(!I.use_tool(src, user, 10 SECONDS, volume = I.tool_volume) || d_state != RWALL_CUT_COVER)
				return
			d_state = RWALL_BOLTS
		...
		if(RWALL_BOLTS)
			...
			if(!I.use_tool(src, user, 2 SECONDS, volume = I.tool_volume) || d_state != RWALL_BOLTS)
				return
			d_state = RWALL_CUT_COVER
...
/turf/simulated/wall/r_wall/screwdriver_act(mob/user, obj/item/I)
	...
	if(!I.use_tool(src, user, 4 SECONDS, volume = I.tool_volume) || state_check != d_state)
		return
	if(d_state == RWALL_SUPPORT_LINES)
		d_state = RWALL_COVER
