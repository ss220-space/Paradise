...
/turf/simulated/wall/r_wall/attackby(obj/item/I, mob/user, params)
	if(d_state == RWALL_COVER && istype(I, /obj/item/gun/energy/plasmacutter))
		...
		if(istype(I, /obj/item/stack/sheet/metal))
			...
			if(do_after(user, max(20 * d_state, 100) * MS.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src) && d_state)
				if(!MS.use(1))
		...
		if(do_after(user, 40 * PS.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src) && !d_state)
			if(!PS.use(2))
...
/turf/simulated/wall/r_wall/try_destroy(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/pickaxe/drill/diamonddrill))
		...
		if(do_after(user, 800 * I.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src)) // Diamond drill has 0.25 toolspeed, so 200
	...
	if(istype(I, /obj/item/pickaxe/drill/jackhammer))
		...
		if(do_after(user, 1000 * I.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src)) // Jackhammer has 0.1 toolspeed, so 100
