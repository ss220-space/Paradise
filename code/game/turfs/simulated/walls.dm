...
/turf/simulated/wall/rpd_act(mob/user, obj/item/rpd/our_rpd)
	if(our_rpd.mode == RPD_ATMOS_MODE)
		if(!our_rpd.ranged)
			...
			user.visible_message(span_notice("[user] начинает сверлить отверстие в [declent_ru(PREPOSITIONAL)]..."),
...
/turf/simulated/wall/proc/try_decon(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/gun/energy/plasmacutter))
		...
		if(do_after(user, delay * I.toolspeed, src, category = DA_CAT_TOOL))
			...
			dismantle_wall()
			visible_message(span_warning("[user] разрезает [declent_ru(ACCUSATIVE)]!"), span_warning("Слышен звук разрезаемого металла."))
			return TRUE
...
/turf/simulated/wall/proc/try_destroy(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/pickaxe/drill/diamonddrill))
		...
		if(do_after(user, delay * I.toolspeed, src, category = DA_CAT_TOOL)) // Diamond pickaxe has 0.25 toolspeed, so 12s./6s.
			to_chat(user, span_notice("Ваш [I.declent_ru(NOMINATIVE)] пробивает последний слой укреплённой обшивки."))
			dismantle_wall()
			visible_message(span_warning("[user] просверливает [declent_ru(ACCUSATIVE)]!"), span_italics("Слышен скрежет металла."))
			return TRUE
		...
		if(do_after(user, delay * jh.wall_toolspeed, src, category = DA_CAT_TOOL)) // Jackhammer has 0.1 toolspeed, so 6s./3s.
			...
			dismantle_wall()
			visible_message(span_warning("[user] дезинтегрирует [declent_ru(ACCUSATIVE)]!"), span_warning("Слышен скрежет металла."))
			return TRUE
...
/turf/simulated/wall/proc/try_wallmount(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/pipe))
		...
		if(P.pipe_type != -1) // ANY PIPE
			...
			user.visible_message(span_notice("[user] начинает сверлить отверстие в [declent_ru(PREPOSITIONAL)]."), span_notice("Вы начинаете сверлить отверстие в [declent_ru(PREPOSITIONAL)]."), span_italics("Слышен звук дрели."))

			if(do_after(user, 8 SECONDS * P.toolspeed, src, category = DA_CAT_TOOL))
				user.visible_message(span_notice("[user] просверливает отверстие в [declent_ru(PREPOSITIONAL)] и проталкивает [P.declent_ru(ACCUSATIVE)] в пустоту."), span_notice("Вы заканчиваете сверление [declent_ru(PREPOSITIONAL)] и проталкиваете [P.declent_ru(ACCUSATIVE)] в пустоту."), span_italics("Слышен звук трещотки."))
