...
/turf/simulated/wall/mech_melee_attack(obj/mecha/M)
	...
	switch(M.damtype)
		if(BRUTE)
			...
			M.visible_message(span_danger("[M.declent_ru_cap(NOMINATIVE)] бьет [declent_ru(ACCUSATIVE)]!"), span_danger("Вы бьете [declent_ru(ACCUSATIVE)]!"))
...
/turf/simulated/wall/proc/try_rot(obj/item/I, mob/user, params)
	if((!I.sharp && I.force >= 10) || I.force >= 20)
		to_chat(user, span_notice("[declent_ru_cap(NOMINATIVE)] рассыпается под воздействием вашего [I.declent_ru(GENITIVE)]."))
		dismantle_wall(1)
		return TRUE
	return FALSE
