...
/turf/simulated/floor/lava/lava_land_surface/krill_act(atom/movable/AM)
	...
	visible_message(span_warning("[capitalize(krill.declent_ru(NOMINATIVE))] медленно тонет в лаве!"))
	sleep(5 SECONDS)
	qdel(krill)
	if(!can_be_fished_on)
...
/turf/simulated/floor/lava/lava_land_surface/plasma/attackby(obj/item/I, mob/user, params)
	...
	if(!I.reagents.add_reagent("plasma", 10))
		to_chat(user, span_warning("[capitalize(I.declent_ru(NOMINATIVE))] уже заполнен[GEND_ENDING_A_O_Y(I)] до краёв."))
		return .
