...
/turf/simulated/floor/lava/lava_land_surface/krill_act(atom/movable/AM)
	...
	visible_message(span_warning("[krill.declent_ru_cap(NOMINATIVE)] медленно тон[PLUR_ET_UT(krill)] в лаве!"))
	sleep(5 SECONDS)
	qdel(krill)
	if(!can_be_fished_on)
...
/turf/simulated/floor/lava/lava_land_surface/plasma/attackby(obj/item/I, mob/user, params)
	...
	if(!I.reagents.add_reagent("plasma", 10))
		to_chat(user, span_warning("[I.declent_ru_cap(NOMINATIVE)] уже заполнен[GEND_A_O_Y(I)] до краёв."))
		return .
