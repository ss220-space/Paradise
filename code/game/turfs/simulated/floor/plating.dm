...
/turf/simulated/floor/plating/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/stack/rods))
		...
		if(R.get_amount() < 2)
			...
			if(do_after(user, 30 * C.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src))
		...
		if(do_after(user, 3 SECONDS * C.toolspeed * ishuman(user) ? user.dna.species.toolspeedmod : 1, target = src))
