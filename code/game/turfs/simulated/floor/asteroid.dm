...
/turf/simulated/floor/plating/asteroid/airless/cave/Initialize(mapload)
	...
	if (!flora_spawn_list)
		...
		if(SSmapping.cave_theme == BLOCKED_BURROWS)
			flora_spawn_list += list(/obj/structure/flora/ash/rock/style_random = 3)
	...
	if(prob(monsterprob))
		...
		for(var/thing in urange(scanrange, T)) //prevents mob clumps
			if(!ishostile(thing) && !istype(thing, /obj/structure/spawner))
				continue
			if((ispath(randumb, /mob/living/simple_animal/hostile/megafauna) || ismegafauna(thing)) && get_dist(T, thing) <= megafaunarange)
