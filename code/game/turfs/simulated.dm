...
/turf/simulated/handle_slip(mob/living/carbon/slipper, weaken_amount, obj/slippable, lube_flags, tilesSlipped)
	...
	if(!(lube_flags & SLIDE_ICE))
		...
		to_chat(slipper, span_notice("[pluralize_ru(user.gender,"Ты","Вы")] поскользнул[genderize_ru(user.gender,"ся","ась","ся","ись")][slippable ? " на [slippable.declent_ru(PREPOSITIONAL)]" : ""]!"))
