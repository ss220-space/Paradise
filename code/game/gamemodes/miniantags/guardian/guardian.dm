...
/mob/living/simple_animal/hostile/guardian/proc/snapback()
	...
	if(summoner && loc && summoner.loc)
		if(get_dist(get_turf(summoner),get_turf(src)) <= range)
			...
			to_chat(src, span_holoparasite("Вас откинуло назад, так как превышена дальность связи! Ваша дальность всего [range] метр[DECL___A_OV(range)] от [summoner.real_name]!"))
