...
/obj/effect/proc_holder/spell/morph_spell/pass_airlock/cast(list/targets, mob/living/simple_animal/hostile/morph/user)
	...
	if(airlock.locked)
		...
		revert_cast(user)
		return
	user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] начинает протискиваться в шлюз [airlock.declent_ru(GENITIVE)]!"))
	...
	user.visible_message(span_warning("[user.declent_ru_cap(NOMINATIVE)] ненадолго приоткрывает шлюз [airlock.declent_ru(GENITIVE)] и проходит через него!"))
