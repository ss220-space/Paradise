...
/mob/living/simple_animal/hostile/morph/proc/try_eat(atom/movable/item)
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] начинает [pick("жрать", "поглощать")] [target]!"), eat_self_message, "Вы слышите громкий хруст!")
...
/mob/living/simple_animal/hostile/morph/proc/eat(atom/movable/item)
	if(item && item.loc != src)
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] проглатывает [item.declent_ru(ACCUSATIVE)] целиком!"))
...
/mob/living/simple_animal/hostile/morph/attack_hand(mob/living/carbon/human/attacker)
	if(ambush_prepared)
		to_chat(attacker, span_warning("[declent_ru_cap(NOMINATIVE)] кажется немного другим, чем обычно... он кажется более... ") + span_userdanger("СЛИЗИСТЫМ?!"))
...
/mob/living/simple_animal/hostile/morph/attackby(obj/item/item, mob/living/user)
	...
	if(user.a_intent == INTENT_HELP && ambush_prepared)
		to_chat(user, span_warning("Вы пытаетесь использовать [item.declent_ru(ACCUSATIVE)] на [declent_ru_cap(NOMINATIVE)]... он кажется другим, чем раньше..."))
	...
	if(!morphed && prob(50))
		...
		if(food_value + gathered_food > 0 && !(item.item_flags & ABSTRACT) && user.drop_item_ground(item))
			to_chat(user, span_warning("[declent_ru_cap(NOMINATIVE)] только что съел ваш [item.declent_ru(ACCUSATIVE)]!"))
			eat(item)
			return ATTACK_CHAIN_BLOCKED_ALL
		return ..()
...
/mob/living/simple_animal/hostile/morph/proc/ambush_attack(mob/living/dumbass, touched)
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] внезапно прыгает на [dumbass.declent_ru(ACCUSATIVE)]!"), span_warning("Вы атакуете [dumbass.declent_ru(ACCUSATIVE)], когда [GEND_HE_SHE(dumbass)] меньше всего этого ожидает!"), "Вы слышите ужасный хруст!")
