...
/mob/living/simple_animal/revenant/death(gibbed)
	...
	to_chat(src, span_revendanger("НЕТ! Нет... уже слишком поздно, вы чувствуете, как ваша сущность распадается..."))
...
/obj/item/ectoplasm/revenant/attack_self(mob/user)
	...
	user.visible_message(
		span_notice("[user] рассыпает [declent_ru(NOMINATIVE)] во всех направлениях."), \
...
/obj/item/ectoplasm/revenant/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(inert)
		return
	visible_message(span_notice("[declent_ru_cap(NOMINATIVE)] рассыпается на частицы при ударе, которые исчезают в никуда."))
	qdel(src)
...
/obj/item/ectoplasm/revenant/proc/reform()
	...
	if(!reforming)
		inert = TRUE
		visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] оседает и кажется безжизненным."))
		return
	...
	if(!key_of_revenant)
		qdel(new_revenant)
		inert = TRUE
		visible_message(span_revenwarning("[declent_ru_cap(NOMINATIVE)] оседает и кажется безжизненной."))
	...
	visible_message(span_revenboldnotice("[declent_ru_cap(NOMINATIVE)] внезапно поднимается в воздух, а затем исчезает."))
