...
/mob/living/simple_animal/demon/shadow/OnUnarmedAttack(atom/target)
	...
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] начинает обматывать [h_target.declent_ru(ACCUSATIVE)] теневой паутиной."))
	wrapping = TRUE
	if(!do_after(src, 4 SECONDS, h_target, DEFAULT_DOAFTER_IGNORE|DA_IGNORE_HELD_ITEM))
		wrapping = FALSE
		return

	h_target.visible_message(span_warning("<b>[declent_ru_cap(NOMINATIVE)] окутывает [h_target.declent_ru(ACCUSATIVE)] в теневой кокон, и из него начинает расползаться тьма.</b>"))
...
/obj/structure/shadowcocoon/obj_destruction()
	visible_message(span_danger("[declent_ru_cap(NOMINATIVE)] раскрывается, и тени, танцующие вокруг, рассеиваются."))
	return ..()
