...
/obj/item/disk/nuclear/Destroy(force)
	...
	if(force)
		...
		add_game_logs("[src] has been !!force deleted!! in ([diskturf ? "[diskturf.x], [diskturf.y] ,[diskturf.z]":"nonexistent location"]).")
		GLOB.poi_list.Remove(src)
	...
	if(GLOB.blobstart.len > 0)
		...
		add_game_logs("[src] has been destroyed in ([diskturf.x], [diskturf.y], [diskturf.z]). Moving it to ([NEWDISK.x], [NEWDISK.y], [NEWDISK.z]).")
