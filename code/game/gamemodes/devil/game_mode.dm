/datum/game_mode/proc/auto_declare_completion_devil()
	...
	var/list/text = list(span_fontsize2(span_bold("Дьявол[DECL_OM_AMI_AMI(length(devils))]:")))
...
/datum/game_mode/proc/auto_declare_completion_sintouched()
	...
	var/list/text = list(span_fontsize2(span_bold("Грешник[DECL_OM_AMI_AMI(length(devils))]:")))
	for(var/datum/mind/devil in devils)
