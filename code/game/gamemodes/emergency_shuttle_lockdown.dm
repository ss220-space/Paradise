...
/datum/controller/subsystem/shuttle/proc/reload_shuttle(admin_called = FALSE, spec_sound = 'sound/misc/notice2.ogg', from_hostile = FALSE)
	if(emergency.mode == SHUTTLE_STRANDED)
		...
		GLOB.minor_announcement.Announce("[from_hostile? "Угроза устранена" : "Блокировка снята"]. У вас есть 3 минуты, чтобы подняться на борт эвакуационного шаттла.", "Приоритетное оповещение.", spec_sound)
		return TRUE
	return TRUE
