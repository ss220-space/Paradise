...
/datum/game_mode/space_ninja/declare_completion(ragin = FALSE)
	if(finished && !ragin)
		...
		to_chat(world, span_warning(span_fontsize_3("<b> Ниндзя был[(space_ninjas.len>1)?"и":""] убит[(space_ninjas.len>1)?"ы":""] экипажем! Клан Паука ещё не скоро отмоется от этого позора!</b>")))
	..()
	return TRUE
