	...
	if(target && cooldown < world.time)
		switch(user.zone_selected)
			if("mouth")
				...
				target.say(wgw)
				add_game_logs("[user][user.key] made [target][target.key] say [wgw] with a voodoo doll.", user)
			...
			if("r_arm","l_arm")
				...
				if(nearby_mobs.len)
					...
					add_game_logs("[user][user.key] made [target][target.key] click on [T] with a voodoo doll.", user)
					target.ClickOn(T)
					GiveHint(target)
			if("head")
