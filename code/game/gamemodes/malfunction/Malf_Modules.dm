...
/datum/AI_Module/large/break_air_alarms
	...
	description = "Позволяет отключить предохранители на всех атмосферных панелях. После этого их можно будет переключить на режим «Потоп», отключающий вытяжки и проверку давления вентиляций. \
	Любой, кто проверит интерфейс атмосферной панели, может заметить проблему."
	one_purchase = TRUE
	cost = 50
	power_type = /datum/action/innate/ai/break_air_alarms
	unlock_text = "<span class='notice'>Вы снимаете протоколы безопасности всех атмосферных панелей, но не подтверждаете запрос. Вы можете подтвердить его когда угодно… больной вы ублюдок.</span>"

/datum/action/innate/ai/break_air_alarms
	name = "Нарушение протокола безопасности атмосферных панелях"
	desc = "Добавляет режим «Потоп» во все атмосферные панели."
...
/datum/action/innate/ai/break_air_alarms/Activate()
	for(var/obj/machinery/alarm/AA in GLOB.machines)
		if(!is_station_level(AA.z))
			continue
		AA.emagged = TRUE
	to_chat(owner, "<span class='notice'>Все ограничения атмосферных протоколов безопасности станции сняты. Атмосферные панели теперь могут быть переключены в режим «Потоп».")
	owner.playsound_local(owner, 'sound/machines/terminal_off.ogg', 50, 0)
