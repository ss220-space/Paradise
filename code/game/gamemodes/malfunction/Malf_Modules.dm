...
/datum/AI_Module/large/break_air_alarms
	...
	description = "Позволяет отключить предохранители на всех воздушных сигнализациях. После этого их можно будет переключить на режим «Потоп», отключающий вытяжки и проверку давления вентиляций. \
