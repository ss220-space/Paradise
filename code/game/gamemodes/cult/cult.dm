...
/datum/game_mode/cult/declare_completion()
	if(cult_objs.cult_status == NARSIE_HAS_RISEN)
		...
		to_chat(world, span_danger(span_fontsize_3("The cult wins! It has succeeded in summoning [SSticker.cultdat.entity_name]!")))
	else if(cult_objs.cult_status == NARSIE_HAS_FALLEN)
		SSticker.mode_result = "cult draw - narsie died, nobody wins"
		to_chat(world, span_danger(span_fontsize_3("Nobody wins! [SSticker.cultdat.entity_name] was summoned, but banished!")))
	else
		SSticker.mode_result = "cult loss - staff stopped the cult"
		to_chat(world, span_warning(span_fontsize_3("The staff managed to stop the cult!")))
