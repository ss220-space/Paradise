...
/obj/structure/clockwork/functional/celestial_gateway/examine(mob/user)
	...
	if(isclocker(user) || isobserver(user))
		switch(seconds_until_activation)
			if(-INFINITY to GATEWAY_REEBE_FOUND)
				to_chat(user, "<span class='clocklarge'>The Ark is feeding power into the bluespace field.</span>")
			if(GATEWAY_REEBE_FOUND to GATEWAY_RATVAR_COMING)
				to_chat(user, "<span class='clocklarge'>The field is ripping open a copy of itself in Ratvar's prison.</span>")
			if(GATEWAY_RATVAR_COMING to INFINITY)
				to_chat(user, "<span class='clocklarge'>With the bluespace field established, Ratvar is preparing to come through!</span>")
	else
		switch(seconds_until_activation)
			if(-INFINITY to GATEWAY_REEBE_FOUND)
	...
	switch(seconds_until_activation)
		...
		if(GATEWAY_RATVAR_ARRIVAL to INFINITY)
			if(!purpose_fulfilled)
				...
				sound_to_playing_players(volume = 100, channel = CHANNEL_JUSTICAR_ARK, pressure_affected = FALSE, sound = sound('sound/effects/ratvar_rises.ogg')) //End the sounds
				sleep(12.5 SECONDS)
