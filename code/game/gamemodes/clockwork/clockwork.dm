...
/datum/game_mode/proc/clock_give_item(obj/item/item_path, mob/living/carbon/human/H)
	...
	if(!where)
		to_chat(H, span_userdanger("К несчастью, не получилось выдать [item_name] по техническим причинам. Это очень не хорошо и ты должен сообщить об этом администрации (через F1)."))
		return FALSE
	else
		to_chat(H, span_danger("Теперь у тебя есть [item_name] в [where]."))
		return TRUE
