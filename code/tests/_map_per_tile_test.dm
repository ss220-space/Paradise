 ...
 * Map per-tile test.
 *
 * Per-tile map tests iterate over each tile of a map to perform a check, and fails the test if a tile does not pass the check.
 * A new test can be written by extending /datum/map_per_tile_test, and implementing the check in CheckTile.
 */
