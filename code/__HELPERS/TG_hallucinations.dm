 ...
 */
/proc/visible_hallucination_pulse(atom/center, radius = 7, hallucination_duration = 500, hallucination_max_duration, list/optional_messages)
	for(var/mob/living/nearby_living in view(center, radius))
		...
		nearby_living.Hallucinate(hallucination_duration * dist)
 ...
 * optional_messages - optional list of messages passed. Those affected by pulses will be given one of the messages in said list.
 */
/proc/hallucination_pulse(atom/center, radius = 7, hallucination_duration = 500, hallucination_max_duration, list/optional_messages)
...
/proc/get_random_hallucination(tier = HALLUCINATION_TIER_COMMON, strict = FALSE)
	...
	return pick_weight_classic(pool)
...
/proc/debug_hallucination_weighted_list()
	...
	to_chat(usr, span_boldnotice("Общий вес списка галлюцинаций обладающих весом составляет [total_weight]."))
	return total_weight
...
/proc/select_hallucination_type(mob/user, message = "Выберите подтип галлюцинации", title = "Выберите галлюцинацию")
...
/proc/create_delusion(mob/user)
	...
	var/chosen = tgui_input_list(user, "Выберите тип иллюзии. Пользовательский позволит ввести пользовательскую иконку.", "Выбор иллюзии", delusions)
