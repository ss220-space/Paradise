...
/proc/log_attack(attacker, defender, message, newhp)
	if(config.log_attack)
		rustg_log_write(GLOB.world_game_log, "ATTACK: [attacker] against [defender]: [message] [newhp][GLOB.log_end]") //Seperate attack logs? Why?

/proc/log_conversion(text, mob/converting)
	if(config.log_conversion)
		rustg_log_write(GLOB.world_game_log, "CONVERSION: [converting.simple_info_line()]: [html_decode(text)][GLOB.log_end]")
