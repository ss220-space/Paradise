...
/datum/hud/proc/check_parallax()
	...
	if(rock.displaying_layers)
		ADD_TRAIT(src, TRAIT_PARALLAX_DISPLAYED, GENERIC_TRAIT)
		displaying_client.screen |= rock
	else
		REMOVE_TRAIT(src, TRAIT_PARALLAX_DISPLAYED, GENERIC_TRAIT)
		displaying_client.screen -= rock
...
/datum/hud/proc/apply_parallax_pref()
	...
	if(check_level_trait(screen_location?.z, ZTRAIT_NOPARALLAX))
