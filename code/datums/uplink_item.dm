...
/datum/uplink_item/dangerous/combat_defib
	...
	desc = "Модуль на МЭК. Практически во всём является копией обычного модуля дефибриллятора, за одним лишь исключением — встроенная система защиты \
			была переписана, так что теперь его можно использовать в качестве оружия."
...
/datum/uplink_item/suits/modsuit
	name = "МЭК Синдиката"
	desc = "Знаменитый МЭК, выполненный в зловещих чёрно-красных тонах, обладающий хорошей защитой и практически не стесняющий пользователя в движениях. \
			В комплект также входит оборудование для поддержания жизнедеятельности. Экипаж, при виде данного МЭКа, обычно впадает в панику, так что о скрытном \
			подходе можете не расчитывать."
...
/datum/uplink_item/suits/springlock_module
	...
	desc = "Модуль для МЭК, представляющий из себя серьезную модификацию всего костюма в виде пружинного экзоскелета. \
			При активации раскрывается, значительно ускоряя процесс облачения пользователя в костюм, однако имеет критический \
			недостаток: при воздействии влаги пружинный механизм имеет тенденцию \"защёлкиваться\" в исходное положение, игнорируя положение пользователя. \
			Данная модель позволяет пользователю активировать костюм практически мгновенно, давая пользователю возможность быстро избавиться от своей цели. \
...
/datum/uplink_item/suits/magboot
	...
	desc = "Модуль для МЭК, представляет из себя доработанную версию стандартных магнитных ботинок. Не способен защитить пользователя от смазки, \
			однако предоставляет защиту от гравитации, поскальзывания на воде и мыле и, что самое важное, не замедляет пользователя. Данный модуль \
			Изначально предустановлен в МЭКи Синдиката."
...
/datum/uplink_item/suits/elite_magboot
	...
	desc = "Модуль для МЭК, представляющий из себя украденную технологию магнитных ботинок от \"Киберсан\" Удерживающие магниты последнего поколения \
			позволяют пользователю не скользить на космической смазке, при этом предоставляя те же самые преимущества, что и продвинутые магбутсы. \
			Не забудьте, что вам, для начала, необходимо удалить из вашего МЭКа старые магбутсы с помощью лома."
...
/datum/uplink_item/suits/chameleon_module
	...
	desc = "Модуль для МЭК, использует технологию хамелеон для маскировки выключенного МЭКа под обычный рюкзак. Внимание: модуль не будет работать \
			в случае, если МЭК запущен."
	item = /obj/item/mod/module/chameleon
	cost = 10

/datum/uplink_item/suits/plate_compression
	name = "Модуль уплотнения"
	desc = "Модуль для МЭК, позволяющий сжать блок управления МЭК до достаточно малых размеров, чтобы он поместился в рюкзак. Модуль несовместим со всеми \
			модулями хранения, поэтому вам сначала придётся их удалить."
...
/datum/uplink_item/suits/modsuit_elite
	...
	desc = "МЭК, выполненный в зловещих чёрных тонах. Продвинутая версия МЭК Синдиката. Практически точная копия зловещих костюмов отряда \"АТОМ\" \
			за исключением того, что модули внутри костюма гораздо хуже того, что предоставляется оперативникам. Чрезвычайно крепкий и *чрезвычайно* дорогой. \
			Члены экипажа при виде данного МЭКа впадает в сильную панику, так что не удивляйтесь, если за вами начнет вести охоту вся станция."
	item = /obj/item/mod/control/pre_equipped/traitor_elite
	cost = 60 //45 to start, no holopara / ebow.
	surplus = 60
	excludefrom = list(UPLINK_TYPE_NUCLEAR, UPLINK_TYPE_SST)

/datum/uplink_item/suits/chameleon_hardsuit //temp, more modules incoming
	name = "ИКС-\"Хамелеон\""
	desc = "Скафандр, оснащённый технологией \"Хамелеон\". В комплект также входит оборудование для поддержания жизнедеятельности. \
			...
			инженерный, медицинский, шахтёрский, службы безопасности, а также под стандартный ВКД. Маскировка *КРАЙНЕ* неэффективна \
			в виду того, что данные ИКСы практически нигде более не встречаются."
	cost = 46
...
/datum/uplink_item/suits/elite_mod
	...
	desc = "Легендарный МЭК, выполненный в зловещих чёрных тонах. Продвинутая версия МЭК Синдиката. Предоставляет превосходную защиту от всех угроз, \
			при этом не замедляя пользователя. Двухрежимная система старых ИКСов была полностью убрана, в связи с чем необходимость жонглировать \
			режимами в бою полностью отпала. Костюм обладает повышенной модульной вместимостью, позволяя настроить костюм под предпочтения пользователя."
...
/datum/uplink_item/suits/shielded
	...
	desc = "Модуль персонального защитного силового поля для МЭК, который способен блокировать любые атаки, как в ближнем, так и в дальнем бою. \
			Щит имеет три заряда и со временем восстанавливается, но из-за малого количества зарядов носитель всё ещё может внезапно оказаться смертен."
