...
/datum/uplink_item/implants/warp
	...
	При активации откатывает местоположение пользователя на 125 метров назад.\
	Примечание: Вопреки распространённому мнению, имплант не вызывает киберпсихоза. Данная информация распространяется дезинформаторами \"Нанотрейзен\"."
