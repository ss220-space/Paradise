	...
	if((U.loc in user.contents || (in_range(U.loc, user) && istype(U.loc.loc, /turf))))
		...
		if(I)
			if(ishuman(user))
				...
				if(limited_stock > 0)
					add_game_logs("[key_name(user)] purchased [name]. [name] was discounted to [cost].", user)
					if(!user.mind.special_role)
						message_admins("[key_name_admin(user)] purchased [name] (discounted to [cost]), as a non antagonist.")
				else
					add_game_logs("[key_name(user)] purchased [name].", user)
					if(!user.mind.special_role)
...
/datum/uplink_item/bundles_TC/surplus_crate/spawn_item(turf/loc, obj/item/uplink/U)
	...
	add_game_logs("[key_name(usr)] purchased a surplus crate with [jointext(itemlog, ", ")]", usr)
