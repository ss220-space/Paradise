...
/datum/ai_controller/proc/ai_interact(target, intent = INTENT_HELP, list/modifiers)
	if(!ai_can_interact())
		return FALSE
	...
	var/old_intent = living_pawn.intent
	living_pawn.set_intent(intent)
	living_pawn.ClickOn(final_target, params)
	living_pawn.set_intent(old_intent)
	return TRUE
...
#define TRACK_AI_DATUM_TARGET(tracked_datum, key) do { \
	if(isweakref(tracked_datum)) { \
		...
		else if(!HAS_TRAIT_FROM(_tracked_datum, TRAIT_AI_TRACKING, "[src.UID()]_[key]")) { \
			RegisterSignal(_tracked_datum, COMSIG_QDELETING, PROC_REF(sig_remove_from_blackboard), override = TRUE); \
			ADD_TRAIT(_tracked_datum, TRAIT_AI_TRACKING, "[src.UID()]_[key]"); \
		}; \
	}; \
} while(FALSE)
...
#define CLEAR_AI_DATUM_TARGET(tracked_datum, key) do { \
	if(isdatum(tracked_datum)) { \
		var/datum/_tracked_datum = tracked_datum; \
		REMOVE_TRAIT(_tracked_datum, TRAIT_AI_TRACKING, "[src.UID()]_[key]"); \
