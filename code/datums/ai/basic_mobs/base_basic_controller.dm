...
/datum/ai_controller/basic_controller/TryPossessPawn(atom/new_pawn)
	...
	RegisterSignal(basic_mob, list(POST_BASIC_MOB_UPDATE_VARSPEED, COMSIG_MOB_MOVESPEED_UPDATED), PROC_REF(update_speed))
...
/datum/ai_controller/basic_controller/setup_able_to_run()
	...
	if(ai_traits & PAUSE_DURING_DO_AFTER)
		RegisterSignal(pawn, list(COMSIG_DO_AFTER_BEGAN, COMSIG_DO_AFTER_ENDED), PROC_REF(update_able_to_run))
