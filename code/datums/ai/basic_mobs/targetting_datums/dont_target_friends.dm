...
/datum/targetting_datum/basic/not_friends/can_attack(mob/living/living_mob, atom/target, vision_range)
	if(attack_unsim_turf && (iswallturf(target) || ismineralturf(target)))
		return TRUE

	if(attack_obj && isobj(target))
		var/obj/target_object = target
		if(!(target_object.resistance_flags & INDESTRUCTIBLE))
			return TRUE
