...
/datum/targeting_strategy/basic/not_friends
	/// If we can try to wall turfs or not
	var/attack_wall_turf = FALSE
...
/datum/targeting_strategy/basic/not_friends/can_attack(mob/living/living_mob, atom/target, vision_range)
	if(attack_wall_turf && iswallturf(target))
		return TRUE
...
/datum/targeting_strategy/basic/not_friends/attack_wall_turfs
	attack_wall_turf = TRUE
