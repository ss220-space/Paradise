...
/datum/ai_behavior/break_spine/perform(seconds_per_tick, datum/ai_controller/controller, target_key)
	...
	batman.visible_message(span_warning("[batman] gets a slightly too tight hug from [big_guy]!"),\
	span_userdanger("You feel your body break as [big_guy] embraces you!"))

	for(var/zone in GLOB.body_zones - BODY_ZONE_HEAD)
		batman.apply_damage(15, BRUTE, zone)
...
/datum/ai_behavior/give/proc/try_to_give_item(datum/ai_controller/controller, mob/living/target, obj/item/held_item, actually_give)
	...
	var/has_left_pocket = target.can_equip(held_item, ITEM_SLOT_POCKET_LEFT)
	var/has_right_pocket = target.can_equip(held_item, ITEM_SLOT_POCKET_RIGHT)
	var/has_valid_hand

	if(target.can_put_in_hands(held_item))
		has_valid_hand = TRUE
	...
	if(!has_valid_hand || prob(50))
		target.equip_to_slot_if_possible(held_item, (!has_left_pocket ? ITEM_SLOT_POCKET_RIGHT : (prob(50) ? ITEM_SLOT_POCKET_LEFT : ITEM_SLOT_POCKET_RIGHT)))
	else
...
/datum/ai_behavior/consume/perform(seconds_per_tick, datum/ai_controller/controller, target_key, hunger_timer_key)
	...
	if(QDELETED(target) || !living_pawn.is_in_hands(target))
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_FAILED

	controller.ai_interact(target = living_pawn, intent = INTENT_HELP)
...
/datum/ai_behavior/consume/finish_action(datum/ai_controller/controller, succeeded, target_key, hunger_timer_key)
	...
	if(!QDELETED(target) && !DOING_INTERACTION_WITH_TARGET(living_pawn, target))
		controller.clear_blackboard_key(target_key)
		living_pawn.drop_item_ground(target) // drops empty drink glasses

	for(var/obj/item/trash/trash in living_pawn.get_held_items())
		living_pawn.drop_item_ground(trash) // drops spawned trash items
...
/datum/ai_behavior/navigate_to_and_pick_up/setup(datum/ai_controller/controller, target_key, drop_held = TRUE) // Похуй потом
	. = ..()
	set_movement_target(controller, controller.blackboard[target_key])
...
/datum/ai_behavior/navigate_to_and_pick_up/setup(datum/ai_controller/controller, target_key, drop_held = TRUE)
	...
	if(living_pawn.is_in_hands(target)) // already in hands
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_SUCCEEDED
	if(!target.IsReachableBy(living_pawn)) // can't reach it, despite being adjacent
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_FAILED
	if(living_pawn.get_active_hand()) // something is in our hands already
		if(!drop_held || !living_pawn.drop_item_ground(living_pawn.get_active_hand()))
			return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_FAILED
...
/datum/ai_behavior/drop_item/perform(seconds_per_tick, datum/ai_controller/controller)
	var/mob/living/living_pawn = controller.pawn
	var/list/my_held_items = living_pawn.get_held_items() - GetBestWeapon(controller, null, living_pawn.get_held_items())
	if(!length(my_held_items))
		return AI_BEHAVIOR_FAILED | AI_BEHAVIOR_DELAY
	living_pawn.drop_item_ground(pick(my_held_items))
...
/datum/ai_behavior/perform_speech_radio/perform(seconds_per_tick, datum/ai_controller/controller, speech, obj/item/radio/speech_radio, list/try_channels = list(PUB_FREQ_NAME))
	var/mob/living/living_pawn = controller.pawn
