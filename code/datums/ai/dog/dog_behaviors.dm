...
/datum/ai_behavior/simple_equip/proc/pickup_item(datum/ai_controller/controller, obj/item/target)
	...
	pawn.visible_message(span_notice("[pawn] берет [target.declent_ru(ACCUSATIVE)] в свою пасть."))
...
/datum/ai_behavior/deliver_item/proc/deliver_item(datum/ai_controller/controller)
	...
	if(ismob(return_target))
		controller.pawn.visible_message(span_notice("[controller.pawn] доставляет [carried_item.declent_ru(ACCUSATIVE)] к ногам [return_target]."))
	else // not sure how to best phrase this
		controller.pawn.visible_message(span_notice("[controller.pawn] доставляет [carried_item.declent_ru(ACCUSATIVE)] к [return_target]."))
...
/datum/ai_behavior/play_dead/finish_action(datum/ai_controller/controller, succeeded)
	...
	simple_pawn.visible_message(span_notice("[simple_pawn] резко вскакивает на ноги, взволнованно пыхтя!")) //я не знаю как на русском по-умному будет по-умному эта отдышка у собак
