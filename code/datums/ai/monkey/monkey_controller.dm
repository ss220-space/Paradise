...
/datum/ai_controller/monkey/UnpossessPawn()
	UnregisterSignal(pawn, list(COMSIG_PARENT_ATTACKBY, COMSIG_ATOM_ATTACK_HAND, COMSIG_ATOM_ATTACK_PAW, COMSIG_ATOM_BULLET_ACT, COMSIG_ATOM_HITBY, COMSIG_MOVABLE_CROSS, COMSIG_LIVING_START_PULL,\
...
/datum/ai_controller/monkey/proc/TryFindWeapon()
	...
	if(W && !HAS_TRAIT(W, TRAIT_NODROP) && !blackboard[BB_MONKEY_BLACKLISTITEMS][W] && W.force > blackboard[BB_MONKEY_BEST_FORCE_FOUND])
		...
		if(H)
			W = pick(list(H.get_active_hand(), H.get_inactive_hand()))
...
/datum/ai_controller/monkey/proc/on_Crossed(datum/source, atom/movable/AM)
	var/mob/living/living_pawn = pawn
	if(!living_pawn.incapacitated() && ismob(AM) && (!is_monkeybasic(AM) || prob(15)))
