...
/datum/idle_behavior/idle_dog/perform_idle_behavior(delta_time, datum/ai_controller/dog/controller)
	...
	if(controller.blackboard[BB_SIMPLE_CARRY_ITEM] && SPT_PROB(5, delta_time))
		...
		living_pawn.visible_message(span_notice("[living_pawn.declent_ru_cap(NOMINATIVE)] мягко впивается зубами в [carry_item.declent_ru(ACCUSATIVE)] в [GEND_HIS_HER(living_pawn)] пасти."))
