ADMIN_VERB(station_traits_panel, R_EVENT, "Modify Station Traits", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_EVENTS)
...
/datum/station_traits_panel/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("revert")
			...
			var/datum/station_trait/station_trait = locateUID(uid)

			if(!istype(station_trait))
				return TRUE

			if(too_late_to_revert())
				to_chat(usr, span_warning("Раунд уже начался, ивенты нельзя изменить!"))
				return TRUE
		...
		if("setup_future_traits")
			if(too_late_for_future_traits())
				to_chat(usr, span_warning("Слишком поздно назначать трейты для следующего раунда!"))
				return TRUE
			...
			rustlib_file_write(json_encode(params["station_traits"]), FUTURE_STATION_TRAITS_FILE)

			return TRUE

		if("clear_future_traits")
			if(!future_traits)
				to_chat(usr, span_warning("Трейты станции для следующего раунда не заданы."))
				return TRUE
