...
/datum/map_generator/cave_generator/generate_terrain(list/turfs, area/generate_in)
	...
	if(!(generate_in.generate_caves))
		return
	var/start_time = REALTIMEOFDAY
	string_gen = rustg_cnoise_generate("[initial_closed_chance]", "[smoothing_iterations]", "[birth_limit]", "[death_limit]", "[world.maxx]", "[world.maxy]") //Generate the raw CA data

	// Area var pullouts to make accessing in the loop faster
	var/flora_allowed = (generate_in.generate_flora) && length(flora_spawn_list)
	var/feature_allowed = (generate_in.generate_flora) && length(feature_spawn_list)
	var/mobs_allowed = (generate_in.generate_fauna) && length(mob_spawn_list)
	var/megas_allowed = (generate_in.generate_megafauna) && length(megafauna_spawn_list)
	...
	for(var/i in turfs) //Go through all the turfs and generate them
		...
		if(mobs_allowed && !spawned_something && prob(mob_spawn_chance))
			...
			if(ispath(picked_mob, /obj/structure/spawner/lavaland))
				for(var/obj/structure/spawner/lavaland/spawn_blocker in range(4, new_turf))
					can_spawn = FALSE
					break
