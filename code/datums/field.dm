...
/*
/obj/effect/abstract/proximity_checker/on_receiver_enter(turf/source, atom/movable/entered, turf/old_loc)
	. = ..()
	if(get_dist(source, parent) == radius)
		field_edge_crossed(entered, old_loc, source)
	else
		field_turf_crossed(entered, old_loc, source)
*/

/datum/component/proximity_monitor/advanced/on_receiver_move(atom/movable/movable, atom/old_loc)
	. = ..()
	/*
	if(ignore_if_not_on_turf)
		//Early return if it's not the parent that has moved.
		if(movable != parent)
			return
		//Cleanup the field if the parent was on a turf but isn't anymore.
...
/datum/component/proximity_monitor/advanced/cheker_initialized(obj/effect/abstract/proximity_checker/cheker, turf/location, atom/created, init_flags)
	cheker.on_entered(location, created, null)


/*
/datum/component/proximity_monitor/advanced/on_uncrossed(turf/source, atom/movable/gone, direction)
	if(get_dist(source, parent) == radius)
...
/datum/component/proximity_monitor/advanced/proc/update_new_turfs()
	//if(ignore_if_not_on_turf && !isturf(parent.loc))
	...
	if(radius > 0)
		local_field_turfs += RANGE_TURFS(radius - 1, center)

	if(radius > 1)
		local_edge_turfs = RANGE_TURFS(radius, center) - local_field_turfs
...
/datum/component/proximity_monitor/advanced/proc/get_edgeturf_direction(turf/T, turf/center_override = null)
	...
	if(((T.x == (checking_from.x + radius)) || (T.x == (checking_from.x - radius))) && ((T.y == (checking_from.y + radius)) || (T.y == (checking_from.y - radius))))
		return get_dir(checking_from, T)

	if(T.x == (checking_from.x + radius))
		return EAST

	if(T.x == (checking_from.x - radius))
		return WEST

	if(T.y == (checking_from.y - radius))
		return SOUTH
...
// (DEBUG FIELD ITEM removed during port — master220 already defines /obj/item/multitool/field_debug
//  for its native /datum/proximity_monitor system, so it would collide.)
