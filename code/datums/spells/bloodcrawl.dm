...
/obj/effect/proc_holder/spell/bloodcrawl/proc/sink_animation(atom/enter_point, mob/living/user)
	...
	visible_message(span_danger("[user] погружается в [enter_point.declent_ru(ACCUSATIVE)]."))
...
/obj/effect/proc_holder/spell/bloodcrawl/proc/handle_consumption(mob/living/user, mob/living/victim, atom/enter_point, obj/effect/dummy/slaughter/holder)
	...
	if(victim.stat == CONSCIOUS)
		enter_point.visible_message(span_warning("[victim] вырывается из [enter_point.declent_ru(GENITIVE)] в последний момент!"))
		user.stop_pulling()
		return

	victim.emote("scream")
	victim.forceMove(holder)
	enter_point.visible_message(span_warning("<b>[user] затягивает [victim] в [enter_point.declent_ru(ACCUSATIVE)]!</b>"))
