/obj/effect/proc_holder/spell/aoe/knock
	name = "Knock"
	desc = "Данное заклинание открывает ближайшие двери. Для него не требуется роба."
...
/obj/effect/proc_holder/spell/aoe/knock/greater
	name = "Greater Knock"
	desc = "При первом прочтении снимет ограничения на доступ ко всем шлюзам на станции и объявит о прочтении этого заклинания. При любом последующем прочтении откроет все двери в поле зрения. Нельзя вернуть после покупки!"
...
/obj/effect/proc_holder/spell/aoe/knock/greater/cast(list/targets, mob/user = usr)
	if(!used)
		...
		GLOB.command_announcement.Announce(
			message = "Мы убрали все доступы у шлюзов вашей станции. Вы можете поблагодарить нас позже!",
