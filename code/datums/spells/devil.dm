...
/obj/effect/proc_holder/spell/summon_contract/cast(list/targets, mob/user = usr)
	for(var/target in targets)
		...
		if(!C.mind || !user.mind)
			to_chat(user, span_notice("[C.declent_ru_cap(NOMINATIVE)] не выглядит разумным и не сможет подписать контракт."))
			continue
...
/mob/living/proc/infernalphaseout(obj/effect/proc_holder/spell/infernal_jaunt/spell)
	...
	visible_message(span_warning("[declent_ru_cap(NOMINATIVE)] исчезает в огненной вспышке!"))
...
/mob/living/proc/infernalphasein(obj/effect/proc_holder/spell/infernal_jaunt/spell)
	...
	visible_message(span_warning("<b>[declent_ru_cap(NOMINATIVE)] появляется в огненной вспышке!</b>"))
