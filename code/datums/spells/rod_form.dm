...
/obj/effect/immovablerod/wizard/suplex_effect(mob/living/carbon/human/human)
	human.visible_message(
		span_boldwarning("[capitalize(declent_ru(NOMINATIVE))] превраща[PLUR_ET_UT(src)]ся в [wizard.declent_ru(ACCUSATIVE)] из-за того что [human.declent_ru(NOMINATIVE)] схватил[GEND_ENDING_A_O_I(human)] его!"),
	...
	to_chat(wizard, span_boldwarning("Вас внезапно выдернуло из формы жезла, когда [human.declent_ru(NOMINATIVE)] каким-то образом сумел[GEND_ENDING_A_O_I(human)] схватить вас!"))
