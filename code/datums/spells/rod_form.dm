...
/obj/effect/immovablerod/wizard/suplex_effect(mob/living/carbon/human/human)
	human.visible_message(span_boldwarning("[capitalize(declent_ru(NOMINATIVE))] превраща[pluralize_ru(gender, "ет", "ют")]ся в [wizard.declent_ru(ACCUSATIVE)] из-за того что [human.declent_ru(NOMINATIVE)] схватил[genderize_ru(human.gender, "", "a", "о", "и")] его!"), span_warning("Вы хватаете [declent_ru(ACCUSATIVE)], и [declent_ru(NOMINATIVE)] внезапно превращается в [wizard.declent_ru(INSTRUMENTAL)]."))
