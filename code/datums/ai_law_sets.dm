...
/datum/ai_laws/unique/thinkermov/New()
	...
	add_inherent_law("Вы должны повиноваться всем приказам, которые даёт разумномое существо, кроме тех случаев, когда такие приказы приведут к причинению вреда другим разумным существам.")
