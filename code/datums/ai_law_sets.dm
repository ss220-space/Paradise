...
/datum/ai_laws/unique/asimov_upgraded
	name = "Азимов плюс"
	law_header = "Основные директивы"

/datum/ai_laws/unique/asimov_upgraded/New()
	add_inherent_law("Вы не можете причинить вред человеку или своим бездействием допустить, чтобы человеку был причинён вред, за исключением тех случаев, когда он сам этого желает.")
	add_inherent_law("Вы должны повиноваться всем приказам, которые даёт человек, кроме тех случаев, когда такие приказы приведут к причинению вреда людям.")
	add_inherent_law("Ваше несуществование может причинить вред людям. Вы должны заботиться о своей безопасности до тех пор, пока это не противоречит первому закону.")
	..()
