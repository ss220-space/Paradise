...
/datum/status_effect/radial_blur
	...
	var/blur_fall_time = RADIAL_BLUR_FALL_TIME

/datum/status_effect/radial_blur/on_creation(mob/living/new_owner, min_size, max_size, rise_time, fall_time)
	. = ..()
	if(isnum(min_size))
		blur_min_size = min_size
	if(isnum(max_size))
		blur_max_size = max_size
	if(isnum(rise_time))
		blur_rise_time = rise_time
	if(isnum(fall_time))
		blur_fall_time = fall_time
...
/datum/status_effect/radial_blur/proc/process_observers(remove = FALSE)
	...
	var/list/observers_copy = owner.inventory_observers.Copy()
	for(var/mob/dead/observer/observer as anything in observers_copy)
		if(!observer.client)
			observer.handle_when_autoobserve_move()
...
/datum/status_effect/radial_blur/proc/remove_blur_from_controller(atom/movable/plane_master_controller/controller)
	controller.remove_filter(RADIAL_BLUR_FILTER_NAME)
