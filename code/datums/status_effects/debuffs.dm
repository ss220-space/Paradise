...
/datum/status_effect/transient/jittery/get_examine_text()
	switch(strength)
		if(600 SECONDS to INFINITY)
			return span_warning("<b>[GEND_HE_SHE_CAP(owner)] бь[pluralize_ru(owner.gender,"ётся","ются")] в судорогах!</b>")
		...
		if(200 SECONDS to 400 SECONDS)
			return span_warning("[GEND_HE_SHE_CAP(owner)] слегка дёрга[pluralize_ru(owner.gender,"ется","ются")].")
