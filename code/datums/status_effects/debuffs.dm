...
/datum/status_effect/transient/jittery/get_examine_text()
	switch(strength)
		if(600 SECONDS to INFINITY)
			return span_warning("<b>[owner.p_they(TRUE)] [owner.p_are()] бьётся в судорогах!</b>")
		if(400 SECONDS to 600 SECONDS)
			return span_warning("[owner.p_they(TRUE)] [owner.p_are()] крайне нервничает.")
		if(200 SECONDS to 400 SECONDS)
			return span_warning("[owner.p_they(TRUE)] [owner.p_are()] слегка дёргается.")
...
/datum/status_effect/bubblegum_curse/proc/get_over_here()
	...
	to_chat(owner, span_danger("[attacker] разрывает вас!"))
	...
	sleep(8)
	to_chat(owner, span_danger("[attacker] тащит вас по крови!"))
