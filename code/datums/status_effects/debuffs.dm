...
/datum/status_effect/transient/jittery/get_examine_text()
	switch(strength)
		if(600 SECONDS to INFINITY)
			return span_warning("<b>[GEND_HE_SHE_CAP(owner)] бьётся в судорогах!</b>")
		if(400 SECONDS to 600 SECONDS)
			return span_warning("[GEND_HE_SHE_CAP(owner)] крайне нервничает.")
		if(200 SECONDS to 400 SECONDS)
			return span_warning("[GEND_HE_SHE_CAP(owner)] слегка дёргается.")
