...
/datum/status_effect/stun_absorption/get_examine_text()
	if(can_absorb_stun())
		return replacetext(examine_message, "%EFFECT_OWNER_THEYRE", genderize_ru(owner.gender, "его", "её", "его", "их"))
 ...
 *
 * When we get stunned, we will try to absorb a number of seconds from the stun, and return [COMPONENT_NO_EFFECT] if we succeed.
 */
...
/**
 * Signal proc for [COMSIG_LIVING_GENERIC_INCAPACITATE_CHECK]. (Note, this includes being stamcrit)
 *
