...
/datum/chatmessage/proc/finish_image_generation(mheight, atom/movable/source, mob/owner, complete_text, lifespan)
	...
	var/starting_height = source.maptext_height
	...
	if(owned_by.seen_messages)
		...
		for(var/datum/chatmessage/m as anything in owned_by.seen_messages[source])
			...
			if(time_spent >= lifespan_until_fade)
			...
			var/remaining_time = lifespan_until_fade * (CHAT_MESSAGE_EXP_DECAY ** idx++) * (CHAT_MESSAGE_HEIGHT_DECAY ** combined_height)
	...
	message.pixel_x = -source.base_pixel_x
	...
	// View the message
	LAZYADDASSOCLIST(owned_by.seen_messages, source, src)
