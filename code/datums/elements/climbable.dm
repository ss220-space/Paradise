...
/datum/element/climbable/proc/can_climb(atom/source, mob/user)
	...
	if(source.flags & ON_BORDER && user.loc != source.loc && (dir_step & source.dir) == source.dir)
		return FALSE
	return TRUE
...
/datum/element/climbable/proc/attack_hand(atom/climbed_thing, mob/user)
	...
	for(var/mob/living/structure_climber as anything in climbers)
		if(structure_climber == user)
			return
		user.changeNext_move(CLICK_CD_MELEE)
		user.do_attack_animation(climbed_thing)
		structure_climber.Paralyse(4 SECONDS)
	...
	*/
	...
	if(do_after(user, adjusted_climb_time, climbed_thing))
		...
		if(do_climb(climbed_thing, user, params))
			...
			var/obj/structure/structure_target = climbed_thing
			if(istype(structure_target))
				structure_target.clumse_stuff(user)
...
/datum/element/climbable/proc/do_climb(atom/climbed_thing, mob/living/user, params)
	...
	if(climbed_thing.flags & ON_BORDER && (same_loc || !(dir_step & REVERSE_DIR(climbed_thing.dir))))
