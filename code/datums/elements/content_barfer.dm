...
/datum/element/content_barfer/Attach(datum/target)
	...
	RegisterSignal(target, list(COMSIG_LIVING_DEATH, COMSIG_MOB_CHANGED_TYPE), PROC_REF(barf_contents))

/datum/element/content_barfer/Detach(datum/target)
	UnregisterSignal(target, list(COMSIG_LIVING_DEATH, COMSIG_MOB_CHANGED_TYPE))
	return ..()
