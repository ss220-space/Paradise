...
/datum/strippable_item/proc/start_equip(atom/source, obj/item/equipping, mob/user)
	source.visible_message(
		span_notice("[user] пытается надеть [equipping.declent_ru(ACCUSATIVE)] на [source.declent_ru(ACCUSATIVE)]."),
		span_notice("[user] пытается надеть на вас [equipping.declent_ru(ACCUSATIVE)]."),
	)
	if(ishuman(source))
...
/datum/strippable_item/proc/start_unequip(atom/source, mob/user)
	...
	if(!in_thief_mode(user))
		source.visible_message(
			span_warning("[user] пытается снять [item.declent_ru(ACCUSATIVE)] с [source.declent_ru(GENITIVE)]."),
			span_userdanger("[user] пытается снять с вас [item.declent_ru(ACCUSATIVE)]!"),
			"Слышно шуршание."
		)
