...
/datum/element/material_heal/proc/self_stack_heal(obj/item/stack/item, obj/item/organ/external/bodypart, mob/living/user, params)
	...
	user.visible_message(
		span_notice("[user] начинает лечить свои раны на [GEND_HIS_HER(user)] [bodypart.declent_ru(PREPOSITIONAL)], используя [item.declent_ru(ACCUSATIVE)]."),
		ignored_mobs = user
	)
...
/datum/element/material_heal/proc/heal_message(mob/living/carbon/target, mob/living/user, obj/item/organ/external/bodypart, obj/item/item)
	if(user == target)
		user.visible_message(span_green("[user] залечивает раны на [GEND_HIS_HER(target)] [bodypart.declent_ru(PREPOSITIONAL)], используя [item.declent_ru(ACCUSATIVE)]."), \
							ignored_mobs = user)
	else
		user.visible_message(span_green("[user] залечивает раны [target] на [GEND_HIS_HER(target)] [bodypart.declent_ru(PREPOSITIONAL)], используя [item.declent_ru(ACCUSATIVE)]."), \
							ignored_mobs = user)
