...
/datum/element/swimming_tile/Attach(turf/target, stamina_entry_cost = 7, ticking_stamina_cost = 5, ticking_oxy_damage = 2, exhaust_swimmer_prob = 30)
	...
	for(var/mob/living/drownee in target.contents)
		if(!(drownee.flags & INITIALIZED)) //turfs initialize before movables
			continue
...
/datum/element/swimming_tile/proc/dip_in(mob/living/floater)
	...
	var/effective_stamina_entry_cost = floater.get_strength_level_number() > (/datum/strength_level/normal::level_num) ? (stamina_entry_cost + clothing_weight(floater)) : ((stamina_entry_cost + clothing_weight(floater)) / 2)
...
/datum/status_effect/swimming/tick(seconds_between_ticks)
	...
	if(under_pressure)
		owner.AdjustLoseBreath(oxygen_per_interval)
