...
/datum/element/venomous/proc/do_venom(datum/element_owner, atom/venom_source, mob/living/target, hit_zone, throw_hit)
	...
	if(isliving(element_owner) && !target.can_inject(user = element_owner, target_zone = hit_zone/*, injection_flags = injection_flags*/))
		return
	...
	tmp_holder.trans_to(target, tmp_holder.total_volume)
	qdel(tmp_holder)
