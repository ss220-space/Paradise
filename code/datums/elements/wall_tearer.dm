...
/datum/element/wall_tearer/Attach(datum/target, allow_reinforced = TRUE, tear_time = 2 SECONDS, reinforced_multiplier = 2, do_after_key = null)
	...
	RegisterSignal(target, list(COMSIG_ATOM_ATTACK_ANIMAL, COMSIG_LIVING_UNARMED_ATTACK), PROC_REF(on_attacked_wall))


/datum/element/wall_tearer/Detach(datum/source)
	. = ..()
	UnregisterSignal(source, list(COMSIG_ATOM_ATTACK_ANIMAL, COMSIG_LIVING_UNARMED_ATTACK))
