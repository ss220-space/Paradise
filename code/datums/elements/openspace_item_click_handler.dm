...
/datum/element/openspace_item_click_handler/proc/on_afterattack(obj/item/source, atom/target, mob/user, proximity_flag, list/modifiers)
	SIGNAL_HANDLER
	if(target.z == user.z)
		return
	...
	while(!isnull(checked_turf))
		...
		if(checked_turf?.z == user.z)
			INVOKE_ASYNC(source, TYPE_PROC_REF(/obj/item, handle_openspace_click), checked_turf, user, user.Adjacent(checked_turf), modifiers)
			break
