...
/datum/element/rusted_weapon/proc/after_process_fire(datum/source, atom/target, mob/living/user)
	...
	to_chat(user, span_userdanger("[gun.declent_ru_cap(NOMINATIVE)] взрывается прямо у вас перед лицом!"))
...
/datum/element/rusted_weapon/proc/destroy_gun(obj/item/gun/gun, mob/living/user)
	...
	if(user.tkgrabbed_objects[gun])
		...
		to_chat(user, span_userdanger("БА-БАХ! [gun.declent_ru_cap(NOMINATIVE)] взрывается!"))
	else
		user.take_organ_damage(0, 30)
		user.flash_eyes()
		to_chat(user, span_userdanger("БА-БАХ! [gun.declent_ru_cap(NOMINATIVE)] взрывается у вас в руках!"))
