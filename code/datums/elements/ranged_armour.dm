...
/datum/element/ranged_armour/proc/pre_bullet_impact(atom/parent, obj/projectile/bullet)
	...
	if(below_projectile_multiplier == 0)
		parent.visible_message(span_danger("[parent.declent_ru(NOMINATIVE)] не получает повреждений от [bullet.declent_ru(GENITIVE)]!"))
		bullet.damage = 0
		return
	bullet.damage *= below_projectile_multiplier
	parent.visible_message(span_danger("[parent.declent_ru(NOMINATIVE)] защищен[genderize_ru(parent.gender, "", "а", "о", "ы")] от [bullet.declent_ru(GENITIVE)]!"))
...
/datum/element/ranged_armour/proc/pre_thrown_impact(atom/parent, obj/item/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(hit_atom.throwforce >= minimum_thrown_force)
		return
	parent.visible_message(span_danger("[hit_atom.declent_ru(NOMINATIVE)] [throw_blocked_message] [parent.declent_ru(GENITIVE)]!")) /// "копье отскакивает от голиафа"
