...
/datum/element/noticable_organ/proc/should_show_text(mob/living/carbon/examined)
	return !(body_zone && (body_zone in examined.get_covered_body_zones()))
...
/*
	if(examined.ai_controller?.ai_status == AI_STATUS_ON && !examined.dna.species.ai_controlled_species)
		return TRUE
*/
