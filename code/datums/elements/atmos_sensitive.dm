...
/datum/element/atmos_sensitive/proc/react_to_move(datum/source, atom/movable/oldloc, direction, forced)
	...
	var/turf/turf_source = atom_source.loc
	if(istype(turf_source) && !isunsimulatedturf(turf_source) && !turf_source.blocks_air)
		var/turf/new_open = atom_source.loc
...
/turf/simulated/process_exposure()
