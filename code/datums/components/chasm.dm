		...
		if(isliving(dropped_mob))
			...
			if(ishuman(dropped_mob))
				...
				if(jaunter)
					...
					if(!fall_into_chasm)
						chasm.visible_message(span_boldwarning("[dropped_mob.declent_ru_cap(NOMINATIVE)] пада[PLUR_ET_YUT(dropped_mob)] в [chasm.declent_ru(ACCUSATIVE)]!")) //To freak out any bystanders
...
/datum/component/chasm/proc/drop(atom/movable/dropped_thing)
	...
	if(below_turf)
		...
		dropped_thing.visible_message(span_boldwarning("[dropped_thing.declent_ru_cap(NOMINATIVE)] пада[PLUR_ET_YUT(dropped_thing)] в [atom_parent]!"), span_userdanger("[fall_message]"))
		below_turf.visible_message(span_boldwarning("[dropped_thing.declent_ru_cap(NOMINATIVE)] падает сверху!"))
	...
	// send to oblivion
	dropped_thing.visible_message(span_boldwarning("[dropped_thing.declent_ru_cap(NOMINATIVE)] пада[PLUR_ET_YUT(dropped_thing)] в [atom_parent]!"), span_userdanger("[oblivion_message]"))
	if(isliving(dropped_thing))
	...
	if(!dropped_thing.forceMove(storage))
		atom_parent.visible_message(span_boldwarning("[atom_parent.declent_ru_cap(NOMINATIVE)] выплёвывает [dropped_thing.declent_ru(ACCUSATIVE)]!"))
...
/obj/effect/abstract/chasm_storage/proc/on_revive(mob/living/escapee)
	...
	if(ourturf.GetComponent(/datum/component/chasm))
		...
		ourturf.visible_message(span_boldwarning("[escapee.declent_ru_cap(NOMINATIVE)] пробивается сквозь [ourturf.declent_ru(ACCUSATIVE)], выпрыгивая из пропасти под ней!"))
