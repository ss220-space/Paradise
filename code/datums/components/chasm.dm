		...
		if(isliving(dropped_mob))
			...
			if(ishuman(dropped_mob))
				...
				if(jaunter)
					...
					if(!fall_into_chasm)
						chasm.visible_message(span_boldwarning("[capitalize(dropped_mob.declent_ru(NOMINATIVE))] падает в [chasm.declent_ru(ACCUSATIVE)]!")) //To freak out any bystanders
...
/datum/component/chasm/proc/drop(atom/movable/dropped_thing)
	...
	if(below_turf)
		...
		dropped_thing.visible_message(span_boldwarning("[capitalize(dropped_thing.declent_ru(NOMINATIVE))] падает в [atom_parent]!"), span_userdanger("[fall_message]"))
	...
	// send to oblivion
	dropped_thing.visible_message(span_boldwarning("[capitalize(dropped_thing.declent_ru(NOMINATIVE))] падает в [atom_parent]!"), span_userdanger("[oblivion_message]"))
	if(isliving(dropped_thing))
