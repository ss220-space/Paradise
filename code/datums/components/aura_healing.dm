	...
	for(var/mob/living/candidate in (requires_visibility ? view(range, parent) : range(range, parent)))
		...
		if(iscarbon(candidate)) //another if, because porotic parts
			if(ishuman(candidate)) //humans, tajarans...
				...
				candidate.adjustBruteLoss(-brute_heal * seconds_per_tick, updating_health = FALSE, robotic = robot_heal)
				candidate.adjustFireLoss(-burn_heal * seconds_per_tick, updating_health = FALSE, robotic = robot_heal)
			else
