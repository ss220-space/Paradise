...
/datum/component/callouts/proc/callout_picker(mob/user, atom/clicked_atom)
	...
	var/datum/callout_option/selection = show_radial_menu(user, get_turf(clicked_atom), callout_items)
	if(!selection)
		return
	...
	if(voiceline)
		user.say((!isnull(radio_prefix) ? radio_prefix : "") + selection::voiceline)
...
/obj/effect/temp_visual/callout/Initialize(mapload, mob/creator, datum/callout_option/callout, atom/target)
	...
	color = colorize_string(creator.name, 2, 0.9)
	update_appearance()
