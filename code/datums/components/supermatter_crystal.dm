...
/datum/component/supermatter_crystal/proc/blob_hit(datum/source, obj/structure/blob/blob)
	...
	if(blob.get_integrity() > 100)
		...
		blob.visible_message(
			span_danger("\The [blob] strikes at \the [atom_source] and rapidly flashes to ash."),
			span_hear("You hear a loud crack as you are washed with a wave of heat."),
		)
...
/datum/component/supermatter_crystal/proc/animal_hit(datum/source, mob/living/simple_animal/user, list/modifiers)
	...
	if(!user.melee_damage_upper && !user.melee_damage_lower)
		murder = user.friendly
	else
		murder = user.attacktext
...
/datum/component/supermatter_crystal/proc/unarmed_hit(datum/source, mob/user, list/modifiers)
	...
	if(isliving(user))
		...
		if(living_mob.incorporeal_move || HAS_TRAIT(living_mob, TRAIT_GODMODE))
			return
	var/atom/atom_source = source
	if(isrobot(user) && atom_source.Adjacent(user))
...
/datum/component/supermatter_crystal/proc/hand_hit(datum/source, mob/living/user, list/modifiers)
	...
	var/obj/item/organ/external/head/forehead = user.get_bodypart(BODY_ZONE_HEAD)
	if(forehead)
		dust_mob(source, user,
...
/datum/component/supermatter_crystal/proc/attackby_hit(datum/source, obj/item/item, mob/living/user, params)
	...
	if(istype(item, /obj/item/clothing/mask/cigarette))
		...
		if(clumsy)
			var/obj/item/organ/external/dust_arm = user.get_active_hand()
			dust_arm.droplimb()
			user.visible_message(
				span_danger("The [item] flashes out of existence on contact with \the [atom_source], resonating with a horrible sound..."),
		...
		if(cig.lit || user.a_intent == INTENT_HARM)
			user.visible_message(
				span_danger("A hideous sound echoes as [item] is ashed out on contact with \the [atom_source]. That didn't seem like a good idea..."),
			)
			playsound(atom_source, 'sound/effects/supermatter.ogg', 150, TRUE)
			...
			user.visible_message(
				...
				"[span_danger("Time seems to slow to a crawl as you touch \the [atom_source] with \the [item].")]\n[span_notice("\The [item] flashes alight with an eerie energy as you nonchalantly lift your hand away from \the [atom_source]. Damn.")]",
			)
	...
	if(user.drop_item_ground(item))
		user.visible_message(
			span_danger("As [user] touches \the [atom_source] with \a [item], silence fills the room..."),
			"[span_userdanger("You touch \the [atom_source] with \the [item], and everything suddenly goes silent.")]\n[span_notice("\The [item] flashes into dust as you flinch away from \the [atom_source].")]",
	...
	if(atom_source.Adjacent(user)) //if the item is stuck to the person, kill the person too instead of eating just the item.
		if(user.incorporeal_move || HAS_TRAIT(user, TRAIT_GODMODE))
			return
		dust_mob(source, user,
			span_danger("[user] reaches out and touches [atom_source] with [item], inducing a resonance... [item] starts to glow briefly before the light continues up to [user]'s body. [capitalize(user.p_they())] burst[user.p_s()] into flames before flashing into dust!"),
...
/datum/component/supermatter_crystal/proc/bumped_hit(datum/source, atom/movable/hit_object)
	...
	if(isliving(hit_object))
		hit_object.visible_message(
			span_danger("\The [hit_object] slams into \the [atom_source] inducing a resonance... [hit_object.p_their()] body starts to glow and burst into flames before flashing into dust!"),
		...
		hit_object.visible_message(
			span_danger("\The [hit_object] smacks into \the [atom_source] and rapidly flashes to ash."),
			null,
			span_hear("You hear a loud crack as you are washed with a wave of heat."),
		)
	else
...
/datum/component/supermatter_crystal/proc/on_z_impact(datum/source, turf/impacted_turf, levels)
	...
	for(var/mob/living/poor_target in impacted_turf)
		...
		poor_target.visible_message(
			span_danger("\The [atom_source] slams into \the [poor_target] out of nowhere inducing a resonance... [poor_target.p_their()] body starts to glow and burst into flames before flashing into dust!"),
	...
	for(var/atom/movable/hit_object as anything in impacted_turf)
		...
		atom_source.visible_message(
			span_danger("\The [atom_source], smacks into the plating out of nowhere, reducing everything below to ash."),
			null,
			span_hear("You hear a loud crack as you are washed with a wave of heat."),
		)
...
/datum/component/supermatter_crystal/proc/consume(atom/source, atom/movable/consumed_object)
	...
	if(!iseffect(consumed_object) && !effects_calculated)
		if(isitem(consumed_object))
			...
			var/obj/real_consumed_object = consumed_object
			matter_increase += min(0.5 * real_consumed_object.max_integrity, 1000)
	...
	for(var/mob/living/near_mob in range(10))
		...
		if(near_mob in view())
			near_mob.show_message(
				span_danger("As \the [atom_source] slowly stops resonating, you find your skin covered in new radiation burns."), EMOTE_VISIBLE,
				span_danger("The unearthly ringing subsides and you find your skin covered in new radiation burns."), EMOTE_AUDIBLE,
			)
		else
			near_mob.show_message(
