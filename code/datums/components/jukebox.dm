...
/datum/jukebox/proc/load_songs_from_config()
	...
	if(isnull(config_songs))
		...
		for(var/track_file in tracks)
			...
			if(length(track_data) >= 3) // Bandaid for legacy tracks to not use the length for the bpm rather then the actual beats.
...
/// Loads presets if there are no songs in the config folder.
/datum/jukebox/proc/fill_songs_static_list()
	var/songs_list = list()
	for(var/datum/track/new_track as anything in subtypesof(/datum/track/engineering))
...
/datum/jukebox/proc/get_ui_data(list/data)
	...
	for(var/song_name, value in songs)
		var/datum/track/one_song = value
...
/datum/jukebox/proc/register_listener(mob/new_listener)
	...
	RegisterSignal(new_listener, list(COMSIG_MOVABLE_MOVED, COMSIG_MOB_JUKEBOX_PREFERENCE_APPLIED), PROC_REF(listener_moved))
...
/datum/track/engineering/basic
	song_name = "Engineering's Basic Beat"
	song_path = 'sound/misc/disco.ogg'
	song_length = 1 MINUTES
	song_beat_deciseconds = 5 DECISECONDS

/datum/track/engineering/domination
	song_name = "Engineering's Domination Dance"
	song_path = 'sound/misc/e1m1.ogg'
	song_length = 1 MINUTES + 35 SECONDS
	song_beat_deciseconds = 6 DECISECONDS

/datum/track/engineering/superiority
	song_name = "Engineering's Superiority Shimmy"
	song_path = 'sound/misc/paradox.ogg'
	song_length = 4 MINUTES
	song_beat_deciseconds = 4 DECISECONDS

/datum/track/engineering/ultimate
	song_name = "Engineering's Ultimate High-Energy Hustle"
	song_path = 'sound/misc/boogie2.ogg'
