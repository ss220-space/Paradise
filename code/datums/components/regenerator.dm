...
/datum/component/regenerator/process(seconds_per_tick)
	...
	var/heal_mod = living_parent.stat == UNCONSCIOUS ? 2 : 1
	...
	if(!heals_wounds || !ishuman(parent))
		if(!need_mob_update)
			return
	...
	for(var/obj/item/organ/external/bodypart as anything in human_parent.bodyparts)
		if(!SPT_PROB(2, seconds_per_tick))
			continue

		if(!bodypart.has_fracture() && !bodypart.has_internal_bleeding())
			continue
...
/datum/component/regenerator/proc/should_be_regenning(mob/living/who)
	...
	if(!heals_wounds || !ishuman(who))
		if(who.health != who.maxHealth)
			return TRUE

		return FALSE

	var/mob/living/carbon/human/human_parent = who
	for(var/obj/item/organ/external/bodypart as anything in human_parent.bodyparts)
		if(!bodypart.has_fracture() && !bodypart.has_internal_bleeding())
			continue
