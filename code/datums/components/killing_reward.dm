...
/datum/component/killing_reward/proc/on_death()
	...
	for(var/datum/money_account/account as anything in killers_accs)
		...
		account.notify_pda_owner("<b>Поступление вознаграждения </b>\"На ваш привязанный аккаунт поступил[DECL_0_O_O(bounty)] [bounty] кредит[DECL_0_A_OV(bounty)].\" (Невозможно Ответить)", FALSE)
