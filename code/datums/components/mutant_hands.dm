...
/datum/component/mutant_hands/RegisterWithParent()
	...
	RegisterSignal(parent, list(COMSIG_CARBON_POST_ATTACH_LIMB, COMSIG_CARBON_POST_REMOVE_LIMB), PROC_REF(try_reapply_hands))
...
/datum/component/mutant_hands/proc/apply_mutant_hands()
	...
	for(var/obj/item/hand_slot as anything in human_parent.get_held_items())
		...
		if(!isnull(hand_slot))
			...
			human_parent.drop_item_ground(hand_slot)

		// Put in hands has a sleep somewhere in there
		human_parent.put_in_hands(new mutant_hand_path(), qdel_on_fail = TRUE)
...
/datum/component/mutant_hands/proc/remove_mutant_hands()
	var/mob/living/carbon/human/human_parent = parent
	for(var/obj/item/hand_slot in human_parent.get_held_items())
...
/obj/item/mutant_hand/visual_equipped(mob/living/carbon/human/user, slot)
	. = ..()
...
/mob/living/carbon/human/proc/is_holding(obj/item/I)
	return get_held_index_of_item(I)


/mob/living/carbon/human/proc/get_held_index_of_item(obj/item/item)
	var/list/held = get_held_items()
	return held.Find(item)
