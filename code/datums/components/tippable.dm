...
/datum/component/tippable/proc/interact_with_tippable(mob/living/source, mob/user)
	...
	if(DOING_INTERACTION_WITH_TARGET(user, source))
		return
	if(istype(living_user) && !INTENT_DISARM)
		return
...
/datum/component/tippable/proc/try_tip(mob/living/tipped_mob, mob/tipper)
	...
	if(tip_time > 0)
		to_chat(tipper, span_warning("Вы начинаете опрокидывать [tipped_mob]..."))
		tipped_mob.visible_message(
			span_warning("[tipper] начинает опрокидывать [tipped_mob]."),
			span_userdanger("[tipper] начинает тебя опрокидывать!"),
			ignored_mobs = tipper
		)

		if(!do_after(tipper, tip_time, target = tipped_mob))
			if(!isnull(tipped_mob.client))
				add_attack_logs(tipper, "Неудачная попытка перевернуть [key_name(tipped_mob)]", ATKLOG_ALL)
			to_chat(tipper, span_danger("Вам не удаётся опрокинуть [tipped_mob]."))
			return
...
/datum/component/tippable/proc/do_tip(mob/living/tipped_mob, mob/tipper)
	...
	to_chat(tipper, span_warning("Вы опрокидываете [tipped_mob]."))
	if (!isnull(tipped_mob.client))
		add_attack_logs(tipper, "перевернул [key_name(tipper)].", ATKLOG_ALL)
	tipped_mob.visible_message(
		span_warning("[tipper] опрокидывает [tipped_mob]."),
		span_userdanger("[tipper] опрокинул вас!"),
		ignored_mobs = tipper
		)
...
/datum/component/tippable/proc/try_untip(mob/living/tipped_mob, mob/untipper)
	if(untip_time > 0)
		to_chat(untipper, span_notice("Вы начинаете поднимать [tipped_mob]..."))
		tipped_mob.visible_message(
			span_notice("[untipper] Начинает поднимать [tipped_mob]."),
			span_notice("[untipper] Начинает поднимать вас."),
			ignored_mobs = untipper
		)

		if(!do_after(untipper, untip_time, target = tipped_mob))
			to_chat(untipper, span_warning("Вы не подняли [tipped_mob]."))
			return
...
/datum/component/tippable/proc/do_untip(mob/living/tipped_mob, mob/untipper)
	...
	to_chat(untipper, span_notice("Вы подняли [tipped_mob]."))
	tipped_mob.visible_message(
		span_notice("[untipper] поднял [tipped_mob]."),
		span_notice("Вы подняты с помощью [untipper]!"),
		ignored_mobs = untipper
		)
...
/datum/component/tippable/proc/right_self(mob/living/tipped_mob)
	...
	tipped_mob.visible_message(
		span_notice("[tipped_mob] поднимает себя."),
		span_notice("Вы подняли себя.")
		)
