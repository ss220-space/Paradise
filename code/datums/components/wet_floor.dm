/datum/component/wet_floor
	...
	var/static/mutable_appearance/ice_overlay = mutable_appearance('icons/effects/water.dmi', "ice_floor")
...
/datum/component/wet_floor/proc/update_flags()
	...
	switch(highest_strength)
		if(TURF_WET_WATER)
			intensity = SLIPPERY_TIME_WATER
			lube_flags |= NO_SLIP_WHEN_WALKING
		if(TURF_WET_LUBE)
			intensity = SLIPPERY_TIME_LUBE
			lube_flags |= (SLIDE|SLIP_IGNORE_NO_SLIP_WATER|SLIP_WHEN_LYING)
		if(TURF_WET_ICE)
			intensity = SLIPPERY_TIME_ICE
			lube_flags |= (SLIDE|SLIP_IGNORE_NO_SLIP_WATER|SLIP_WHEN_LYING)
		if(TURF_WET_PERMAFROST)
			intensity = SLIPPERY_TIME_ICE
	...
	parent.LoadComponent(/datum/component/slippery, intensity, 0, lube_flags, CALLBACK(src, PROC_REF(AfterSlip)))
