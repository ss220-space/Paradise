...
/datum/component/obeys_commands/proc/add_friend(datum/source, mob/living/new_friend)
	SIGNAL_HANDLER
	// RegisterSignal(new_friend, COMSIG_KB_LIVING_VIEW_PET_COMMANDS, PROC_REF(on_key_pressed)) Похуй потом
...
/datum/component/obeys_commands/proc/remove_friend(datum/source, mob/living/old_friend)
	SIGNAL_HANDLER
	// UnregisterSignal(old_friend, list(//Похуй потом
...
/datum/component/obeys_commands/proc/display_menu(mob/living/friend)
	...
	if(radial_viewers[friend.UID()])
		return
	if(!friend.can_see(parent, DEFAULT_RADIAL_VIEWING_DISTANCE))
		return
...
/datum/component/obeys_commands/proc/display_radial_menu(mob/living/friend)
	...
	var/pick = show_radial_menu(friend, parent, radial_options, radius = radial_menu_radius,  custom_check = CALLBACK(src, PROC_REF(check_menu_viewer), friend))
	remove_from_viewers(friend)
	if(!pick)
		return
...
/datum/component/obeys_commands/proc/check_menu_viewer(mob/living/user)
	...
	if(!user.can_see(parent, viewing_distance))
		return FALSE
	return TRUE
