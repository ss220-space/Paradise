...
/datum/component/eatable/proc/try_eat_item(mob/living/carbon/human/target, mob/user)
	...
	switch(user.a_intent)
		...
		if(INTENT_HARM)
			chat_message_to_user = "В [target == user ? "вас" : target] больше не лезет. Но [target == user ? "вы" : user] насильно запихива[target == user ? "ете" : "ет"] [item.declent_ru(ACCUSATIVE)] в рот!"
			if(target != user)
...
/datum/component/eatable/proc/forceFed(mob/target, mob/user, instant_application = FALSE)
	...
	if(!instant_application)
		item.visible_message(span_warning("[user] пытается накормить [target], запихивая в рот [item.declent_ru(ACCUSATIVE)]."))
