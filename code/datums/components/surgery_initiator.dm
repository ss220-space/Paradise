	...
	if(the_surgery.step_number == 1)
		...
		if(affected_organ)
			user.visible_message(
				span_notice("[user] прерывает операцию на [affected_organ.declent_ru(PREPOSITIONAL)] [patient], используя [tool.declent_ru(ACCUSATIVE)]."),
			...
			user.visible_message(
				span_notice("[user] прерывает операцию на [parse_zone(selected_zone)] [patient], используя [tool.declent_ru(ACCUSATIVE)]."),
...
/datum/component/surgery_initiator/proc/show_starting_message(mob/user, mob/living/target, datum/surgery/procedure)
	...
	if(affected_organ)
		user.visible_message(
			span_notice("[user] готовится начать операцию на [affected_organ.declent_ru(PREPOSITIONAL)] [target], удерживая [tool.declent_ru(ACCUSATIVE)] в руке."),
		...
		user.visible_message(
			span_notice("[user] готовится начать операцию на [parse_zone(selected_zone)] [target], удерживая [tool.declent_ru(ACCUSATIVE)] в руке."),
