...
/datum/component/hunger_effects/Initialize()
	...
	RegisterSignal(parent, COMSIG_CARBON_NUTRITION_UPDATE, .proc/update_hunger_effects)
...
/datum/component/hunger_effects/proc/update_hunger_effects()
	...
	H.toolspeedmod = base_toolspeedmod + current_level.tool_mod
	if(H.toolspeedmod != base_toolspeedmod)
		H.add_or_update_variable_actionspeed_modifier(
			/datum/actionspeed_modifier/species_tool_mod,
			multiplicative_slowdown = H.toolspeedmod
		)

	H.sound_environment_override = current_level.sound_env
