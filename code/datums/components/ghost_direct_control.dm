...
/datum/component/ghost_direct_control/proc/attempt_possession(mob/our_mob, mob/dead/observer/hopeful_ghost)
	var/ghost_asked = tgui_alert(usr, "[question_text? question_text : "Стать [our_mob.declent_ru_cap(INSTRUMENTAL)]?"]", "Стать [our_mob.declent_ru_cap(INSTRUMENTAL)]?", list("Да", "Нет"))
...
/datum/component/ghost_direct_control/proc/assume_direct_control(mob/harbinger)
	...
	if(respawnable_check && cannotPossess(harbinger))
		...
		return
	if(new_body.key)
		to_chat(harbinger, span_warning("[new_body.declent_ru_cap(NOMINATIVE)] уже является разумным!"))
		qdel(src)
		return
