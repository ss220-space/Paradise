...
/datum/component/spawner/Initialize/(spawn_types = list(), spawn_time = 30 SECONDS, max_spawned = 5, max_spawn_per_attempt = 1 , faction = list("mining"), spawn_text = null, datum/callback/spawn_callback = null, spawn_distance = 1, spawn_distance_exclude = 0, initial_spawn_delay = 0 SECONDS)
...
/datum/component/spawner/proc/try_spawn_mob()
	...
	for(var/i in 1 to adjusted_spawn_count)
		...
		if(spawn_distance == 1)
			...
			picked_spot = pick(circle_range_turfs(spawner.loc, spawn_distance))
			created = new chosen_mob_type(picked_spot)
		created.flags |= (spawner.flags & ADMIN_SPAWNED)
	...
	if(spawn_text)
		spawner.visible_message(span_danger("[spawner] [spawn_text] новое существо."))
