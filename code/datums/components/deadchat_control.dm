...
/datum/component/deadchat_control/Destroy(force, silent)
	var/atom/atom_parent = parent
	var/message = "<span class='deadsay italics bold'>[capitalize(atom_parent.declent_ru(NOMINATIVE))] теперь не контролируется призраками.")
...
/datum/component/deadchat_control/proc/democracy_loop()
	...
	if(!isnull(result))
		...
		if(!(deadchat_mode & MUTE_DEADCHAT_DEMOCRACY_MESSAGES))
			var/input_cooldown_s = input_cooldown * 0.1
			var/message = "<span class='deadsay italics bold'>[capitalize(atom_parent.declent_ru(NOMINATIVE))] выполнил команду [result]!<br>Новое голосование начато. Оно закончится через [input_cooldown_s] секунд[declension_ru(input_cooldown_s,"у", "ы", "")].")
			for(var/mob/dead/observer/M in orbiters)
				to_chat(M, message)
	else if(!(deadchat_mode & MUTE_DEADCHAT_DEMOCRACY_MESSAGES))
		var/message = "<span class='deadsay italics bold'>В этом цикле не было голосов.")
		for(var/mob/dead/observer/M in orbiters)
			to_chat(M, message)
...
/datum/component/deadchat_control/proc/on_examine(atom/object, mob/user, list/examine_list)
	...
	if(!(user in orbiters))
		examine_list += "<span class='deadsay bold'>Прыгнете на [genderize_ru(object.gender, "него", "неё", "него", "них")]] и осмотрите снова, чтобы увидеть список доступных команд.")
		return
