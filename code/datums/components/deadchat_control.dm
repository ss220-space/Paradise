...
/datum/component/deadchat_control/Initialize(_deadchat_mode, _inputs, _input_cooldown = 12 SECONDS, _on_removal)
	...
	notify_ghosts("[atom_parent.declent_ru_cap(NOMINATIVE)] теперь может контролироваться призраками из чата! Возможные команды: [english_list(input_names)]", source = parent, action = NOTIFY_FOLLOW, title="Контроль дедчата!")
...
/datum/component/deadchat_control/Destroy(force)
	var/atom/atom_parent = parent
	var/message = span_deadsay(span_bolditalics("[atom_parent.declent_ru_cap(NOMINATIVE)] теперь не контролируется призраками."))
...
/datum/component/deadchat_control/proc/democracy_loop()
	...
	if(!isnull(result))
		...
		if(!(deadchat_mode & MUTE_DEADCHAT_DEMOCRACY_MESSAGES))
			var/input_cooldown_s = input_cooldown * 0.1
			var/message = span_deadsay(span_bolditalics("[atom_parent.declent_ru_cap(NOMINATIVE)] выполнил команду [result]!<br>Новое голосование начато. Оно закончится через [input_cooldown_s] секунд[DECL_SEC_MIN(input_cooldown_s)]."))
			for(var/mob/dead/observer/M in orbiters)
				to_chat(M, message)
...
/datum/component/deadchat_control/proc/orbit_begin(atom/source, atom/orbiter)
	...
	if(isobserver(orbiter))
		...
		if(O.client && !(O.client.prefs.toggles & PREFTOGGLE_CHAT_DEAD))
			...
			to_chat(O, span_deadsay("[atom_parent.declent_ru_cap(NOMINATIVE)] контролируется призраками через чат! Осмотрите [atom_parent.declent_ru(ACCUSATIVE)] чтобы увидеть команды управления, которые вы можете использовать пока летаете вокруг н[GEND_HIS_HER(atom_parent)]!"))
