	...
	if(isliving(crossing_movable))
		...
		if(crossing_mob.mob_size > MOB_SIZE_SMALL && !(crossing_mob.movement_type & FLYING))
			if(HAS_TRAIT(crossing_mob, TRAIT_PACIFISM))
				crossing_mob.visible_message(span_notice("[crossing_mob] аккуратно переступает через [parent_as_living.declent_ru(ACCUSATIVE)]"), span_notice("вы аккуратно переступаете через [parent_as_living.declent_ru(ACCUSATIVE)]"))
				return
			if(should_squash)
				crossing_mob.visible_message(span_notice(" [crossing_mob] раздавливает [parent_as_living.declent_ru(ACCUSATIVE)]!"), span_notice("Вы раздавливаете [parent_as_living.declent_ru(ACCUSATIVE)]."))
				Squish(parent_as_living)
			else
				parent_as_living.visible_message(span_notice("[parent_as_living.declent_ru(NOMINATIVE)] избегает смерти!"))

	if(isstructure(crossing_movable) && !crossing_movable.anchored)
		if(should_squash)
			crossing_movable.visible_message(span_notice("[parent_as_living.declent_ru(NOMINATIVE)] раздавливается под [crossing_movable.declent_ru(INSTRUMENTAL)]"))
			Squish(parent_as_living)
		else
			parent_as_living.visible_message(span_notice("[parent_as_living.declent_ru(NOMINATIVE)] избегает смерти!"))
