...
/datum/brain_trauma
	...
	var/desc = "Травма, вызванная повреждением головного мозга."
	...
	var/gain_text = span_notice_alt("Вы чувствуете что с вашим разумом что-то не так.")
	var/lose_text = span_notice_alt("Вы больше не чувствуете что с вашим разумом что-то не так.")
	var/can_gain = TRUE
	var/random_gain = TRUE //can this be gained through random traumas?
	var/resilience = TRAUMA_RESILIENCE_BASIC //how hard is this to cure?

	/// Tracks abstract types of brain traumas, useful for determining traumas that should not exist
	/// (abstract_type is already declared on the base /datum in master220, so just assign it here.)
...
/datum/brain_trauma/proc/on_gain()
	...
	// master220 has no organ-level trauma tick, so drive on_life() off the owner's Life signal.
	RegisterSignal(owner, COMSIG_LIVING_LIFE, PROC_REF(_on_owner_life))
	return TRUE
...
/// Relays the owner's Life tick to on_life().
/datum/brain_trauma/proc/_on_owner_life(datum/source, deltatime, times_fired)
	SIGNAL_HANDLER
	on_life(deltatime, times_fired)
