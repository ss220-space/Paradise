...
/datum/symptom/hallucigen/Activate(datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB))
		...
		switch(A.stage)
			if(1, 2)
				to_chat(M, "<span class='warning'>[pick("Что-то мелькает на границе вашего зрения, и сразу исчезает.", "Вы слышите лёгкий шёпот непонятно откуда.", "У вас болит голова.")]</span>")
			if(3, 4)
