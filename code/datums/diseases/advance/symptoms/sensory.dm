	...
	if(prob(SYMPTOM_ACTIVATION_PROB * 5))
		...
		switch(A.stage)
			if(4, 5)
				...
				if(prob(SYMPTOM_ACTIVATION_PROB * 5))
					to_chat(M, "<span class='notice'>[pick("Вы всё очень хорошо видите.","Ваши глаза, кажется, теперь фокусируются легче и точнее обычного.", "Кажется, вам больше не нужно моргать.","Вы слышите всё очень чётко.","Ваш слух становится очень чутким.")]</span>")
