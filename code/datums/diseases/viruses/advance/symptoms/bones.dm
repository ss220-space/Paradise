	...
	if(prob(SYMPTOM_ACTIVATION_PROB * 4))
		switch(A.stage)
			...
			if(3, 4)
				switch(rand(1, 3))
					if(1)
						playsound(M, SFX_BONEBREAK, 15, TRUE)
					...
					if(3)
						M.Slowed(1 SECONDS)
						M.visible_message(span_warning("[capitalize(M.declent_ru(NOMINATIVE))] хрома[PLUR_ET_YUT(M)]."), span_warning("Ваша нога больше не держит форму!"))
			...
			if(5)
				switch(rand(1, 2))
					...
					if(2)
						playsound(M, SFX_BONEBREAK, 50, TRUE)
