	...
	switch(stage)
		...
		if(4)
			...
			if(prob(3) && barklimit <= 10)
				...
				affected_mob.visible_message(span_danger("[affected_mob] отрыгивает [initial(puppypicked.name)]!"), span_userdanger("Вы отрыгиваете [initial(puppypicked.name)]?!"))
			...
			if(prob(1))
				...
				affected_mob.visible_message(span_danger("[affected_mob] отрыгивает [coughfox.declent_ru(ACCUSATIVE)]!"), span_userdanger("Вы отрыгиваете [coughfox.declent_ru(ACCUSATIVE)]?!"))
			if(prob(50))
				affected_mob.emote("cough")
