...
/datum/disease/virus/nuclefication/stage_act()
	...
	switch(stage)
		...
		if(5)
			H.visible_message(span_danger("[H.declent_ru(NOMINATIVE)] превраща[PLUR_ET_UT(affected_mob)]ся в нуклеацию!"), span_userdanger("ВЫ ПРЕВРАЩАЕТЕСЬ В НУКЛЕАЦИЮ. ОПЯТЬ!"))
			H.setOxyLoss(0)
			H.SetJitter(0)
