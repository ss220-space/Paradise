...
/datum/disease/virus/anxiety/stage_act()
	...
	switch(stage)
		...
		if(4)
			...
			if(prob(5))
				affected_mob.visible_message(span_danger("[affected_mob] меч[PLUR_ET_YUT(affected_mob)]ся в панике."), span_userdanger("У вас паническая атака!"))
			...
			if(prob(3))
				affected_mob.visible_message(span_danger("[affected_mob] откашлива[PLUR_ET_YUT(affected_mob)] бабочек!"), span_userdanger("Вы откашливаете бабочек!"))
