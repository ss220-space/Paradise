...
/datum/disease/virus/wizarditis/stage_act()
	...
	switch(stage)
		if(2)
			if(prob(5))
				affected_mob.say(pick("Ты не пройдёшь!", "Экспеллиармус!", "Клянусь бородой Мерлина!", "Почувствуй мощь Тёмной Стороны!"))
			if(prob(3))
				to_chat(affected_mob, span_danger("Вы чувствуете, что [pick("вам не хватает маны", "ветры магии исчезли", "вам хочется призвать фамильяра")]."))
		...
		if(3)
			...
			if(prob(6))
				to_chat(affected_mob, span_danger("Вы чувствуете, как [pick("магия бурлит в ваших жилах", "это место даёт вам +1 к интеллекту", "вам хочется призвать фамильяра")]."))
