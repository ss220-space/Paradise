		...
		if(2)
			...
			if(prob(15))
				affected_mob.visible_message(
					span_danger("[affected_mob] дёргается в конвульсиях!")
				)
			...
			if(prob(33))
				if(affected_mob.incapacitated())
					affected_mob.visible_message(
						span_danger("[affected_mob] бьётся в судорогах и дёргается!")
					)
					return
				affected_mob.visible_message(
					span_danger("[affected_mob] яростно мечется!")
				)
				for(var/mob/living/carbon/M in range(1, affected_mob))
					...
					if(prob(80))
						...
						affected_mob.visible_message(
							span_danger("[affected_mob] ударяет [M.declent_ru(ACCUSATIVE)] своими конвульсиями!")
						)
						M.adjustBruteLoss(damage)
					else
						playsound(affected_mob.loc, 'sound/weapons/punchmiss.ogg', 25, TRUE, -1)
						affected_mob.visible_message(
							span_danger("[affected_mob] не попадает по [M.declent_ru(ACCUSATIVE)] своими конвульсиями!")
						)
