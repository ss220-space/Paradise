			...
			if(prob(33))
				...
				for(var/mob/living/carbon/M in range(1, affected_mob))
					...
					if(prob(80))
						...
						affected_mob.visible_message(span_danger("[affected_mob] ударя[PLUR_ET_UT(affected_mob)] [M.declent_ru(ACCUSATIVE)] своими конвульсиями!"))
						M.adjustBruteLoss(damage)
					else
						playsound(affected_mob.loc, 'sound/weapons/punchmiss.ogg', 25, TRUE, -1)
						affected_mob.visible_message(span_danger("[affected_mob] не попада[PLUR_ET_UT(affected_mob)] по [M.declent_ru(ACCUSATIVE)] своими конвульсиями!"))
