	...
	switch(stage)
		...
		if(2)
			...
			if(prob(5))
				...
				affected_mob.say(speak)
			if(prob(15))
				affected_mob.visible_message(span_danger("[affected_mob] дёрга[PLUR_ET_YUT(affected_mob)]ся в конвульсиях!"))
			...
			if(prob(33))
				...
				for(var/mob/living/carbon/M in range(1, affected_mob))
					...
					if(prob(80))
						...
						affected_mob.visible_message(span_danger("[affected_mob] ударя[PLUR_ET_YUT(affected_mob)] [M.declent_ru(ACCUSATIVE)] своими конвульсиями!"))
						M.adjustBruteLoss(damage)
					else
						playsound(affected_mob.loc, 'sound/weapons/punchmiss.ogg', 25, TRUE, -1)
						affected_mob.visible_message(span_danger("[affected_mob] не попада[PLUR_ET_YUT(affected_mob)] по [M.declent_ru(ACCUSATIVE)] своими конвульсиями!"))
