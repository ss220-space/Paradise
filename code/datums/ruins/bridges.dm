...
/obj/effect/spawner/bridge/Initialize(mapload)
	. = ..()
	INVOKE_ASYNC(src, PROC_REF(load_bridge_async))

// TODO: Remove this, move to generating bridges on late initializing by map loader
/obj/effect/spawner/bridge/proc/load_bridge_async()
	if(QDELETED(src))
		return
	...
	if((ismineralturf(east) || istype(east, /turf/simulated/floor/plating/asteroid)) && \
			...
			!(ismineralturf(s1) || istype(s1, /turf/simulated/floor/plating/asteroid)))
		template = GLOB.bridge_horizontal_templates[pick("lavaland_bridge_horizontal_1.dmm",
														"lavaland_bridge_horizontal_2.dmm",
														"lavaland_bridge_horizontal_3.dmm",
														"lavaland_bridge_horizontal_4.dmm",
														"lavaland_bridge_horizontal_5.dmm",
														"lavaland_bridge_horizontal_6.dmm")]
	else if((ismineralturf(north) || istype(north, /turf/simulated/floor/plating/asteroid)) && \
			(ismineralturf(south) || istype(south, /turf/simulated/floor/plating/asteroid)) && \
			!(ismineralturf(e1) || istype(e1, /turf/simulated/floor/plating/asteroid)) && \
			!(ismineralturf(w1) || istype(w1, /turf/simulated/floor/plating/asteroid)))
		template = GLOB.bridge_vertical_templates[pick("lavaland_bridge_vertical_1.dmm",
													"lavaland_bridge_vertical_2.dmm",
													"lavaland_bridge_vertical_3.dmm",
													"lavaland_bridge_vertical_4.dmm",
