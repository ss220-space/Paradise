...
/datum/award/proc/get_changed_rows(key, value)
	if(!database_id || !key || !name)
		return
	return list("ckey" = "'[key]'","achievement_key" = "'[database_id]'", "value" = "'[value]'")

/datum/award/proc/get_metadata_row()
	return list("achievement_key"  = "'[database_id]'", "achievement_version" = "'[achievement_version]'", "achievement_type" = "'award'", "achievement_name" = "'[name]'", "achievement_description" = "'[desc]'")

///Get raw numerical achievement value from the database
/datum/award/proc/get_raw_value(key)
	set waitfor = FALSE

	var/datum/db_query/Q = SSdbcore.NewQuery("SELECT value FROM [format_table_name("achievements")] WHERE ckey = '[key]' AND achievement_key = '[database_id]'")
...
/datum/award/score/proc/LoadHighScores()
	...
	var/datum/db_query/Q = SSdbcore.NewQuery("SELECT ckey,value FROM [format_table_name("achievements")] WHERE achievement_key = '[database_id]' ORDER BY value DESC LIMIT 50")
	if(!Q.Execute(async = TRUE))
		qdel(Q)
		return
	else
