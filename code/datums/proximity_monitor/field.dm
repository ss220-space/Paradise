...
/datum/proximity_monitor/advanced/proc/get_edgeturf_direction(turf/tile, turf/center_override = null)
	var/turf/checking_from = get_turf(host)
	if(istype(center_override))
		checking_from = center_override

	if(!(tile in edge_turfs))
		return

	if(((tile.x == (checking_from.x + current_range)) || (tile.x == (checking_from.x - current_range))) && ((tile.y == (checking_from.y + current_range)) || (tile.y == (checking_from.y - current_range))))
		return get_dir(checking_from, tile)

	if(tile.x == (checking_from.x + current_range))
		return EAST

	if(tile.x == (checking_from.x - current_range))
		return WEST

	if(tile.y == (checking_from.y - current_range))
		return SOUTH
...
/datum/proximity_monitor/advanced/debug
	current_range = 5
	var/set_fieldturf_color = COLOR_PROXIMITY_FIELD
	var/set_edgeturf_color = COLOR_PROXIMITY_EDGE
